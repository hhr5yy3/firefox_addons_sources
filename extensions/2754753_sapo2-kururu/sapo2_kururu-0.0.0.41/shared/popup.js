...
PopUp.prototype = {
    ...
    window : function(self) {
        ...
        var efeito = function() {
            ...
            if (contador_de_frames-- === 0) {
                ...
                self.pano.style.background = 'rgba(145, 145, 145, 0.8)';
    ...
    /**
     * Bloqueia o acesso à janela e retorna a função para desbloquear.
     * @param self {Object} Referência à instância de PopUp()
     * @param mensagem {String} Texto que deve ser exibido na frente da janela.
     * @param cor {String} BackgroundColor
     * @returns {Function} Retorna a função para desbloquear a janela que não precisa de qualquer parâmetro.
     */
    bloquear : (self, mensagem, cor="rgba(255,255,248,0.9)")=>{
        const aceitarEsc = self.aceitaEsc;
        const clicarForaSair = self.clicafora_sair;
        self.aceitaEsc = self.clicafora_sair = false;
        const d1 = self.cria('div', {
            innerText: mensagem,
            className: 'bloqueio',
            id: 'popup_msg_bloqueio', // SERVIR DE REFERÊNCIA PARA MODIFICAR O VALOR DA MENSAGEM
            style: {
                width: "100%", // `${getComputedStyle(self.div).width}px`,
                height: "100%", // `${getComputedStyle(self.div).height}px`,
                top: "0px", // `${getComputedStyle(self.div).top}px`,
                left: "0px", // `${getComputedStyle(self.div).left}px`,
                position: 'absolute',
                background: cor || 'rgba(255,251,248,0.9)',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                zIndex: '1000',
                fontFamily: getComputedStyle(self.div).fontFamily,
                fontSize: getComputedStyle(self.div).fontSize,
            }
        });
        self.div.appendChild(d1);
        console.log(d1);
        const desbloquear = ()=>{
            d1.parentNode.removeChild(d1);
            self.aceitaEsc = aceitarEsc;
            self.clicafora_sair = clicarForaSair;
        }
        return desbloquear;
    }
};
