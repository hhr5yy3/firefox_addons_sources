(() => {
  ...
  // ../common/content-scripts/serps/swisscows.ts
  (async () => {
  ...
  function processResults(searchResultToggler) {
    ...
    const Q = `div.web-results article.item-web:not([${ATTR_NAME}])`;
    const items = document.querySelectorAll(Q);
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      item.setAttribute(ATTR_NAME, "1");
      const a = item.querySelector("a:not(.site)");
      if (!a) {
      ...
      getCategorization_default(a.href).then((categorization) => {
        ...
        const target = a.querySelector("h2");
        if (!target) {
