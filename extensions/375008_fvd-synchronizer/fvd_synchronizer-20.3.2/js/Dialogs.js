(function(){
	...
	var Dialogs = function(){
        ...
        //if (window == chrome.extension.getBackgroundPage()) { // ff method switch
        if (window.location == chrome.extension.getURL("background.html")) {
	...
	Dialogs.prototype = {
		...
		alertWindow: function(title, text, params){
			...
			fvdSynchronizer.Utils.Async.chain([function(chainCallback){
				...
				chrome.windows.create({
					url: chrome.extension.getURL("alert.html?title=" + encodeURIComponent(title) + "&text=" + encodeURIComponent(text)),
					width: INITIAL_WIDTH,
		...
		choose: function(title, text, buttons, callback){

			var btns = {};
            
            buttons.forEach((button)=>{
                btns[button.text] = function(){
                    dlg.close();
			...
			var dlg = new Dialog({
				...
				onShow: function(){
                    ...
                    */
					...
					document.getElementById( "dialogAlertCheck_text" ).innerText = String(text);
					document.getElementById( "dialogAlertCheck_checkBoxLabel" ).innerText = String(cbText);
		...
		syncProgressSimple: function(syncDriver, syncAction, callback, additional) {            
			...
			if (syncAction == "makeBackup" || syncAction == "makeBackupZip") {
			...
			var dlg = new Dialog({
				...
				onShow: function( _dlg ){
					...
					var unsortedLink = _dlg.container.querySelector(".warning-unsorted a"); // Task #2046
					if(unsortedLink){ // Task #2046
						unsortedLink.addEventListener("click", function(e) {
					...
					var fvdButton = _dlg.container.querySelector(".fvdButton");
					if(fvdButton) fvdButton.setAttribute("hidden", true);
                    ...
                    if (syncAction == "makeBackupZip") {
						fvdSynchronizer.Driver[syncDriver].Backup.makeZip(function(){
                    ...
                    if (syncAction == "restoreBackupZip") {
                        ...
                        fvdSynchronizer.Driver[syncDriver].Backup.restoreZip( additional.file, function(){
				...
				onShow: function(){
					if (groupId) {
						...
						fvdSpeedDial.Storage.getGroup(groupId, function(group){
                            ...
                            if(group.global_id == "default" && (typeof group.name == "undefined" || !group.name || group.name == "undefined")){
