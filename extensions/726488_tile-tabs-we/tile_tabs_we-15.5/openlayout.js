/************************************************************************/
/*                                                                      */
/*      Tile Tabs WE - Generic WebExtension - Open Layout Page          */
/*                                                                      */
/*      Javascript for Open Layout Page                                 */
/*                                                                      */
...
/* Listener for open layout page load */
...
function onLoadPage()
{
    ...
    document.getElementById("open-buttons-open").addEventListener("click",onOpen,false);
    document.getElementById("open-buttons-cancel").addEventListener("click",onCancel,false);
    ...
    function(object)
    {
        ...
        select = document.getElementById("open-layoutlist-select");
...
{
    document.getElementById("open-message-1").style.setProperty("display","none","");
}
...
function onOpen(event)
{
    ...
    {
        document.getElementById("open-message-1").style.setProperty("display","inline","");
        event.preventDefault();
        return;
    }
    
    chrome.runtime.sendMessage({ type: "openLayoutReply", layoutname: select.options[select.selectedIndex].getAttribute("value"), result: true });
}
...
{
    chrome.runtime.sendMessage({ type: "openLayoutReply", result: false });
}
