(() => {
  ...
  // ../common/content-scripts/serps/yahoojp.ts
  (async () => {
  ...
  function processResults(searchResultToggler) {
    const items = document.querySelectorAll("#contents .sw-CardBase");
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      const a = item.querySelector(".sw-Card__title a");
      if (!a) {
      ...
      getCategorization_default(a.href).then((categorization) => {
        ...
        const target = a.querySelector("h3 span");
        if (!target) {
