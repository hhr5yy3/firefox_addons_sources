...
browser.runtime.sendMessage({ pageLoad: true }, function(response) {
    alterDOM(response);
});
...
browser.storage.onChanged.addListener(function(changes) {
	...
	browser.storage.sync.get("telEnabled", function (obj) {
...
function loadConfig() {
	browser.storage.sync.get("protocol", function (obj) {
		protocol = (obj.protocol != undefined) ? obj.protocol : "dial";
	});
	
	browser.storage.sync.get("useSlashes", function (obj) {
		useSlashes = (obj.useSlashes != undefined) ? obj.useSlashes : true;
	});
	
	browser.storage.sync.get("minTelVal", function (obj) {
		minTelVal = (obj.minTelVal != undefined) ? parseInt(obj.minTelVal) : 8;
	});
	
	browser.storage.sync.get("maxTelVal", function (obj) {
		maxTelVal = (obj.maxTelVal != undefined) ? parseInt(obj.maxTelVal) : 20;
	});
	
	browser.storage.sync.get("replaceList", function (obj) {
	...
	browser.storage.sync.get("telEnabled", function (obj) {
		enabled = (obj.telEnabled != undefined) ? obj.telEnabled : true;
		if (enabled) {
			clearDOM();
			parseDOM(document.body);
		}
...
function parseDOM(node) {
	...
	if (nodeName == 'A') {
		...
		breakA: {
			return inputDialBtn(node); //return 0;
		}
	}
				...
				if (node.firstChild.firstChild != undefined) {
					for (var n = 0; n < node.firstChild.firstChild.childElementCount; n++) {
						...
						if (node.firstChild.firstChild.childNodes[n].nextSibling != null && node.firstChild.firstChild.childNodes[n].nextSibling.className == "DetectedNumber") {
							$(node.firstChild.firstChild.childNodes[n].nextSibling).on('mouseup', function(ev) {
								ev.preventDefault()}
								);
							$(node).on('mouseup', function(ev) {
								...
								$(node.firstChild.firstChild.childNodes[n].nextSibling).on('click', function(ev) {
								ev.preventDefault()}
								);
...
function nodeUpdate(resultArray, node, insertBefore=true) {
	for (var n = 0; n < resultArray.length; n++) {
		...
		var nodeValueRaw = (node.nodeValue == undefined) ? (node.value == undefined)? node.textContent : node.value : node.nodeValue;
...
function buildNode(telNumber, cleanNumber, fullSize) {
	...
	if(fullSize) {
		spanNode = $('<div style="display: inline-block; position: relative; cursor: pointer;padding-left:3px;padding-right:20px;-webkit-border-radius: 8px;-webkit-box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;white-space: nowrap;" title="Call: ' + telNumber + '" class="DetectedNumber">' + telNumber + '<img src="data:image/png;base64,' + handsetImage + '" style="display: inline-block; width: initial; border: 0px; padding: 0px; margin: 0px; float: right; bottom: 1px; right: 1px; position: absolute; background-color:rgba(0,0,0,0);" class="DetectedNumberImg"/></div>')[0];
	} else {
		spanNode = $('<div style="display: inline-block;cursor: pointer;padding-left:3px;-webkit-border-radius: 8px;-webkit-box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;box-shadow: 0 1px 1px #ddd, 0 1px 1px #555;white-space: nowrap;"  title="Call: ' + telNumber + '" class="DetectedNumber"><img src="data:image/png;base64,' + handsetImage + '" style="width: initial; border: 0px; padding: 0px; margin: 0px; float: none; bottom: 1px; right: 1px; background-color:rgba(0,0,0,0);" class="DetectedNumberImg"/></div>')[0];
	}

	spanNode.addEventListener('click', function(ev){ location.href= protocol + ':' + ((useSlashes)?"//":"") + cleanNumber; ev.stopPropagation(); return(false);  }, false);
...
function populateRules() {
	...
	if (validNumbers.length == 0) {
		...
		validNumbers[validNumbers.length] = /(\+?[\s\-]?\d*[\s\.\-\\\/]?)?([\(\[][\d]*[\)\]])?[\s\.\-\\\/]?[\d]*[\s\.\-\\\/]?[\d]{3,4}?[\s\.\-\\\/]?[\d]+(([\s\.\-\\\/]?[\D]{1,8}?[\s\.\-\\\/]|\,)?[\d]+)?/g;
...
function parsePhoneNumbers(node) {
	...
	if (bestResult != null && bestResult.length > 0) {
		...
		if (partialresult != undefined) {
			var numresult = partialresult[0].replace(/[\s\/\-]/g,'').replace(/[^\d\(\)]{2,}/,'_').split('_')[0];
