...
scparser.$add('t', function(a, scnodes) {
    ...
    for (let i = 0; i < scnodes.length; i++) {
        if (scnodes[i]) {
            const n = scnodes[i];
    ...
    chrome.storage.local.set({addedNode: handles});
});
...
scparser.$add('u', function(a) {
    ...
    if (n) {
        ...
        chrome.storage.local.set({updatedNode: n});
    }
});
...
scparser.$add('d', function(a) {
    ...
    if (pmdb) {
        ...
        // When we have file attachment on the password node and if the password nodes are deleted then we need to
        // handle it to traverse tree the same way as webclient does
        delete M.c[M.d[a.n].p][a.n];
        delete M.d[a.n];

        chrome.storage.local.set({deletedNode: a.n});
    }
});
...
scparser.$finalize = async() => {
    ...
    if (loadavatars.length) {
        await useravatar.refresh(loadavatars).catch(dump);
        chrome.runtime.sendMessage({type: 'reloadAvatar', avatarMeta: generateAvatarMeta(loadavatars[0])});
        loadavatars = [];
    }
...
function initworkerpool() {
    ...
    decWorkerPool.init(worker_procmsg, 8, workerStateData);
...
function tree_node(node) {
    ...
    if (M.d[node.h] && (M.d[node.h].name || M.d[node.h].k === undefined)) {
        // already decrypted
        return;
    }
...
function worker_procmsg(ev) {
    ...
    if (ev.data.scqi >= 0) {
        ...
        if (ev.data.scni >= 0) {
            ...
            // As we do not have usfc cache, doing this manually but if we have file attachments later,
            // possible put this back.
