...
function HTMLActuator(store) {
  ...
  this.score = 0;
  this.leaderboard = new Leaderboard(store);
  this.store = store.view;
}
    ...
    if (metadata.terminated) {
      if (metadata.over) {
        if (!self.leaderboardBtn.classList.contains('hidden') && metadata.score > self.leaderboard.bestLBScore()) {
          self.showLeaderboardSuggestion(metadata.scoreObj, false, function(state) {
            if (state !== 'bestscore') {
              metadata.tileAnimating = false;
              self.actuate(grid, metadata);
            } else {
              self.message(false);
            }
          });
...
HTMLActuator.prototype.showLeaderboardSuggestion = function(scoreObj, userClick, onHide) {
  if (userClick && this.leaderboard.isOpened()) {
    this.leaderboard.hide();
  } else {
    this.store && this.store.hideStore();
    this.leaderboard.showSuggestion(scoreObj, userClick, onHide);
  }
}
...
HTMLActuator.prototype.showStore = function() {
  this.store && this.store.showStore();
}
...
function GameManager(size, InputManager, Actuator, StorageManager) {
  ...
  //alert("Actuator");
  this.actuator       = new Actuator(this.store);
  this.store = this.store.controller;
...
GameManager.prototype.restart = function () {
  if (this.won && !this.keepPlaying && this.score > this.actuator.leaderboard.bestLBScore()) {
    this.actuator.showLeaderboardSuggestion({
      score: this.score,
      steps: this.steps,
...
GameManager.prototype.checkCanUndo = function (need_load, callback) {
  if (!paymentSupports) {
  ...
  self.store.getInfo(need_load, function(info) {
...
GameManager.prototype.gameStateAvailableUndo = function () {
  return this.direction !== undefined && this.storageManager.hasSavedState();
};

// show store
GameManager.prototype.showStore = function () {
...
GameManager.prototype.actuate = function (tileAnimating = true) {
...
GameManager.prototype.buildTraversals = function (vector, undo) {
  var traversals = { x: [], y: [] };
