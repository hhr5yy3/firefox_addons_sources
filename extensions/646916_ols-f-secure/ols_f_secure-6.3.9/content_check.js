...
class ContentChecker {
    ...
    setPaymentCategory(tabId, url, categories) {
        ...
        for (const i in categories) {
            if (Object.keys(categories[i])[0] == "banking" || Object.keys(categories[i])[0] == "paymentservice" || 
                Object.keys(categories[i])[0] == "fso_banking" || Object.keys(categories[i])[0] == "fso_paymentservice") {
    ...
    async checkResources(tabId, getOrsp) {
        ...
        if (this.#tabs[tabId] && this.#tabs[tabId].resources && (this.#tabs[tabId].paymentForm || this.#tabs[tabId].paymentCategory)) {
            logd(`Performing resource check, payment form found: ${this.#tabs[tabId].paymentForm}, payment category: ${this.#tabs[tabId].paymentCategory}`);
            let unsafeRequests = [];
