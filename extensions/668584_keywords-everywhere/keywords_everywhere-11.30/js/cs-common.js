var Common = (function(){
  ...
  var subscriptionPromoCondition = function(){
    if (_config.areSubsEnabled && (!_plan.plan || !_plan.credits)) {
      if (_plan.expiry_date) {
        var activeSubscription = new Date(_plan.expiry_date) > new Date();
        if (activeSubscription) return 'valid_sub';
      }
      return 'freeuser';
    }
    return '';
  ...
  var setPlan = function(val, isValidAPIKey){
    _plan = val;
  ...
  var getIsValidAPIKey = function(){
    return _isValidAPIKey;
  };
  ...
  var addCopyExportButtons = function($parent, res, filename){
    $('#xt-suggestions-export').remove();
    var  html = '<a href="#" data-type="copy">Copy</a><a href="#" data-type="export">Export</a>';
    var $div = $('<div>', {id: 'xt-suggestions-export'}).html(html).appendTo($parent);
    $div.find('a').click(function(e){
      e.preventDefault();
      var type = this.dataset.type;
      if (type === 'export') {
        Common.exportKeywords(res, '', filename);
      }
      else if (type === 'copy') {
        Common.exportKeywords(res, 'copy');
      }
    });
  };


  var exportKeywords = function(storage, copy, filename){
    if (!storage) storage = keywordsStorage;
    if (copy) copy = true;
    ...
    if (!filename) filename = document.location.host + '_' + Date.now() + '.csv';
    exportTableToCSV({
      table: $result,
      filename: filename,
      method: copy ? 'copy' : 'export'
    });
  };
  ...
  var renderWidgetTable = function(rows, params){
    ...
    var html = [
      ...
      '</div>',
      '<h3 class="' + (json ? '' : 'xt-ke-h-no-credits') + '"><img src="' + chrome.runtime.getURL('/img/icon24.png') + '" width="24" height="24"> ' + title + '</h3>',
    ...
    var validSubParam = subscriptionPromoCondition() === 'valid_sub' ? '&validSub=1' : '';
    var validAPIKeyParam = _isValidAPIKey ? '&validAPIKey=1' : '';
    var planParam = '&plan=' + _plan?.plan;

    var iframeSrc = 'https://keywordseverywhere.com/ke/widget.php?apiKey=' + apiKey + '&source=' + iframeSrcParam + '&enabled=' + iframeSettingEnabledParam + '&country=' + country + '&version=' + version + '&pur=' + pur + validSubParam + planParam + validAPIKeyParam + '&query=' + encodeURIComponent(Common.escapeHtml(params.query));
  ...
  var renderIframeHTML = function(params){
    ...
    var apiKey = settings.apiKey || '';
    var pur = _credits > 0 ? 0 : 1;
    var iframeSettingEnabledParam = true;
    if (typeof params.settingEnabled !== 'undefined') iframeSettingEnabledParam = params.settingEnabled;

    var version = chrome.runtime.getManifest().version;
    var validSubParam = subscriptionPromoCondition() === 'valid_sub' ? '&validSub=1' : '';
    var validAPIKeyParam = _isValidAPIKey ? '&validAPIKey=1' : '';
    var planParam = '&plan=' + _plan?.plan;
    // var validSubParam = params.iframeValidSubParam ? '&validSub=1' : '';
    var iframeSrc = 'https://keywordseverywhere.com/ke/widget.php?apiKey=' + apiKey + '&source=' + params.iframeSrcParam + validSubParam + planParam + validAPIKeyParam + '&enabled=' + iframeSettingEnabledParam + '&country=' + country + '&version=' + version + '&pur=' + pur + '&query=' + encodeURIComponent(Common.escapeHtml(params.query));
    if (params.darkMode) iframeSrc += '&darkmode=' + params.darkMode;
  ...
  const formatNumber = n => {
    ...
    else if (Number.isInteger(n)) return res;
    else res = n.toFixed(2);
    return res;
  };
  ...
  const renderGenericWidget = function(params){
    ...
    if (!$root[0]) {
      $root = $(rootTagName, {
        id: rootId,
        class: 'xt-ke-card xt-widget-generic'
      });
      let $addTo = $(params.parentSelector);
      $root[addMethod]($addTo);
      if (params.onAdded) params.onAdded($root);
  ...
  const getIcon = function(icon) {
    switch (icon) {
      ...
      case 'screenshot':
        return `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="currentColor" stroke-width="1.5"><rect x="2" y="2" width="20" height="20" fill="none" stroke="none"/><path d="M18 7H15.83L14 5H10L8.17 7H6C4.9 7 4 7.9 4 9V17C4 18.1 4.9 19 6 19H18C19.1 19 20 18.1 20 17V9C20 7.9 19.1 7 18 7Z" stroke-linecap="round" stroke-linejoin="round"/><circle cx="12" cy="13" r="3" stroke-linecap="round" stroke-linejoin="round"/><circle cx="18" cy="6" r="1" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
      case 'star':
        return `<svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>`;
    }
  };
  ...
  return {
    ...
    setPlan: setPlan,
    getPlan: getPlan,
    getIsValidAPIKey: getIsValidAPIKey,
    subscriptionPromoCondition: subscriptionPromoCondition,
    setCountry: setCountry,
    getCountry: getCountry,
    clearKeywordsStorage: clearKeywordsStorage,
    addKeywords: addKeywords,
    addCopyExportButtons: addCopyExportButtons,
