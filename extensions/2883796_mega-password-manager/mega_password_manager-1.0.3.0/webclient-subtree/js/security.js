...
var security = {
    ...
    persistAccountSession(masterKeyArray32, decryptedSessionIdBase64, decodedPrivateRsaKey) {
        'use strict';
        const temp = security.login.rememberMe === false && !localStorage.sid && !is_pwm;
...
security.login = {
     ...
     */
    checkLoginMethod: async function(email, password, pinCode, rememberMe, oldStartCallback, newStartCallback) {
        'use strict';
        ...
        // Fetch the user's salt from the API
        const res = await security.fetchAccountVersionAndSalt(email).catch(tell);
        ...
        if (res.version === 2) {
            return newStartCallback(email, password, pinCode, rememberMe, res.salt);
        }

        // Otherwise using the old method
        if (res.version === 1) {
            return oldStartCallback(email, password, pinCode, rememberMe);
        }
    },
            ...
            callback: function(result) {
                ...
                if (typeof result === 'object') {
                    ...
                    if (typeof temporarySessionIdBase64 !== 'undefined') {
                        // We don't have key generation in Extension so return incomplete error
                        security.login.loginCompleteCallback(EINCOMPLETE);
                    }
                    else {
    ...
    checkForCommonErrors: function(result, oldStartLoginCallback, newStartLoginCallback) {
        ...
        // Check for suspended account, incomplete registration
        if (result === EBLOCKED || result === EINCOMPLETE) {
            return result;
        }
