...
"use strict";
/*!******************************************************!*\
  !*** ./src/background/downloader-v2.js + 13 modules ***!
  \******************************************************/
...
/* harmony default export */ const config = ({
  ...
  ffmpegCoreVersion: "0.12.6",
  wasmFFmpegCacheName: "ffmpeg-0.12.6.wasm",
  exclude: "gvRn,*Screenshot",
  manifest: "v2",
  wasm: "include",
  ...
  getUtmUrl: "https://r.clckgo.online/user",
  socialMediaSharingUrl: "https://r.clckgo.online/udl-helper?share=1",
  findOrCreateUrl: "https://api.videodlservice.com/api/storage/findOrCreate",
  shortUrl: "https://r.clckgo.online/r/{%bundleIncrement%}",
  serviceUrl: "https://r.clckgo.online/vid/{%encodedString%}?{%addFrom%}",
  contactUsUrl: "https://r.clckgo.online/contact-us?{%addFrom%}",
  desktopClientUrl: "https://r.clckgo.online/udl-client-youtube-downloader-and-converter",
  ...
  uninstallLandingUrl: "https://r.clckgo.online/post-remove-helper",
  onlineVideoPlatform: "YouTube",
  ratingUrl: "https://addons.mozilla.org/ru/firefox/addon/youtube-downloader-udl-helper/",
  privacyUrl: "https://r.clckgo.online/privacy-helper",
  eulaUrl: "https://r.clckgo.online/eula-helper",
  removeHelperUrl: "https://r.clckgo.online/remove-helper",
  modules: [
    {
      path: "gv",
      name: "gv"
...
;// CONCATENATED MODULE: ./src/utils/ffmpeg/createFFmpeg-ff.js
const createFFmpeg = (_options = {}) => {
  ...
  /* utils/log */
  let _logging = false;
  let customLogger = (...args) => console.log("customLogger: ", args);
  ...
  const detectCompletion = (message, progressRatio) => {
    if (progressRatio === 1 || (message.startsWith("Aborted(") && runResolve !== null)) {
      runResolve();
      runResolve = null;
      running = false;
    }
   ...
   * Load ready wasm binary from options.ffmpegWasmBinary
   *
  ...
  const load = async () => {
    ...
    if (Core === null) {
      ...
      Core = await _options.createFFmpegCore({ wasmBinary: options.ffmpegWasmBinary });
      Object.assign(Core, {
        // In current wasm build logs received with `type="stderr"`! Nothing with `type="stdout"`
        logger: parseMessage, // receive logs from `printErr` and `print`

        // Progress calculation:
        // https://github.com/ffmpegwasm/ffmpeg.wasm/blob/ae1cda/src/fftools/ffmpeg.c#L1637-L1653
        // Quote:
        // > only works when the duration of
        // > input and output file are the same, other cases (ex. trim)
        // > still WIP.
        progress: ({ progress: r, time }) => { // receive progress from wasm
          progress({ ratio: r, time });
          detectCompletion("", r); // detect complete by progress
        },
      });
  ...
  const run = (..._args) => {
    ...
    if (Core === null) {
      ...
      return new Promise((resolve) => {
        const args = _args.filter(s => s.length !== 0);
        runResolve = resolve;
        Core.exec(...args);
        const r = Core.ret;
        Core.reset();
...
/* harmony default export */ const createFFmpeg_ff = (createFFmpeg);

;// CONCATENATED MODULE: ./src/utils/ffmpeg/ffmpeg-core-ff.js
const createFFmpegCore = __webpack_require__(/*! @ffmpeg/core */ 22); // single-thread udm ffmpeg.wasm core

/* harmony default export */ const ffmpeg_core_ff = (createFFmpegCore);

;// CONCATENATED MODULE: ./src/utils/ffmpeg/get-wasm-code-include.js
// if you have to change here something don't forget to have a look at /src/utils/idbfs-ff.js

/* eslint-disable no-undef */
const runtime = chrome ? chrome.runtime : browser.runtime;
/* eslint-enable no-undef */

const getWasmCode = async () => {
  const response = await fetch(runtime.getURL("/ffmpeg-core.wasm"));
  const arrayBuffer = await response.arrayBuffer();
  return new Uint8Array(arrayBuffer);
};
/* harmony default export */ const get_wasm_code_include = (getWasmCode);

;// CONCATENATED MODULE: ./src/utils/idbfs-ff.js
// fake idbfs for firefox
// works ONLY if wasm blob is web accessible (see manifest and src/assets)
// otherwise couse downloading over 20Mb EVERY time when ffmpeg worker starts




const { wasmFFmpegCacheName } = config;
// eslint-disable-next-line func-names
const getIDBFS = function () {
  const idbfs = {
    deleteFile() {
    ...
    deleteFiles() {},
    existsFile(id) {
      return Promise.resolve(id === wasmFFmpegCacheName);
    },
    // External API
    readFile(id) {
      return new Promise((resolve, reject) => {
        if (id === wasmFFmpegCacheName) {
          resolve(get_wasm_code_include());
        } else {
...
/* harmony default export */ const idbfs_ff = (getIDBFS);

;// CONCATENATED MODULE: ./src/background/ffmpeg/worker.js
/* eslint-disable no-restricted-globals  */
const worker = (createFFmpeg, createFFmpegCore) => {
  const fakeWorker = {
    onmessage: () => {},
    postMessage: () => {},
  };
  const postMessage = (...argv) => { fakeWorker.onmessage(...argv); };
  ...
  const closeFiles = async () => {
    ...
    await ffWorker.FS("close", fHandlers["media.ext"]);
    await ffWorker.FS("close", fHandlers["audio.m4a"]);
    await ffWorker.FS("close", fHandlers["audio.mp4"]);
    await ffWorker.FS("close", fHandlers["video.mp4"]);
    await ffWorker.FS("close", fHandlers["audio.webm"]);
    await ffWorker.FS("close", fHandlers["video.webm"]);
  };
  ...
  const convert = inFormat => outFormat => (async () => {
    const ffWorker = await ctx.ffWorker;
    await closeFiles();
    const args = [
      "-i", `video.${inFormat}`,
    ...
    await ffWorker.run(...args);
    return ffWorker.FS("readFile", `output.${outFormat}`);
  });

  const convertMp3 = async (bitrate) => {
    const ffWorker = await ctx.ffWorker;
    await closeFiles();
    const args = [
      "-i",
      "audio.m4a",
      "-acodec", "libmp3lame", "-ab", bitrate ? `${bitrate}k` : "128k",
      "output.mp3",
    ];
    await ffWorker.run(...args);
    return ffWorker.FS("readFile", "output.mp3");
  };

  const converters = {
    "mp4->mp4": convert("mp4")("mp4"),
    "mp4->webm": convert("mp4")("webm"),
    "webm->mp4": convert("webm")("mp4"),
    "webm->webm": convert("webm")("webm"),
  };

  onmessage = async (msg) => {
    const sendConvertedData = data => postMessage({ data: data.buffer, type: "converted" }, [data.buffer]);
    ...
    if (msg.type === "init") {
      ...
      ctx.ffWorker = new Promise(async (resolve, reject) => {
        try {
          ...
          window._scriptDir = typeof document !== "undefined" && document.currentScript ? document.currentScript.src : undefined;
          const _createFFmpeg = createFFmpeg;
          const result = _createFFmpeg({
            // log: true, // enable ffmpeg logs in console
            createFFmpegCore,
            ffmpegWasmBinary: msg.wasm,
            progress: (progress) => {
              if (ctx.cancelCalled) return;
              postMessage({
      ...
      try {
        // if (Math.random() > 0.7) {
        //   postMessage({ type: "startFailed" });
        //   return;
        // }
        const ffWorker = await ctx.ffWorker;
        if (!ffWorker.isLoaded()) {
          await ffWorker.load();
        }
        fHandlers["media.ext"] = await ffWorker.FS("open", "media.ext", "a");
        fHandlers["audio.m4a"] = await ffWorker.FS("open", "audio.m4a", "a");
        fHandlers["audio.mp4"] = await ffWorker.FS("open", "audio.mp4", "a");
        fHandlers["video.mp4"] = await ffWorker.FS("open", "video.mp4", "a");
        fHandlers["audio.webm"] = await ffWorker.FS("open", "audio.webm", "a");
        fHandlers["video.webm"] = await ffWorker.FS("open", "video.webm", "a");
    ...
    if (msg.type === "chunk") {
      ...
      // DOES'T WORK BY SOME REASONS!!!
      // if (!fHandlers[path]) {
      //   fHandlers[path] = await ffWorker.FS("open", path, "a");
      // }
      const uint8Arr = new Uint8Array(chunk);
      await ffWorker.FS("write", fHandlers[path], uint8Arr, 0, uint8Arr.length);
      return;
    }
    ...
    if (msg.type === "readFile") {
      try {
        ...
        const { path } = msg;
        const data = ffWorker.FS("readFile", path);
        await ffWorker.FS("unlink", path);
        await closeFiles();
        sendConvertedData(data);
      } catch (e) {
        console.log(e);
        error(e);
      }
    ...
    if (msg.type === "mp4->mp3") {
      ...
      try {
        ...
        const data = await convertMp3(bitrate);
        sendConvertedData(data);
      } catch (e) {
        console.log(e);
        error(e);
    ...
    if (msg.type.includes("->")) {
      if (!(msg.type in converters)) {
        error(`converter ${msg.type} not found`);
        return;
      }
      try {
        const ffWorker = await ctx.ffWorker;
        ...
        const conversionFunction = converters[msg.type];
        const data = await conversionFunction();
        sendConvertedData(data);
      } catch (e) {
        console.log("ERROR!!!", e);
        error(e);
  ...
  fakeWorker.postMessage = onmessage;
  return fakeWorker;
};
/* eslint-enable no-restricted-globals */

/* harmony default export */ const ffmpeg_worker = (worker);

;// CONCATENATED MODULE: ./src/background/ffmpeg/start-worker.js
// local imports
// utils
...
// let workerBody;
const { wasmFFmpegCacheName: start_worker_wasmFFmpegCacheName } = config;
const fs = idbfs_ff();
const files = [
  start_worker_wasmFFmpegCacheName,
];

const allFilesPresents = async () => (
  (await Promise.all(files.map(path => fs.existsFile(path)))).every(result => result)
);

const createFiles = async () => {
  const wasm = get_wasm_code_include();
  const rawData = await wasm;
  await fs.writeFile(start_worker_wasmFFmpegCacheName, rawData);
};

const removeFiles = () => (
  files.map(path => fs.deleteFile(path))
);

const startWorker = (progress, getSharedState) => {
  const ctx = {
    ffmpegWorker: null,
    reloadAttempt: 0,
  };

  const tryToCreate = async (resolve, reject) => {
    if (ctx.reloadAttempt > 4) {
      console.log("start failed!!! second attempt");
      ctx.reloadAttempt = 0;
      reject();
      return;
    }
    ...
    try {
      if (!await allFilesPresents()) await createFiles();
      // if (!workerBody) {
      //   const js = `
      //     const createFFmpeg = (${createFFmpeg.toString()});
      //     const createFFmpegCore = (${createFFmpegCore.toString()});
      //     (${worker.toString()})(createFFmpeg, createFFmpegCore);`;
      //   const blob = new Blob([js], { type: "text/javascript" });
      //   workerBody = URL.createObjectURL(blob);
      // }
      ctx.ffmpegWorker = ffmpeg_worker(createFFmpeg_ff, ffmpeg_core_ff);
      ctx.ffmpegWorker.onmessage = async (e) => {
        if (e.type === "started") {
          console.log("STARTED!!!");
          resolve(ctx.ffmpegWorker);
          return;
        }
        if (e.type === "startFailed") {
          console.log("start failed!!! first attempt");
          ctx.ffmpegWorker.terminate();
          ctx.ffmpegWorker = undefined;
          await removeFiles();
          tryToCreate(resolve, reject);
          return;
        }
        if (e.type === "converted") {
          const { cancelCalled, resolveStream } = getSharedState();
          if (!cancelCalled) {
            resolveStream(e.data);
          }
        }
        if (e.type === "progress") {
          progress(e.progress);
      ...
      const wasm = await fs.readFile(start_worker_wasmFFmpegCacheName);
      ctx.ffmpegWorker.postMessage({ type: "init", wasm: wasm.buffer }, [wasm.buffer]);
    } catch (e) {
      await removeFiles();
      console.log("ERROR!!!", e);
      tryToCreate(resolve, reject);
  ...
  return new Promise(async (resolve, reject) => {
    await tryToCreate(resolve, reject);
  });
};

/* harmony default export */ const start_worker = (startWorker);

;// CONCATENATED MODULE: ./src/background/dispatcher.js
// local imports
// utils
...
const dispatch = (msg, ctx) => {
  ...
  if (type.includes("->") && workers[msg.token]) {
    ...
    workers[msg.token].worker.then((worker) => {
      worker.postMessage({ bitrate: msg.bitrate, type });
  ...
  const getMuxingProgress = token => (progress) => {
    sendMessage({ msg: { progress, token }, type: "muxingProgress" });
  };
  ...
  case "createFFMPEGInstance":
    ...
    workers[token] = {
      sharedState,
      worker: start_worker(getMuxingProgress(token), getSharedState),
    };
  ...
  case "get":
    ...
    break;
  case "set":
    if (uninstallLandingUrl && key === "utm") {
      setUninstallUrlWithUTM(value);
    }
...
/* harmony default export */ const dispatcher = (dispatch);

;// CONCATENATED MODULE: ./src/background/send-message-to-all.js
/* eslint-disable no-undef */
...
;// CONCATENATED MODULE: ./src/background/downloader-v2.js
// local imports
// config
...
/* eslint-disable no-undef */
const downloader_v2_runtime = chrome ? chrome.runtime : browser.runtime;
const downloader_v2_tabs = chrome ? chrome.tabs : browser.tabs;
/* eslint-enable no-undef */

const { uninstallLandingUrl } = config;

const setDefaultUninstallUrl = () => downloader_v2_runtime.setUninstallURL(uninstallLandingUrl);
const setUninstallUrlWithUTM = utm => downloader_v2_runtime.setUninstallURL(add_utm(uninstallLandingUrl, utm));
...
const langCheck = () => {
  if (typeof localStorage.lang !== "string") {
    localStorage.lang = "autoDetect";
  }
};
langCheck();
...
const downloader_v2_dispatcher = (message, port) => {
...
const onConnect = (port) => {
  port.onMessage.addListener((message) => {
    downloader_v2_dispatcher(message, port);
  });
};

downloader_v2_runtime.onConnect.addListener(onConnect);
  ...
  return (msg) => {
    ...
    if (type === "getContent") {
      try {
        ...
        getContent(url, { abortController, timeout })
...
downloader_v2_runtime.onConnect.addListener(function (port) {
