...
var settings = {
	...
	test_order: "DU_P", //order in which tests will be performed as a string. D=Download, U=Upload, P=Ping+Jitter, I=IP, _=1 second delay
	...
	xhr_ul_blob_megabytes: 1, //size in megabytes of the upload blobs sent in the upload test (forced to 4 on chrome mobile)
	...
	domain: "https://ping.ezyspeedtest.com/"
};
...
function getIp(done) {
	...
	xhr.open("GET", settings.domain + settings.url_getIp + url_sep(settings.url_getIp) + (settings.mpot ? "cors=true&" : "") + (settings.getIp_ispInfo ? "isp=true" + (settings.getIp_ispInfo_distance ? "&distance=" + settings.getIp_ispInfo_distance + "&" : "&") : "&") + "r=" + Math.random(), true);
	xhr.send();
}
	...
	var testStream = function(i, delay) {
		setTimeout(
			function() {
				...
				xhr[i].open("GET", settings.domain + settings.url_dl + url_sep(settings.url_dl) + (settings.mpot ? "cors=true&" : "") + "r=" + Math.random() + "&ckSize=" + settings.garbagePhp_chunkSize, true); // random string to prevent caching
				xhr[i].send();
			}.bind(this),
			1 + delay
		);
	}.bind(this);
	/* open streams, for download test , creating parallel 6 multistreams simultaneoulsy
	 The test uses multiple streams. If a stream finishes the download, it is restarted. 
	The amount of downloaded data for each stream is tracked using the XHR Level 2 `onprogress` event.
	The test streams are not perfectly synchronized because we don't want them to finish all at
	 the same time if they do. 
	Every 200ms, a timer updates the `dlStatus` string with the current speed and calculates a 
	"bonus" time by which to shorten the test depending on how high the speed is
	*/
...
function ulTest(done) {
	...
	// we create a 1MB buffer with a Uint32Array view referring to the buffer
	var r = new ArrayBuffer(1048576); // payload
			...
			setTimeout(
				function() {
					...
					if (ie11workaround) {
						...
						// send xhr
						xhr[i].open("POST", settings.domain + settings.url_ul + url_sep(settings.url_ul) + (settings.mpot ? "cors=true&" : "") + "r=" + Math.random(), true); // random string to prevent caching
						try {
							xhr[i].setRequestHeader("Content-Encoding", "identity"); // disable compression (some browsers may refuse it, but data is incompressible anyway)
						} catch (e) {}
						//No Content-Type header in MPOT branch because it triggers bugs in some browsers
...
function pingTest(done) {
	...
	var doPing = function() {
		...
		xhr[0].open("GET", settings.domain + settings.url_ping + url_sep(settings.url_ping) + (settings.mpot ? "cors=true&" : "") + "r=" + Math.random(), true); // random string to prevent caching
		xhr[0].send();
	}.bind(this);
