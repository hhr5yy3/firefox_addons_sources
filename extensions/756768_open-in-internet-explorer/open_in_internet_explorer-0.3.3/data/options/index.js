...
function restore() {
  ...
  chrome.storage.local.get({
    ...
    $('custom-script').value = prefs['custom-script'];
  });
}
...
function save() {
  ...
  const urls = $('urls').value.split(/\s*,\s*/).filter((h, i, l) => h && l.indexOf(h) === i);
  ...
  chrome.storage.local.set({
    ...
    'custom-script': $('custom-script').value
  }, () => {
    restore();
    const status = $('status');
...
if (typeof URLPattern === 'undefined') {
  document.getElementById('usp').classList.add('disabled');
}
