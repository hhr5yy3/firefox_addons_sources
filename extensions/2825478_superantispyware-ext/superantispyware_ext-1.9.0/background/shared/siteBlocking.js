(() => {
  ...
  var TAB_DOMAIN_STORAGE_KEY_SFX = "domain";
  ...
  async function removeFromAllowlist(domain, reason) {
    chrome.storage.session.remove(storeKey(domain, reason));
  }

  // ../common/background/siteBlocking.ts
  var ignoreTab = ({ url, frameId }) => frameId !== 0 || !isHttp(url);
  var domainStoreKey = (tabId) => `${TAB_STORAGE_KEY_PFX}:${tabId}:${TAB_DOMAIN_STORAGE_KEY_SFX}`;
  chrome.webNavigation.onBeforeNavigate.addListener(async (details) => {
    if (!ignoreTab(details)) {
      chrome.storage.session.set({
        [domainStoreKey(details.tabId)]: getDomain(details.url)
      });
    }
  });
  chrome.tabs.onRemoved.addListener(async (tabId) => {
    const key = domainStoreKey(tabId);
    const domain = await getStoredData(key, "session");
    if (domain) {
      const allTabs = (
        // Promisify for MV2
        await new Promise((res) => chrome.tabs.query({}, res))
      );
      const otherTabs = allTabs.filter((tab) => tab.id !== tabId);
      await Promise.all([
        manageAllowlisting(domain, otherTabs, SiteBlockReason_default.ContentFilter),
        manageAllowlisting(domain, otherTabs, SiteBlockReason_default.HighRisk)
      ]);
      chrome.storage.session.remove(key);
    }
  });
  async function manageAllowlisting(domain, otherTabs, siteBlockReason) {
    const allowlisted = await isAllowlisted(domain, siteBlockReason);
    if (allowlisted) {
      ...
      removeFromAllowlist(domain, siteBlockReason);
    }
  }
})();
