const WEBSTORE_LINK = `https://addons.mozilla.org/firefox/addon/adblock-for-youtube-tm/`;
 ...
 */
 class SkipVideoAds {
  constructor( enabled = false) {
    this.enabled = enabled
    this.vObserver = false
    if( enabled)  this.MountObserver()
  }
  ...
  MountObserver() {
    domReady( () => {
      waitForProm( '.html5-video-player', document.body).then( player => {
        ...
        this.vObserver = new MutationObserver( check )
        this.vObserver.observe( player, {
  ...
  UnmountObserver() {
    if( this.vObserver != false) {
      this.vObserver.disconnect()
      this.vObserver = false
    }
  }
