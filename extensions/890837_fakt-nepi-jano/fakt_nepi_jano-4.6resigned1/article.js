...
utils.getArticleVideos = function(html) {
	...
	while (i--) {
		...
		utils.getArticleVideo(iosVideos[i], 'https://www.sme.sk/storm/mmdata_get.asp?id=' + iosVideoId + '&hd1=1');
	}
};
...
utils.getArticle = function(url) {
	...
	request.setRequestHeader('User-Agent', ''); /* The server blocks connections from standardly recognized agents */
	request.send();
};
