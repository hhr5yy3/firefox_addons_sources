...
TspTransportProtocol.prototype.Start = function ()
{
  ...
  if (!this.onMiniguiExecuteUrl)
    throw this.constructor.name + '.Start() Event onMiniguiExecuteUrl is undefined!';
...
TspTransportProtocol.prototype.MiniguiExecuteUrl = function (Url)
{
  if (!Url)
    return;
  if (this.onMiniguiExecuteUrl)
    this.onMiniguiExecuteUrl(Url);
};

TspTransportProtocol.prototype.CloseMinigui = function ()
{
  var message = {};
  message.Action = 'CloseMinigui';
  this.PostMiniguiMessage(message);
};
...
TspTransportProtocol.prototype.mgOnMessage = function (message)
{
  ...
  switch (message.Action)
  {
    ...
    default:
      if (message.type == 'ExecuteUrl')
        this.MiniguiExecuteUrl(message.params.Url);
      else if (this.MiniguiProtocol)
  ...
  try
  {
    switch (this.Protocol.ProtocolState)
    {
      ...
      case tpsAuthenticated:
        ...
        {
          if (this.Protocol.MiniguiProtocol)
            this.Protocol.MiniguiProtocol.tsOnMessage(message.data);
          else if (this.Protocol.Log.Info)
            spLog.logError(
              this.constructor.name + '.tsOnMessage() ' +
              'MiniguiProtocol is undefined for "' + message.data.type + '" message!'
            );
        }
        else
          this.Protocol.onMessage(message);
