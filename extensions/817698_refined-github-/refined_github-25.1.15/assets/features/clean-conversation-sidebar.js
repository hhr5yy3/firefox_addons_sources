...
const canEditSidebar = () => elementExists('.discussion-sidebar-item [data-hotkey="l"]');

function getNodesAfter(node) {
...
async function cleanSidebar() {
	...
	if (assignees.children.length === 0) {
		...
		if (assignYourself) {
			...
			expectElement('[aria-label="Select assignees"] summary').append(
				React.createElement('span', { style: {fontWeight: 'normal'},}, " – "  , assignYourself),
			);
	...
	if (createBranchLink && !openWorkspaceButton) {
		...
		expectElement('[aria-label="Link issues"] summary').append(
			React.createElement('span', { style: {fontWeight: 'normal'},}, " – "  , createBranchLink),
		);
	}
