const tabTypes = {};
const XML_TYPES = ['text/xml', 'application/xml;charset=utf-8'];
...
chrome.contextMenus.create({
    "title": "XML code to Viewer",
    "contexts": ["selection"],
    "onclick": (info, tab) => {
        var textToWrite = info.selectionText,
            content = textToWrite;
        var file = new File([vkbeautify.xml(content)], null, {
        ...
        fetch(URL.createObjectURL(file), myInit)
            .then(response => {
                browser.tabs.create({
                    url: response.url
                });
