	...
	xmlHttp.onreadystatechange = function() {
		if (xmlHttp.readyState == 4 && xmlHttp.status == 200 && xmlHttp.response != null){
			...
			if(jsonData != null && jsonData.length > 0 && jsonData[0].result == "success"){
				...
				document.getElementById("tab-container-2").innerHTML = DOMPurify.sanitize(jsonData[0].data[0].MESSAGE, {ADD_ATTR: ['target']});
			}
		}
	}
	xmlHttp.open("GET", 'https://www.oinkandstuff.com/oink-remote/api/v1/RemoteMessage?app=FirefoxOptionsGeneric', true); // true for asynchronous 
	xmlHttp.send(null);	
