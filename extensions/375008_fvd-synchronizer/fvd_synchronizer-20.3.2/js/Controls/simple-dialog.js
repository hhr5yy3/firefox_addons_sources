...
Dialog.prototype = {
	...
	open: function() {
		...
		ws.top = (document.body.scrollTop || document.documentElement.scrollTop || window.scrollY) + this.options.top + 'px';
	...
	setTitle: function(title) {
		...
		//this.header.innerHTML = title; // Task #1708
		this.header.innerText = String(title);
	},
	...
	_makeNodes: function() {
		...
		if (typeof this.options.title == 'string' && this.options.title != '') {
			var header = document.createElement('div');
			header.className = 'dialog-header';
			//header.innerHTML = this.options.title;// Task #1708
			header.innerText = String(this.options.title);
			this.header = header;
		}
	...
	_makeButtons: function(buttons) {
		...
		for (var buttonText in buttons) {
			...
			button.innerText = String(buttonText);
