var Tool = (function(){
  ...
  var processChildList = function(children, mutation){
    ...
    for (var i = 0, len = children.length; i < len; i++) {
      ...
      else if (node.tagName === 'ESS-CELL' && node.className && node.className.indexOf && node.className.indexOf('xt-col') === -1) {
        found = true;
        var $parent = $(node).closest('.particle-table-row');
        if ($parent.find('.xt-col')[0]) continue;
        var $target = $parent.find('[essfield=text]');
        if (!$target[0]) {
          $target = $parent.find('[essfield=keyword]');
        }
        var metricsList = Starter.getSettings().metricsList;
        if(metricsList.trend) {
          $target.after('<ess-cell role="grid-cell" tabindex="-1" class="xt-col"></ess-cell>');
