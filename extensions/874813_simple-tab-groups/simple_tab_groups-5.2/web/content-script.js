...
;// CONCATENATED MODULE: ./js/hotkeys.js
...
const NOT_SUPPORTED_CODE_KEYS = new Set([
...
function isValidHotkeyEvent(event) {
...
function isValidHotkeyValue(value) {
    const parts = value.split('+');
    return (parts.length >= 2 && parts.every(key => key)) || isFuncKeyValue(value);
}

function eventToHotkeyValue(event) {
    const valueParts = [];
...
;// CONCATENATED MODULE: ./web/content-script.js
