...
import ArrowDownIcon from '../npm/octicons-plain-react-components-ArrowDown.js';
import CheckCircleFillIcon from '../npm/octicons-plain-react-components-CheckCircleFill.js';
...
const getPositiveReactions = memoize((comment) => {
...
function highlightBestComment(bestComment) {
	expectElement('.unminimized-comment', bestComment).classList.add('rgh-highest-rated-comment');
	expectElement('.unminimized-comment .timeline-comment-header > h3', bestComment).before(
...
, React.createElement(CheckCircleFillIcon, null )
),
	);
}

function linkBestComment(bestComment) {
	...
	const text = expectElement('.comment-body', bestComment).textContent.slice(0, 100);
	const {hash} = expectElement('a.js-timestamp', bestComment);
	const avatar = expectElement('img.avatar', bestComment).cloneNode();

	bestComment.parentElement.firstElementChild.after(
		React.createElement('a', { href: hash, className: "no-underline rounded-1 rgh-highest-rated-comment timeline-comment color-bg-subtle px-2 d-flex flex-items-center"       ,}
, avatar

, React.createElement('h3', { className: "timeline-comment-header-text f5 color-fg-muted text-normal text-italic css-truncate css-truncate-overflow mr-2"       ,}
, React.createElement('span', { className: "Label mr-2" ,}, "Highest-rated"), text
)

, React.createElement('div', { className: "color-fg-muted f6 no-wrap"  ,}
, React.createElement(ArrowDownIcon, { className: "mr-1",} ), "Jump to comment"
)
),
	);
}
