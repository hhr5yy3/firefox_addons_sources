...
config.post = {
  ...
  action: (d, tab) => getCookies(d.referrer).then(async cookies => {
    ...
    const referrer = d.referrer || tab.url;
    if (referrer) {
      body.append('referer', referrer);
    }
...
const locales = {
  extract: 'No link can be extracted from the text you have selected',
  empty: 'The command box is currently empty. Please use the options page to define a command.',
  path: 'JDownloader cannot be located. Go to the options page and provide the path.',
  permission: 'Allow this permission to extract links from all iframes on this page',
  help: `
      ...
      getCookies(d.referrer).then(cookies => {
        ...
        chrome.runtime.sendNativeMessage('com.add0n.native_client', {
          ...
          if (!res) {
            ...
            return reject(Error('There was no response. Have you installed the native client?'));
          }
          else if (res.code !== 0) {
