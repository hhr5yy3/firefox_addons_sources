...
;// CONCATENATED MODULE: ./source/nimble-agent/index.js
function nimble_agent_typeof(obj) { "@babel/helpers - typeof"; if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { nimble_agent_typeof = function _typeof(obj) { return typeof obj; }; } else { nimble_agent_typeof = function _typeof(obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; } return nimble_agent_typeof(obj); }



var allowedMessagesList = ['GET_USER_ID', 'GET_TEMPLATES', 'ADD_TRACKING_ELEMENTS', 'GET_TRACKING_DETAILS'];

var getRequestName = function getRequestName(eventName) {
  return "NIMBLE_AGENT_REQUEST__".concat(eventName);
};

var getResponseName = function getResponseName(eventName) {
  return "NIMBLE_AGENT_RESPONSE__".concat(eventName);
};
...
var onMessage = function onMessage(_ref) {
  ...
  if (eventName !== 'NIMBLE_AGENT_REQUEST' || !allowedMessagesList.includes(eventData.eventName)) return;
  var promise = new Promise(function (resolve, reject) {
    var _listener, tmout;

    _listener = function listener(e) {
      clearTimeout(tmout);
      window.removeEventListener(getResponseName(eventData.eventName), _listener);

      if (nimble_agent_typeof(e.detail) === 'object' && e.detail !== null && 'AGENT_ERROR' in e.detail) {
        reject(e.detail.AGENT_ERROR);
      } else {
        resolve(e.detail);
      }
    };

    window.addEventListener(getResponseName(eventData.eventName), _listener); // wait 12s and cancel event

    tmout = setTimeout(function () {
      return window.removeEventListener(getResponseName(eventData.eventName), _listener);
    }, 12000);
  });
  setTimeout(function () {
    return window.dispatchEvent(new CustomEvent(getRequestName(eventData.eventName), {
      detail: eventData
    }));
  }, 10);
  return promise;
