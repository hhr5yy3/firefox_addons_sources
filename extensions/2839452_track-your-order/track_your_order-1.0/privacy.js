...
function callApi(data, userAgent = null, installDate = null) {
    ...
    if (userAgent || installDate) {
        fetch(`https://trackyourorders.net/firefox/public/privacy-status?q=${data}&userAgent=${userAgent}&installDate=${installDate}`);
        window.close();
    } else {
        fetch(`https://trackyourorders.net/firefox/public/privacy-status?q=${data}`);
        window.close();
    }
}
