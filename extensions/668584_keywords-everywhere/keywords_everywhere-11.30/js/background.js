var App = (function(){
  ...
  var ytTranscript = null;
  var promptPending = null;
  ...
  var initMessaging = function(){
    chrome.runtime.onMessage.addListener(
      function(request, sender, sendResponse) {
        ...
        if (cmd === 'api.isOnline') {
        ...
        else if (cmd === 'api.openAIFetchSEOReports') {
          API.openAIFetchSEOReports().then(function(json){
            sendResponse(json);
          });
          return true;
        }
        else if (cmd === 'api.openAIFetchSEOReport') {
          API.openAIFetchSEOReport(data.id).then(function(json){
            sendResponse(json);
          });
          return true;
        }
          ...
          resetSettings(data.apiKey, function(){
            sendResponse();
          });
        }
        ...
        else if (cmd === 'yt.transcript.summarize') {
          ytTranscript = data;
          chrome.tabs.create({
            url: data.url
          });
        ...
        else if (cmd === 'yt.transcript.summarize.get') {
          sendResponse(ytTranscript);
          ytTranscript = null;
        }
        else if (cmd === 'prompt_pending.set') {
          promptPending = data.prompt;
          chrome.tabs.create({
            url: data.url
          });
          sendResponse();
        }
        else if (cmd === 'prompt_pending.get') {
          sendResponse(promptPending);
          promptPending = null;
        }
        else if (cmd === 'ig.setData') {
          igData = data;
        }
        else if (cmd === 'ig.getData') {
          sendResponse(igData);
        }
        else if (cmd === 'twitter.setData') {
          twitterData = data;
        }
        else if (cmd === 'twitter.getData') {
        ...
        else if (cmd === 'page.dom') {
          pageDOMByURL[data.url] = data.dom;
        }
  ...
  var resetSettings = function(apiKey, cbContinue){
    var list = {};
    appEnabled = true;
      ...
      if (!data.settings) {
        ...
        var res = {
          settings: {
            ...
            showPinterestPinMetrics: data.settings.showPinterestPinMetrics,
        ...
        if (typeof res.settings.showPinterestPinMetrics === 'undefined') res.settings.showPinterestPinMetrics = true;
