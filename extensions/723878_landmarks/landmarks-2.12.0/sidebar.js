(function () {
    ...
    dismissedSidebarNotAlone: {
      ...
      showOrHide: function (wasDismissed) {
        ...
        browser.storage.sync.get(defaultInterfaceSettings, function (items) {
          "popup" !== items.interface || wasDismissed
            ? hideNote("note-ui")
            : showNote("note-ui");
        });
      },
  ...
  const notes = Object.assign({}, _sidebarNote, _updateNote);
  ...
  function reflectInterfaceChange(ui) {
    browser.storage.sync.get(defaultDismissedSidebarNotAlone, function (items) {
      if (false === items.dismissedSidebarNotAlone) {
        switch (ui) {
          case "sidebar":
            hideNote("note-ui");
            break;
  ...
  function setupNotes() {
    ...
    browser.storage.onChanged.addListener(function (changes) {
      changes.hasOwnProperty("interface") &&
        reflectInterfaceChange(
          changes.interface.newValue ?? defaultInterfaceSettings.interface
        );
