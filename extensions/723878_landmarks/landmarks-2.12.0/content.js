(function () {
  ...
  function LandmarksFinder$1(win, doc, _testUseHeuristics) {
    // Constants
    ...
    let useHeuristics = _testUseHeuristics;
    // parameter is only used by tests
    ...
    function getLandmarks(element, depth, parentLandmark) {
      ...
      const explicitRole = rawRoleValue
        ...
        : null;
      const hasExplicitRole = null !== explicitRole;
    ...
    function isLandmark(role, explicitRole, label) {
      ...
      return true;
      // already a valid role if we were called
    }
  ...
  const landmarksFinderStandard = new LandmarksFinder$1(window, document);
  const landmarksFinderDeveloper = new LandmarksFinder(window, document);
  ...
  function handleHighlightMessage(index, action, actionParam) {
    browser.storage.sync.get(defaultBorderSettings, function (items) {
      if (
        ...
        ("persistent" === items.borderType &&
          landmarksFinder.getCurrentlySelectedIndex() === index)
      ) {
        return;
      }
    ...
    switch (message.name) {
      ...
      case "show-landmark":
        handleHighlightMessage(
          ...
          landmarksFinder.getLandmarkElementInfoWithoutUpdatingIndex(
            message.index
          )
        );
        break;
  ...
  function doUpdateOutdatedResults() {
    let outOfDate = false;
    if (null !== observerReconnectionScanTimer) {
  ...
  function startUpTasks() {
    ...
    browser.storage.onChanged.addListener(function (changes) {
      if ("guessLandmarks" in changes) {
        const setting =
          changes.guessLandmarks.newValue ??
    ...
    document.hidden || observeMutations();
