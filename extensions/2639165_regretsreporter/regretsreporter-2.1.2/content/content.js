!function(modules, entry, mainEntry, parcelRequireName, globalName) {
  ...
  var mainExports = newRequire("hGqht");
  ...
  jnptL: [ function(require, module, exports) {
    require("./helpers/bundle-manifest").register(JSON.parse('{"6vzN4":"content.js"}'));
  }, {
    "./helpers/bundle-manifest": "7NOsp"
  } ],
  "7NOsp": [ function(require, module, exports) {
    "use strict";
    var mapping = {};
    module.exports.register = function(pairs) {
      for (var keys = Object.keys(pairs), i = 0; i < keys.length; i++) mapping[keys[i]] = pairs[keys[i]];
    }, module.exports.resolve = function(id) {
      var resolved = mapping[id];
      if (null == resolved) throw new Error("Could not resolve bundle with id " + id);
      return resolved;
    };
  }, {} ],
  hGqht: [ function(require, module, exports) {
    var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
    parcelHelpers.defineInteropFlag(exports), parcelHelpers.export(exports, "log", (() => log));
    var _webextensionPolyfillTs = require("webextension-polyfill-ts"), _messages = require("../common/messages"), _objectPath = require("object-path"), _reactDom = require("react-dom"), _pageTs = require("bundle-text:./page.ts"), _pageTsDefault = parcelHelpers.interopDefault(_pageTs), _modal = require("./modal"), _react = require("react"), _payloads = require("./payloads"), _common = require("../common/common"), _helpers = require("../common/helpers"), _button = require("./button");
    function log(...args) {
      false;
    }
    let ytApiKey = null, ytRequestHeaders = null;
    async function sendFeedbackRequest(videoId, feedbackToken) {
      log();
      const url = `https://www.youtube.com/youtubei/v1/feedback?key=${ytApiKey}`, rawResponse = await _helpers.universalFetch(url, {
        method: "POST",
        headers: ytRequestHeaders,
        body: JSON.stringify(_payloads.makeFeedbackPayload(videoId, feedbackToken))
      });
      await rawResponse.json();
      log();
    }
    _webextensionPolyfillTs.browser.runtime.onMessage.addListener((async message => {
      if (log(), message.type === _messages.EventType.AuthRecorded && (log(message.keyId), 
      ytApiKey = message.keyId, ytRequestHeaders = message.headers), message.type === _messages.EventType.SendVideoFeedback) switch (message.feedbackType) {
       case _common.FeedbackType.Dislike:
        return async function(videoId) {
          log();
          const url = `https://www.youtube.com/youtubei/v1/like/dislike?key=${ytApiKey}`, rawResponse = await _helpers.universalFetch(url, {
            method: "POST",
            headers: ytRequestHeaders,
            body: JSON.stringify(_payloads.makeDislikePayload(videoId))
          });
          await rawResponse.json(), log();
        }(message.videoId);

       case _common.FeedbackType.NotInterested:
       case _common.FeedbackType.NoRecommend:
        return sendFeedbackRequest(message.videoId, message.feedbackToken);

       case _common.FeedbackType.RemoveFromHistory:
        const feedbackToken = await async function(videoId) {
          log();
          const initDataRegex = /(?:window\["ytInitialData"\]|ytInitialData)\W?=\W?({.*?});/, result = await _helpers.universalFetch("https://www.youtube.com/feed/history", {
            credentials: "include",
            method: "GET",
            mode: "cors"
          ...
          try {
            ...
            const initData = JSON.parse(matchedData[1]), groups = _objectPath.get(initData, "contents.twoColumnBrowseResultsRenderer.tabs.0.tabRenderer.content.sectionListRenderer.contents");
            if (!Array.isArray(groups)) throw new Error("Assert failed");
            let matchingVideo;
            for (const item of groups) {
              const videoRenderers = _objectPath.get(item, "itemSectionRenderer.contents");
              for (const {videoRenderer: videoRenderer} of videoRenderers) if ((null == videoRenderer ? void 0 : videoRenderer.videoId) && videoId === videoRenderer.videoId) {
                matchingVideo = videoRenderer;
                break;
              }
            }
            return matchingVideo ? _objectPath.get(matchingVideo, "menu.menuRenderer.topLevelButtons.0.buttonRenderer.serviceEndpoint.feedbackEndpoint.feedbackToken") : void log();
          } catch (e) {
            throw log(), new Error("Failed to parse YT initial data");
          }
        }(message.videoId);
        return feedbackToken ? sendFeedbackRequest(message.videoId, feedbackToken) : void log();
      }
    })), async function() {
      const scr = document.createElement("script");
      scr.textContent = `(function () {\n\t\tfunction define(m) {\n\t\t\t\tconsole.log("[page loader] page module defined");\n\t\t\t\t// run default export\n\t\t\t\tif (typeof m === 'function') {\n\t\t\t\t\tconst module = m();\n\t\t\t\t\tmodule.default();\n\t\t\t\t}\n\t\t}\n\t\tdefine.amd = true;\n\t\t${_pageTsDefault.default}\n\t})();`, 
      (document.head || document.documentElement).appendChild(scr), setInterval((async () => {
        const cohort = await _common.experimentArm.acquire(), feedbackUiVariantValue = await _common.feedbackUiVariant.acquire(), injectButtons = cohort !== _common.ExperimentArm.NoInject, dataCollectionEnabled = cohort !== _common.ExperimentArm.OptOut, event = {
          type: "ping",
          injectButtons: injectButtons,
          onboardingCompleted: await _common.onboardingCompleted.acquire(),
          dataCollectionEnabled: dataCollectionEnabled,
          feedbackUiVariant: feedbackUiVariantValue
        };
        _button.injectElements(event), window.postMessage(event, window.location.origin);
      }), 2e3);
    }(), window.addEventListener("message", (event => {
      const isAllowedMessage = [ _messages.EventType.VideoViewed, _messages.EventType.VideoBatchRecorded, _messages.EventType.NativeFeedbackSent ].includes(event.data.type);
      if (!(event.origin === window.location.origin) || !isAllowedMessage) return;
      log(event.data);
      const message = event.data;
      message.type === _messages.EventType.VideoBatchRecorded && 0 === message.data.length || _webextensionPolyfillTs.browser.runtime.sendMessage(message);
    })), function() {
      const div = document.createElement("div");
      document.body.append(div), _reactDom.render(_react.createElement(_modal.Modal), div);
    }(), function(url) {
      const elem = document.createElement("link");
      elem.rel = "stylesheet", elem.setAttribute("href", url), document.body.appendChild(elem);
    }(_webextensionPolyfillTs.browser.runtime.getURL("content/content.css"));
  }, {
    "webextension-polyfill-ts": "g4Zvj",
    "../common/messages": "8QxHi",
    "object-path": "9dlrR",
    "react-dom": "cnO8j",
    "bundle-text:./page.ts": "2Xalq",
    "./modal": "8gHJg",
    react: "8Nzqg",
    "./payloads": "8NB0e",
    "../common/common": "jKOy4",
  ...
  "9dlrR": [ function(require, module, exports) {
    var root, factory;
    root = this, factory = function() {
      "use strict";
      var toStr = Object.prototype.toString;
      function hasOwnProperty(obj, prop) {
        return null != obj && Object.prototype.hasOwnProperty.call(obj, prop);
      }
      ...
      function factory(options) {
        var hasShallowProperty, getShallowPropertySafely, objectPath = function(obj) {
          return Object.keys(objectPath).reduce((function(proxy, prop) {
            return "create" === prop || "function" == typeof objectPath[prop] && (proxy[prop] = objectPath[prop].bind(objectPath, obj)), 
            proxy;
          }), {});
        };
        ...
        function set(obj, path, value, doNotReplace) {
          if ("number" == typeof path && (path = [ path ]), !path || 0 === path.length) return obj;
          if ("string" == typeof path) return set(obj, path.split(".").map(getKey), value, doNotReplace);
          var currentPath = path[0], currentValue = getShallowPropertySafely(obj, currentPath);
          return 1 === path.length ? (void 0 !== currentValue && doNotReplace || (obj[currentPath] = value), 
          currentValue) : (void 0 === currentValue && ("number" == typeof path[1] ? obj[currentPath] = [] : obj[currentPath] = {}), 
          set(obj[currentPath], path.slice(1), value, doNotReplace));
        }
        return hasShallowProperty = (options = options || {}).includeInheritedProps ? function() {
          return !0;
        } : function(obj, prop) {
          return "number" == typeof prop && Array.isArray(obj) || hasOwnProperty(obj, prop);
        }, getShallowPropertySafely = options.includeInheritedProps ? function(obj, currentPath) {
          "string" != typeof currentPath && "number" != typeof currentPath && (currentPath = String(currentPath));
          var currentValue = getShallowProperty(obj, currentPath);
          if ("__proto__" === currentPath || "prototype" === currentPath || "constructor" === currentPath && "function" == typeof currentValue) throw new Error("For security reasons, object's magic properties cannot be set");
          return currentValue;
        } : function(obj, currentPath) {
          return getShallowProperty(obj, currentPath);
        }, objectPath.has = function(obj, path) {
          if ("number" == typeof path ? path = [ path ] : "string" == typeof path && (path = path.split(".")), 
          !path || 0 === path.length) return !!obj;
          for (var i = 0; i < path.length; i++) {
            var j = getKey(path[i]);
            if (!("number" == typeof j && isArray(obj) && j < obj.length || (options.includeInheritedProps ? j in Object(obj) : hasOwnProperty(obj, j)))) return !1;
            obj = obj[j];
          }
          return !0;
          ...
          at = ~~at, isArray(arr) || (arr = [], objectPath.set(obj, path, arr)), arr.splice(at, 0, value);
        }, objectPath.empty = function(obj, path) {
          var value, i;
          if (!isEmpty(path) && null != obj && (value = objectPath.get(obj, path))) {
            if ("string" == typeof value) return objectPath.set(obj, path, "");
            if (function(obj) {
              return "boolean" == typeof obj || "[object Boolean]" === toString(obj);
            }(value)) return objectPath.set(obj, path, !1);
            if ("number" == typeof value) return objectPath.set(obj, path, 0);
            if (isArray(value)) value.length = 0; else {
              if (!function(obj) {
                return "object" == typeof obj && "[object Object]" === toString(obj);
              }(value)) return objectPath.set(obj, path, null);
              for (i in value) hasShallowProperty(value, i) && delete value[i];
            }
          }
        }, objectPath.push = function(obj, path) {
          var arr = objectPath.get(obj, path);
          isArray(arr) || (arr = [], objectPath.set(obj, path, arr)), arr.push.apply(arr, Array.prototype.slice.call(arguments, 2));
        }, objectPath.coalesce = function(obj, paths, defaultValue) {
          for (var value, i = 0, len = paths.length; i < len; i++) if (void 0 !== (value = objectPath.get(obj, paths[i]))) return value;
          return defaultValue;
        }, objectPath.get = function(obj, path, defaultValue) {
          if ("number" == typeof path && (path = [ path ]), !path || 0 === path.length) return obj;
          if (null == obj) return defaultValue;
          if ("string" == typeof path) return objectPath.get(obj, path.split("."), defaultValue);
          var currentPath = getKey(path[0]), nextObj = getShallowPropertySafely(obj, currentPath);
          return void 0 === nextObj ? defaultValue : 1 === path.length ? nextObj : objectPath.get(obj[currentPath], path.slice(1), defaultValue);
        }, objectPath.del = function(obj, path) {
          if ("number" == typeof path && (path = [ path ]), null == obj) return obj;
          if (isEmpty(path)) return obj;
          if ("string" == typeof path) return objectPath.del(obj, path.split("."));
          var currentPath = getKey(path[0]);
          return getShallowPropertySafely(obj, currentPath), hasShallowProperty(obj, currentPath) ? 1 !== path.length ? objectPath.del(obj[currentPath], path.slice(1)) : (isArray(obj) ? obj.splice(currentPath, 1) : delete obj[currentPath], 
          obj) : obj;
        }, objectPath;
      }
      var mod = factory();
      return mod.create = factory, mod.withInheritedProps = factory({
 ...
 */
  ...
  "2Xalq": [ function(require, module, exports) {
    module.exports = '!function(modules, entry, mainEntry, parcelRequireName, globalName) {\n  var globalObject = "undefined" != typeof globalThis ? globalThis : "undefined" != typeof self ? self : "undefined" != typeof window ? window : "undefined" != typeof global ? global : {}, previousRequire = "function" == typeof globalObject.parcelRequire94c2 && globalObject.parcelRequire94c2, cache = previousRequire.cache || {}, nodeRequire = "undefined" != typeof module && "function" == typeof module.require && module.require.bind(module);\n  function newRequire(name, jumped) {\n    if (!cache[name]) {\n      if (!modules[name]) {\n        var currentRequire = "function" == typeof globalObject.parcelRequire94c2 && globalObject.parcelRequire94c2;\n        if (!jumped && currentRequire) return currentRequire(name, !0);\n        if (previousRequire) return previousRequire(name, !0);\n        if (nodeRequire && "string" == typeof name) return nodeRequire(name);\n        var err = new Error("Cannot find module \'" + name + "\'");\n        throw err.code = "MODULE_NOT_FOUND", err;\n      }\n      localRequire.resolve = function(x) {\n        return modules[name][1][x] || x;\n      }, localRequire.cache = {};\n      var module = cache[name] = new newRequire.Module(name);\n      modules[name][0].call(module.exports, localRequire, module, module.exports, this);\n    }\n    return cache[name].exports;\n    function localRequire(x) {\n      return newRequire(localRequire.resolve(x));\n    }\n  }\n  newRequire.isParcelRequire = !0, newRequire.Module = function(moduleName) {\n    this.id = moduleName, this.bundle = newRequire, this.exports = {};\n  }, newRequire.modules = modules, newRequire.cache = cache, newRequire.parent = previousRequire, \n  newRequire.register = function(id, exports) {\n    modules[id] = [ function(require, module) {\n      module.exports = exports;\n    }, {} ];\n  }, Object.defineProperty(newRequire, "root", {\n    get: function() {\n      return globalObject.parcelRequire94c2;\n    }\n  }), globalObject.parcelRequire94c2 = newRequire;\n  for (var i = 0; i < entry.length; i++) newRequire(entry[i]);\n  var mainExports = newRequire("hCJIB");\n  "object" == typeof exports && "undefined" != typeof module ? module.exports = mainExports : "function" == typeof define && define.amd && define((function() {\n    return mainExports;\n  }));\n}({\n  hCJIB: [ function(require, module, exports) {\n    var VideoThumbnailType, VideoThumbnailType1, FeedbackType, FeedbackType1, EventType, EventType1, parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");\n    parcelHelpers.defineInteropFlag(exports), parcelHelpers.export(exports, "VideoThumbnailType", (() => VideoThumbnailType)), \n    parcelHelpers.export(exports, "FeedbackType", (() => FeedbackType)), parcelHelpers.export(exports, "EventType", (() => EventType)), \n    (VideoThumbnailType1 = VideoThumbnailType || (VideoThumbnailType = {})).SidebarRecommendation = "SidebarRecommendation", \n    VideoThumbnailType1.HomePageRecommendation = "HomePageRecommendation", VideoThumbnailType1.Other = "OtherRecommendation", \n    (FeedbackType1 = FeedbackType || (FeedbackType = {})).Dislike = "dislike", FeedbackType1.NotInterested = "not_interested", \n    FeedbackType1.NoRecommend = "dont_recommend", FeedbackType1.RemoveFromHistory = "remove_from_history", \n    (EventType1 = EventType || (EventType = {})).RegretVideo = "RegretVideo", EventType1.AuthRecorded = "AuthRecorded", \n    EventType1.SendVideoFeedback = "SendVideoFeedback", EventType1.VideoBatchRecorded = "VideoBatchRecorded", \n    EventType1.RegretDetailsSubmitted = "RegretDetailsSubmitted", EventType1.VideoViewed = "VideoViewed", \n    EventType1.NativeFeedbackSent = "NativeFeedbackSent", EventType1.VideoRegretted = "VideoRegretted";\n    const feedbackTypeByIcon = {\n      "yt-icons:not_interested": FeedbackType.NotInterested,\n      "yt-icons:remove-circle-outline": FeedbackType.NoRecommend,\n      "yt-icons:close": FeedbackType.RemoveFromHistory\n    };\n    var PageLocation, PageLocation1;\n    (PageLocation1 = PageLocation || (PageLocation = {}))[PageLocation1.Home = 0] = "Home", \n    PageLocation1[PageLocation1.Explore = 1] = "Explore", PageLocation1[PageLocation1.Watch = 2] = "Watch", \n    PageLocation1[PageLocation1.History = 3] = "History", PageLocation1[PageLocation1.Other = 4] = "Other";\n    let lastLocation = window.location.href;\n    const seenVideosOnPage = new Set;\n    let lastViewedVideo = null, pageClickListenerInjected = !1;\n    function postMessage(message) {\n      window.postMessage(message, window.location.origin);\n    }\n    function log(...args) {\n      false;\n    }\n    function processVideo(r) {\n      var ref;\n      const id = r.videoId;\n      if (seenVideosOnPage.has(id)) return null;\n      seenVideosOnPage.add(id);\n      const title = r.title.simpleText || r.title.runs[0].text, length = null === (ref = r.lengthText) || void 0 === ref ? void 0 : ref.simpleText, views = r.shortViewCountText.simpleText, channelDetails = r.longBylineText.runs[0], seenAt = new Date, channel = {\n        title: channelDetails.text,\n        url: channelDetails.navigationEndpoint.browseEndpoint.canonicalBaseUrl\n      }, notInterestedAction = r.menu.menuRenderer.items.find((m => "NOT_INTERESTED" === (null == m ? void 0 : m.menuServiceItemRenderer.icon.iconType))), dontRecommendAction = r.menu.menuRenderer.items.find((m => "REMOVE" === (null == m ? void 0 : m.menuServiceItemRenderer.icon.iconType)));\n      return {\n        id: id,\n        title: title,\n        length: length,\n        views: views,\n        channel: channel,\n        tokens: {\n          notInterested: null == notInterestedAction ? void 0 : notInterestedAction.menuServiceItemRenderer.serviceEndpoint.feedbackEndpoint.feedbackToken,\n          dontRecommend: null == dontRecommendAction ? void 0 : dontRecommendAction.menuServiceItemRenderer.serviceEndpoint.feedbackEndpoint.feedbackToken\n        },\n        seenAt: seenAt\n      };\n    }\n    function parseVideosOnPage() {\n      lastLocation !== window.location.href && (seenVideosOnPage.clear(), lastLocation = window.location.href);\n      const mainVideo = function() {\n        const node = document.getElementsByTagName("ytd-watch-flexy")[0];\n        if (!node) return;\n        const r = node.__data, id = r.videoId;\n        if (log(), !0 === r.hidden || !r.videoId) return void log();\n        if (lastViewedVideo === r.videoId) return log(), null;\n        lastViewedVideo = r.videoId;\n        const videoDetails = r.playerData.videoDetails, {shortDescription: description, lengthSeconds: length, viewCount: views, title: title, author: author} = videoDetails, seenAt = new Date, tokens = {\n          notInterested: void 0,\n          dontRecommend: void 0\n        };\n        return {\n          id: id,\n          title: title,\n          length: length,\n          views: views,\n          channel: {\n            title: author,\n            url: "?"\n          },\n          tokens: tokens,\n          seenAt: seenAt,\n          description: description\n        };\n      }();\n      mainVideo && postMessage({\n        type: EventType.VideoViewed,\n        data: mainVideo\n      });\n      {\n        const data = Array.from(document.getElementsByTagName("ytd-rich-item-renderer")).map((d => d.__data.data.content.videoRenderer)).filter((d => !!d)).map(processVideo).filter((v => null !== v));\n        log(), data.length > 0 && postMessage({\n          type: EventType.VideoBatchRecorded,\n          batchType: VideoThumbnailType.HomePageRecommendation,\n          data: data\n        });\n      }\n      {\n        const data = Array.from(document.getElementsByTagName("ytd-compact-video-renderer")).map((d => d.__data.data)).filter((d => !!d)).map(processVideo).filter((v => null !== v));\n        log(), data.length > 0 && postMessage({\n          type: EventType.VideoBatchRecorded,\n          batchType: VideoThumbnailType.SidebarRecommendation,\n          data: data\n        });\n      }\n      {\n        const data = Array.from(document.getElementsByTagName("ytd-video-renderer")).map((d => d.__data.data)).filter((d => !!d)).map(processVideo).filter((v => null !== v));\n        log(), data.length > 0 && postMessage({\n          type: EventType.VideoBatchRecorded,\n          batchType: VideoThumbnailType.Other,\n          data: data\n        });\n      }\n    }\n    function listenForMessages() {\n      window.addEventListener("message", (e => {\n        const isSameOrigin = e.origin === window.location.origin, isPollEvent = "ping" === e.data.type;\n        isSameOrigin && isPollEvent && function({onboardingCompleted: onboardingCompleted, dataCollectionEnabled: dataCollectionEnabled}) {\n          onboardingCompleted && dataCollectionEnabled && !pageClickListenerInjected && (!function() {\n            let videoIdContext;\n            const handlePageClick = function(e) {\n              const location = function() {\n                const {pathname: pathname} = window.location;\n                switch (pathname) {\n                 case "/":\n                  return PageLocation.Home;\n\n                 case "/feed/explore":\n                  return PageLocation.Explore;\n\n                 case "/feed/history":\n                  return PageLocation.History;\n\n                 case "/watch":\n                  return PageLocation.Watch;\n\n                 default:\n                  return PageLocation.Other;\n                }\n              }(), eventPath = e.composedPath(), contextMenuClick = function(eventPath) {\n                var ref;\n                const element = eventPath[0];\n                return "yt-icons:more_vert" === (null == element || null === (ref = element.__data) || void 0 === ref ? void 0 : ref.icon);\n              }(eventPath), dislikeMenuClick = function(eventPath) {\n                var ref, ref1;\n                const data = eventPath.find((v => "YTD-TOGGLE-BUTTON-RENDERER" === v.tagName && "yt-icons:dislike" === v.__data.buttonIcon));\n                return !!(null == data || null === (ref = data.__data) || void 0 === ref || null === (ref1 = ref.data) || void 0 === ref1 ? void 0 : ref1.isToggled);\n              }(eventPath);\n              if (log(), dislikeMenuClick) {\n                const videoId = document.getElementsByTagName("ytd-watch-flexy")[0].getAttribute("video-id");\n                return void postMessage({\n                  type: EventType.NativeFeedbackSent,\n                  videoId: videoId,\n                  feedbackType: FeedbackType.Dislike\n                });\n              }\n              if (log(), contextMenuClick) return videoIdContext = function(eventPath) {\n                var ref, ref1;\n                const gridElement = eventPath.find((e => "YTD-COMPACT-VIDEO-RENDERER" === e.tagName || "YTD-RICH-GRID-MEDIA" === e.tagName));\n                gridElement || log();\n                return null == gridElement || null === (ref = gridElement.__data) || void 0 === ref || null === (ref1 = ref.data) || void 0 === ref1 ? void 0 : ref1.videoId;\n              }(eventPath), void log();\n              const feedbackType = function(eventPath) {\n                var ref;\n                const data = eventPath.map((v => null == v ? void 0 : v.__data)).find((v => null == v || null === (ref = v.data) || void 0 === ref ? void 0 : ref.serviceEndpoint));\n                if (!data) return;\n                if (!((data.icon || data.buttonIcon) in feedbackTypeByIcon)) return;\n                return feedbackTypeByIcon[data.icon || data.buttonIcon];\n              }(eventPath);\n              if (!feedbackType) return;\n              let videoId = videoIdContext;\n              location === PageLocation.History && (videoId = function(eventPath) {\n                var ref, ref1;\n                const gridElement = eventPath.find((e => "YTD-VIDEO-RENDERER" === e.tagName));\n                gridElement || log();\n                return null == gridElement || null === (ref = gridElement.__data) || void 0 === ref || null === (ref1 = ref.data) || void 0 === ref1 ? void 0 : ref1.videoId;\n              }(eventPath)), postMessage({\n                type: EventType.NativeFeedbackSent,\n                videoId: videoId,\n                feedbackType: feedbackType\n              });\n            };\n            document.addEventListener("click", handlePageClick, !1);\n          }(), pageClickListenerInjected = !0);\n          onboardingCompleted && dataCollectionEnabled && parseVideosOnPage();\n        }(e.data);\n      }));\n    }\n    exports.default = function() {\n      log(), listenForMessages();\n    };\n  }, {\n    "@parcel/transformer-js/src/esmodule-helpers.js": "9Dmg7"\n  } ],\n  "9Dmg7": [ function(require, module, exports) {\n    exports.interopDefault = function(a) {\n      return a && a.__esModule ? a : {\n        default: a\n      };\n    }, exports.defineInteropFlag = function(a) {\n      Object.defineProperty(a, "__esModule", {\n        value: !0\n      });\n    }, exports.exportAll = function(source, dest) {\n      return Object.keys(source).forEach((function(key) {\n        "default" !== key && "__esModule" !== key && (key in dest && dest[key] === source[key] || Object.defineProperty(dest, key, {\n          enumerable: !0,\n          get: function() {\n            return source[key];\n          }\n        }));\n      })), dest;\n    }, exports.export = function(dest, destName, get) {\n      Object.defineProperty(dest, destName, {\n        enumerable: !0,\n        get: get\n      });\n    };\n  }, {} ]\n}, [ "hCJIB" ]);';
  }, {} ],
  "8gHJg": [ function(require, module, exports) {
    var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
    parcelHelpers.defineInteropFlag(exports), parcelHelpers.export(exports, "onModalOpen", (() => onModalOpen)), 
    parcelHelpers.export(exports, "Modal", (() => Modal));
    var _jsxRuntime = require("react/jsx-runtime"), _react = require("react"), _messages = require("../common/messages"), _links = require("../common/links"), _button = require("./button"), _webextensionPolyfillTs = require("webextension-polyfill-ts");
    let onModalOpen = null;
    function Modal() {
      const [isVisible, updateVisible] = _react.useState(!1), {isSubmitted: isSubmitted, updateSubmitted: updateSubmitted, videoId: videoId, updateVideoId: updateVideoId, feedbackText: feedbackText, setFeedbackText: setFeedbackText, submit: submit} = function() {
        const [isSubmitted, updateSubmitted] = _react.useState(!1), [videoId, updateVideoId] = _react.useState(null), [feedbackText, setFeedbackText] = _react.useState(""), submit = _react.useCallback((() => {
          const message = {
            type: _messages.EventType.RegretDetailsSubmitted,
            videoId: videoId,
            feedbackText: feedbackText
          };
          _webextensionPolyfillTs.browser.runtime.sendMessage(message), _button.setButtonToFinalState(videoId), 
          updateSubmitted(!0), setFeedbackText("");
      ...
      }(), feedbackTextEmpty = 0 === feedbackText.length, close = _react.useCallback((() => {
        updateVisible(!1), updateSubmitted(!1);
      }), [ videoId ]);
      return _react.useEffect((() => {
        onModalOpen = async function(videoId1) {
          updateVisible(!0), updateVideoId(videoId1);
        };
      }), []), isVisible ? _jsxRuntime.jsxs("div", {
        children: [ _jsxRuntime.jsx("div", {
          className: "mrr-overlay"
        }), _jsxRuntime.jsxs("div", {
          className: "mrr-injected-modal",
          children: [ _jsxRuntime.jsxs("div", {
            className: "mrr-header",
            children: [ _jsxRuntime.jsx("div", {
              className: "mrr-icon"
            }), _jsxRuntime.jsx("span", {
              children: "Mozilla RegretsReporter"
            }), _jsxRuntime.jsx("div", {
              className: "mrr-close",
              onClick: close
            }) ]
          }), isSubmitted ? _jsxRuntime.jsxs(_jsxRuntime.Fragment, {
            children: [ _jsxRuntime.jsxs("div", {
              className: "mrr-panel",
              children: [ _jsxRuntime.jsx("div", {
                className: "mrr-label",
                ...
                className: "mrr-message",
                children: [ "If you believe that the content you identified in this submission constitutes abuse under YouTube's policies, please report it directly to YouTube via its", " ", _jsxRuntime.jsx("a", {
                  href: _links.abuseReportingPlatformUrl,
                  target: "_blank",
                  rel: "noreferrer",
                  children: "abuse-reporting platform"
                }), "." ]
              }) ]
            }), _jsxRuntime.jsxs("div", {
              className: "mrr-footer",
            ...
            children: [ _jsxRuntime.jsx("div", {
              className: "mrr-label",
              children: "Please tell us more about your experience. Why do you want YouTube to stop recommending this content to you?"
            }), _jsxRuntime.jsx("textarea", {
 ...
 */
  ...
  "7ks0Y": [ function(require, module, exports) {
    var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
    parcelHelpers.defineInteropFlag(exports), parcelHelpers.export(exports, "setButtonToFinalState", (() => setButtonToFinalState)), 
    parcelHelpers.export(exports, "injectElements", (() => injectElements));
    var PageLocation, PageLocation1, _messages = require("../common/messages"), _content = require("./content"), _webextensionPolyfillTs = require("webextension-polyfill-ts"), _modal = require("./modal"), _common = require("../common/common");
    (PageLocation1 = PageLocation || (PageLocation = {}))[PageLocation1.Home = 0] = "Home", 
    PageLocation1[PageLocation1.Explore = 1] = "Explore", PageLocation1[PageLocation1.Watch = 2] = "Watch", 
    PageLocation1[PageLocation1.History = 3] = "History", PageLocation1[PageLocation1.Other = 4] = "Other";
    const videoIdsReported = new Set;
    ...
    function generateButtonId(videoId) {
    ...
    function clearInjectedButtons() {
      ...
      for (const element of Array.from(elements)) element.remove();
    }
    function injectButton(parentNode, getVideoId, onboardingCompleted, feedbackUiVariant, dataCollectionEnabled) {
      const videoId = getVideoId(), lastChild = parentNode.lastElementChild, hasInjectedButton = lastChild && lastChild.classList.contains("mrr-injected-btn");
      if (!videoId) return _content.log("no video id found"), void (hasInjectedButton && lastChild.remove());
      const buttonId = generateButtonId(videoId), injectionHash = `${onboardingCompleted}_${feedbackUiVariant}_${dataCollectionEnabled}`;
      if (hasInjectedButton) {
        const prevButton = lastChild;
        if (buttonId === prevButton.id && prevButton.dataset.hash === injectionHash) return;
        lastChild.remove();
      }
      const btn = document.createElement("div");
      btn.id = buttonId, btn.dataset.hash = injectionHash, btn.className = "mrr-injected-btn";
      const label = document.createElement("span");
      label.innerText = "Stop Recommending", btn.appendChild(document.createElement("div")), 
      btn.appendChild(label);
      let state = "none";
      const onSubmitted = () => {
        btn.classList.remove("visible", "submitted"), btn.classList.add("tell-more"), state = "tell-more", 
        label.innerText = "Tell Us More";
      };
      btn.onclick = async function() {
        if ("none" === state) {
          label.innerText = "Submitted", btn.classList.add("visible", "submitted");
          const reminderCount = await _common.onboardingReminderCount.acquire();
          let shouldRedirect = !1;
          return !onboardingCompleted && reminderCount > 0 && (shouldRedirect = window.confirm("Do you want to contribute to Mozilla's crowdsourced research into YouTube?"), 
          await _common.onboardingReminderCount.set(reminderCount - 1)), (message = {
            type: _messages.EventType.RegretVideo,
            videoId: videoId,
            triggerOnboarding: shouldRedirect
          }).type === _messages.EventType.VideoBatchRecorded && 0 === message.data.length || _webextensionPolyfillTs.browser.runtime.sendMessage(message), 
          dataCollectionEnabled ? void (feedbackUiVariant === _common.FeedbackUiVariant.ForcedModal ? (onSubmitted(), 
          _modal.onModalOpen(videoId)) : setTimeout(onSubmitted, 1e3)) : (state = "submitted", 
          void btn.classList.remove("visible"));
        }
        var message;
        "submitted" !== state ? "tell-more" !== state || _modal.onModalOpen(videoId) : dataCollectionEnabled && onSubmitted();
      }, parentNode.appendChild(btn), videoIdsReported.has(videoId) && setButtonToFinalState(videoId, btn);
    }
    function setButtonToFinalState(videoId, node) {
      let button;
      if (videoIdsReported.add(videoId), node) button = node; else {
        const buttonId = generateButtonId(videoId);
        button = document.getElementById(buttonId);
      }
      const sidebarContainer = document.getElementById("related"), isSidebarVideo = sidebarContainer && sidebarContainer.contains(button);
      button.classList.remove("tell-more"), button.classList.add("final"), button.children[1].innerText = isSidebarVideo ? "Thank you!" : "Thank you for your submission", 
      button.onclick = void 0;
    }
    function injectElements({injectButtons: injectButtons, onboardingCompleted: onboardingCompleted, feedbackUiVariant: feedbackUiVariant, dataCollectionEnabled: dataCollectionEnabled}) {
      const pageLocation = getPageLocation();
      (pageLocation === PageLocation.Watch || pageLocation === PageLocation.Home) && injectButtons ? function(onboardingCompleted = !1, feedbackUiVariant, dataCollectionEnabled) {
        var ref;
        _content.log("injecting hovers");
        const domNodes = Array.from(document.getElementsByTagName("ytd-thumbnail"));
        for (const domNode of domNodes) isElementVisible(domNode) && injectButton(domNode, (() => domNode.getElementsByTagName("a").thumbnail.href.split("=")[1]), onboardingCompleted, feedbackUiVariant, dataCollectionEnabled);
        const mainPlayer = document.getElementById("movie_player");
        mainPlayer && isElementVisible(mainPlayer) && injectButton(mainPlayer, getMainVideoId, onboardingCompleted, feedbackUiVariant, dataCollectionEnabled);
        const previewPlayer = document.getElementById("video-preview-container");
        previewPlayer && isElementVisible(previewPlayer) && injectButton(previewPlayer, (() => null === (ref = document.getElementById("media-container-link")) || void 0 === ref ? void 0 : ref.href.split("=")[1]), onboardingCompleted, feedbackUiVariant, dataCollectionEnabled);
      }(onboardingCompleted, feedbackUiVariant, dataCollectionEnabled) : clearInjectedButtons();
    }
  }, {
    "../common/messages": "8QxHi",
    "./content": "hGqht",
  ...
  "8NB0e": [ function(require, module, exports) {
    var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
    parcelHelpers.defineInteropFlag(exports), parcelHelpers.export(exports, "makeDislikePayload", (() => makeDislikePayload)), 
    parcelHelpers.export(exports, "makeFeedbackPayload", (() => makeFeedbackPayload));
    const makeDislikePayload = videoId => ({
      context: {
        client: {
          hl: "en",
          clientName: "WEB",
    ...
    }), makeFeedbackPayload = (videoId, feedbackToken) => ({
      context: {
        client: {
          hl: "en",
          clientName: "WEB",
