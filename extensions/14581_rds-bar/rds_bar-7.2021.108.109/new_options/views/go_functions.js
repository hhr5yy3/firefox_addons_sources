    ...
    var FunctionView = Backbone.View.extend({
        ...
        renderExtra: function (html) {
            ...
            if (this.model.get('name') === 'SiteAnalysis') {
                var extraSEO = window.GoParameters.get('Seo').get('extra');
                price = 0;
                hideCoins = true;
                for (name in extra) {
        ...
        showOptions: function (event) {
            Contextmenu.open({e: event, m: this.model, parent: this, parameters_branch: 'GoParameters'});
        },
        toggle: function (e) {
            this.model.toggle();
            Google.changeFunction(e, this.model);

            $('#save').removeAttr('disabled');
        },
        returnLocale: function (name) {
            return AppLocale.get('options.google.functions.' + name);
        },
        ...
        render: function () {
            var html = $('<div class="col"><p><b>' + AppLocale.get('options.view.functions_ttl') + '</b></p><div>' //'<div class="functions-title">' +
                    /*AppLocale.get('options.yandex.functions.ttl') +
                     '</div>'*/),
                functions = Google.get('functions'),
                f,
                view,
            ...
            for (f in functions) {
                if (Bar.get('locale') === 'ru' || rdz.utils.locale.ru.functions.indexOf(f) === -1) {
