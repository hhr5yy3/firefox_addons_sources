...
$("#windowsize_" + window_size).prop("checked", true);

$("input[name='windowsize']").change(function (e) {
  var elm = $(e.currentTarget);
  if (elm.prop("checked")) {
    CAMELIZER.setting("window_size", elm.val());
    save_settings();
    $("#pricehistory_tab_btn").click();
    set_window_size();
    set_chart_height();
    select_correct_period();
...
$("#shortcut_link").click(function (e) {
  e.preventDefault();
  browser.tabs.create({
    url: $(this).attr("href")
  });
  ...
  browser.tabs.query({active: true, currentWindow: true}).then(function(tab) {
    tab = tab[0];
    if (!is_amazon(tab.url)) {
      show_loading_error_message(browser.i18n.getMessage("not_amazon_error_title"), browser.i18n.getMessage("not_amazon_error_msg"));
      return;
    }
    if (is_amazon_blacklist(tab.url)) {
      show_loading_error_message(browser.i18n.getMessage("amazon_blacklist_error_title"), browser.i18n.getMessage("amazon_blacklist_error_msg"));
      return;
    }
    get_asin(function(response) {
      CAMELIZER.ephemeral("current_tab_url", tab.url);
      parse_amazon_url_and_update_ui(response["asin"]);
    });
  }, function(err) {
    dmsg("Error");
    dmsg(err);
