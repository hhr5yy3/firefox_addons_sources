...
browserAPI.storage.local.get("extensionTabs", async ({ extensionTabs }) => {
...
browserAPI.storage.local.get("allTabsModeIsOn", ({ allTabsModeIsOn }) => {
  if (allTabsModeIsOn) {
...
toggleBtn.onclick = async () => {
  ...
  browserAPI.storage.local.get(["extensionTabs"], async ({ extensionTabs }) => {
    ...
    if (extensionModePopUp === "single") {
      ...
      browserAPI.storage.local.set({ extensionTabs });
      ...
      if (allTabsModeIsOn_POPUP) {
        ...
        currentTabIsOn_POPUP = false;
        changeToggleButton(false);
        browserAPI.storage.local.set({ extensionTabs, allTabsModeIsOn: false });
      } else {
        ...
        currentTabIsOn_POPUP = true;
        changeToggleButton(true);
        browserAPI.storage.local.set({ extensionTabs, allTabsModeIsOn: true });
      }
    }
  });
};
...
function enableOrDisableTab(tab, disable = false) {
  browserAPI.storage.local.get("disabledTabs", ({ disabledTabs }) => {
