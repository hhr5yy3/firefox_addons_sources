var Tool = (function(){
  ...
  var initAVGWidget = function(){
    ...
    var followers = $('._ac2a[title]').attr('title') || 0;
    if (!followers) {
      $('header ul li a[role=link]').map(function(i, node){
        if (node.href.indexOf('followers') !== -1) {
          followers = $(node).find('span[title]').attr('title');
        }
      });
    }
    if (followers) followers = parseInt(followers.replace(/[,.]/g, ''));
    var isTags = false;
    var igData = [];
    $templates.map(function(i, template){
      ...
      try {
        ...
        console.log(isTags, template.dataset.url);
        if (isTags) {
    ...
    if (counts.posts) {
      if (!$widget[0]) {
        var selector = 'main header section ul';
        ...
        `);
        if (!isTags) {
          addIframe($widget.find('#xt-instagram-avg-widget-footer'), darkMode, 'xt-instagram-avg-widget-iframe', 'instgr_avg');
        }
        else {
