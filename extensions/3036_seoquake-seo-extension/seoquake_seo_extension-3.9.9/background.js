...
var clientExcludeMatches = [/^http(s)?:\/\/.*\.facebook\.(com|net)\/.*/i, /^http(s)?:\/\/.*\.fbcdn\.(com|net)\/.*/i, /^http(s)?:\/\/.*\.akamaihd\.(com|net)\/.*/i, /^http(s)?:\/\/.*\.virtualearth\.(com|net)\/.*/i, /^http(s)?:\/\/.*addons.opera\.com\/.*/i];
...
function _loadContentCss(tab) {
  ...
  if (clientMatches.every(function (regexp) {
    ...
    return new Promise(function (resolve) {
      return browser.tabs.insertCSS(tab.getIdentifier(), scriptConfig, resolve);
    });
  } else {
...
function _loadContentScripts(tab) {
  ...
  if (clientMatches.every(function (regexp) {
    ...
    return new Promise(function (resolve) {
      return browser.tabs.executeScript(tab.getIdentifier(), scriptConfig, resolve);
    }).then(function () {
...
function _sendMessage(tab, data) {
  return new Promise(function (resolve, reject) {
    browser.tabs.sendMessage(tab.getIdentifier(), data, function (result) {
      if (result === undefined && browser.runtime.lastError) {
        if (browser.runtime.lastError.message === 'Could not establish connection. Receiving end does not exist.') {
          _loadContentScripts(tab).then(function () {
            return browser.tabs.sendMessage(tab.getIdentifier(), data, resolve);
          }).catch(reject);
        } else {
          resolve();
        }
...
var FirefoxBrowser = function (_Browser) {
  _inherits(FirefoxBrowser, _Browser);

  function FirefoxBrowser() {
    _classCallCheck(this, FirefoxBrowser);

    return _possibleConstructorReturn(this, (FirefoxBrowser.__proto__ || Object.getPrototypeOf(FirefoxBrowser)).call(this));
  }

  _createClass(FirefoxBrowser, [{
    key: 'isNewInstall',
    ...
    value: function getUserUniqId() {
      ...
      return this.getConfigValue('uuid', false).then(function (uuid) {
        if (!uuid) {
          uuid = lib.getUUID();
          _this3.setConfigValue('uuid', uuid);
        }
    ...
    value: function getConfigValue(configKey, defaultValue) {
      return new Promise(function (resolve) {
        browser.storage.local.get(configKey, function (value) {
          if (value === undefined || !value.hasOwnProperty(configKey)) {
            resolve(defaultValue);
            return;
          }

          try {
            resolve(JSON.parse(value[configKey]));
          } catch (e) {
            resolve(defaultValue);
          }
        });
    ...
    value: function setConfigValue(configKey, value) {
      return browser.storage.local.set(_defineProperty({}, configKey, JSON.stringify(value)));
    }
  }, {
    ...
    value: function loadConfiguration(defaultValues, storageKey, callback) {
      ...
      this.getConfigValue(storageKey).then(function (configuration) {
        callback(defaultValues, configuration, function (values) {
          return _this4.setConfigValue(storageKey, values);
        });
      });
    }
  }, {
    ...
    value: function loadCoreConfiguration(callback) {
      ...
      this.getConfigValue('version').then(function (version) {
        if (version === null || version === undefined) {
          _this5.setConfigValue('installDate', JSON.stringify(new Date()));
        }

        _this5.setConfigValue('version', _this5.version);
      });
    ...
    value: function addMessageListener(action, callback) {
      ...
      this.port.on(action, function (data) {
        ...
        callback(data.payload, sendAnswer.bind(_this6, action, data));
      });
    }
  }, {
    ...
    value: function openModuleTab(page, params, newWindow) {
      ...
      browser.windows.getAll({ populate: true }, function (windows) {
        ...
        var modulePageUrl = browser.extension.getURL(page);
        ...
        windows.forEach(function (windowItem) {
          windowItem.tabs.forEach(function (tab) {
            if (lib.trimHash(tab.url) === lib.trimHash(modulePageUrl)) {
              ...
              browser.windows.update(windowItem.id, { focused: true });
        ...
        if (!preferencesOpened) {
          if (newWindow) {
            browser.windows.create({ url: modulePageUrl });
          } else {
            browser.tabs.create({ url: modulePageUrl });
          }
        }
      });
    }
    ...
    value: function registerContentScripts(messagesDispatcher) {
      ...
      browser.tabs.onUpdated.addListener(function (tabId, updateInfo) {
...
FirefoxBrowser.prototype.createContextMenuItem = require('./ContextMenu');

instance = new FirefoxBrowser();
...
var Analytics = function () {
  ...
  _createClass(Analytics, [{
    ...
    value: function sendRequest(data) {
      ...
      request += '&cs=' + encodeURIComponent('3.9.9') + '&cm=firefox&ds=' + encodeURIComponent('3.9.9');
    ...
    value: function _updateTokens(text) {
      ...
      if (answer.expires_in) {
        ...
        this._refreshTokenTimeoutId = setTimeout(function () {
          _this15.requestRefreshToken(function () {
            return null;
          });
