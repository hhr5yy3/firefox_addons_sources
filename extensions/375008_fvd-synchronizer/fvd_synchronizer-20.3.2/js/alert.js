...
window.addEventListener( "load", function(){
		...
		this.syncQuotaExceed = function(){
			...
			document.querySelector("#specialMessage_syncQuotaExceed .buttons .main").addEventListener( "click", function(){
				...
				fvdSynchronizer.Server.Sync.loginInAdminPanel(function( result ){
					...
					chrome.tabs.create({
						url: "http://everhelper.pro/client/?l=/components/MainOptions/quota",
						active: true
					});
	...
	if( vars.type ){
		...
		if( vars.text ){
			...
			//document.getElementById("simpleText").innerHTML = decodeURIComponent( vars.text ); // Task #1708
			document.getElementById("simpleText").innerText = String(decodeURIComponent( vars.text ));
