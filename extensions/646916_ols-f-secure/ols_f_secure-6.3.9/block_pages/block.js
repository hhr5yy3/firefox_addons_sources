...
class BlockPage {
    ...
    constructor(url, allowDisabled, customization, platform) {
        this.#url = url;
    ...
    loadImages() {
        const shieldIcon = document.querySelector('.shield_icon');
        if (this.isRunningOnMac && shieldIcon) {
...
async function loadBlockPage() {
    try {
        ...
        const page = new BlockPage(block.url, block.allowDisabled, storageData.customization, storageData.platform);
