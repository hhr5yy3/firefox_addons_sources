(() => {
  var __webpack_modules__ = {
    ...
    2756: (module, __unused_webpack_exports, __webpack_require__) => {
      "use strict";
      ...
      var applyBind = __webpack_require__(6898);
    ...
    4903: (module, __unused_webpack_exports, __webpack_require__) => {
      "use strict";
    ...
    1701: (module, __unused_webpack_exports, __webpack_require__) => {
      "use strict";
    ...
    6930: (module, __unused_webpack_exports, __webpack_require__) => {
      "use strict";
      var GetIntrinsic = __webpack_require__(9718);
    ...
    7213: (__unused_webpack_module, exports, __webpack_require__) => {
      "use strict";
    ...
    6818: (__unused_webpack_module, exports, __webpack_require__) => {
      var process = __webpack_require__(6490);
      ...
      exports.isBuffer = __webpack_require__(6098);
    ...
    4912: (module, __unused_webpack_exports, __webpack_require__) => {
      "use strict";
  ...
  (() => {
    "use strict";
    function bitbucket_SelectorSet() {
      if (!(this instanceof bitbucket_SelectorSet)) return new bitbucket_SelectorSet;
      this.size = 0;
      this.uid = 0;
      this.selectors = [];
    ...
    var bitbucket_docElem = window.document.documentElement;
    var bitbucket_matches = bitbucket_docElem.matches || bitbucket_docElem.webkitMatchesSelector || bitbucket_docElem.mozMatchesSelector || bitbucket_docElem.oMatchesSelector || bitbucket_docElem.msMatchesSelector;
    bitbucket_SelectorSet.prototype.matchesSelector = function(el, selector) {
      return bitbucket_matches.call(el, selector);
    };
    bitbucket_SelectorSet.prototype.querySelectorAll = function(selectors, context) {
      return context.querySelectorAll(selectors);
    };
    bitbucket_SelectorSet.prototype.indexes = [];
    var bitbucket_idRe = /^#((?:[\w\u00c0-\uFFFF\-]|\\.)+)/g;
    bitbucket_SelectorSet.prototype.indexes.push({
      name: "ID",
      selector: function matchIdSelector(sel) {
        var m;
        if (m = sel.match(bitbucket_idRe)) return m[0].slice(1);
      },
      element: function getElementId(el) {
        if (el.id) return [ el.id ];
      }
    });
    var bitbucket_classRe = /^\.((?:[\w\u00c0-\uFFFF\-]|\\.)+)/g;
    bitbucket_SelectorSet.prototype.indexes.push({
      name: "CLASS",
      selector: function matchClassSelector(sel) {
        var m;
        if (m = sel.match(bitbucket_classRe)) return m[0].slice(1);
      },
    ...
    var bitbucket_tagRe = /^((?:[\w\u00c0-\uFFFF\-]|\\.)+)/g;
    bitbucket_SelectorSet.prototype.indexes.push({
      name: "TAG",
      selector: function matchTagSelector(sel) {
        var m;
        if (m = sel.match(bitbucket_tagRe)) return m[0].toUpperCase();
      },
      element: function getElementTagName(el) {
        return [ el.nodeName.toUpperCase() ];
      }
    });
    bitbucket_SelectorSet.prototype.indexes["default"] = {
      name: "UNIVERSAL",
      selector: function() {
        return true;
      },
    ...
    var bitbucket_Map;
    if (typeof window.Map === "function") bitbucket_Map = window.Map; else bitbucket_Map = function() {
      function Map() {
        this.map = {};
      }
    ...
    var bitbucket_chunker = /((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g;
    function bitbucket_parseSelectorIndexes(allIndexes, selector) {
      allIndexes = allIndexes.slice(0).concat(allIndexes["default"]);
      var i, j, m, dup, key, index, allIndexesLen = allIndexes.length, rest = selector, indexes = [];
      do {
        bitbucket_chunker.exec("");
        if (m = bitbucket_chunker.exec(rest)) {
          rest = m[3];
    ...
    function bitbucket_findByPrototype(ary, proto) {
      var i, len, item;
    ...
    bitbucket_SelectorSet.prototype.logDefaultIndexUsed = function() {};
    bitbucket_SelectorSet.prototype.add = function(selector, data) {
      ...
      selectorIndexes = bitbucket_parseSelectorIndexes(this.indexes, selector);
      for (i = 0; i < selectorIndexes.length; i++) {
        selectorIndex = selectorIndexes[i];
        key = selectorIndex.key;
        indexProto = selectorIndex.index;
        index = bitbucket_findByPrototype(indexes, indexProto);
        if (!index) {
          index = Object.create(indexProto);
          index.map = new bitbucket_Map;
          indexes.push(index);
        }
    ...
    bitbucket_SelectorSet.prototype.remove = function(selector, data) {
      if (typeof selector !== "string") return;
    ...
    function bitbucket_sortById(a, b) {
      return a.id - b.id;
    }
    bitbucket_SelectorSet.prototype.queryAll = function(context) {
      ...
      return results.sort(bitbucket_sortById);
    };
    bitbucket_SelectorSet.prototype.matches = function(el) {
      if (!el) return [];
      ...
      return matches.sort(bitbucket_sortById);
    };
    var bitbucket_el = null;
    var bitbucket_observer = null;
    var bitbucket_queue = [];
    function bitbucket_scheduleBatch(document, callback) {
      var calls = [];
      ...
      function scheduleBatchQueue() {
        ...
        calls.push(args);
        if (calls.length === 1) bitbucket_scheduleMacroTask(document, processBatchQueue);
      }
      return scheduleBatchQueue;
    }
    function bitbucket_scheduleMacroTask(document, callback) {
      if (!bitbucket_observer) bitbucket_observer = new MutationObserver(bitbucket_handleMutations);
      if (!bitbucket_el) {
        bitbucket_el = document.createElement("div");
        bitbucket_observer.observe(bitbucket_el, {
          attributes: true
        });
      }
      bitbucket_queue.push(callback);
      bitbucket_el.setAttribute("data-twiddle", "" + Date.now());
    }
    function bitbucket_handleMutations() {
      var callbacks = bitbucket_queue;
      bitbucket_queue = [];
    ...
    var bitbucket_initMap = new WeakMap;
    var bitbucket_initializerMap = new WeakMap;
    var bitbucket_subscriptionMap = new WeakMap;
    var bitbucket_addMap = new WeakMap;
    function bitbucket_applyChanges(selectorObserver, changes) {
      for (var i = 0; i < changes.length; i++) {
        var change = changes[i];
        var type = change[0];
        var el = change[1];
        var observer = change[2];
        if (type === bitbucket_ADD) {
          bitbucket_runInit(observer, el);
          bitbucket_runAdd(observer, el);
        } else if (type === bitbucket_REMOVE) bitbucket_runRemove(observer, el); else if (type === bitbucket_REMOVE_ALL) bitbucket_runRemoveAll(selectorObserver.observers, el);
      }
    }
    function bitbucket_runInit(observer, el) {
      if (!(el instanceof observer.elementConstructor)) return;
      var initIds = bitbucket_initMap.get(el);
      if (!initIds) {
        initIds = [];
        bitbucket_initMap.set(el, initIds);
      }
      if (initIds.indexOf(observer.id) === -1) {
        var initializer = void 0;
        if (observer.initialize) initializer = observer.initialize.call(void 0, el);
        if (initializer) {
          var initializers = bitbucket_initializerMap.get(el);
          if (!initializers) {
            initializers = {};
            bitbucket_initializerMap.set(el, initializers);
          }
    ...
    function bitbucket_runAdd(observer, el) {
      if (!(el instanceof observer.elementConstructor)) return;
      var addIds = bitbucket_addMap.get(el);
      if (!addIds) {
        addIds = [];
        bitbucket_addMap.set(el, addIds);
      }
      if (addIds.indexOf(observer.id) === -1) {
        ...
        if (observer.subscribe) {
          ...
          if (subscription) {
            var subscriptions = bitbucket_subscriptionMap.get(el);
            if (!subscriptions) {
              subscriptions = {};
              bitbucket_subscriptionMap.set(el, subscriptions);
            }
    ...
    function bitbucket_runRemove(observer, el) {
      if (!(el instanceof observer.elementConstructor)) return;
      var addIds = bitbucket_addMap.get(el);
      if (!addIds) return;
      ...
      if (addIds.length === 0) bitbucket_addMap.delete(el);
    }
    function bitbucket_runRemoveAll(observers, el) {
      var addIds = bitbucket_addMap.get(el);
      if (!addIds) return;
      ...
      bitbucket_addMap.delete(el);
    }
    var bitbucket_innerHTMLReplacementIsBuggy = null;
    function bitbucket_detectInnerHTMLReplacementBuggy(document) {
      if (bitbucket_innerHTMLReplacementIsBuggy === null) {
        ...
        b.appendChild(c);
        a.innerHTML = "";
        bitbucket_innerHTMLReplacementIsBuggy = c.parentNode !== b;
      }
      return bitbucket_innerHTMLReplacementIsBuggy;
    }
    function bitbucket_supportsSelectorMatching(node) {
    ...
    var bitbucket_ADD = 1;
    var bitbucket_REMOVE = 2;
    var bitbucket_REMOVE_ALL = 3;
    function bitbucket_handleMutations$1(selectorObserver, changes, mutations) {
      for (var i = 0; i < mutations.length; i++) {
        var mutation = mutations[i];
        if (mutation.type === "childList") {
          bitbucket_addNodes(selectorObserver, changes, mutation.addedNodes);
          bitbucket_removeNodes(selectorObserver, changes, mutation.removedNodes);
        } else if (mutation.type === "attributes") bitbucket_revalidateObservers(selectorObserver, changes, mutation.target);
      }
      if (bitbucket_detectInnerHTMLReplacementBuggy(selectorObserver.ownerDocument)) bitbucket_revalidateOrphanedElements(selectorObserver, changes);
    }
    function bitbucket_addNodes(selectorObserver, changes, nodes) {
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        if (bitbucket_supportsSelectorMatching(node)) {
          var matches = selectorObserver.selectorSet.matches(node);
          for (var j = 0; j < matches.length; j++) {
            var data = matches[j].data;
            changes.push([ bitbucket_ADD, node, data ]);
          }
        }
        if ("querySelectorAll" in node) {
          ...
          for (var _j = 0; _j < matches2.length; _j++) {
            ...
            for (var k = 0; k < elements.length; k++) changes.push([ bitbucket_ADD, elements[k], _data ]);
          }
        }
      }
    }
    function bitbucket_removeNodes(selectorObserver, changes, nodes) {
      for (var i = 0; i < nodes.length; i++) {
        var node = nodes[i];
        if ("querySelectorAll" in node) {
          changes.push([ bitbucket_REMOVE_ALL, node ]);
          var descendants = node.querySelectorAll("*");
          for (var j = 0; j < descendants.length; j++) changes.push([ bitbucket_REMOVE_ALL, descendants[j] ]);
        }
      }
    }
    function bitbucket_revalidateObservers(selectorObserver, changes, node) {
      if (bitbucket_supportsSelectorMatching(node)) {
        var matches = selectorObserver.selectorSet.matches(node);
      ...
      if ("querySelectorAll" in node) {
        var ids = bitbucket_addMap.get(node);
        if (ids) for (var _i = 0; _i < ids.length; _i++) {
          var observer = selectorObserver.observers[ids[_i]];
          if (observer) if (!selectorObserver.selectorSet.matchesSelector(node, observer.selector)) changes.push([ bitbucket_REMOVE, node, observer ]);
        }
      }
    }
    function bitbucket_revalidateDescendantObservers(selectorObserver, changes, node) {
      if ("querySelectorAll" in node) {
        bitbucket_revalidateObservers(selectorObserver, changes, node);
        var descendants = node.querySelectorAll("*");
        for (var i = 0; i < descendants.length; i++) bitbucket_revalidateObservers(selectorObserver, changes, descendants[i]);
      }
    }
    function bitbucket_revalidateInputObservers(selectorObserver, changes, inputs) {
      for (var i = 0; i < inputs.length; i++) {
        var input = inputs[i];
        var els = input.form ? input.form.elements : selectorObserver.rootNode.querySelectorAll("input");
        for (var j = 0; j < els.length; j++) bitbucket_revalidateObservers(selectorObserver, changes, els[j]);
      }
    }
    function bitbucket_revalidateOrphanedElements(selectorObserver, changes) {
      for (var i = 0; i < selectorObserver.observers.length; i++) {
        ...
        if (observer) {
          ...
          for (var j = 0; j < elements.length; j++) {
            var el = elements[j];
            if (!el.parentNode) changes.push([ bitbucket_REMOVE_ALL, el ]);
          }
        }
      }
    }
    function bitbucket_whenReady(document, callback) {
      var readyState = document.readyState;
      if (readyState === "interactive" || readyState === "complete") bitbucket_scheduleMacroTask(document, callback); else document.addEventListener("DOMContentLoaded", bitbucket_scheduleMacroTask(document, callback));
    }
    var bitbucket_typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function(obj) {
      return typeof obj;
    } : function(obj) {
      return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj;
    };
    var bitbucket_uid = 0;
    function bitbucket_SelectorObserver(rootNode) {
      ...
      this.observers = [];
      this.selectorSet = new bitbucket_SelectorSet;
      this.mutationObserver = new MutationObserver(bitbucket_handleRootMutations.bind(this, this));
      this._scheduleAddRootNodes = bitbucket_scheduleBatch(this.ownerDocument, bitbucket_addRootNodes.bind(this, this));
      this._handleThrottledChangedTargets = bitbucket_scheduleBatch(this.ownerDocument, bitbucket_handleChangedTargets.bind(this, this));
      this.rootNode.addEventListener("change", bitbucket_handleChangeEvents.bind(this, this), false);
      bitbucket_whenReady(this.ownerDocument, bitbucket_onReady.bind(this, this));
    }
    bitbucket_SelectorObserver.prototype.disconnect = function() {
      this.mutationObserver.disconnect();
    };
    bitbucket_SelectorObserver.prototype.observe = function(a, b) {
      var handlers = void 0;
      if (typeof b === "function") handlers = {
        selector: a,
        initialize: b
      }; else if ((typeof b === "undefined" ? "undefined" : bitbucket_typeof(b)) === "object") {
        handlers = b;
        handlers.selector = a;
      } else handlers = a;
      var self = this;
    ...
    bitbucket_SelectorObserver.prototype._abortObserving = function(observer) {
      var elements = observer.elements;
      for (var i = 0; i < elements.length; i++) bitbucket_runRemove(observer, elements[i]);
      this.selectorSet.remove(observer.selector, observer);
      delete this.observers[observer.id];
    };
    bitbucket_SelectorObserver.prototype.triggerObservers = function(container) {
      var changes = [];
      bitbucket_revalidateDescendantObservers(this, changes, container);
      bitbucket_applyChanges(this, changes);
    };
    function bitbucket_onReady(selectorObserver) {
    ...
    function bitbucket_addRootNodes(selectorObserver) {
      var changes = [];
      bitbucket_addNodes(selectorObserver, changes, [ selectorObserver.rootNode ]);
      bitbucket_applyChanges(selectorObserver, changes);
    }
    function bitbucket_handleRootMutations(selectorObserver, mutations) {
      var changes = [];
      bitbucket_handleMutations$1(selectorObserver, changes, mutations);
      bitbucket_applyChanges(selectorObserver, changes);
    }
    function bitbucket_handleChangeEvents(selectorObserver, event) {
      selectorObserver._handleThrottledChangedTargets(event.target);
    }
    function bitbucket_handleChangedTargets(selectorObserver, inputs) {
      var changes = [];
      bitbucket_revalidateInputObservers(selectorObserver, changes, inputs);
      bitbucket_applyChanges(selectorObserver, changes);
    }
    var bitbucket_documentObserver = void 0;
    function bitbucket_getDocumentObserver() {
      if (!bitbucket_documentObserver) bitbucket_documentObserver = new bitbucket_SelectorObserver(window.document);
      return bitbucket_documentObserver;
    }
    function bitbucket_index_esm_observe() {
      var _getDocumentObserver;
      return (_getDocumentObserver = bitbucket_getDocumentObserver()).observe.apply(_getDocumentObserver, arguments);
    }
    var bitbucket_bitbucket_url_to_object = __webpack_require__(8787);
    const bitbucket_DEFAULT_LANGUAGE = "java";
    const bitbucket_USAGE_THRESHOLD_PERCENT = 5;
    const bitbucket_HUNDRED_PERCENT = 100;
    class bitbucket_Language_Language {
      #name;
      #percentage;
      ...
      get isRelevant() {
        return this.percentage > bitbucket_USAGE_THRESHOLD_PERCENT;
      }
      static Default=new bitbucket_Language_Language(bitbucket_DEFAULT_LANGUAGE, bitbucket_HUNDRED_PERCENT);
    }
    const bitbucket_SUPPORTED_LANGUAGES = {
      [bitbucket_DEFAULT_LANGUAGE]: [ "idea" ],
      kotlin: [ "idea" ],
      groovy: [ "idea" ],
      scala: [ "idea" ],
    ...
    class bitbucket_Tool {
      ...
      static Default=new bitbucket_Tool("IntelliJ IDEA", "idea", bitbucket_intellij_idea_namespaceObject);
    }
    const bitbucket_idea = bitbucket_Tool.Default;
    const bitbucket_appcode = new bitbucket_Tool("AppCode", "appcode", bitbucket_appcode_namespaceObject);
    const bitbucket_clion = new bitbucket_Tool("CLion", "clion", bitbucket_clion_namespaceObject);
    const bitbucket_pycharm = new bitbucket_Tool("PyCharm", "pycharm", bitbucket_pycharm_namespaceObject);
    const bitbucket_phpstorm = new bitbucket_Tool("PhpStorm", "php-storm", bitbucket_phpstorm_namespaceObject);
    const bitbucket_rubymine = new bitbucket_Tool("RubyMine", "rubymine", bitbucket_rubymine_namespaceObject);
    const bitbucket_webstorm = new bitbucket_Tool("WebStorm", "web-storm", bitbucket_webstorm_namespaceObject);
    const bitbucket_rider = new bitbucket_Tool("Rider", "rd", bitbucket_rider_namespaceObject);
    const bitbucket_goland = new bitbucket_Tool("GoLand", "goland", bitbucket_goland_namespaceObject);
    const bitbucket_rustrover = new bitbucket_Tool("RustRover", "rustrover", bitbucket_rustrover_namespaceObject);
    const bitbucket_SUPPORTED_TOOLS = {
      idea: bitbucket_idea,
      appcode: bitbucket_appcode,
      clion: bitbucket_clion,
      pycharm: bitbucket_pycharm,
      phpstorm: bitbucket_phpstorm,
      rubymine: bitbucket_rubymine,
      webstorm: bitbucket_webstorm,
      rider: bitbucket_rider,
      goland: bitbucket_goland,
      rustrover: bitbucket_rustrover
    };
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.idea, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.webstorm, bitbucket_SUPPORTED_TOOLS.phpstorm, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.webstorm, bitbucket_SUPPORTED_TOOLS.phpstorm, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.webstorm, bitbucket_SUPPORTED_TOOLS.phpstorm, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.webstorm, bitbucket_SUPPORTED_TOOLS.phpstorm, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.webstorm, bitbucket_SUPPORTED_TOOLS.phpstorm, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.webstorm, bitbucket_SUPPORTED_TOOLS.phpstorm, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.goland, bitbucket_SUPPORTED_TOOLS.idea, 
    bitbucket_SUPPORTED_TOOLS.phpstorm, bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.pycharm, 
    bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.pycharm, bitbucket_SUPPORTED_TOOLS.idea, 
    bitbucket_SUPPORTED_TOOLS.rider, bitbucket_SUPPORTED_TOOLS.rider, bitbucket_SUPPORTED_TOOLS.clion, 
    bitbucket_SUPPORTED_TOOLS.clion, bitbucket_SUPPORTED_TOOLS.rubymine, bitbucket_SUPPORTED_TOOLS.idea, 
    bitbucket_SUPPORTED_TOOLS.rustrover, bitbucket_SUPPORTED_TOOLS.clion, bitbucket_SUPPORTED_TOOLS.idea, 
    bitbucket_SUPPORTED_TOOLS.rubymine, bitbucket_SUPPORTED_TOOLS.idea, bitbucket_SUPPORTED_TOOLS.appcode, 
    bitbucket_SUPPORTED_TOOLS.appcode;
    const bitbucket_convertNumberToIndex = number => {
      const normalizedNumber = Number.isInteger(number) ? number : 1;
      return normalizedNumber - 1;
    };
    const bitbucket_parseLineNumber = lineNumber => {
      const parsedValue = Number.parseInt(lineNumber, 10);
      return isNaN(parsedValue) ? 1 : parsedValue;
    };
    const bitbucket_Toolbox_getToolboxCloneUrl = (toolTag, cloneUrl) => `jetbrains://${toolTag}/checkout/git?checkout.repo=${cloneUrl}&idea.required.plugins.id=Git4Idea`;
    const bitbucket_getToolboxNavigateUrl = (toolTag, project, filePath, lineNumber = null) => {
      const lineIndex = bitbucket_convertNumberToIndex(lineNumber == null ? 1 : lineNumber);
      const columnIndex = bitbucket_convertNumberToIndex(1);
    ...
    const bitbucket_Toolbox_callToolbox = action => {
    ...
    const bitbucket_PROTOCOLS = {
      HTTPS: "HTTPS",
      SSH: "SSH"
    };
    const bitbucket_CLONE_BUTTON_PAGE_HEADER_WRAPPER_SELECTOR = '[data-qa="page-header-wrapper"] > div > div > div > div > div > button:last-of-type';
    const bitbucket_CLONE_BUTTON_CONTENT_SELECTOR = '#root [data-testid="Content"] > div > div > div > div > div > div > div > div + div > div:last-child > button:last-child';
    const bitbucket_CLONE_BUTTON_NARROW_PAGE_SELECTOR = "#root > div > div > div > header + div + div + div > div > div > div > div + div > div + div > button:last-child";
    const bitbucket_CLONE_BUTTON_NARROW_PAGE_SIDE_PANEL_OPEN = "#root > div > div > div > div:last-child > div > div > div > div + div > div:last-child > button:last-child";
    const bitbucket_cloneButtonSelectors = [ bitbucket_CLONE_BUTTON_CONTENT_SELECTOR, bitbucket_CLONE_BUTTON_PAGE_HEADER_WRAPPER_SELECTOR, bitbucket_CLONE_BUTTON_NARROW_PAGE_SELECTOR, bitbucket_CLONE_BUTTON_NARROW_PAGE_SIDE_PANEL_OPEN ];
    const bitbucket_CLONE_BUTTON_JS_CSS_CLASS = "js-toolbox-clone-button";
    const bitbucket_OPEN_BUTTON_JS_CSS_CLASS = "js-toolbox-open-button";
    const bitbucket_fetchMetadata = () => new Promise(((resolve, reject) => {
      const metadata = bitbucket_bitbucket_url_to_object(window.location.toString());
      if (metadata) {
        metadata.api_url = metadata.api_url.replace("api.bitbucket.org/2.0", "bitbucket.org/!api/2.0");
        fetch(`${metadata.api_url}?fields=links.clone`).then((response => response.json())).then((parsedResponse => {
    ...
    const bitbucket_fetchLanguages = bitbucketMetadata => new Promise(((resolve, reject) => {
      fetch(`${bitbucketMetadata.api_url}?fields=language`).then((response => response.json())).then((parsedResponse => {
        resolve(parsedResponse.language);
      })).catch((() => {
        reject();
      }));
    }));
    const bitbucket_selectTools = language => new Promise((resolve => {
      const normalizedLanguage = language === "html/css" ? "html" : language;
      const toolIds = normalizedLanguage && bitbucket_SUPPORTED_LANGUAGES[normalizedLanguage.toLowerCase()];
      const normalizedToolIds = toolIds && toolIds.length > 0 ? toolIds : bitbucket_SUPPORTED_LANGUAGES[bitbucket_DEFAULT_LANGUAGE];
      const tools = normalizedToolIds.sort().map((toolId => bitbucket_SUPPORTED_TOOLS[toolId]));
      resolve(tools);
    }));
    const bitbucket_fetchTools = bitbucketMetadata => bitbucket_fetchLanguages(bitbucketMetadata).then(bitbucket_selectTools);
    let bitbucket_onMessageHandler = null;
    const bitbucket_renderPageAction = bitbucketMetadata => new Promise((resolve => {
      ...
      bitbucket_onMessageHandler = (message, sender, sendResponse) => {
        switch (message.type) {
         case "get-tools":
          bitbucket_fetchTools(bitbucketMetadata).then(sendResponse);
          return true;

         case "perform-action":
          const toolboxAction = bitbucket_Toolbox_getToolboxCloneUrl(message.toolTag, message.cloneUrl);
          bitbucket_Toolbox_callToolbox(toolboxAction);
          break;
        }
        return;
      };
    ...
    const bitbucket_getCloneUrl = (links, which) => {
      const link = links.clone.find((l => l.name === which));
      return link ? link.href : "";
    };
    const bitbucket_getHttpsCloneUrl = links => bitbucket_getCloneUrl(links, "https");
    const bitbucket_getSshCloneUrl = links => bitbucket_getCloneUrl(links, "ssh");
    const bitbucket_addStyleSheet = () => {
    ...
    const bitbucket_createButtonTooltip = (button, text) => {
    ...
    const bitbucket_cloneButtonsRendered = () => document.getElementsByClassName(bitbucket_CLONE_BUTTON_JS_CSS_CLASS).length > 0;
    const bitbucket_addCloneButtonEventHandler = (btn, bitbucketMetadata) => {
      btn.addEventListener("click", (e => {
        ...
        chrome.runtime.sendMessage({
          ...
          const cloneUrl = protocol === bitbucket_PROTOCOLS.HTTPS ? bitbucket_getHttpsCloneUrl(bitbucketMetadata.links) : bitbucket_getSshCloneUrl(bitbucketMetadata.links);
          const action = bitbucket_Toolbox_getToolboxCloneUrl(toolTag, cloneUrl);
          bitbucket_Toolbox_callToolbox(action);
        }));
      }));
    };
    const bitbucket_createCloneButton = (tool, cloneButton, bitbucketMetadata) => {
      const button = document.createElement("a");
      button.setAttribute("class", `${cloneButton.className} jt-button ${bitbucket_CLONE_BUTTON_JS_CSS_CLASS}`);
    ...
    const bitbucket_removeCloneButtons = () => {
      const buttonGroup = document.querySelector(".jt-button-group");
      if (buttonGroup) buttonGroup.parentElement.removeChild(buttonGroup);
    };
    const bitbucket_renderCloneButtons = (tools, bitbucketMetadata, cloneButton = null) => {
      if (bitbucket_cloneButtonsRendered()) return;
      if (!cloneButton) cloneButton = document.querySelector(bitbucket_cloneButtonSelectors.join(", "));
      if (!cloneButton || !cloneButton.textContent.includes("Clone")) return;
      bitbucket_addStyleSheet();
      const buttonGroup = document.createElement("div");
      buttonGroup.setAttribute("class", "jt-button-group");
      tools.forEach((tool => {
        const btn = bitbucket_createCloneButton(tool, cloneButton, bitbucketMetadata);
        buttonGroup.appendChild(btn);
        const tooltip = bitbucket_createButtonTooltip(btn, `Clone in ${tool.name}`);
        buttonGroup.appendChild(tooltip);
      }));
      cloneButton.insertAdjacentElement("beforebegin", buttonGroup);
    };
    const bitbucket_removeOpenButtons = () => {
      document.querySelectorAll(`.${bitbucket_OPEN_BUTTON_JS_CSS_CLASS}`).forEach((b => {
        b.remove();
      }));
    };
    const bitbucket_addOpenButtonEventHandler = (domElement, tool, bitbucketMetadata) => {
      domElement.addEventListener("click", (e => {
        e.preventDefault();
        const filePathIndex = 5;
        const filePath = location.pathname.split("/").splice(filePathIndex).join("/");
        const lineNumber = bitbucket_parseLineNumber(location.hash.replace("#lines-", ""));
        bitbucket_Toolbox_callToolbox(bitbucket_getToolboxNavigateUrl(tool.tag, bitbucketMetadata.repo, filePath, lineNumber));
      }));
    };
    const bitbucket_createOpenButton = (tool, sampleButton, bitbucketMetadata) => {
      const button = sampleButton.cloneNode(true);
      button.classList.add(bitbucket_OPEN_BUTTON_JS_CSS_CLASS);
      ...
      const tooltip = bitbucket_createButtonTooltip(actionButton, `Open this file in ${tool.name}`);
      button.appendChild(tooltip);
      return button;
    };
    const bitbucket_openButtonsRendered = () => document.getElementsByClassName(bitbucket_OPEN_BUTTON_JS_CSS_CLASS).length > 0;
    const bitbucket_renderOpenButtons = (tools, bitbucketMetadata) => {
      if (bitbucket_openButtonsRendered()) return;
      const actionAnchorElement = document.querySelector('[data-qa="bk-file__actions"] > [data-qa="bk-file__action-button"]');
      if (actionAnchorElement) tools.forEach((tool => {
        const action = bitbucket_createOpenButton(tool, actionAnchorElement, bitbucketMetadata);
        actionAnchorElement.insertAdjacentElement("beforebegin", action);
      }));
    };
    const bitbucket_startTrackingDOMChanges = () => {
      const cloneButtonsObserver = bitbucket_index_esm_observe(bitbucket_cloneButtonSelectors.join(", "), {
        add(el) {
          if (el.textContent.includes("Clone")) bitbucket_fetchMetadata().then((metadata => {
            bitbucket_fetchTools(metadata).then((tools => {
              bitbucket_renderCloneButtons(tools, metadata, el);
            }));
          }));
        },
        remove() {
          bitbucket_removeCloneButtons();
        }
      });
      const openButtonsObserver = bitbucket_index_esm_observe('[data-qa="bk-file__header"] > div > [data-qa="bk-file__actions"]', {
        add() {
          bitbucket_fetchMetadata().then((metadata => {
            bitbucket_fetchTools(metadata).then((tools => {
              bitbucket_renderOpenButtons(tools, metadata);
            }));
          }));
        },
        remove() {
          bitbucket_removeOpenButtons();
        }
      });
      return [ cloneButtonsObserver, openButtonsObserver ];
    };
    const bitbucket_stopTrackingDOMChanges = observers => {
      if (observers) observers.forEach((o => {
        o.abort();
      }));
    };
    const bitbucket_enablePageAction = bitbucketMetadata => {
      chrome.runtime.sendMessage({
        type: "enable-page-action",
        project: bitbucketMetadata.repo,
        https: bitbucket_getHttpsCloneUrl(bitbucketMetadata.links),
        ssh: bitbucket_getSshCloneUrl(bitbucketMetadata.links)
      });
    };
    const bitbucket_disablePageAction = () => {
      chrome.runtime.sendMessage({
        type: "disable-page-action"
      });
    };
    const bitbucket_refreshPageAction = () => {
      bitbucket_fetchMetadata().then((metadata => {
        bitbucket_renderPageAction(metadata).then((() => {
          bitbucket_enablePageAction(metadata);
        }));
      })).catch((() => {
        bitbucket_disablePageAction();
      }));
    };
    const bitbucket_startTrackingClientNavigation = () => {
      const titleObserver = new MutationObserver((() => {
        bitbucket_refreshPageAction();
      }));
      titleObserver.observe(document.querySelector("title"), {
        childList: true
      });
    };
    const bitbucket_toolboxify = () => {
      bitbucket_refreshPageAction();
      bitbucket_startTrackingClientNavigation();
      chrome.runtime.sendMessage({
        ...
        chrome.runtime.onMessage.addListener((message => {
          switch (message.type) {
           case "modify-pages-changed":
            if (message.newValue) DOMObservers = bitbucket_startTrackingDOMChanges(); else bitbucket_stopTrackingDOMChanges(DOMObservers);
            break;
          }
        }));
      }));
    };
    const bitbucket_bitbucket = bitbucket_toolboxify;
    bitbucket_bitbucket();
  })();
})();