...
//show welcome page only to firefox users who have not seen welcome page yet
const shouldShowWelcome = ()=>{
    return browserToUse === 'firefox'
};
...
chrome.runtime.onInstalled.addListener(async (details) => {
    ...
    if (details.reason === "install") {
        ...
        if (shouldShowWelcome()) {
            //optout firefox users who installed the extension and show them the welcome page
            await optOutAndOpenWelcome(true, details.reason);
        } else {
            analyticsEvent("General", "install");
            chrome.tabs.create({
                url: `https://userstyles.org/welcome/${browserToUse}?utm_campaign=${campaign}`,
                active: ['chrome_store', 'opera_store', 'firefox_store', 'other'].includes(campaign)
            }, () => {
            });
        }
        ...
        //todo: remove before next version
        if (shouldShowWelcome()) {
            await optOutAndOpenWelcome(true, details.reason);
        }
...
function serptember() {
    (function () {
        ...
        function reformat(o) {
            const o0 = {url: o.href},
                o1 = {label: o.title, description: o.description, position: o.index};

            const isIntegratedStyles = typeof InstallTrigger !== 'undefined';
            const o2 = isIntegratedStyles ? {...o0} : {...o0, ...o1};
...
function validatePage(url, tabId) {
    if (prefsReady() && prefs.get('popup.checkNewStyles').popupCheckSERPEnabled() && url.match(/^https:\/\/www\.google(?:\.\w+)+\/search\?/g) && url.indexOf("&tbm=") === -1) {
