/**
 * (c) 2014 Rob Wu [rob@robwu.nl](mailto:rob@robwu.nl) (https://robwu.nl)
 * License: MIT/X11 https://github.com/Rob--W/chrome-api/tree/master/worker_proxy/
 *
 ...
 * used in a content script. This limitation has been resolved before by
 * patch-worker.js (available at
 ...
 * - Origin-specific features such as IndexedDB or the FileSystem API use the web
 *   page's origin instead of the extension's.
 * - Cross-origin XMLHttpRequest is not possible because the extension's
 *   permissions are not available.
 *
 * All of the previous issues are solved with this patch, because it runs the
 * Worker on the extension's origin. The patch has two constraints:
 *
 ...
 * - Transferable messages must NOT be used until http://crbug.com/334408 is fixed.
 *
 ...
 *
 * "content_scripts": [{ "js": [ "worker_proxy.js", ... ], "matches": [ ... ]
 * }], "background": { "scripts": ["worker_proxy.js", ...] },
 * "web_accessible_resources": ["worker_proxy.html"]
 *
 * Worker_proxy.html contains:<script src="worker_proxy.js"></script>
 */
...
(function () {
   ...
   * @param {MessagePort} metadataPort Port used for sending internal data such
   *   as error events.
   * @param {string} url URL of Web worker (relative to the location of the HTML
   *   file that embeds this script).
   */
  ...
  function postMessageToWorkerProxy(message, transferables) {
    ...
    if (!proxyFrame) {
      ...
      chrome.runtime.sendMessage(MSG_GET_TOKEN, function (token) {
        if (typeof token != 'string') {
          ...
          console.warn(
            'Failed to initialize Worker because of a ' +
    ...
    function flushMessages(token) {
      ...
      if (!contentWindow) {
        ...
        console.warn(
          'WARNING: The worker proxy frame was removed; ' +
