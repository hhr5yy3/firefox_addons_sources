	...
	Alertify = function () {
		...
		_alertify = {
			...
			notify : function (message, type, wait) {
				...
				// log.innerHTML = message; // Firefox Add-on Review system complains about this
			...
			setup : function (fromQueue) {
				...
				// elDialog.innerHTML = this.build(item); // Firefox Add-on Review complains about this
				
				// alternative implementation for the row above
				var htmlElement = jQuery.parseHTML(this.build(item));
				while (elDialog.firstChild) {
					elDialog.firstChild.remove();
				}
				elDialog.appendChild(htmlElement[0]);
