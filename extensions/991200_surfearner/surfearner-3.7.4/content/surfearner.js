...
//        chrome.storage.local.get('state', function (d) {
        surfearner.state = 'true';
        if (surfearner.page !== 'loading') {
          var frame = document.querySelector('#surfearner #surfearner_frame');
          if (surfearner.anim_type === 1) classes = document.body.className;
          if (frame && frame.className !== 'surfearner_frame') {
            ...
            if (document.getElementById('surfearner').className === 'se_top') {
              if (surfearner.anim_type === 1) document.body.className = classes + ' surfearned';
              if (surfearner.anim_type === 1) scrolling(65);
...
//        });
      ...
      banner_hide: function () {
        ...
        if (frame && frame.className === 'surfearner_frame') {
          ...
          if (surfearner.anim_type === 1) document.body.className = classes.replace('surfearned', '');
          surfearner.pause();
          if (document.getElementById('surfearner').className === 'se_top') {
            if (surfearner.anim_type === 1) scrolling(-65);
            if (surfearner.css_injected)
