...
var __publicField = (obj, key, value) => __defNormalProp(obj, typeof key !== "symbol" ? key + "" : key, value);
  ...
  (function(module, exports) {
    (function(root, factory) {
      ...
      return {
         ...
         */
        instrument: function StackTrace$$instrument(fn2, callback, errback, thisArg) {
          if (typeof fn2 !== "function") {
            throw new Error("Cannot instrument non-function object");
          } else if (typeof fn2.__stacktraceOriginalFn === "function") {
            return fn2;
          }
          var instrumented = (function StackTrace$$instrumented() {
            try {
              this.get().then(callback, errback)["catch"](errback);
              return fn2.apply(thisArg || this, arguments);
            } catch (e) {
          ...
          instrumented.__stacktraceOriginalFn = fn2;
          return instrumented;
        },
        /**
         ...
         */
        deinstrument: function StackTrace$$deinstrument(fn2) {
          if (typeof fn2 !== "function") {
            throw new Error("Cannot de-instrument non-function object");
          } else if (typeof fn2.__stacktraceOriginalFn === "function") {
            return fn2.__stacktraceOriginalFn;
          } else {
            return fn2;
          }
        },
  ...
  }), Ai = c.record(c.string(), us), Ri = (e) => us.safeParse(e).success, wi = (e) => Ai.safeParse(e).success;
  function Ci(e) {
    return ls.safeParse(e).success;
  }
  const ih = (e) => {
    if (!wi(e))
      throw new Error("Error: the customer list data is invalid.");
    return { tenants: Object.entries(e).map(
      ([r, s]) => ki(r, s)
    ) };
  }, ki = (e, t) => {
    if (!Ri(t))
      throw new Error(`Invalid customer: ${t}`);
    const { name: r, region: s } = t, n = { id: e, name: r, region: s };
    if (!Ci(n))
      throw new Error(`Invalid tenant: ${JSON.stringify(n)}`);
    return n;
  }, ds = c.object({
    parserType: c.string(),
  ...
  const lh = (e) => Ni.safeParse(e).success;
  function hh(e) {
    return fs.safeParse(e).success;
  }
  function Cs(e) {
  ...
  const qc = "5.14.0", Us = () => qc, Bc = ["com.au", "com", "ca", "co.uk", "de", "fr", "nl"], Mc = Bc.flatMap((e) => [
    ...
    version: c.string()
  }), mh = (e) => Uc.safeParse(e).success, Jc = (e) => typeof e == "object" && e !== null;
  c.object({
    url: c.string(),
  ...
  const zd = (e, t) => {
    ...
    const s = Ei(t);
    s.append("fast", "true"), s.append("breakout", "true"), r && (r.parserName && s.append("targetTitle", r.parserName), s.append("referrer", r.referrer));
  ...
  const storage = browser.storage.sync ? browser.storage.sync : browser.storage.local;
  background;
  const legacyActiveTenantV2Schema = z$1.object({
    code: z$1.string()
  });
  const isLegacyActiveTenant = (value) => legacyActiveTenantV2Schema.safeParse(value).success;
  const removeTenant = async () => {
    await storage.remove("selectedTenant");
  };
  ...
  const fetchTenant = async () => {
    const result2 = await storage.get(["selectedTenant", "activeTenant"]);
    const { selectedTenant, activeTenant } = result2;
    if (!selectedTenant && isLegacyActiveTenant(activeTenant)) {
      return await fetchTenantById(activeTenant.code);
    }
    if (!selectedTenant) {
      return null;
    }
    if (Ci(selectedTenant)) {
      return selectedTenant;
    } else {
      await removeTenant();
      const selectedTenantString = stringify(selectedTenant);
      await logError(
        new Error(`invalid selectedTenant: ${selectedTenantString}`)
      );
      return null;
    }
  };
  const migrateLegacyTenant = async () => {
    const result2 = await storage.get(["activeTenant"]);
    const { activeTenant } = result2;
    if (isLegacyActiveTenant(activeTenant)) {
      const tenant = await fetchTenantById(activeTenant.code);
      if (tenant) {
        await logTenantLegacyMigrated();
        await updateTenant(tenant);
        await storage.remove("activeTenant");
      }
    }
  };
  const updateTenant = async (tenant) => {
    await storage.set({ selectedTenant: tenant });
    await logTenantUpdated();
  };
  background;
  const config = {
    customerConfigurationUrl: "https://resource-parsers.talis.com/1/customer_list",
    regions: {
      EUAPAC: {
        apiBaseUrl: "https://resource-parsers.talis.com"
      },
      CA: {
        apiBaseUrl: "https://resource-parsers.ca.talis.com"
      }
    }
  };
  const getBaseUrlForCustomerOrDefault = async () => {
    const selectedTenant = await fetchTenant();
    if (!selectedTenant) {
      return config.regions.EUAPAC.apiBaseUrl;
    }
    return config.regions[selectedTenant.region].apiBaseUrl;
  };
  const getConfigurationUrl = () => {
    return config.customerConfigurationUrl;
  };
  background;
  const send$1 = async (url, method = "GET", body) => {
    ...
    if (!response.ok) {
      throw Error(`HTTP response failure to ${url}: ${response.status}`);
    }
    try {
      return await response.json();
    } catch {
      throw Error(`HTTP parsing failure to ${url}`);
    }
  };
  const postParseResource = async (parsingRequest) => {
    const apiOrigin = await getBaseUrlForCustomerOrDefault();
    const url = new URL("2/parse_resource", apiOrigin);
    const response = await send$1(url, "POST", JSON.stringify(parsingRequest));
    if (!lh(response)) {
      throw Error("Invalid parsing response returned from serverless API");
    }
    return response;
  };
  const getTenantList = async () => {
    const configurationUrl = getConfigurationUrl();
    const response = await send$1(new URL(configurationUrl));
    try {
      const { tenants } = ih(response);
      return tenants;
    } catch {
      throw Error("Invalid customer list returned from serverless API");
    }
  };
  const fetchApiRegistry = async () => {
    try {
      const apiOrigin = await getBaseUrlForCustomerOrDefault();
      const response = await send$1(new URL("1/configuration", apiOrigin));
      if (!mh(response)) {
        return fh;
      }
      return response;
  ...
  background;
  const REGISTRY_STORAGE_KEY = "registry";
  const getRegistryFromStorage = async () => {
    const result2 = await browser.storage.local.get(REGISTRY_STORAGE_KEY);
    if (mh(result2.registry)) {
      return result2.registry;
    } else {
      return void 0;
    }
  };
  const saveRegistryToStorage = async (registry) => {
    await browser.storage.local.set({ [REGISTRY_STORAGE_KEY]: registry });
  };
  const getRegistry = async () => {
    const storageRegistry = await getRegistryFromStorage();
    if (storageRegistry) {
  ...
  const updateRegistry = async () => {
    const apiRegistry = await fetchApiRegistry();
    await saveRegistryToStorage(apiRegistry);
    return apiRegistry;
  };
  background;
  const sessionDataSchema = z$1.object({
    sessionId: z$1.string(),
    timestamp: z$1.number()
  });
  const GOOGLE_ANALYTICS_BASE_URL = "https://www.google-analytics.com";
  const GOOGLE_ANALYTICS_PARAMS_MEASUREMENT_ID = "measurement_id";
  const GOOGLE_ANALYTICS_PARAMS_SECRET = "api_secret";
  const GOOGLE_ANALYTICS_ENGAGEMENT_TIME = 100;
  const SESSION_EXPIRATION = minutes(30);
  const LIBRARY_VERSION = Us();
  const createGoogleAnalyticsUrl = (path) => {
    const url = new URL(path, GOOGLE_ANALYTICS_BASE_URL);
    url.searchParams.append(
      GOOGLE_ANALYTICS_PARAMS_MEASUREMENT_ID,
      "G-M5LH3H6ZBV"
    );
    url.searchParams.append(
      GOOGLE_ANALYTICS_PARAMS_SECRET,
      "cxPWr5ovTwuPf1nNy3cvnA"
    );
    return url;
  };
  const fetchClientId = async () => {
    const result2 = await browser.storage.local.get("clientId");
    if (typeof result2.clientId === "string" && Boolean(result2.clientId)) {
      return result2.clientId;
    }
    const clientId = crypto.randomUUID();
    await browser.storage.local.set({ clientId });
    return clientId;
  };
  const isSessionData = (value) => sessionDataSchema.safeParse(value).success;
  const fetchSessionId = async () => {
    const result2 = await browser.storage.local.get("sessionData");
    const timestamp = Date.now();
    if (isSessionData(result2.sessionData)) {
      const duration = timestamp - result2.sessionData.timestamp;
      if (duration < SESSION_EXPIRATION) {
        await browser.storage.session.set({
          sessionData: {
            sessionId: result2.sessionData.sessionId,
            timestamp
          }
        });
        return result2.sessionData.sessionId;
      }
    }
    const sessionId = crypto.randomUUID();
    await browser.storage.local.set({ sessionData: { sessionId, timestamp } });
    return sessionId;
  };
  const createEventModel = async (events) => {
    const [clientId, sessionId, registry, tenant] = await Promise.all([
      fetchClientId(),
      fetchSessionId(),
      getRegistry(),
      fetchTenant()
    ...
    const mappedEvents = events.map((event) => ({
      ...event,
      params: {
        engagement_time_msec: GOOGLE_ANALYTICS_ENGAGEMENT_TIME,
        extension_version: version,
        library_version: LIBRARY_VERSION,
        region: tenant ? tenant.region : null,
  ...
  const send = async (model) => {
    const url = createGoogleAnalyticsUrl("mp/collect");
    try {
      await fetch(url, {
        method: "POST",
      ...
      console.debug("Error sending Google Analytics event", error);
      return;
    }
  };
  const logToConsole = (model) => {
    console.debug("bibliographic resource collector events", model);
  };
  const formatTraceFilename = (filename) => {
    if (filename && URL.canParse(filename)) {
      return new URL(filename).pathname;
    }
    return filename;
  };
  const logEvents = async (...events) => {
    const model = await createEventModel(events);
    logToConsole(model);
    {
      await send(model);
    }
  };
  const logInstalled = () => logEvents({
    name: "extension_installed",
    params: {}
  });
  const logUpgraded = () => logEvents({
    name: "extension_upgraded",
    params: {}
  });
  const logIconClicked = (url) => logEvents({
    name: "extension_icon_clicked",
    params: {
      origin: url
    }
  });
  const logTenantUpdated = () => logEvents({
    name: "extension_tenant_updated",
    params: {}
  });
  const logTenantLegacyMigrated = () => logEvents({
    name: "extension_tenant_legacy_migrated",
    params: {}
  });
  const logPageParsingResult = (url, parserMechanism, parserName) => logEvents({
    name: "extension_page_parsed",
    params: {
      origin: url,
      parser_mechanism: parserMechanism,
      parser_name: parserName
    }
  });
  const logError = async (error) => {
    const stack = await stacktraceExports.fromError(error, { offline: true });
    const params = {
      message: error.message
    };
    if (stack.length > 0) {
      const [trace] = stack;
      params.trace_file_name = formatTraceFilename(trace.fileName);
      params.trace_function_name = trace.functionName;
      params.trace_line_number = trace.lineNumber;
      params.trace_column_number = trace.columnNumber;
    }
    await logEvents({
  ...
  const handleTabUpdated = (tabId, changeInfo) => {
    if (changeInfo.status) {
      tabStatusObservable.update(tabId, changeInfo.status);
    }
  };
  ...
  const startTabStatusMonitoring = () => {
  ...
  const getUrlFromParsingResponse = ({ id: id2, region }, parsingResponse, tab) => {
    if (!lh(parsingResponse)) {
      return createFallbackBookmarkingUrl(id2, region, tab);
    }
    if (Cs(parsingResponse)) {
      const baseUrl = getTarlBaseBookmarkingUrl(id2, region);
      return Eh(
        baseUrl,
  ...
  const parsePageViaApi = async (parsingResult, selectedTenant, activeTab) => {
    const result2 = await postParseResource({
    ...
    if (lh(result2)) {
      const mechanism = determineApiParserMechanism(result2);
      return {
        mechanism,
        parserName: result2.meta.parserName,
  ...
  background;
  const REGISTRY_ALARM_NAME = "registry-update";
  const createAlarm = () => {
    const updateIntervalInMinutes = Number(
      "720"
    );
    browser.alarms.create(REGISTRY_ALARM_NAME, {
      // how long to wait before the first alarm fires
      delayInMinutes: updateIntervalInMinutes,
      // how often the alarm should fire
      periodInMinutes: updateIntervalInMinutes
    });
  };
  const ensureRegistryAlarmExists = async () => {
    const alarm = await browser.alarms.get(REGISTRY_ALARM_NAME);
    if (!alarm) {
      createAlarm();
    }
  };
  ...
  const getActiveTab = async (tab) => {
    ...
    if (currentTab.success) {
      ...
      await logError(Error(`Active tab error: ${currentTab.errors}`));
      return void 0;
    }
  };
  const extensionButtonClicked = async (tab) => {
    const activeTab = await getActiveTab(tab);
    if (activeTab) {
      await Promise.all([
        logIconClicked(activeTab.url),
        performBookmarking(activeTab)
      ]);
    }
  };
  const extensionInstalled = async ({
    reason
  }) => {
    switch (reason) {
      case "install":
        await updateRegistry();
        await Promise.all([logInstalled(), browser.runtime.openOptionsPage()]);
        break;
      case "update":
  ...
  const definition = defineBackground({
    main() {
      browser.action.onClicked.addListener(extensionButtonClicked);
      browser.runtime.onInstalled.addListener(extensionInstalled);
      browser.alarms.onAlarm.addListener(updateRegistry);
      startTabStatusMonitoring();
      void ensureRegistryAlarmExists();
    }
  });
  background;
