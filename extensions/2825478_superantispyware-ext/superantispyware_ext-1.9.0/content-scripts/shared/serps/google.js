(() => {
  ...
  // ../common/content-scripts/serps/google.ts
  (async () => {
  ...
  function processResults(searchResultToggler) {
    const ATTR_NAME = "data-wsp-scanned";
    const divs = document.querySelectorAll(`div.g:not([${ATTR_NAME}])`);
    let previousA = null;
    for (let i = 0; i < divs.length; i++) {
      const div = divs[i];
      div.setAttribute(ATTR_NAME, "1");
      ...
      getCategorization_default(a.href).then((categorization) => {
        ...
        const target = a.querySelector("h3");
        if (!target) {
          log("Missing H3 element.");
          return;
        }
