(function () {
  ...
  service.$inject = ['$q', '$http', '$timeout', '$rootScope', 'sources_list', 'storage', 'userService'];
  
  function service ($q, $http, $timeout, $rootScope, sourcesList, storage, userService) {
        ...
        return userService
          ...
          .then(async (user) => {
            ...
            if (!_shortcuts) {
              ...
              //Find out sources that user explicitly added
              user.sources?.filter(source => source.enabled).forEach(source => {
