...
const configSyncManager = ConfigSyncManager.getInstance();
class EventSession {
  ...
  async attemptConnection() {
    ...
    if ((/* @__PURE__ */ new Date()).getTime() > rpc.speculativeWebSocketAttemptProhibitedUntil.getTime()) {
      ...
      try {
        ...
        KeeLog.debug(`HTTP request failed: ${error2.message} (${error2.status} ${error2.statusText}), not attempting web socket connection`);
      }
    }
  }
  sendMessage(data) {
...
var SessionType$1 = /* @__PURE__ */ ((SessionType2) => {
  SessionType2["Event"] = "event";
  SessionType2["Websocket"] = "websocket";
  return SessionType2;
})(SessionType$1 || {});
const _FeatureFlags = class _FeatureFlags {
};
_FeatureFlags.offered = [
...
let FeatureFlags = _FeatureFlags;
...
class JWT {
  static async verify(sig) {
    ...
    const isValid = await crypto.subtle.verify(
      ...
      kee.utils.base64urltoByteArray(sigParts[2]),
...
class kprpcClient {
  constructor(store) {
    ...
    this.websocketSessionManager = new WebsocketSessionManager(
      () => accountManager.featureEnabledMultiSessionTypes || !this.eventSessionManager.isActive(),
  ...
  getSessionManagerByType(sessionType) {
    return sessionType === SessionType$1.Event ? this.eventSessionManager : this.websocketSessionManager;
  }
  ...
  processJSONRPCresponse(obj, sessionManager) {
    const sessionType = sessionManager instanceof EventSessionManager ? SessionType$1.Event : SessionType$1.Websocket;
  ...
  setupEventSession(features) {
    if (!accountManager.featureEnabledMultiSessionTypes && this.websocketSessionManager.isActive()) {
      KeeLog.debug(
  ...
  setupWebsocketSession() {
    if (!accountManager.featureEnabledMultiSessionTypes && this.eventSessionManager.isActive()) {
      KeeLog.debug(
        "Session activation aborted: Existing session already active and account does not have the multiple sessions feature."
      );
      this.websocketSessionManager.closeSession();
...
class jsonrpcClient {
  ...
  get eventSessionManagerIsActive() {
    return this.kprpcClient.getSessionManagerByType(SessionType$1.Event).isActive();
  }
  get websocketSessionManagerIsActive() {
    return this.kprpcClient.getSessionManagerByType(SessionType$1.Websocket).isActive();
  }
  //***************************************
  ...
  selectAndFocusDatabase(vaultFileName, keepassFilename) {
    let sessionManager;
    const smEvent = this.kprpcClient.getSessionManagerByType(SessionType$1.Event);
    const smWebsocket = this.kprpcClient.getSessionManagerByType(SessionType$1.Websocket);
    ...
    return sessionManager instanceof EventSessionManager ? SessionType$1.Event : SessionType$1.Websocket;
  }
  ...
  async findLogins(fullURL, httpRealm, uuid, dbFileName, freeText, username) {
    ...
    const sessionManagers = potentialSessionManagers.filter(
      (sm) => sm instanceof EventSessionManager && this.store.state.KeePassDatabases.some((db) => db.sessionType == SessionType$1.Event) || sm instanceof WebsocketSessionManager && this.store.state.KeePassDatabases.some(
        (db) => db.sessionType == SessionType$1.Websocket
      )
    );
  ...
  async getAllDatabases() {
    ...
    const dbs = [];
    sessionResponses.sort((s) => s.sessionType === SessionType$1.Event ? -1 : 1);
    for (const sessionResponse of sessionResponses) {
      if (sessionResponse.resultWrapper.result !== null) {
        const recievedDBs = sessionResponse.sessionType === SessionType$1.Event ? sessionResponse.resultWrapper.result.dbs : sessionResponse.resultWrapper.result;
        ...
        if (sessionResponse.sessionType === SessionType$1.Event) {
  ...
  updateAddonSettings(settings, version) {
    const sessionManager = this.kprpcClient.getSessionManagerByType(SessionType$1.Event);
    if (!sessionManager) {
      return;
    }
...
class KFCommands {
  ...
  init() {
    ...
    chrome.contextMenus.onClicked.addListener((info) => {
      ...
      if (id.startsWith("matchedLogin-")) {
        kee.tabStates.get(kee.foregroundTabId).framePorts.get(info.frameId).postMessage({
  ...
  async setupContextMenuItems() {
    ...
    try {
      ...
      if (kee.foregroundTabId >= 0 && kee.tabStates.has(kee.foregroundTabId) && kee.tabStates.get(kee.foregroundTabId).frames) {
...
async function browserPopupMessageHandler(msg) {
  ...
  if (msg.removeNotification) {
    kee.removeUserNotifications((n) => n.id != msg.removeNotification);
  }
  ...
  if (msg.action === Action.CreateEntry || msg.action === Action.UpdateEntry) {
    if (kee.store.state.connected) {
      ...
      const clearSubmittedData = () => {
        var _a, _b;
        if (((_b = (_a = kee.persistentTabStates.get(tabId)) == null ? void 0 : _a.items) == null ? void 0 : _b.length) > 0) {
          kee.persistentTabStates.get(
            tabId
          ).items = kee.persistentTabStates.get(tabId).items.filter((item) => item.itemType !== "submittedData");
        }
      };
  ...
  if (msg.action == Action.ManualFill && msg.selectedEntryIndex != null) {
    kee.tabStates.get(kee.foregroundTabId).framePorts.get(msg.frameId || 0).postMessage(msg);
    kee.tabStates.get(kee.foregroundTabId).framePorts.get(0).postMessage({ action: Action.CloseAllPanels });
  }
...
async function pageMessageHandler(msg) {
  ...
  if (msg.entries) {
    kee.tabStates.get(this.sender.tab.id).frames.get(this.sender.frameId).entries = msg.entries;
  }
  if (msg.submittedData) {
    ...
    if (kee.persistentTabStates.get(this.sender.tab.id)) {
      kee.persistentTabStates.get(
        this.sender.tab.id
      ).items = kee.persistentTabStates.get(this.sender.tab.id).items.filter((item) => item.itemType !== "submittedData");
    }
    kee.persistentTabStates.get(this.sender.tab.id).items.push(persistentItem);
    if (kee.store.state.entryUpdateStartedAtTimestamp >= Date.now() - 9e4)
      return;
  ...
  if (msg.action === Action.PageHide) {
    try {
      kee.tabStates.get(this.sender.frameId).framePorts.forEach((port, key, map2) => {
        try {
          port.disconnect();
        } catch (e) {
          if (KeeLog && KeeLog.debug) {
...
class KeeBrowserActionIconConfiguration {
  constructor(saveAvailable, notificationAvailable, connectionAvailable, dbAvailable, matchedEntries) {
...
class Kee {
  ...
  constructor() {
    this.store = new BackgroundStore((mutation, excludedPort) => {
      function tryPostMessage(port, mutationObj2) {
        try {
          port.postMessage({ mutation: mutationObj2 });
          return true;
        } catch (e) {
      ...
      if (this.browserPopupPort !== excludedPort)
        tryPostMessage(this.browserPopupPort, mutationObj);
      if (this.vaultPort !== excludedPort)
        tryPostMessage(this.vaultPort, mutationObj);
    ...
    this.onPortConnected = async function(p) {
      var _a, _b;
      if (KeeLog && KeeLog.debug)
      ...
      switch (name) {
        case "browserPopup": {
          ...
          p.onDisconnect.addListener(() => {
            ...
            this.currentSearchTermTimer = self.setTimeout(() => {
              var _a2, _b2;
              (_a2 = this == null ? void 0 : this.store) == null ? void 0 : _a2.updateCurrentSearchTerm(null);
              (_b2 = this == null ? void 0 : this.store) == null ? void 0 : _b2.updateSearchResults(null);
          ...
          if (this.persistentTabStates.get(this.foregroundTabId)) {
            this.persistentTabStates.get(this.foregroundTabId).items.forEach((item) => {
        ...
        case "page": {
          ...
          if (frameId === 0) {
            ...
            if (((_b = (_a = this.persistentTabStates.get(tabId)) == null ? void 0 : _a.items) == null ? void 0 : _b.length) > 0) {
              this.persistentTabStates.get(
                tabId
              ).items = this.persistentTabStates.get(tabId).items.filter(
        ...
        case "iframe": {
          ...
          p.onDisconnect.addListener(() => {
            var _a2, _b2;
            (_b2 = (_a2 = this.tabStates.get(p.sender.tab.id)) == null ? void 0 : _a2.ourIframePorts) == null ? void 0 : _b2.delete(p.sender.frameId);
          });
          const connectMessage = {
            initialState: this.store.state,
            frameState: this.tabStates.get(p.sender.tab.id).frames.get(parentFrameId),
            frameId: p.sender.frameId,
            tabId: p.sender.tab.id,
            isForegroundTab: p.sender.tab.id === this.foregroundTabId
          };
          ...
          p.postMessage(connectMessage);
          this.tabStates.get(p.sender.tab.id).ourIframePorts.set(p.sender.frameId, p);
          break;
        }
      }
    };
  ...
  async init() {
    ...
    accountManager.addListener(() => {
      if (accountManager.featureEnabledMultiSessionTypes && !this.KeePassRPC.eventSessionManagerIsActive) {
    ...
    if (chrome.runtime.lastError != null) {
      KeeLog.warn(
        "KeeFox was unable to disable built-in password manager saving - confusion may ensue! " + chrome.runtime.lastError.message
      );
    }
    return true;
  }
  notifyUser(notification, nativeNotification) {
    this.removeUserNotifications((n) => n.name != notification.name);
  ...
  refreshFormStatus(action) {
    this.tabStates.forEach((ts, tabId) => {
      ts.framePorts.forEach((port, key, map2) => {
        try {
  ...
  openKeePass() {
    ...
    const supportsWebsocketFocus = this.store.state.KeePassDatabases.some(
      (db) => db.sessionType === SessionType$1.Websocket && db.sessionFeatures.indexOf("KPRPC_OPEN_AND_FOCUS_DATABASE") >= 0
    );
  ...
  async loginToPasswordManager() {
    ...
    if (sessionType !== SessionType$1.Websocket) {
      const vaultTabs = await chrome.tabs.query({
        url: [
          "https://keevault.pm/*",
...
class NetworkAuth {
  ...
  async provideCredentialsAsync(requestDetails) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      chrome.runtime.onMessage.addListener(handleMessage);
      let wind;
      function declineHandling(closingWindowId) {
        if (closingWindowId !== (wind == null ? void 0 : wind.id))
          return;
...
const initialised = new Promise((resolve, _) => {
...
function updateForegroundTab(tabId) {
  ...
  if (kee && kee.foregroundTabId !== tabId) {
    ...
    const ports = (_a = kee.tabStates.get(tabId)) == null ? void 0 : _a.framePorts;
    if (ports) {
      if (KeeLog && KeeLog.debug)
      ...
      ports.forEach((port, frameIndex) => {
        try {
          ...
          if (KeeLog && KeeLog.warn)
            KeeLog.warn("Failed to post a message to a foreground tab port. Will remove the port now.");
...
if (!isFirefox()) {
  chrome.runtime.onInstalled.addListener(() => {
    var _a;
    (_a = chrome.runtime.getManifest().content_scripts) == null ? void 0 : _a.forEach(async (script) => {
      ...
      const loadContentScripts = (tab) => {
        ...
        if (((_a2 = script == null ? void 0 : script.js) == null ? void 0 : _a2.length) > 0) {
          loadOperations.push(chrome.scripting.executeScript({
            target: { tabId: tab.id, allFrames },
            files: script.js
          }));
        }
        if (((_b = script == null ? void 0 : script.css) == null ? void 0 : _b.length) > 0) {
      ...
      try {
        ...
        if (!e.message.endsWith(" is showing error page")) {
          if (KeeLog && KeeLog.error)
            KeeLog.error(e.message);
          else
            console.error(e);
        }
      }
...
if (isFirefox()) {
  chrome.webRequest.onAuthRequired.addListener(
    async (requestDetails) => {
      if (KeeLog == null ? void 0 : KeeLog.debug)
        KeeLog.debug("onAuthRequired request started");
      await Promise.race([initialised, new Promise((resolve) => self.setTimeout(resolve, 2e4))]);
      if (KeeLog == null ? void 0 : KeeLog.debug)
        KeeLog.debug("onAuthRequired request ongoing");
  ...
  chrome.webRequest.onAuthRequired.addListener(
    (requestDetails, callback) => {
      ...
      Promise.race([
        initialised,
        new Promise((resolve) => self.setTimeout(resolve, 2e4))
      ]).then(() => {
        if (KeeLog == null ? void 0 : KeeLog.debug)
          KeeLog.debug("onAuthRequired request ongoing");
