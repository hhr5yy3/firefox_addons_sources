...
import ReplyIcon from '../npm/octicons-plain-react-components-Reply.js';
...
import { isArchivedRepoAsync, getUsername } from '../github-helpers/index.js';
...
function mentionUser({delegateTarget: button}) {
	...
	const newComment = expectElement(fieldSelector);
	newComment.focus();
...
function add(avatar) {
	...
	if (avatar.classList.contains('TimelineItem-avatar')) {
		...
		wrap(avatar, React.createElement('div', { className: "avatar-parent-child TimelineItem-avatar d-none d-md-block"   ,} ));
	}

	const userMention = expectElement('img', avatar).alt;
	avatar.after(
