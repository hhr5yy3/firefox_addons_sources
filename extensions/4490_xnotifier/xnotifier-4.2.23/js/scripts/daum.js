...
function init(){
  ...
  this.loginData=["https://logins.daum.net/accounts/login.do",
                      "id",null,"pw=&stln=1&ipSecurity=checked"];
  this.dataURL="https://cmail.daum.net/v2/folders";
  this.viewURL="https://mail.daum.net";
...
function checkLogin(aData){
  ...
  case ST_CHECK:
    ...
    return false;
  case ST_CHECK+1:
    var reg=new RegExp("DAUMID\\s*:\\s*'"+this.user+"'","i");
...
function process(aData,aHttp){
  switch(this.stage){
  case ST_PRE:
    this.getHtml("https://track.tiara.daum.net/queen/footsteps");//set cookies
    return false;
  case ST_PRE_RES:
    this.getHtml("https://logins.daum.net/accounts/loginform.do?url=https%3A%2F%2Fmail.daum.net%2F");
    return false;
  case ST_PRE_RES+1:
    this.post=this.getForm(aData,"loginForm");  
    this.stage=ST_LOGIN;
  ...
  case ST_LOGIN:
    ...
    if(result.rescode=="200"){
      this.getHtml("https://logins.daum.net/accounts/srp.do?slevel=1&"+daumlogin.srp.init2(result),this.post+"&"+this.loginData[LOGIN_POST]);
      delete this.post;
      return false;
    }
    delete this.post;
  ...
  case ST_DATA:
    ...
    this.getHtml(this.viewURL);//check login
    return true;
  case ST_DATA_RES+1:
    var reg=new RegExp("DAUMID\\s*:\\s*'"+this.user+"'","i");
    var fnd=aData.match(reg);
...
function getData(aData){
  ...
  if(fnd){
    ...
    for(var i=0;i<fnd.length;i++){
      ...
      if(o.id=="SENT")continue;
      if(o.id=="DRAFT")continue;
      if(o.id=="SPAM")continue;
      if(o.id=="TRASH")continue;
      var n=o.mailsUnread;
      if(this.inboxOnly){
        if(o.id=="INBOX")num=n;
      }else num+=n;
...
function getViewURL(aFolder){
  if(aFolder){
    return "https://mail.daum.net/#"+aFolder;
  }
  return this.viewURL;  
}
