			...
			get: function() {
				return new Promise(function(resolve) {
					const default_options = {
						extension_ignore_url: ['post', 'user', 'discussion', 'people', 'private'],
						extension_depth: 3,
		...
		return {
			...
			ids: function(tags) {
				return new Promise(function(resolve) {
					...
					const list = [];
					for (const i in tags) {
						list.push(`tag${i}:tag(name:"${tags[i]}") { 
									id
									name
									synonyms
									mainTag { 
					...
					fetch($this.api, {
						...
						for (const [i, tag] of Object.entries(response.data)) {
							if (tag === null)
								continue;
							result[tag.mainTag.name] = tag.mainTag.id.val();
							if (tag.mainTag.synonyms !== null) {
								let synonyms = tag.mainTag.synonyms.split(', ');
								for (var j in synonyms) {
									result[synonyms[j]] = tag.mainTag.id.val();
								}
							}
						}
						resolve(result);
