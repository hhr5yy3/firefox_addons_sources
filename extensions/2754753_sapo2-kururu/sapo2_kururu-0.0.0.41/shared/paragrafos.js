class Paragrafos {
    /**
     *
     * @param tarefa {Object} A própria tarefa vinda do Super
     * @param textos {Array.<Object>} Lista de textos vindos de https://{SERVER}/compendio/modelos
     * @param interessados {Array.<Object>} [Opcional] Lista dos interessados do processo, obtida por request anterior. Ex.: let interessados = await this.super_get(this.get_processo_interessados(this.tarefa.processo.id), true);
     * @param campos
     * @param orgaoSuperior
     */
    constructor(tarefa, textos, interessados, campos, orgaoSuperior) {
        this.tarefa = tarefa;
        this.textos = textos || []; // Ex.: [{rowid: 1, texto: 'alguma_coisa'}, {rowid: 2, texto: 'algo mais'}]
        ...
        this.campos = campos; // Ex.: [{identificador: "###NUMERO_CONTRATO" : valor: "03/2020", precedido_artigo: false, artigo_uppercase: false}, {identificador: "###INTERESSADOS", valor: ["abc", "def"], precedido_artigo: false, artigo_uppercase: false}]
        this.orgaoSuperior = orgaoSuperior || {nome: 'CONSULTORIA-GERAL DA UNIÃO', sigla: 'CGU'};
    }
     ...
     */
    cabecalho(tarefa, nome_peca, num_peca) {
        // console.log(tarefa);

        let p = '<p class="centralizado"><img src="';
        p += this.brasao() + '"/><br />';
        p += '<strong>ADVOCACIA-GERAL DA UNIÃO</strong><br />';
        p += `${this.orgaoSuperior.nome}<br/>`; // 'CONSULTORIA-GERAL DA UNIÃO<br />';
        if (tarefa.setorResponsavel.unidade.nome !== this.orgaoSuperior.nome) p += tarefa.setorResponsavel.unidade.nome + '<br />'; // quando a unidade esta dentro da CGU
        // if (tarefa?.setorResponsavel?.numeracaoDocumentoUnidade) p += tarefa.setorResponsavel.nome + '<br /></p>';
        p += tarefa.setorResponsavel.nome + '<br /></p>';
        p += '<hr />';
        ...
        p += `<p class="esquerda"><strong>PROCESSO</strong>: <span data-method="nup" data-options="" data-service="sapiens_administrativo.template_renderer">${this.formatanup(tarefa.processo.NUP)}</span></p>`;
    ...
    /**
     *
     * @param data {String} Ex.: Teresina, 13 de janeiro de 2022.
     */
    finalizar_html(data) {
        if (!data) throw new Error('Erro: Data precisa estar no formato "Cidade, XX, de ... de XXXX');
        this.html += this._par();
        this.html += `<p class="direita">${data}</p>`;
        this.html += this._par();
        this.tarefa.usuarioResponsavel.assinaturaHTML.split('\n').forEach((d, i)=>{
            if (i === 0) this.html += this._par(`${d}`, 'centralizado');
            else this.html += this._par(d, 'centralizado');
        });
        this.html += this._par();
        if (this?.tarefa?.processo?.chaveAcesso) this.html += `<hr/><p class="esquerda">Chave de acesso ao Processo: ${this.tarefa.processo.chaveAcesso} - https://supersapiens.agu.gov.br</p>`;
