vflscrsh.action = {};
//------------------------------------------------------------------------------
vflscrsh.action.init = function(tab_data) {
		...
		chrome.tabs.captureVisibleTab(null, { 'format' : 'png', 'quality': 100}, function(dataURI) {
			if (dataURI) {
				...
				image.onload = function() {
					...
					vflscrsh.action.prepare_done(tab_data);
				}
				image.src = dataURI;
			}
		});
...
//------------------------------------------------------------------------------
vflscrsh.action.set_selection = function(tab_data, dimensions) {
	chrome.tabs.executeScript(tab_data.id, { 'file': '/content/page.js' }, function(callback) {
		if (chrome.runtime.lastError) {}
...
//------------------------------------------------------------------------------
vflscrsh.action.prepare_done = function(tab_data) {
	var insertText = vflscrsh.utils.prefs_get('insertTextInImage');
	if (insertText) {
		...
		tab_data.screenshot.ctx.fillText(vflscrsh.utils.createTemplateName('templateImageName', tab_data), 3, 15);
	}


	if (tab_data.method == 'upload') {
		vflscrsh.upload.init(tab_data);
	}
	//------------------------------------------------------------------------
	else if (tab_data.method == 'preview') {
		vflscrsh.preview.init(tab_data);
	}
	//------------------------------------------------------------------------
	else if (tab_data.method == 'save') {
		vflscrsh.save.init(tab_data);
	}
	//------------------------------------------------------------------------
	else if (tab_data.method == 'save_mhtml') {
		vflscrsh.save.init(tab_data, true);
	}
	//------------------------------------------------------------------------
	else if (tab_data.method == 'copy') {
		vflscrsh.copy.init(tab_data);
	}
	//------------------------------------------------------------------------
	else {
		vflscrsh.open_url(tab_data.screenshot.canvas.toDataURL('image/png'), tab_data.id, true);
	}
	vflscrsh.tab_list['tab' + tab_data.id] = {};
}
//------------------------------------------------------------------------------
vflscrsh.action.prepare_page = function(data) {
	var tab_data = vflscrsh.tab_list['tab' + data.tab_id];
	...
	chrome.tabs.update(tab_data.id, { 'active' : true }, function() {
		chrome.tabs.captureVisibleTab(null, { 'format' : 'png', 'quality': 100}, function(dataURI) {
			if (dataURI) {
				var image = new Image();
				image.onload = function() {
					vflscrsh.action.prepare_page_part(data, image, tab_data);
				}
				image.src = dataURI;
			}
		});
...
//------------------------------------------------------------------------------
vflscrsh.action.prepare_page_drawWindow = function(data) {
	var tab_data = vflscrsh.tab_list['tab' + data.tab_id];
	if (! tab_data.id) { return; }
	if (data.site_description) {
		tab_data.site_description = data.site_description;
	...
	if (data.image_data) {
		var image = new Image();
		image.onload = function() {
			vflscrsh.action.prepare_page_drawWindow_end(data, image, tab_data);
		}
		image.src = data.image_data;
	}
}
//------------------------------------------------------------------------------
vflscrsh.action.prepare_page_drawWindow_end = function(data, image, tab_data) {
	if (! tab_data.id) { return; }
	//------------------------------------------------------------------------
	var canvas = document.createElement('canvas');
	var max_size = 32767;
...
//------------------------------------------------------------------------------
vflscrsh.action.prepare_page_part = function(data, image, tab_data) {
	if (! tab_data.id) { return; }
...
//------------------------------------------------------------------------------
vflscrsh.action.prepare_page_end = function(data) {
	var tab_data = vflscrsh.tab_list['tab' + data.tab_id];
...
//------------------------------------------------------------------------------
vflscrsh.action.end_action = function(tab_data) {
