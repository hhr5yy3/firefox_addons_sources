...
chrome.storage.onChanged.addListener((changes, areaName) => {
	if (areaName === "local" && config.nextCleanupTime !== changes.cfg?.newValue.nextCleanupTime) {
...
function setArDescriptionContent() {
	if (!config.autoRemoveOption) {
	...
	} else if (config.nextCleanupTime === 0) {
		...
		const mapObj = {
			"%DATE%": `${nextCleanupTime.getDate()}.${formatWithLeadingZero(
				nextCleanupTime.getMonth() + 1
			)}.${nextCleanupTime.getFullYear()}`,
