...
async function insertReminders(reminders) {
    ...
    for (const insert of reminders) {
        ...
        if (found === false && override === false) toAdd.push(insert);
    }
    if (toAdd.length > 0 || override === true) chrome.storage.sync.set({ "reminders": [...storage["reminders"], ...toAdd] });
}
...
async function reminderWatch() {
    ...
    const alertPeriod = 1000 * 60 * 60 * 6 * 50; // 6 hours
