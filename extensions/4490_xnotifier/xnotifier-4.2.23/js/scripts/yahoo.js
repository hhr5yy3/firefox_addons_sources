...
function init(){
  ...
  if(ar[1]=="yahoo.co.jp"){
    ...
    this.loginData=["https://login.yahoo.com/account/challenge/password?","username","password","verifyPassword=Sign+in"];
...
function checkLogin(aData){
  ...
  case ST_CHECK+1:
    ...
    var fnd3=aData.match(/"https:&#x2F;&#x2F;login.yahoo.com&#x2F;account&#x2F;logout\?/);
    if(fnd||fnd2||fnd3){
...
if(this.debug)dlog(this.id+"\t"+this.user+"\t"+this.stage,aData);
  ...
  case ST_PRE+2:
    ...
    if(fnd){
      var url=fnd[1];
      if(fnd[1].indexOf("http")!=0)url="https://login.yahoo.com"+url;
      this.loginData[LOGIN_URL]=url;
      this.stage=ST_LOGIN;
      this.getHtml(url);
      return true;
    }
...
function getData(aData){
  ...
  if(this.mode==3){      
    fnd=aData.match(/,"folders":(\[[\s\S]+?\]),"hasMoreItems"/);
    var num=0;
    try{
