...
browser.runtime.sendMessage({msg: "page_action_loaded"}).then(msg => {
    switch (msg.msg) {
    case "show_buttons":
	show_buttons(msg);
	break;
...
async function show_buttons(msg) {
	...
	await browser.runtime.sendMessage({msg: "page_action_action", action: action, altKey: e.altKey, ctrlKey: e.ctrlKey, metaKey: e.metaKey, shiftKey: e.shiftKey}).catch(error => {
	    console.log(error.message);
	});
	if (["translate_in_page_action", "translate_on_2nd_in_page_action"].includes(action)) {
	    let items = document.getElementById("items");
