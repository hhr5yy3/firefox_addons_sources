...
if (window.location.search.includes('openProspector')) {
  var emails = window.location.search.replace('?', '').split('&').find(function (p) {
    return p.includes('emails=');
  });

  if (emails) {
    emails = emails.replace('emails=', '').split(',');
  } else {
    emails = [];
  }
  ...
  sendMessageToExtension('NIMBLE_GMAIL_SHOW_PROSPECTOR_REQUEST', {
