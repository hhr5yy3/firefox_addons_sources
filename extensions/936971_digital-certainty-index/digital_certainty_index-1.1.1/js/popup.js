...
core.getCurrentTabId().then(function (tab) {
  ...
  document.getElementById("extension-view").setAttribute("src", values.dciUrl + currentTab.url);
  api.getDCI(currentTab.url).then(function (dci) {
    iconCtrl.setIconBadge(dci.mainDCI, currentTab.id);
  });
});
