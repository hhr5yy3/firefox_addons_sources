...
document.body.classList.add('recursion--added');
                ...
                fullResult = walkDOM(
                    ...
                    function (node) {
                        ...
                        if (node.nodeType === 1) {
                            ...
                            var rolePresentation = !skipTo.tag &&
                                !skipTo.role &&
                                !hasName &&
                                nTag !== "iframe" &&
                                nRole &&
                            ...
                            if (!isSeparatChildFormField) {
                                ...
                                if (
                                    ...
                                    !hasName &&
                                    node === refNode &&
                                    btnType && ["button", "submit", "reset"].indexOf(btnType) !== -1
                                ) {
                                    if (btnValue) {
                                        name = btnValue;
                                    } else {
                            ...
                            if (!skipTo.tag && !skipTo.role && isSeparatChildFormField) {
                                ...
                                if (
                                    ...
                                    if (isRangeWidgetRole) {
                                        ...
                                        isNativeFormField && ["input", "textarea"].indexOf(nTag) !== -1 &&
                          ...
                          "email"
                            ...
                            if (
                                !isSkipTo &&
                                aOwns && ["input", "img", "progress"].indexOf(nTag) === -1
                            ) {
                                ids = aOwns.split(/\s+/);
                                parts = [];
                                for (i = 0; i < ids.length; i++) {
            ...
            var isBlockLevelElement = function (node, cssObj) {
                ...
                for (var prop in blockStyles) {
                    ...
                    for (var i = 0; i < values.length; i++) {
                        if (
                            styleObject[prop] &&
                            ((values[i].indexOf("!") === 0 && [values[i].slice(1), "inherit", "initial", "unset"].indexOf(
                                    styleObject[prop]
                                ) === -1) ||
