...
// AJAX запросы
  if (request.query) {
    ...
    // и уже не запущен повтор - пробуем снова
...
chrome.tabs.onUpdated.addListener(function (tabId, change, t) {
  ...
  if (change.status && 'complete' === change.status && t.status == 'complete' && t.url != undefined) {
    ...
    if (host === 'surfearner.loc' || host === 'surfearner.com') {
      ...
      code += "document.getElementsByTagName('body')[0].appendChild(el);undefined;";
...
function insertSurfJs(fail) {
  ...
  var src = chrome.runtime.getURL('/content/surf.js');
  var src2 = chrome.runtime.getURL('/content/json2html.js');
  var code = '(function(n){';
  if (fail) {
