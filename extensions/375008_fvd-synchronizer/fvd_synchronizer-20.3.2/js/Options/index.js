(function () {
    browser.tabs.query({
        ...
        for (var i = tabs.length - 1; i >= 0; i--)
            if (tabs.url != needUrl) tabs.splice(i, 1); // ff
        if (tabs.length > 0) {
            var tab = tabs[0];
    ...
    this.Options = new function () {
        ...
        function refreshTabsVisibility() {
            ...
            fvdSynchronizer.Utils.Async.chain([
                ...
                , function () {
                    ...
                    var tabHistory = document.getElementById("tabHeadBackupHistory");
                    var tabAccount = document.getElementById("tabHeadAccount");
                    
                    if (state == "logged") {
                        /* ESM
                        fvdSynchronizer.Driver.Speeddial.isAllowed(function (allowed) {
                            if (allowed) {
                                //if (allowed || true) { // Task #963
        ...
        this.loadAccountFrame = function () {
            ...
            url += "/stats?addon=chrome_sync&content=bookmarks";
        ...
        function (next) {
                    ...
                    fvdSynchronizer.Dialogs.confirm(_("dlg_bookmarks_confirm_overwrite_server_data_title"), _("dlg_bookmarks_confirm_overwrite_server_data_text"), function (result) {
                        if (!result) {
                            return;
                        }
                        next();
        ...
        this.sdBackupMake = function (callback, zip) {
            fvdSynchronizer.Dialogs.syncProgressSimple("Speeddial", zip ? "makeBackupZip" : "makeBackup", function () {
                if (callback) {
                    callback();
                }
            });
        };
        this.sdBackupChoose = function () {
            fvdSynchronizer.Dialogs.choose(_("options_default_desc"), _("backup_confirm_create_title"), [
                {
                    value: "localdb"
                    , text: _("backup_confirm_create_btn_db")
                }
                , {
                    value: "file"
                    , text: _("backup_confirm_create_btn_file")
                }
                ], function (result) {
                if (result == "file") {
                    fvdSynchronizer.Options.sdBackupMake(false, true);
                }
                else
                if (result == "localdb") {
                    fvdSynchronizer.Options.sdBackupMake(false, false);
                }
            });
        };
        this.sdBackupReadZip = function (file) {
            fvdSynchronizer.Driver.Speeddial.Backup.readZip(file);
        };
        ...
        this.sdBackupRestoreZip = function (file, callback) {
            fvdSynchronizer.Dialogs.syncProgressSimple("Speeddial", "restoreBackupZip", function () {
                self.refreshBookmarksBackupState();
                self.refreshInitialSync();
                if (callback) {
                    callback();
            ...
            //fvdSynchronizer.Options.sdBackupReadZip(evt.target.files[0]);
        };
        ...
        this.hightBackups = function (force) {
            if(force || fvdSynchronizer.Prefs.get( "sd.highlight" ) == "backups"){
                var btns = document.querySelectorAll(".highlight-backup");

                for (var i = 0; i != btns.length; i++) {
                    btns[i].className = String(btns[i].className).replace("highlight-green", "");
                }

                setTimeout(()=>{
                    for (var i = 0; i != btns.length; i++) {
                        btns[i].className += " highlight-green";
                    }
                }, 1000);
                
                fvdSynchronizer.Prefs.set( "sd.highlight", "" );
            }
        };
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            document.getElementById("buttonpremiumForShareButton").addEventListener("click", function () {
                fvdSynchronizer.Utils.Opener.newTab(browser.runtime.getURL("premiumforshare.html"));
            }, false);
        ...
        function () {
                    ...
                    var events = [
                        ...
                        , {
                            ...
                            , callback: function () {
                                if (!isBookmarksInitialization) {
                                    fvdSynchronizer.Options.sdBackupChoose();
                                }
                            }
            }
                        , {
                            event: "click"
                            , elem: "backupExportToFile"
                            , callback: function () {
                                if (!isBookmarksInitialization) {
                                    fvdSynchronizer.Options.sdBackupChoose();
                                }
                            ...
                            event: "change"
                            , elem: "backupReadFromFile"
                            , callback: function (evt) {
                                if (evt.target.files.length) {
                                    fvdSynchronizer.Dialogs.confirm(_("confirm_really_restore_sd_title"), _("confirm_really_restore_sd_text"), function (r) {
                                        if (r) {
                                            self.sdBackupRestoreZip(evt.target.files[0]);
                                        }
                                    });
                                }
                            }
                    ...
                    events.forEach(function (event) {
                        if(document.getElementById(event.elem)){
                            document.getElementById(event.elem).addEventListener(event.event, event.callback, false);
                        }
                    });
        ...
        //browser.extension.onMessage.addListener(function(r, sender, sendResponse) { // ff switch to runtime
        browser.runtime.onMessage.addListener(function (r, sender, sendResponse) {
            if (r.action) {
                ...
                else if (r.action === "refreshIframe") { // Task #845
                    //document.getElementById("accountFrame").contentDocument.location.reload(true);
                    
                    var src = String(document.getElementById('accountFrame').getAttribute('src')).split('&rand')[0];
                    
                    //document.getElementById('accountFrame').setAttribute('src', 'about:blank');
                    document.getElementById('accountFrame').setAttribute('src', src + "&rand=" + String(Math.round(Math.random()*100000)));                    
                }
                else if (r.action === "sdBackupMakedZip") {
                    saveAs(r.content, "speeddial.sd");
                }
            }else
            if (r.subject == "changeActivityState") {// Task #1541
                self.refreshActivityState();
            }
        });
        
        /*// Task #1541
        //browser.extension.onRequest.addListener( function( r ){ // ff switch because 'onRequest' is depricated
        browser.runtime.onMessage.addListener(function (r) {
            console.info(r);
