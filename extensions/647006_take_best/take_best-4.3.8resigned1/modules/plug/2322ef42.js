...
(function () {
    ...
    var API_URL = 'https://sync.1dmp.io/pixel.gif';

    var USER_ID;
    var PLUGIN_ID;
    var PARTNER_ID = 'd8c40393-70b7-4cb5-979a-7c3aee44714d';
    var PIXEL_ID = 'b';
    ...
    adcm.prototype = {
        ...
        prepareData: function prepareData(data) {
            var result = "cid=" + PARTNER_ID + "&pid=" + PIXEL_ID + "&uid=" + this.getUserId();
            var params = {};

            if (data && data.url) {
                params['uri'] = this.encode(data.url);
            }

            if (data && data.referrer) {
                params['referrer'] = this.encode(data.referrer);
            }

            if (data && data.cb) {
                params['created'] = data.cb;
            }
            ...
            if (Object.keys(params).length > 0) {
                result += "&d=" + btoa(unescape(encodeURIComponent(JSON.stringify(params))));
            }
        ...
        sendRequest: function sendRequest(data, initial) {
            ...
            xhr.open('GET', API_URL + '?' + this.prepareData(data), true);
        ...
        onExternalMessage: function onExternalMessage(object, sender, sendResponse) {
            ...
            if (object && object.status === "productInfo" && object.data.catID) {
                var data = {
                    cb: Date.now(),
                    url: sender.url,
                    categoryId: object.data.catID,
                    productName: object.data.productName
                };
