(function () {
    ...
    angular.module('muzli').directive('adminBar', ['server', '$rootScope', 'storage', 'userService',
        function (server, $rootScope, storage, userService) {
            return {
                restrict: 'E',
                ...
                link: function (scope, element) {
                    ...
                    scope.resetBubbles = function () {
                        $rootScope.vm.ftxLeft = ['settings', 'scroll', 'shortcuts', 'sidebar', 'speed-dial'];
                        storage.set({
                    ...
                    scope.resetPermissions = function () {
                        chrome.permissions.remove({
                            permissions: ['topSites', 'bookmarks', 'favicon'],
                        }, () => { console.log('Permissions reset') });
                    }

                    scope.resetJobs = function () {
                        localStorage.removeItem('disableJobsCard');
                        location.reload();
                    }

                    scope.resetFTX = async () => {

                        let { UUID } = await storage.get('UUID');
                    ...
                    scope.resetHomeScroll = () => {

                        storage.set({
                            lastHomeScrolled: new Date('2020-01-01'),
                        })
            ...
            return {
                link: function ($scope, element, attrs) {
                    ...
                    let checkScrollPosition = () => {
                        ...
                        const distanceUntillVisible = element.get(0).getBoundingClientRect().top - window.innerHeight;
                        const renderTreshhold = Math.min(window.innerHeight, 800);
                        const zoneId = $scope.row === '1' ? 'rectangle_1' : 'rectangle_3';
                    ...
                    function resizeIframe() {
                        let iframeScale = (element.get(0).offsetWidth / 300).toFixed(4);
