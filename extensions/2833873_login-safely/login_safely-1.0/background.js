var UNINSTALL_PAGE = "https://loginonline.co/firefox/public/feedback";
const DOMAIN = "*://loginonline.co/*";
...
browser.runtime.onInstalled.addListener(function (details) {
  if (details.reason === "install") {
    fetch("https://loginonline.co/firefox/public/install")
  }
});
...
browser.runtime.onInstalled.addListener(function () {
  ...
  browser.tabs.query({ url: DOMAIN }, function (tabs) {
    tabs.map((tab) => {
      browser.tabs.sendMessage(tab.id, { type: 'extensionInstalled', data: 'loginonlineInstalled' });
    })
    ...
    currentEngine.map(async (item, i) => {
      if (item.name == "web search by Yahoo" && item.isDefault == true) {
        if (!call) {
          browser.tabs.query({ url: DOMAIN }, function (tabs) {
            tabs.map((tab) => {
              browser.tabs.sendMessage(tab.id, { type: 'yesButtonClicked', data: "loginonlineYesClicked" });
            });
          });
          call = true;
        }
        ...
        if (!notcall) {
          browser.tabs.query({ url: DOMAIN }, function (tabs) {
            tabs.map((tab) => {
              browser.tabs.sendMessage(tab.id, { type: 'noButtonClicked', data: "loginonlineNoClicked" });
            });
          });
          notcall = true;
        }
...
browser.extension.isAllowedIncognitoAccess().then(isAllowed => {
  browser.tabs.query({ url: DOMAIN }, function (tabs) {
    tabs.map((tab) => {
      browser.tabs.sendMessage(tab.id, { type: 'okayBtnClicked', data: { id: 'loginonlineOkayClicked', isAllowed } });
    });
  });
});
