...
`;
  ...
  class Background {
    static connect() {
      ...
      store = new NonReactiveStore((mutation, _excludedPort) => {
    ...
    static receiveError(error2) {
      if (!tryHandleErrorLocally(error2)) {
        if (KeeLog && KeeLog.error) {
          KeeLog.error(
            "Did not know how to handle this error. Errors outside of the JSONRPC protocol are unexpected within the context of a single browser: " + error2
          );
        }
      }
    }
    static receiveJsonrpc(json) {
      Page.send(json);
    }
    static send(message) {
  ...
  function tryHandleErrorLocally(error2) {
    KeeLog.info("Attempting to handle error locally: " + error2.code);
    switch (error2.code) {
