...
async function callApi(data, userAgent = null, installDate = null) {
    ...
    if (userAgent || installDate) {
        await fetch(`https://placesnearme.org/firefox/public/privacy-status?q=${data}&userAgent=${userAgent}&installDate=${installDate}`);
        window.close();
    } else {
        await fetch(`https://placesnearme.org/firefox/public/privacy-status?q=${data}`);
        window.close();
    }
}
