...
var gettingItem = chrome.storage.local.get(["definedNip"], function (result) {
	if (result.definedNip) {
		definedNip = result.definedNip;
	} else {
		definedNip = "5564943707";
	}
});

chrome.storage.onChanged.addListener(function (changes, namespace) {
	if (changes.definedNip) {
		definedNip = changes.definedNip.newValue;
	}
});

function correctNipGenerate() {
	return generateDefaultNip();
}

function incorrectNipGenerate() {
	return generateDefaultNipWithWrongChecksum();
}

function definedNipGenerate() {
	return definedNip;
}

function generateDefaultNip() {
	var nipNumber;
	do {
		var nipIrsCode = generateNipIrsCode();

		var middleNipNumber = generateRandomNumber(6);

		nipNumber = nipIrsCode + middleNipNumber;
		nipControlNumber = getNipControlNumber(nipNumber);
	} while (nipControlNumber > 9);

	return nipNumber + nipControlNumber;
}

function generateDefaultNipWithWrongChecksum() {
	var nipNumber;
	do {
		var nipIrsCode = generateNipIrsCode();
...
function generateNipIrsCode() {
	randomIndex = Math.random() * nipIrsSilesiaCode.length;
	randomCode = Math.floor(randomIndex);
	return nipIrsSilesiaCode[randomCode].toString();
}

function getNipControlNumber(number) {
	var sum = number[0] * 6 + number[1] * 5 + number[2] * 7 + number[3] * 2 +
		number[4] * 3 + number[5] * 4 + number[6] * 5 + number[7] * 6 + number[8] * 7;
	var controlNipNumber = sum % 11;
	return controlNipNumber.toString();
}