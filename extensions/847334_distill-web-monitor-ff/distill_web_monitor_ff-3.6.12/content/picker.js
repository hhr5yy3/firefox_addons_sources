define('picker', ['jquery', 'underscore', 'id', 'api', 'util', 'locator'],
  function($, _, ID, Api, util, Locator) {
    const Picker = {
      MODE_NOOP: 'NOOP',
      MODE_SELECT: 'SELECT',
    ...
    function Selection(options) {
      ...
      function setLocatorType(type) {
        if(locator.type == type) return;
        let expr;
        if (type == 'xpath') {
          expr = Locator.findXPath(currentTarget, Picker.XPATH_ATTRS);
        } else if (type == 'css') {
          expr = Locator.findCSSPath(currentTarget);
        } else if (type == 'js') {
          expr = Locator.findJSPath(currentTarget);
        } else {
          throw new Error('Unknown selector type: ' + type);
        }
        setLocator({type, expr});
      }
