angular.module('starter').controller('LoginCtrl', function ($scope, api, utils, $state, $rootScope, $http) {
	...
	if (chrome.i18n.getMessage('nameApp') === "Kopikot") {
		$scope.socialBtns = [{
				class: 'but vk',
				url: 'https://oauth.vk.com/authorize?client_id=4622348&redirect_uri=https://www.kopikot.ru/oauth-login?type=vkontakte&response_type=token&scope=email,offline&display=page'
			},
			{
				class: 'but ok',
				url: 'https://connect.ok.ru/oauth/authorize?client_id=1107767808&scope=GET_EMAIL&response_type=token&redirect_uri=https://www.kopikot.ru/oauth-login?type=ok&layout=w'
			},
			{
				class: 'but mail',
				url: 'https://connect.mail.ru/oauth/authorize?client_id=726874&response_type=token&redirect_uri=https://www.kopikot.ru/oauth-login?type=mailru'
			},
			{
				class: 'but fb',
				url: 'https://www.facebook.com/v2.5/dialog/oauth?response_type=token&client_id=753295054750257&redirect_uri=https://www.kopikot.ru/oauth-login?type=facebook&scope=email,public_profile,user_friends'
			},
			{
				class: 'but google',
				url: 'https://accounts.google.com/o/oauth2/v2/auth?redirect_uri=https://www.kopikot.ru/oauth-login?type=google&prompt=consent&response_type=code&client_id=350518387970-57r0uafh686ig61fou0k65r4jmmriovo.apps.googleusercontent.com&scope=https://www.googleapis.com/auth/userinfo.email+https://www.googleapis.com/auth/userinfo.profile&access_type=offline'
			}
		];
	...
	} else if (chrome.i18n.getMessage('nameApp') === "Kopikot.com.ua") {
		$scope.socialBtns = [{
				class: 'but vk',
				url: 'https://oauth.vk.com/authorize?client_id=6225867&redirect_uri=https://www.kopikot.com.ua/oauth-login?type=vkontakte&response_type=token&scope=email,offline&display=page'
			},
			{
				class: 'but ok',
				url: 'https://connect.ok.ru/oauth/authorize?client_id=1257105920&scope=GET_EMAIL&response_type=token&redirect_uri=https://www.kopikot.com.ua/oauth-login?type=ok&layout=w&state=fuck'
			},
			{
				class: 'but yandex',
				url: 'https://oauth.yandex.ru/authorize?response_type=token&client_id=63d46d641f81466cad774f3611fdf3a3'
			},
			{
				class: 'but fb',
				url: 'https://www.facebook.com/v2.5/dialog/oauth?response_type=token&client_id=123979644962005&redirect_uri=https://www.kopikot.com.ua/oauth-login?type=facebook&scope=email,public_profile,user_friends'
			},
			{
				class: 'but google',
				url: 'https://accounts.google.com/o/oauth2/v2/auth?redirect_uri=https://www.kopikot.com.ua/oauth-login?type=google&prompt=consent&response_type=code&client_id=350518387970-57r0uafh686ig61fou0k65r4jmmriovo.apps.googleusercontent.com&scope=https://www.googleapis.com/auth/userinfo.email+https://www.googleapis.com/auth/userinfo.profile&access_type=offline'
			}
		];
	} else {
		$scope.socialBtns = [{
