...
const onBeforeRootFrameRequest = function(fctxt) {
    ...
    // Check ACS list
    fetch("./assets/thirdparties/adaware/acs.txt").then(function(response) {
        if (response.status === 200) {
            response.text().then(function(text) {
                let data = text.split('\r\n');

                if (data.indexOf(requestHostname.replace('www.','')) > -1) {
                    if (result === 2) {
                        ({ result, logData } = { result: 2, logData: { result: 2, source: 'static' } })
                    } else {
                        ({ result, logData } = { result: (strictBlockBypasser.isBypassed(requestHostname)) ? 2 : 1, logData: { result: (strictBlockBypasser.isBypassed(requestHostname)) ? 2 : 1, source: 'static' } })
                    }
                } 
                
                // Not blocked
                if ( result !== 1 || µb.blockDangerousUrlsStatus === false ) { return; }
