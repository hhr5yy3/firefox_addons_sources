        ...
        setTimeout(async function() {
            ...
            if (accountsList.length) {
                ...
                function handleDragStart(e) {
                    sourceIndex = accounts.findIndex(account => account.getEmail() == this.getAttribute("email"));
                    destIndex = accounts.findIndex(account => account.getEmail() == this.getAttribute("email"));

                    console.log("drag start", e, sourceIndex, destIndex);
                    ...
                    if (accountAddingMethod == "autoDetect") {
                        ...
                        const content = new DocumentFragment();
                        content.append("Reordering accounts is not available when using the auto-detect method.");
                ...
                function handleDragEnter(e) {
                    const thisSourceIndex = accounts.findIndex(account => account.getEmail() == globalThis.dragSrcEl.getAttribute("email"));
                    const thisDestIndex = accounts.findIndex(account => account.getEmail() == this.getAttribute("email"));

                    if (sourceIndex == thisSourceIndex && destIndex == thisDestIndex) {
                        console.log("same index", destIndex, thisDestIndex);
                        return;
                    } else {
                        sourceIndex = thisSourceIndex;
                        destIndex = thisDestIndex;
                    }

                    console.log("drag enter", e, sourceIndex, globalThis.dragSrcEl.getAttribute("email"), destIndex, this.getAttribute("email"));
                    ...
                    accounts.forEach(account => {
                        console.log("order: ", account.getEmail());
                    });
                ...
                function handleDragLeave(e) {
                    ...
                    if (sourceIndex == thisSourceIndex && destIndex == thisDestIndex) {
                        console.log("same index on leave", destIndex, thisDestIndex);
                        return;
                    } else {
...
function highlightAddAccount() {
    const $accountAddingMethod = byId("accountAddingMethod");
