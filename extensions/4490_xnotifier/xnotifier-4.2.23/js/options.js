...
function onLoad(){
  ...
  $("include-spam").addEventListener("click",function(){setValue('includeSpam',this.checked?2:0);});
  ...
  $("link").addEventListener("input",function(){setValue(this.id,this.value);});
  ...
  keyNum=0;
  arDeleted=[];
  prefs=exlib.loadPrefs("prefs",DEF_PREF);
  ...
  arDefault=exlib.loadPrefs("defaults",{});
  checkDefault();
  onSelect(-1);
...
/*    $("show-folders").disabled=!wmn.getScriptVal(host,"supportShowFolders");
    ...
    */
    var locale=getScriptVal(host,"needLocale");
    if(getScriptVal(host,"needServer")||locale){
      $("lb-server").textContent=exlib.i18nString(locale?"locale":"server");//isAOL;
      $("server-box").style.visibility="visible";
    }else{
      $("server").value="";
      $("server-box").style.visibility="hidden";
    }
    if(getScriptVal(host,"needLink")){
      $("link-box").style.display="table-row";
    }else{
      $("link").value="";
      $("link-box").style.display="none";
    }
    var isRSS=(host=="rss");
    $("lb-username").textContent=exlib.i18nString(isRSS?"url":"username");
    $("pwd-box").style.display=isRSS?"none":"table-row";
...
//    $("show-folders").checked=o.showFolders;
    $("include-spam").checked=o.includeSpam!=0;
    $("auto-open").checked=o.autoOpen;
    $("alias").value=o.alias?o.alias:"";
    $("link").value=o.link?o.link:"";
...
function _getValues(o,isNew,byUser){
  ...
  if(isNew&&byUser){//user input
    ...
    o.includeSpam=$("include-spam").checked?2:0;
...
function addItem(o){
  ...
  obj.id=key;
    ...
    ch = document.createElement("td");
      ...
      ch.innerHTML="<div class=\"hdiv\" style=\"max-width:"+thead[1].style.width+"\">"
                  +hosts[o.id].name+"</div>";
    obj.appendChild(ch);
    ch = document.createElement("td");
      ch.style.width=thead[2].style.width;
      ch.innerHTML="<div class=\"hdiv\" style=\"max-width:"+thead[2].style.width+"\">"
                  +o.user+"</div>";
    obj.appendChild(ch);
    ch = document.createElement("td");
...
function onImportPwd(pwd){
  ...
  while ((o = re.exec(str)) != null){
    if(o[1]=="<info>"){
      ...
      if(p!=p.match(/\d+/)){
        window.alert(exlib.i18nString("importWrongPassword"));
        return;
      }
...
function onExportPwd(pwd){
  ...
  var ar=exlib.loadPrefs("scripts",{});
  for(var o in ar){
    str+="[---"+o+"---]\r\n";
    str+=exlib.loadUserscript(o)+"\r\n";
  }
  str+="[---<preferences>---]\r\n";
  var pf=exlib.loadPrefs("prefs",DEF_PREF);
  function getType(o){
  ...
  pf=exlib.loadPrefs("defaults",{});
  for(var i in pf){
    var o=pf[i];
    str+="defaults."+i+"\t"+getType(o)+"\t"+o+"\r\n";
  }
  var ar=main.getAccounts();
  for(var j in ar){
    var ac=ar[j];
    for(var i in ac){
      var o=ac[i];
