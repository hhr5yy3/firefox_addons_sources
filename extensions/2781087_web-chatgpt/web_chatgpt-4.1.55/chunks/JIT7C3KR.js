...
// src/features/larkBot/utils/larkBot.ts
var import_aes = __toESM(require_aes());
var import_webextension_polyfill = __toESM(require_browser_polyfill());
var sendLarkBotMessage = (title, message = "", attr) => {
  const data = {
    env: APP_IS_PROD ? "prod" : "dev",
    type: "WebChatGPT_Notification",
    app: "WebChatGPT_Extension",
    data: {
      message
    },
    app_version: import_webextension_polyfill.default.runtime.getManifest().version,
    browser: getBrowserAgent(),
    browser_version: getBrowserVersion(),
    platform: navigator.platform,
    languages: navigator.languages
  };
  const msg = import_aes.default.encrypt(JSON.stringify(data), "MaxAI").toString();
  return fetch(APP_NOTIFICATION_HOST + "/extensionhub/send_notification", {
    method: "POST",
    headers: {
...
// src/features/larkBot/utils/index.ts
var sendLarkBotMessageInContentScript = (title, message = "", attr) => {
  const port = new ContentScriptConnectionV2({
    runtime: "client"
  });
  return port.postMessage({
    event: "LarkBot_sendMessage",
    data: {
      title,
      message,
      attr
...
var sendLarkBotMessageInBgScript = (title, message = "", attr) => {
  return sendLarkBotMessage(title, message, attr);
};

// src/utils/dataHelper/fileHelper.ts
...
var convertBase64ToBlob = (base64, contentType = "") => {
...
// src/features/chatgpt/core/chromeExtensionArkoseTokenGenerator/utils.ts
var import_webextension_polyfill2 = __toESM(require_browser_polyfill());
var import_js_sha3 = __toESM(require_sha3());
var CACHE_ARKOSE_TOKEN_KEY = "CACHE_ARKOSE_TOKEN_KEY";
var setArkoseToken = async (token) => {
  await import_webextension_polyfill2.default.storage.local.set({
    [CACHE_ARKOSE_TOKEN_KEY]: token
  });
};
var getArkoseToken = async () => {
  const cache = await import_webextension_polyfill2.default.storage.local.get(CACHE_ARKOSE_TOKEN_KEY);
  const token = cache[CACHE_ARKOSE_TOKEN_KEY];
  await setArkoseToken({});
  return token;
};
var generateSentinelChatRequirementsToken = async (jwtToken, kind = "primary_assistant") => {
  var _a2;
  try {
    ...
    if (result.status === 200) {
      ...
      return {
        chatRequirementsToken: (data == null ? void 0 : data.token) || "",
        dx: ((_a2 = data == null ? void 0 : data.arkose) == null ? void 0 : _a2.dx) || "",
        proofofwork: (data == null ? void 0 : data.proofofwork) || {
          difficulty: "",
          seed: ""
        }
      };
...
function getProofConfig() {
  return [
    ...
    new Date().toString(),
    get_default(performance, "memory.jsHeapSizeLimit", 0),
    0,
    navigator.userAgent,
    "",
    "",
...
async function calcProofToken(seed, diff) {
  ...
  for (let i = 0; i < 5e5; i++) {
    ...
    if ((0, import_js_sha3.sha3_512)(seed + base).slice(0, diff.length) <= diff) {
      return base;
    }
  }
...
export {
  ...
  setArkoseToken,
  getArkoseToken,
  generateSentinelChatRequirementsToken,
  calcProofToken,
  convertBlobToBase64,
  convertBase64ToBlob
};
