...
browser.tabs.query({
  ...
  return Account.getApiKey(function(api_key) {
    ...
    browser.storage.local.get('current_leads_list_id', function(value) {
  ...
  return {
    ...
    fetch: function() {
      ...
      return $.ajax({
        ...
        headers: {
          "Email-Hunter-Origin": "firefox_addon"
        },
        type: "GET",
        data: {
          format: "json"
    ...
    render: function() {
      ...
      if (!this.trial) {
        $(".header-search-link").attr("href", "https://hunter.io/search/" + this.domain + "?utm_source=firefox_addon&utm_medium=firefox_addon&utm_campaign=extension&utm_content=browser_popup");
      ...
      if (this.results_count > 10) {
        ...
        return $(".search-results").append("<a class='see-more h-button h-button--sm' target='_blank' href='https://hunter.io/search/" + DOMPurify.sanitize(this.domain) + "?utm_source=firefox_addon&utm_medium=firefox_addon&utm_campaign=extension&utm_content=browser_popup'>" + chrome.i18n.getMessage("see_all_the_results", DOMPurify.sanitize(Utilities.numberWithCommas(remaining_results))) + "</a>");
      }
    },
    ...
    feedbackNotification: function() {
      browser.storage.local.get("calls_count", function(value) {
        if (value["calls_count"] >= 10) {
          return browser.storage.local.get("has_given_feedback", function(value) {
  ...
  appendSelector: function() {
    ...
    return _this.getLeadsLists(function(json) {
      ...
      if (json !== "none") {
        ...
        $(".leads-manager__link").attr("href", "https://hunter.io/leads?leads_list_id=" + selected_list_id + "&utm_source=firefox_addon&utm_medium=firefox_addon&utm_campaign=extension&utm_content=browser_popup");
  ...
  updateCurrent: function() {
    return $(".leads-manager__list").on("change", function() {
      if ($(this).val() === "new_list") {
        return Utilities.openInNewTab("https://hunter.io/leads-lists/new?utm_source=firefox_addon&utm_medium=firefox_addon&utm_campaign=extension");
      } else {
        browser.storage.local.set({
          "current_leads_list_id": $(this).val()
        });
        window.current_leads_list_id = $(this).val();
        return $(".leads-manager__link").attr("href", "https://hunter.io/leads?leads_list_id=" + $(this).val() + "&utm_source=firefox_addon&utm_medium=firefox_addon&utm_campaign=extension&utm_content=browser_popup");
      }
    });
  },
