...
const definitions = [{
  ...
  callbacks: [
    ...
    { 
      name: 'Toggle pinning current tab',
      fn: async () => {
        let tab = await browser.tabs.query({ currentWindow: true, active: true });
        browser.tabs.update(tab[0].id, { pinned: !tab[0].pinned });
      },
    },
    { 
      name: 'Toggle muting current tab',
      fn: async () => {
        let tab = await browser.tabs.query({ currentWindow: true, active: true });
        browser.tabs.update(tab[0].id, { muted: !tab[0].mutedInfo.muted });
      },
    },
    { 
      name: 'Mute all tabs',
