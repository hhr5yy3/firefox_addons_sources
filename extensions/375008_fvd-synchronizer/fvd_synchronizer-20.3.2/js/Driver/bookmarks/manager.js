  ...
  actions.forEach(function( action ){
    self[action] = function(){
      var sentArguments = arguments;
        //console.debug("chrome.bookmarks", action, sentArguments[0].title, sentArguments[0]);
        //if(sentArguments[0].title == "From Google Chrome") console.trace();
...
var BookmarksManager = function() {
  ...
  function getGlobalId(id){
    ...
    if( id == CHROME_UNSORTED_ID ){// ff added safe
      return "unsorted";
    }else{
        var gid = bookmarksDatabase.getGuid(id);
  ...
  function refreshBookmarksRootIds(){
    ...
    //chrome.bookmarks.getChildren("0", function(children) { // ff changed default ID
    //chrome.bookmarks.getChildren("unfiled_____", function(children) { // ff
    chrome.bookmarks.getChildren("root________", function(children) {
      if(children.length >= 2) {
        //if(children[1].title.toLowerCase() == "bookmarks bar") {
        if(children[1].title.toLowerCase() == "bookmarks bar" || String(children[1].id).split('_').join('').trim() == "toolbar") { // ff safe
          CHROME_MENU_ID = children[0].id;
          CHROME_TOOLBAR_ID = children[1].id;
          CHROME_UNSORTED_ID = children[2].id; // ff
        }
        else {
  ...
  function getId( guid, params ) {
      var trimId = String(guid).split('_').join('').trim(); // ff safe
      
    if( guid == "toolbar" || trimId == "toolbar"){ 
      return CHROME_TOOLBAR_ID;
    }
    if( guid == "menu" || trimId == "menu"){ 
      return CHROME_MENU_ID;
    }
    if(trimId == "unsorted" || trimId == "unfiled"){ 
      return CHROME_UNSORTED_ID;
    }
  ...
  function correctBookmark(bookmark){// Task #1524
      //return true;
      
      if(bookmark.type == "separator") return false;
      else
      if(
          bookmark.type == "folder" 
          && 
          (typeof bookmark.children != "object" || !bookmark.children.length)
          &&
          false // Task #1540
      ) return false;
      else
      if(
          ...
          && 
          String(bookmark.url).indexOf('place:') === 0
      ) return false;
  ...
  function prepareBookmarksTree( tree, onlyIds, callback ){      
    ...
    if(correctBookmark(tree)){// Task #1524
    ...
    var bookmarkExistsCount = {all:0, exists:0};
  function bookmarkExists( id, callback ){
    ...
    if( !id ){
      return callback( false );
          ...
          bookmarkExistsCount.all++;
    chrome.bookmarks.get( id, function( bookmarks ){
        
      if( bookmarks && bookmarks.length > 0 &&  correctBookmark(bookmarks[0])){ // Task #1524
          bookmarkExistsCount.exists++;
  ...
  this.isRootFolder = function( globalId ){
    var trimId = String(globalId).split('_').join('').trim(); // ff safe
          
      //if( bookmarksDatabase.systemGuids().indexOf( globalId ) != -1 ){
    if(// ff safe
        bookmarksDatabase.systemGuids().indexOf( globalId ) != -1 
        ||
        bookmarksDatabase.systemGuids().indexOf( trimId ) != -1 
    ){
      return true;
    }
  ...
  this.getRootParentIdByBookmarkId = function(id, cb) {
    ...
    fvdSynchronizer.Utils.Async.cc(function(next) {
      chrome.bookmarks.get(currId, function(res) {
        ...
        if(!b.parentId || b.parentId === "0" || String(b.parentId).split('_').join('').trim() === "root") { // ff safe
          // is root
  ...
  this.save = function( bookmark, callback, params ){
     //if( bookmark.type == "folder" ){
         //console.debug("Save Bookmark", bookmark.title, ++this.savedCount, bookmark);
         //console.trace();
     //}
