...
(function () {
  ...
  var CHANNEL_PAGE_CHANNEL_NAME_ELEMENT =
    'div#channel-header-container yt-formatted-string.style-scope.ytd-channel-name';

  var VIDEO_PAGE_MAIN_VIDEO_ELEMENT = 'video.html5-main-video';

  var VIDEO_PAGE_CHANNEL_NAME_ELEMENT =
    'ytd-video-secondary-info-renderer div#top-row a.yt-simple-endpoint.style-scope.yt-formatted-string';
  ...
  var WATCH_PAGE_VIDEO_NAME =
    'div#info ytd-video-primary-info-renderer h1 yt-formatted-string';

  var VIDEO_PAGE_CATEGORY_ELEMENT =
    'div#microformat ytd-player-microformat-renderer script';
    ...
    youtubeApp.addEventListener('yt-page-data-updated', function () {
      ...
      if (url.pathname.startsWith('/watch')) {
        ...
        if (!channelAnchorElement) {
          var videoUnavailable = pageManager.querySelector('yt-player-error-message-renderer');
          if (videoUnavailable) {
            pageManager.style.visibility = 'initial';
            return;
          }
        }
        var channelURL = new URL(channelAnchorElement.href);
        var channelName = channelAnchorElement.textContent;
  ...
  document.onmousemove = function (event) {
    ...
    var message = { action: 'mouse_move', url: event.currentTarget.URL };
    chrome.runtime.sendMessage(message, function (response) {
      //console.log("mouse move message has been sent");
    });
  };

  document.onmousewheel = function (event) {
    var message = { action: 'mouse_wheel', url: event.currentTarget.URL };
    chrome.runtime.sendMessage(message, function (response) {
      //console.log("mouse move message has been sent");
    });
  };
