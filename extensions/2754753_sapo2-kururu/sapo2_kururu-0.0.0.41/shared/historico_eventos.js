class HistoricoEventos extends Payloads {
    ...
    async inicializar() {
        ...
        this.todosEventos = await this.obterTodosEventos(MFt.criaElem('div', null, d1));
        console.log(this.todosEventos);
        MFt.clear(this.wrapper);
        this.todosEventos.sort((a, b)=>{
        ...
        this.showEventos(wrapperTarefas);
    }

    async obterTodosEventos(elem) {
        let total = 0;
        let tarefas = [];
        let offset = 0;
    ...
    showEventos(wrapperTarefas) {
        const newDiv = e=>{
            return MFt.criaElem('div', {

            }, e);
        };
        for(let e of this.todosEventos) {
            const d1 = MFt.criaElem('div', {
                style: {
                    padding: '0 0 20px 0',
                    margin: '0 0 20px 0'
            ...
            this.exibirCampo("Data", e?.criadoEm ? this.date2normal(this.valida_data_hora(e.criadoEm), true) : '---', newDiv(d1));
            this.exibirCampo("Evento", e?.descricao ? e.descricao : '---', newDiv(d1));
            this.exibirCampo("Autor", e?.criadoPor?.nome ? e.criadoPor.nome : '---', newDiv(d1));
            this.exibirCampo("e-mail", e?.criadoPor?.email ? e.criadoPor.email : '---', newDiv(d1));
            this.exibirCampo("ID", e?.id ? e.id.toString() : '---', newDiv(d1));
        }
    }
}