import { DownloadState } from "../common/task-data.js";
import { ChunkStorage, ChunkStorageWriter } from "./chunk-storage.js";
import { Connection, StreamFilterConnection } from "./connection.js";
...
function getPreferredClass(implementations, preference) {
    if (implementations[preference].isAvailable)
        return implementations[preference];
    return undefined;
}
export class MultithreadedTask extends Task {
    constructor() {
        super(...arguments);
        this.connectionClass = MultithreadedTask.getPreferredConnectionClass() || StreamFilterConnection;
        this.connections = new Map();
        this.currentMaxThreads = 1;
        this.currentWarnings = 0;
        this.currentSize = 0;
        this.startSize = 0;
        this.lastFlushTime = 0;
        this.lastFlushInterval = Infinity;
        this.lastFlushIntervalScaled = Infinity;
        this.siteHandlerInvoker = new SiteHandlerInvoker();
        this.checksumSentry = {};
        ...
        this.cachedConnectionTimes = new class {
            constructor() {
                this.data = Array(16);
                this.position = 0;
            }
        ...
        this.flushBeforeRemove = new CriticalSection();
    }
    ...
    static remainingSimultaneousTasks() {
    ...
    async init(isLoaded) {
        ...
        const dataRW = this.data;
        if (!this.data.storageAPI)
            dataRW.storageAPI = S.storageAPI || defaultStorageAPI;
        const fallbackStorageAPI = 'SegmentedFile';
        let chunkStorageClass = getPreferredClass(ChunkStorage.implementations, this.data.storageAPI);
        let initError;
        ...
        if (isLoaded && this.data.totalSize !== undefined) {
            if (DownloadState.areChunksFinished(this.data.state)) {
                this.firstChunk = new Chunk(undefined, new ChunkStorageWriter(undefined, 0, this.data.totalSize));
                this.lastChunk = new Chunk(this.firstChunk, new ChunkStorageWriter(undefined, this.data.totalSize));
                ...
                if (chunk)
                    this.lastChunk = new Chunk(chunk, new ChunkStorageWriter(undefined, this.data.totalSize));
            }
        }
        ...
        if (!isLoaded)
            this.update({}); // persist
        if (initError)
            setImmediate(() => this.fail(initError));
    }
    *getChunks() {
        // [firstChunk, lastChunk)
        // [] if !firstChunk && !lastChunk
        for (let chunk = this.firstChunk; chunk !== this.lastChunk; chunk = chunk.next)
            yield chunk;
    }
    async start() {
        if (!DownloadState.canStart(this.data.state))
            return;
        ...
        this.logger.i(M('i_api', this.connectionClass.name, this.chunkStorageClass.name));
        ...
        if (!this.firstChunk) {
            ...
            if (S.connectionTimeout !== '') {
                connectionTimer = setTimeout(() => {
                    if (conn.lastTransferTime !== undefined)
                        return;
                    conn.abortWithError(new ReportedError(M.e_timeout, performance.now() - conn.startTime));
            ...
            this.logger.i(M('i_initialConnectionInfo', info.totalSize || '?', info.acceptRanges ? M.yes : M.no), info);
            // Check if the task is aborted
            if (this.initialConnection !== conn)
                return;
            this.firstChunk = new Chunk(undefined, this.chunkStorage.writer(0));
            const { totalSize } = info;
            this.lastChunk = new Chunk(this.firstChunk, new ChunkStorageWriter(undefined, totalSize !== undefined ? totalSize : Infinity));
            const newData = {
            ...
            if (info.substituteFilename || info.finalURL !== this.data.url)
                newData.filename = resolveFilenameTemplate({ ...this.data, ...newData });
            this.update(newData);
    ...
    createConnection(position, isInitial) {
        this.logger.i(M('i_createConnection', position));
        const headers = position > 0 ? { Range: `bytes=${position}-`, } : {};
        let { url } = this.data;
        try {
            const o = new URL(url);
            if (o.username || o.password) { // fetch does not support basic auth in URL
                headers['Authorization'] = 'Basic ' + btoa(unescape(encodeURIComponent(o.username + ":" + o.password)));
                o.username = o.password = '';
                url = o.href;
            }
        }
        catch { }
        const connection = new this.connectionClass(new Request(url, {
            credentials: 'include',
            headers,
        ...
        connection.info.then(() => {
            if (connection.lastTransferTime === undefined)
                return;
    ...
    setChunkConnection(chunk, connection = this.createConnection(chunk.writer.startPosition + chunk.currentSize, false)) {
        this.connections.set(connection, chunk);
    }
    async pipeConnectionToChunk(connection, checkNow) {
        const chunk = this.connections.get(connection);
        if (!chunk)
            return true;
        const receivedData = connection.read();
        if (!receivedData)
            return true; // return true if there is no more data
        ...
        if (checkNow !== undefined)
            connection.lastTransferTime = checkNow;
    ...
    getProgress(chunks = [...this.getChunks()]) {
        const items = {};
    ...
    async onConnectionComplete(connection) {
    ...
    addThreads() {
        ...
        const divisibles = [...this.connections.values()].map(chunk => ({ size: chunk.remainingSize, chunk, count: 0 })).filter(v => v.size);
        if (!divisibles.length)
            return;
        ...
        for (const divisible of divisibles) {
            ...
            do {
                ...
                if (spaceSize > 0 && spaceSize >= minChunkSize)
                    break;
                divisible.count--;
    ...
    removeAllConnections() {
        ...
        if (this.firstChunk && this.data.state !== 'completed' &&
            ...
            this.flushBeforeRemove.sync(() => Promise.all(chunks.map(chunk => chunk.writer.flush()))).then(() => {
                if (firstChunkSentry !== this.firstChunk)
                    return;
                this.syncProgress(this.getProgress(chunks));
            });
    ...
    pause() {
        if (!DownloadState.canPause(this.data.state))
            return;
...
MultithreadedTask.startQueuedTasksTimer = new Timer(() => {
...
MultithreadedTask.initialization = Task.initialization.then(() => {
    MultithreadedTask.startQueuedTasksTimer.startOnce();
});
Task.registerType(MultithreadedTask);
class Chunk {
    constructor(prev, writer) {
        this.prev = prev;
        this.writer = writer;
        if (prev) {
            this.next = prev.next;
