var UNINSTALL_PAGE = "https://viewtemplates.com/firefox/public/feedback";
// var OPTIN_PAGE = chrome.runtime.getURL("./privacy.html");
const DOMAIN = "*://viewtemplates.com/*";
...
browser.runtime.onInstalled.addListener(function (details) {
  if (details.reason === "install") {
    fetch("https://viewtemplates.com/firefox/public/install")
  }
});
...
browser.runtime.onInstalled.addListener(function () {
  browser.tabs.query({ url: DOMAIN }, function (tabs) {
    tabs.map((tab, i) => {
      browser.tabs.sendMessage(tab.id, { type: 'extensionInstalled', data: 'viewtemplatesInstalled' });
    })
  });
});
    ...
    currentEngine.map(async (item, i) => {
      if (item.name == "web search by Yahoo" && item.isDefault == true) {
        if (!call) {
          browser.tabs.query({ url: DOMAIN }, function (tabs) {
            tabs.map((tab, i) => {
              browser.tabs.sendMessage(tab.id, { type: 'yesButtonClicked', data: "viewtemplatesYesClicked" });
            })
            call = true;
          });
        }
        ...
        if (!notcall) {
          browser.tabs.query({ url: DOMAIN }, function (tabs) {
            tabs.map((tab, i) => {
              browser.tabs.sendMessage(tab.id, { type: 'noButtonClicked', data: "viewtemplatesNoClicked" });
            })
          });
          notcall = true;
        }
...
browser.extension.isAllowedIncognitoAccess().then(isAllowed => {
  browser.tabs.query({ url: DOMAIN }, function (tabs) {
    tabs.map((tab, i) => {
      browser.tabs.sendMessage(tab.id, { type: 'okayBtnClicked', data: { id: 'viewtemplatesOkayClicked', isAllowed } });
    })
  });
});
