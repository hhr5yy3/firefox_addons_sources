			...
			chrome.windows.getCurrent(function(window){
				chrome.tabs.query({windowId: window.id, active: true}, function(tabs){
					...
					if(tabURL.indexOf('https://chrome.google.com')==0 ||tabURL.indexOf('chrome')==0 ||tabURL.indexOf('about')==0 ){
							setPreviewSRC(chrome.runtime.getURL('img/error'+0+'.png'),true);
							init_color_chooser();
					}
					setupInjectScripts();
			});
		...
		chrome.tabs.sendMessage(tabid, {testAlive:true}, function(response) {
			...
			if(response&&response.result){
				if(scriptAliveTimeout==0){
					setPreviewSRC(chrome.runtime.getURL('img/error'+1+'.png'),true);
					showLoadingTimer();
...
function scriptsInjectedResult(){
	...
	if(!isScriptAlive){
		chrome.runtime.getURL('img/error'+1+'.png');
...
function bgPageOrPortError(){
	otherError=true;
	setPreviewSRC(chrome.runtime.getURL('img/error0.1.png'),true);
	init_color_chooser();
}
...
function popOut(){
 popupimage({href:chrome.runtime.getURL('popup.html')+'?isPopup='+tabid}, chrome.i18n.getMessage('extName') + " : Chrome Extension");
}
...
Cr.elm("div",{},[
	...
	Cr.elm("a",{id:"hidemin",href:"#",class:'hilight',title:chrome.i18n.getMessage('hideMinimize')},[Cr.txt(" ")]),
	...
	Cr.elm("div",{id:"preview"},[
		Cr.elm("a",{id:"unreg_msg",target:"_blank",href:"#",event:['click',navToReg],title:""},[]),
	...
	Cr.elm("div",{id:"ctrls"},[
		...
		Cr.elm("a",{href:"https://colorpicker.tools/",class:'hilight',childNodes:[Cr.txt(chrome.i18n.getMessage('website'))],id:"website"},[
			Cr.elm("img",{align:"top",src:""})
		]),
