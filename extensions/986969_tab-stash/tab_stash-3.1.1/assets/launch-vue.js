...
let eventClass;
if ((_a = globalThis.mock) == null ? void 0 : _a.events) {
...
const altKeyName = () => PLATFORM_INFO.os === "mac" ? "Option" : "Alt";
const bgKeyName = () => PLATFORM_INFO.os === "mac" ? "Cmd" : "Ctrl";
const bgKeyPressed = (ev) => PLATFORM_INFO.os === "mac" ? ev.metaKey : ev.ctrlKey;
...
async function resolveNamed(promises) {
  ...
  for (const k of Object.getOwnPropertyNames(promises)) {
    const p2 = promises[k];
    objects[k] = p2 instanceof Promise ? await p2 : p2;
  }
  return objects;
}
const later = globalThis.setImmediate ?? globalThis.setTimeout;
function onceRefHasValue(r, fn) {
  return new Promise((resolve2) => {
    let cancel = void 0;
    let cancelImmediately = false;
    cancel = watchEffect(() => {
      if (r.value === void 0 || r.value === null) return;
      fn(r.value);
      resolve2();
      if (cancel) {
        cancel();
...
async function nextTick() {
  await new Promise((resolve2) => later(resolve2));
...
const backingOffDefaults = ((_b = globalThis.mock) == null ? void 0 : _b.events) ? {
  max_delay_ms: 2,
  first_delay_ms: 1,
  exponent: 0,
  ...
  next() {
    return new Promise((resolve2, reject) => {
      if (this._queue.length > 0) {
        resolve2({ done: false, value: this._queue.shift() });
        return;
      }
      if (this._done) {
...
function get(store, key, def2) {
  return _FACTORY.get(store, key, def2);
}
class _StoredObjectFactory {
  ...
  async get(store, key, def2) {
    ...
    object = new StoredObjectImpl(this, store, key, def2);
...
class StoredObjectImpl {
  constructor(factory, store, key, def2) {
    ...
    const state = {};
    for (const k in def2) state[k] = def2[k].default;
...
function launch(component, options) {
  const loc = new URL(document.location.href);
  const trace = (loc.searchParams.get("trace") ?? "").split(",");
  for (const t of trace) {
    globalThis.trace[t] = true;
  }
  const loader = async function() {
    document.documentElement.dataset.view = loc.searchParams.get("view") ?? "tab";
    const plat = await resolveNamed({
      browser: browser.runtime.getBrowserInfo ? browser.runtime.getBrowserInfo() : { name: "chrome" },
      platform: browser.runtime.getPlatformInfo ? browser.runtime.getPlatformInfo() : { os: "unknown" },
      options: Model.live()
    });
    ...
    function updateStyle(opts2) {
      document.documentElement.dataset.metrics = opts2.state.ui_metrics;
      document.documentElement.dataset.theme = opts2.state.ui_theme;
    }
    ...
    Object.assign(globalThis, { app, app_options: opts });
    app.mount("body");
  };
  window.addEventListener("load", asyncEvent(loader));
}
function pageref(path) {
