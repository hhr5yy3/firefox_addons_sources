function TransportAdapter(onConnectCallback, onReceivedCallback, onDisconnectCallback)
{
    ...
    function OnMessageResendToPort(port)
    {
        return message =>
            {
                try
                {
                    CheckLastError();
                    AvNs.TrySendMessage(port, message);
                }
                catch (e)
                {
    ...
    function ProcessNewContentConnection(port)
    {
        if (port.name === "resend")
        {
            ...
            resendPort.onMessage.addListener(OnMessageResendToPort(port));
            port.onMessage.addListener(OnMessageResendToPort(resendPort));
            return;
        }
            ...
            port.onDisconnect.addListener(() =>
                {
                    try
                    {
                        const shutdownCommand = { callId: 0, command: "shutdown", commandAttribute: "disconnect", timestamp: (new Date()).toISOString() };
                        if (m_nmServer)
