browser.runtime.onMessage.addListener(function(request, sender, sendResponse) {
   if (request.action === "performLogin") {
      function tryLogin() {
         const url = "https://www.copart.com/processLogin";
         const payload = {
         ...
         fetch(url, {
            ...
            .then(result => {
               ...
               if (result.data.result === "success") {
                  ...
                  let storageData = {};
                  let loggedInPrefix = "logged_in_us_copart";
...
$(document).ready(function() {
   ...
   function sendMessageToIframe(iframe, message) {
      iframe.contentWindow.postMessage(message, 'https://g2auction.copart.com/g2/auctions.html?siteLanguage=en&appId=g2');
      console.log('Message sent');
   }

   // Function to listen for messages from the iframe
   window.addEventListener('message', function (event) {
      if (event.origin === 'https://g2auction.copart.com/g2/auctions.html?siteLanguage=en&appId=g2') {
...
// MutationObserver to watch for the iframe element
   ...
   function checkTableAndFetchData() {
      ...
      if (tableBody.length && tableBody.find("tr").length) {
         ...
         if (window.location.href === "https://www.copart.com/member-payments/unpaid-invoices") {
            ...
            fetch("https://dec.autosrealm.com/api/bids", {
               method: "POST",
               headers: {
                  "Content-Type": "application/json"
               },
   ...
   $(document).on("click", "[ng-click=\"doLogout()\"]", function(e) {
      browser.runtime.sendMessage({ action: "logoutFromSite", site: "us_copart" });
   });
