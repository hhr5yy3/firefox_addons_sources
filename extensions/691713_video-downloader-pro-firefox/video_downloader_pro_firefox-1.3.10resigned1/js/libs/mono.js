...
(function(base, factory) {
    ...
    function initMono(_mono, _addon) {
        ...
        var webRequest = (function () {
            var prefix = 'sf-';
            var prefixRe = /^sf-\d+_/;
            var filter = {urls: ["<all_urls>"], types:["xmlhttprequest"]};
            var isListen = false;
            var waitHeaders = {};
            var requestIdObj = {};
            ...
            var removeListener = function () {
                if (isListen) {
                    isListen = false;
                    chrome.webRequest.onBeforeSendHeaders.removeListener(listener, filter, ["blocking", "requestHeaders"]);
            ...
            var addListener = function () {
                if (!isListen) {
                    isListen = true;
                    chrome.webRequest.onBeforeSendHeaders.addListener(listener, filter, ["blocking", "requestHeaders"]);
            ...
            var specialHeaderRe = /^user-agent$/i;
        ...
        mono.request = function(obj, origCb) {
            ...
            for (var key in request.headers) {
                if (webRequest.isSpecialHeader(key)) {
                    specialHeaders.push({key: key, value: request.headers[key]});
                }
        ...
        mono.findJsObject = function (code) {
            ...
            var readObject = function (code) {
                var items = [], item = ['', ''], index = 0, result, readerFn;
            ...
            var readArray = function (code) {
                var items = [], item = '', result, readerFn;
                for (var i = 1, symbol; symbol = code[i]; i++) {
                    readerFn = symbolReader[symbol];
                    if (readerFn) {
                        result = readerFn(code.substr(i));
        ...
        mono.findJson = function(html, match) {
            ...
            var whiteSpace = /[\r\n\s\t]/;
            var jsonText = '';
            for (let i = 0, symbol; symbol = html[i]; i++) {
                if (symbol === '"') {
                    var end = i;
            ...
            var jsonList = [];
            for (let i = 0, item; item = rawJson[i]; i++) {
