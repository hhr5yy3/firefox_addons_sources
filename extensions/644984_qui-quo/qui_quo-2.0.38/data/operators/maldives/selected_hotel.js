...
function createOption(img) {
    ...
    let option = {
        ...
        href: $1('.hotel-item-location a').href,
...
function getOccupancy(dataForm) {
    const occupancyDataArray = $$('span', dataForm[2]).extractNodesText();
    const adultsCount = parseInt((occupancyDataArray[0]).match(/\d+/));
    if (occupancyDataArray.length > 3) {
        return {
            adultsCount,
            childrenCount: parseInt((occupancyDataArray[1]).match(/детей (\d+)/)[1]),
            childAges: (occupancyDataArray[1]).match(/\d+/g).slice(1).join(', '),
        }
    }
    return {
        adultsCount,
