...
FMDB.prototype.init = async function(wipe) {
    "use strict";
    assert(!this.db && !this.opening, 'Something went wrong... PMDB is already ongoing...');

    // prefix database name with options/capabilities, plus making it dependent on the current schema.
    const dbpfx = `pm1${FMDB.perspex.substr(1)}${MurmurHash3(JSON.stringify(this.schema), 0x6f01f).toString(16)}`;
