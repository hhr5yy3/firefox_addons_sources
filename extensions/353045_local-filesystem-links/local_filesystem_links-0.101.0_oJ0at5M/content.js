...
(function fileLinkAddon($) {
  $.noConflict();
  var fileLinkSelectors = ['a[href^="file:"]'
    /*'a[href^="smb://"]',
    'a[href^="afp://"]'*/],
    appTextMessages = {
  ...
  function updateLinkTooltip() {
    var tooltipText = options.revealOpenOption == 'O' ? appTextMessages.tooltips.linkText : appTextMessages.tooltips.openFolder;
    $('a').filter(fileLinkSelectors.join(', ')).attr('title', function (i, val) {
  ...
  function activate() {
    if (options.enableLinkIcons) {
      currentIconClass = 'aliensun-link-icon' + (options.revealOpenOption == 'R' ? '-arrow' : '');
  ...
  browser.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    switch (request.action) {
      case 'ping':
        // used to test that we're having a connection to the content script
  ...
  function registerEvents() {
    ...
    // icon click handler
    $(document).on('click', "[class^='aliensun-link-icon']",
    // folder or arrow
    openFolderHandler);
  }
  ...
  */
  ...
  function updateLink($element) {
    ...
    var iconTooltip = options.revealOpenOption == 'O' ? appTextMessages.tooltips.openFolder : appTextMessages.tooltips.linkText;
    ...
    $element.each(function (index, el) {
      if (!$(el).next().is('.aliensun-link-icon,.aliensun-link-icon-arrow')) {
        // icon not added
        $icon.attr('title', iconTooltip).clone().data('link', $(el).attr('href')) // added to container
        .insertAfter($(el));
      }
    });
  }
  ...
  function createObserver() {
    ...
    var observer = new MutationObserver(function (mutations) {
      mutations.forEach(function (mutation) {
        var $changedEl = $(mutation.target).filter('a').filter(fileLinkSelectors.join(', '));
        if (mutation.type === 'attributes') {
          ...
          var $addedNodes = $(mutation.addedNodes).find(fileLinkSelectors.join(', '));
          if ($addedNodes.length == 0) {
            // no chillds with a file link --> check added element
            $addedNodes = $(mutation.addedNodes).filter(fileLinkSelectors.join(', '));
          }

          // Check that next element is not a file link icon
          var $elements = $addedNodes.filter(function () {
            return !$(this).next().is('.aliensun-link-icon,.aliensun-link-icon-arrow');
          });
