var Starter = (function(){
    ...
    return new Promise(function(resolve, reject){
      chrome.runtime.sendMessage({cmd: 'api.getPlan'}, function(json){
        if (json.error || json.ext_error) {
          if (json.data === 'Invalid API Key') {
            Common.setPlan({}, false);
          }
          else if (json.data === 'subscription not found') {
            Common.setPlan({}, true);
          }
          resolve(null);
          return;
        }
        Common.setPlan(json.data, true);
  ...
  var checkURL = function( sources, plan, config ){
    ...
    if (sources[source]) {
      ...
      if (hasCredits) {
        ...
        var freeSources = 'bingco youtub gtrend instgr ebayco etsyco amazon pntrst openai gemini claude xtwttr';
        // config.areSubsEnabled = true;
        if (config.areSubsEnabled) freeSources = 'instgr openai gemini claude pntrst youtub bingco duckgo xtwttr';
        if ((source === 'gsearc' && isGSearchURL()) || freeSources.indexOf(source) !== -1) {
          Tool.init();
          if (source === 'openai' || source === 'gemini' || source === 'claude') addSettingsButton();
        }
      }
    }
  };
  ...
  var addSettingsButton = function(){
    ...
    if (settings.showAddAllButton) {
      if ('amazon ebayco youtub instgr yahoo openai gemini claude xtwttr'.indexOf(source) === -1) {
    ...
    if (settings.showChatGPTactions) {
      if (source === 'openai' || source === 'gemini' || source === 'claude') {
