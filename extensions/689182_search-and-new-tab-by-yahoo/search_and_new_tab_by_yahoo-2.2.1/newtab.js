...
var initNewTab = function (newTabData) {
    extGlobal.distributionChannelConfig = newTabData.distributionChannelConfig;
    if (!newTabData && retryAttempts > 0) {
        ...
        if (extGlobal.constants.aolUI) {
            title.textContent = extGlobal.browserGap.getLocalizedString("newtab_extension_tab_title_aol");
        } else {
        ...
        var frValue = ((extGlobal.browserGap.isFirefox || extGlobal.browserGap.isWebExtension) ? extGlobal.distributionChannelConfig.frCodeFirefox : (extGlobal.browserGap.isSafari ? extGlobal.distributionChannelConfig.frCodeSafari : extGlobal.distributionChannelConfig.frCodeChrome));
        ...
        var typeParam = extGlobal.distributionChannelConfig.typeParam ? extGlobal.distributionChannelConfig.typeParam : extGlobal.constants.typeParam;
...
function ViewRenderer(newTabData, unittest) { //jshint ignore: line
    ...
    if (extGlobal.constants.aolUI) {
        ...
        defaultSites = extGlobal.siteConfig.aolDefaultSites;
    } else if (extGlobal.constants.mapQuestUI) {
        partners = [
            {
                title: "Yahoo",
            ...
            {
                title: "MapQuest",
                url: extGlobal.constants.mapquest_url,
                position: "left"
            }
        ];
        defaultSites = extGlobal.siteConfig.defaultSites;
    } else {
        partners = [
            {
                title: "Yahoo",
    ...
    async function renderBackground() {
        ...
        var photoData = await extGlobal.browserGap.localStorage.getItem("bgPhoto");
        photoData = JSON.parse(photoData || "{}").data;
        if(photoData){
            var readyStateCheckInterval = setInterval(function() {
            if (document.readyState === "complete") {
                clearInterval(readyStateCheckInterval);
                css.addSelector(".bgImage", {"background-image": "url(" + photoData + ")"});
            }
        }, 10);
        } else {
            imageBackground.setAttribute("class", imageBackground.getAttribute("class")+" offlineBG");
            css.addSelector(".bgImage", {"background-image": "url(" + extGlobal.constants.offlinePhotos[0].url_l + ")"});
        }
    }

    //display (and link) the owner of the background image
    async function renderOwnerData(){
        var bgPhoto = await extGlobal.browserGap.localStorage.getItem("bgPhoto");
        bgPhoto = JSON.parse(bgPhoto || "{}");
        var owner = bgPhoto.owner || extGlobal.constants.offlinePhotos[0].owner;
        var ownername = bgPhoto.ownername || extGlobal.constants.offlinePhotos[0].ownername;
        var photoId = bgPhoto.id || extGlobal.constants.offlinePhotos[0].id;

        if(ownername && owner) {
            var photographBy = document.getElementById("photographBy");
            var ownerLink = document.getElementById("ownerName");

            photographBy.setAttribute("href", "https://flickr.com/photos/"+owner+"/"+photoId);
            ownerLink.setAttribute("href", "https://flickr.com/photos/"+owner+"/"+photoId);

            photographBy.onclick = function(){
                var beaconConfig = {};
                var beaconParams = {};
                beaconParams.sec = photographBy.getAttribute("id");
                beaconParams.slk = photographBy.getAttribute("id"); // Ideally slk should contain the ui text, but we are doing this hack as FLickr is an svg here.
                beaconParams.tar = photographBy.getAttribute("href");
            ...
            var photoLabel = ownername;
            ownerLink.textContent = photoLabel;
            photographBy.textContent = extGlobal.browserGap.getLocalizedString("newtab_extension_on_flickr") + " ";
        }
    }
    ...
    async function renderTrendingNow() {
        ...
        if (typeof newTabData.trendingNowData === "undefined" || !newTabData.trendingNowData ||
            !enableTN || extGlobal.constants.financeUI || extGlobal.constants.aolUI || extGlobal.constants.sportsUI ||
            extGlobal.constants.mapQuestUI || extGlobal.constants.feelGoodUI) {
            return;
        }
    ...
    async function renderNewTopSites(parent, newFav) {
        ...
        if (extGlobal.distributionChannelConfig.partnerSite) {
            partners.push(extGlobal.distributionChannelConfig.partnerSite);
        }
    ...
    function renderCentralFooter() {
        if(!extGlobal.constants.financeUI && !extGlobal.constants.sportsUI && !extGlobal.constants.feelGoodUI) {
            var tos = document.getElementById("gdprTos");
            var policy = document.getElementById("gdprPolicy");
            var privacyD = document.getElementById("privacyD");
            if (tos) {
            ...
            if (privacyD) {
                privacyD.textContent = extGlobal.constants.privacy_dashboard_label;
                privacyD.setAttribute("href", extGlobal.constants.privacy_dashboard_url);
            }
        }
    }
    ...
    function renderArrowInterstitial() {
        ...
        if (extGlobal.constants.aolUI) {
            firstTabDetailsDiv.textContent = extGlobal.browserGap.getLocalizedString("newtab_extension_chrome_aol_protect_2");
        }
        else {
    ...
    async function render() {
        ...
        if((typeof RightPanel !== 'undefined') && (newTabData.financeData || newTabData.sportsData  || newTabData.aolData || extGlobal.constants.mapQuestUI || newTabData.feelGoodData)) { //for mapQuest we render even if data is not loaded yet - data is only geolocation for now
            rightPanel = new RightPanel(newTabData, frValue, typeParam, css);
        ...
        if (window.addEventListener) {
            window.addEventListener('resize', function() {
                ...
                if (rightPanelDiv && rightPanelDiv.style.display !== "none") {
                    rightPanel && rightPanel.resizeRightPanelBlur();
                }
            });
        ...
        if(extGlobal.browserGap.isChrome)
        {
            sendPageViewBeacon((extGlobal.distributionChannelConfig.spaceid && extGlobal.distributionChannelConfig.spaceid.chrome ? extGlobal.distributionChannelConfig.spaceid.chrome : extGlobal.constants.chrome_space_id));
        }
        else if(extGlobal.browserGap.isFirefox || extGlobal.browserGap.isWebExtension) {
            sendPageViewBeacon(extGlobal.distributionChannelConfig.firefox_space_id || extGlobal.constants.firefox_space_id);
        }
        else if(extGlobal.browserGap.isSafari)
        {
            sendPageViewBeacon(extGlobal.distributionChannelConfig.safari_space_id || extGlobal.constants.safari_space_id);
        }
