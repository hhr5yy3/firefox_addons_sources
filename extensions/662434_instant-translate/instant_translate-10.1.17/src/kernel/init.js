...
(function (undefined) {
    ...
    if (!ke.IS_SAFARI) {
        var o_gu = chrome.extension.getURL.clone();
    ...
    pl.extend(ke, {
        ...
        get isLegacyProUser() {
            return ke.ext.util.storageUtil.isTrueOption("chr_pro_flag");
        },

        get isProUser() {
            let sub_data = ke.ext.util.storageUtil.getDecodedVal('sub_data');
            return ke.isLegacyProUser // legacy pro users
        ...
        initAnalytics: function (callback) {
            ...
            ke.import('ext.const.split_tests', callback || function() {});
        },
        ...
        getPrefix: function () {
            return ke.section === 'content' || ke.section === 'pdf_tooltip' ? ke.getConst('prefix') + '-' : '';
        },
