...
<li class="js-allowlist-list-item">
    ...
    "shared/js/background/storage/tds.js"() {
      ...
      tds_default = {
        ...
        async ready(configName) {
          await settings.ready();
          const tdsStorage3 = this.tdsStorage;
          if (!tdsStorage3) {
            return Promise.resolve();
          }
          if (configName && listNames.includes(configName)) {
            return tdsStorage3[configName].ready;
          }
          return Promise.all(listNames.map((n) => tdsStorage3[n].ready));
        },
  ...
  // shared/js/background/popupMessaging.js
  var popupMessaging_exports = {};
  __export(popupMessaging_exports, {
    popupConnectionOpened: () => popupConnectionOpened,
    postPopupMessage: () => postPopupMessage
  });
  function popupConnectionOpened(port, messageHandlers2) {
    activePort = port;
    ...
    port.onMessage.addListener(async (message) => {
      ...
      if (!messageType || !(messageType in messageHandlers2)) {
        console.error(
          "Unrecognized message (privacy-dashboard -> background):",
          message
        );
        return;
      }
      const response = await messageHandlers2[messageType](message?.options, port, message);
  ...
  var require_companies = __commonJS({
    "shared/js/background/companies.js"(exports2, module2) {
      ...
      var browserWrapper5 = (init_wrapper(), __toCommonJS(wrapper_exports));
      var { postPopupMessage: postPopupMessage2 } = (init_popupMessaging(), __toCommonJS(popupMessaging_exports));
      var Companies3 = (() => {
  ...
  async function sendPageloadsWithAdAttributionPixelAndResetCount() {
    ...
    const count = import_settings2.default.getSetting("m_pageloads_with_ad_attribution.count");
    if (typeof count === "number" && count > 0) {
      await sendPixelRequest("m_pageloads_with_ad_attribution", {
        count
      });
    }
    import_settings2.default.updateSetting("m_pageloads_with_ad_attribution.count", 0);
  }
  var import_rulePriorities, import_utils2, import_settings2, getFeatureSettings2, getBaseDomain2, browserWrapper, getNextSessionRuleId2, appVersion, manifestVersion, AdClickAttributionPolicy, AdClick;
      ...
      AdClick = class _AdClick {
        ...
        requestWasAllowed(tab) {
          if (!tab.firstAdAttributionAllowed) {
            import_settings2.default.incrementNumericSetting("m_pageloads_with_ad_attribution.count");
  ...
  var import_tldts2, settings4, utils, tdsStorage, privacyPractices, Grade, browserWrapper2, TabState2, Site;
  var init_site = __esm({
    "shared/js/background/classes/site.js"() {
      "use strict";
      import_tldts2 = __toESM(require_cjs());
      settings4 = require_settings();
      ...
      Site = class {
        ...
        setListStatusFromGlobal() {
          ...
          globalLists.forEach((name) => {
            const list = settings4.getSetting(name) || {};
    ...
    ruleUpdateLock = ruleUpdateLock.then(async () => {
      if (configName === "tds") {
        ...
        const {
          ...
          matchDetailsByRuleId,
          allowingRulesByClickToLoadAction
        } = await (0, import_tds5.generateTdsRuleset)(
          configValue,
          supportedSurrogates,
  ...
  var import_settings5, import_trackers, import_extensionConfiguration, import_tds5, import_utils5, import_combined, ruleUpdateLock;
  var init_dnr_config_rulesets = __esm({
    "shared/js/background/dnr-config-rulesets.js"() {
      ...
      import_tds5 = __toESM(require_tds());
  ...
  async function generateDnrAllowingRules(tab, ruleAction) {
    ...
    const allowingDnrRulesByClickToLoadRuleAction = import_settings7.default.getSetting("allowingDnrRulesByClickToLoadRuleAction");
  ...
  var require_tab_manager = __commonJS({
    "shared/js/background/tab-manager.js"(exports2, module2) {
      ...
      var { getCurrentTab: getCurrentTab2 } = (init_utils(), __toCommonJS(utils_exports));
      ...
      var TabManager = class {
        ...
        async getOrRestoreTab(tabId) {
          if (!tabManager5.has(tabId)) {
            await tabManager5.restore(tabId);
          }
          return tabManager5.get({ tabId });
        }
        /**
        ...
        async getOrRestoreCurrentTab() {
          const currentTabDetails = await getCurrentTab2();
          if (currentTabDetails?.id) {
            return await tabManager5.getOrRestoreTab(currentTabDetails.id);
          }
          return null;
        }
        /**
        ...
        createOrUpdateTab(id, info) {
          if (!tabManager5.get({ tabId: id })) {
            info.id = id;
            return tabManager5.create(info);
          } else {
            const tab = tabManager5.get({ tabId: id });
            if (tab && info.status) {
        ...
        updateTabUrl(request) {
          const tab = tabManager5.get({ tabId: request.tabId });
          if (tab) {
      ...
      var tabManager5 = new TabManager();
      module2.exports = tabManager5;
    }
  });

  // shared/js/background/broken-site-report.js
  function fire(pixelName, querystring) {
  ...
  function constructUrl(pixelName, querystring, truncate) {
    ...
    const browserName2 = browserInfo2?.browser;
    const extensionVersion = browserWrapper3.getExtensionVersion();
    const atb = settings10.getSetting("atb");
    ...
    if (browserName2) {
      url += `_${browserName2.toLowerCase()}`;
    }
    url += `?${randomNum}&`;
    let extraParams = "";
  ...
  async function computeLastSentDay(urlString) {
    ...
    const reportTimes = settings10.getSetting("brokenSiteReportTimes") || {};
    const lastSentDay = reportTimes[hostnameHashPrefix];
    reportTimes[hostnameHashPrefix] = dayOutput;
    settings10.updateSetting("brokenSiteReportTimes", reportTimes);
    return lastSentDay;
  }
  ...
  async function sendBreakageReportForCurrentTab({ pixelName, currentTab, category, description, reportFlow }) {
    await settings10.ready();
    await tdsStorage2.ready("config");
    const tab = currentTab || await tabManager.getOrRestoreCurrentTab();
    if (!tab) {
      return;
    }
    const pageParams = await browser4.tabs.sendMessage(
      tab.id,
      { getBreakagePageParams: true }
    ) || {};
    const tds = settings10.getSetting("tds-etag");
    const remoteConfigEtag = settings10.getSetting("config-etag");
    const remoteConfigVersion = tdsStorage2.config.version;
  ...
  var browser4, load2, browserWrapper3, settings10, parseUserAgentString2, getURLWithoutQueryString2, getURL2, tdsStorage2, tabManager, maxPixelLength, truncatableFields, requestCategoryMapping;
  var init_broken_site_report = __esm({
    "shared/js/background/broken-site-report.js"() {
      "use strict";
      browser4 = require_browser_polyfill();
      load2 = require_load();
      ...
      tdsStorage2 = (init_tds(), __toCommonJS(tds_exports)).default;
  ...
  var utils2, tabManager2, trackerutils, settings11, isActive2, constants2, LegacyTabTransfer2;
  ...
  var import_webextension_polyfill5, import_settings8, import_tab_manager, ToggleReports;
    ...
    "shared/js/background/components/toggle-reports.js"() {
      ...
      ToggleReports = class _ToggleReports {
        ...
        constructor() {
          ...
          import_webextension_polyfill5.default.alarms.onAlarm.addListener(async ({ name }) => {
        ...
        async toggleReportStarted(sender) {
          ...
          const response = { data: [] };
          for (const paramId of _ToggleReports.PARAM_IDS) {
        ...
        async toggleReportFinished(accepted, sender) {
          ...
          await Promise.all(
            [
              reloadCurrentTab(),
              resolveAfterDelay(accepted ? 5e3 : 0)
            ]
          );
  ...
  async function setLists(options) {
    ...
    for (const listItem of options.lists) {
      ...
      await tabManager4.setList(listItem);
    }
  ...
  function submitBrokenSiteReport(breakageReport) {
  ...
  async function getPrivacyDashboardData(options) {
    ...
    await tds_default.ready("config");
    const tab = await tabManager4.getOrRestoreTab(tabId);
  ...
  var import_webextension_polyfill6, import_parse_user_agent_string2, utils3, settings13, tabManager4, Companies2, browserName, devtools2, browserWrapper4, messageHandlers;
  ...
  var import_webextension_polyfill7, import_tldts5, MENU_ITEM_ID, REFETCH_ALIAS_ALARM, REFETCH_ALIAS_ATTEMPT, EmailAutofill, formatAddress, isValidUsername, isValidToken;
    ...
    "shared/js/ui/base/ui-wrapper.js"() {
      ...
      backgroundMessage = (thisModel) => {
        import_webextension_polyfill8.default.runtime.onMessage.addListener((req, sender) => {
          if (sender.id !== import_webextension_polyfill8.default.runtime.id) return;
