function Tracker(unittests){ //jshint ignore: line
    ...
    async function sendBeacon (spaceId, beaconType, beaconConfig) {
        var ctid = await extGlobal.browserGap.localStorage.getItem('ctid');
        var defbrows = await extGlobal.browserGap.localStorage.getItem('defbrows');
        if(ctid === null) {
        ...
        try {
            var frCode = (extGlobal.browserGap.isFirefox || extGlobal.browserGap.isWebExtension ? extGlobal.distributionChannelConfig.frCodeFirefox : (extGlobal.browserGap.isSafari ? extGlobal.distributionChannelConfig.frCodeSafari : extGlobal.distributionChannelConfig.tracker.frCode.chrome));
            var trackingData = [{
                ...
                "trackParams":
                {
                    ...
                    "pc": extGlobal.distributionChannelConfig.partnerCode,
                    "ver": "{ver}",
                    "itype": "{itype}",
                    "fr": "{fr}",
                    "ctid": "{ctid}",
                ...
                if (trackTypeCur.trackSpaceID && (trackTypeCur.trackEvt.toLowerCase() === beaconType.toLowerCase()))
                {
                    ...
                    for (var paramOn in objTrackParams) //jshint ignore: line
                    {
                        ...
                        if (paramVal === '{intl}'){
                            ...
                            paramVal = await extGlobal.browserGap.localStorage.getItem('ctid');
                        }else if (paramVal === '{defbrows}'){
                            paramVal = await extGlobal.browserGap.localStorage.getItem('defbrows');
                    ...
                    if (trackTypeCur.useYLC)
                    {
                        ...
                        trackURL = ("https://geo.yahoo.com/" + getBeaconPath(beaconConfig.params.pt) + "?t=" + Math.random());
    ...
    async function sendAlivePing(spaceId, pageInfo, beaconConfig) {
        ...
        var partnerSites = await extGlobal.browserGap.localStorage.getItem("partnerSites");
    ...
    function triggerAcceptBeacon(spaceId, browser, subType) {
        var beaconConfig = {};
        var beaconParams = {};
        beaconParams.itype = extGlobal.constants.tracker_accept;
        beaconParams.browser = browser;
        beaconParams.ttype = subType;
        beaconConfig.params = beaconParams;
        sendBeacon(spaceId, extGlobal.constants.tracker_page_info, beaconConfig);
    }
    ...
    function getAmpDesc() {
        ...
        var distributionCode = extGlobal.distributionChannelConfig.amp_desc_dist || extGlobal.constants.amp_desc_dist_default;
        return extGlobal.constants.amp_desc_type + "_" + browser + "_" + distributionCode;
    }

    function getBeaconPath(pt) {
        var beaconPath = extGlobal.constants.beaconDefaultPath;
        if (pt && pt === "newtab_page" && extGlobal.distributionChannelConfig.beaconPath) {
            beaconPath = extGlobal.distributionChannelConfig.beaconPath;
        }
        return beaconPath;
    }
    ...
    this.triggerAcceptBeacon = triggerAcceptBeacon;
