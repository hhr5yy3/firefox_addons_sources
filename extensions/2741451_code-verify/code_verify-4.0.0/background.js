(function () {
    ...
    const DYNAMIC_STRING_MARKER = '/*BTDS*/';
    ...
    ***************************************************************************** */
     ...
     */
    class FrameStateMachine extends StateMachine {
    ...
    function getChromeV3Action() {
        if (self.chrome.runtime.getManifest().manifest_version >= 3) {
            ...
            return {
                setIcon: (_) => self.chrome.pageAction.setIcon(_),
                disable: (tabId) => { var _a, _b; return (_b = (_a = self.chrome.pageAction).hide) === null || _b === void 0 ? void 0 : _b.call(_a, tabId); },
                enable: (tabId) => { var _a, _b; return (_b = (_a = self.chrome.pageAction).show) === null || _b === void 0 ? void 0 : _b.call(_a, tabId); },
                setPopup: (_) => self.chrome.pageAction.setPopup(_),
            };
        }
    }
    /**
     ...
     * in it.
     */
    class TabStateMachine extends StateMachine {
        ...
        updateStateForFrame(frameId, newState) {
            if (!(frameId in this._frameStates)) {
                ...
                throw new Error(`State machine for frame: ${frameId} does not exist for tab: ${this._tabId}`);
            }
        ...
        updateStateIfValid(newState) {
            ...
            if (newState === STATES.VALID &&
                !Object.values(this._frameStates).every(fsm => fsm.getState() === STATES.VALID || fsm.getState() === STATES.START)) {
                return;
            }
        ...
        genDisableTab() {
            return __awaiter(this, void 0, void 0, function* () {
                const tab = yield chrome.tabs.get(this._tabId);
                if (tab.url) {
    ...
    function getOrCreateTabStateMachine(tabId, origin) {
        var _a;
        const tabState = (_a = tabStateTracker.get(tabId)) !== null && _a !== void 0 ? _a : new TabStateMachine(tabId, origin);
        if (!tabStateTracker.has(tabId)) {
            tabStateTracker.set(tabId, tabState);
        }
        return tabState;
    }
    function recordContentScriptStart(sender, origin) {
        // This is a top-level frame initializing
        if (sender.frameId === 0) {
            tabStateTracker.delete(sender.tab.id);
        }
        getOrCreateTabStateMachine(sender.tab.id, origin).addFrameStateMachine(sender.frameId);
    }
    function updateContentScriptState(sender, newState, origin) {
        getOrCreateTabStateMachine(sender.tab.id, origin).updateStateForFrame(sender.frameId, newState);
    }
     ...
     * LICENSE file in the root directory of this source tree.
     */
    function setOrUpdateMapInMap(outerMap, outerKey, innerKey, value) {
        var _a;
        const innerMap = (_a = outerMap.get(outerKey)) !== null && _a !== void 0 ? _a : new Map();
    ...
    function setOrUpdateSetInMap(outerMap, outerKey, value) {
        var _a;
        const innerSet = (_a = outerMap.get(outerKey)) !== null && _a !== void 0 ? _a : new Set();
        innerSet.add(value);
        if (!outerMap.has(outerKey)) {
     ...
     * LICENSE file in the root directory of this source tree.
     */
    function setupCSPListener(cspHeadersMap, cspReportHeadersMap) {
        chrome.webRequest.onHeadersReceived.addListener(details => {
            if (details.responseHeaders) {
                const cspHeaders = getCSPHeadersFromWebRequestResponse(details);
                const cspReportHeaders = getCSPHeadersFromWebRequestResponse(details, true);
                let frameId = details.frameId;
                if (
                // @ts-expect-error Missing: type definitions in @types/chrome
                (details === null || details === void 0 ? void 0 : details.documentLifecycle) === 'prerender' &&
                    // @ts-expect-error Missing: type definitions in @types/chrome
                    (details === null || details === void 0 ? void 0 : details.frameType) === 'outermost_frame' &&
                ...
                if (details.tabId !== 0) {
                    setOrUpdateMapInMap(cspHeadersMap, details.tabId, frameId, cspHeaders.map(h => h.value));
                    setOrUpdateMapInMap(cspReportHeadersMap, details.tabId, frameId, cspReportHeaders.map(h => h.value));
                }
                else {
                    ...
                    chrome.tabs.query({ active: true, currentWindow: true }).then(tabs => {
                        if (tabs.length !== 0) {
                            setOrUpdateMapInMap(cspHeadersMap, tabs[0].id, frameId, cspHeaders.map(h => h.value));
                            setOrUpdateMapInMap(cspReportHeadersMap, tabs[0].id, frameId, cspReportHeaders.map(h => h.value));
                        }
                    });
                }
            }
     ...
     * LICENSE file in the root directory of this source tree.
     */
    const isMetaInitiatedResponse = (response) => {
    ...
    function checkResponseMIMEType(response) {
        ...
        if (((_b = (_a = response.responseHeaders) === null || _a === void 0 ? void 0 : _a.find(header => header.name.toLowerCase().includes('x-content-type-options'))) === null || _b === void 0 ? void 0 : _b.value) !== 'nosniff') {
    ...
    function setUpWebRequestsListener(cachedScriptsUrls) {
        chrome.webRequest.onResponseStarted.addListener(response => {
            ...
            if (response.type === 'xmlhttprequest' &&
                ((_a = cachedScriptsUrls.get(response.tabId)) === null || _a === void 0 ? void 0 : _a.has(response.url))) {
                ...
                (_b = cachedScriptsUrls.get(response.tabId)) === null || _b === void 0 ? void 0 : _b.delete(response.url);
                return;
            }
            if (response.type === 'script') {
                checkResponseMIMEType(response);
     ...
     * LICENSE file in the root directory of this source tree.
     */
    function getCFRootHash(host, version) {
        return new Promise(resolve => {
            fetch('https://staging-api.privacy-auditability.cloudflare.com/v1/hash/' +
                `${encodeURIComponent(host)}/${encodeURIComponent(version)}`, { method: 'GET' })
                .then(response => {
                resolve(response);
            })
                .catch(error => {
     ...
     * LICENSE file in the root directory of this source tree.
     */
    function validateMetaCompanyManifest(rootHash, otherHashes, leaves, host, version) {
        return __awaiter(this, void 0, void 0, function* () {
            const cfResponse = yield getCFRootHash(host, version);
            ...
            const cfPayload = yield cfResponse.json();
            ...
            const jsHashArray = Array.from(new Uint8Array(yield crypto.subtle.digest('SHA-256', encodedMegaHash)));
            ...
            if (jsHash === otherHashes.main || jsHash === otherHashes.longtail) {
                const combinedHashArray = Array.from(new Uint8Array(yield crypto.subtle.digest('SHA-256', encoder.encode(otherHashes.longtail + otherHashes.main))));
    ...
    function validateSender(sender) {
        ...
        if ((sender === null || sender === void 0 ? void 0 : sender.documentLifecycle) === 'prerender') {
            frameId = 0;
        }
        return {
            frameId,
    ...
    function plugin(BaseParser) {
        ...
        return class extends BaseParser {
            parseLiteral(value) {
                ...
                const before = this.input.substring(node.start - markerLength, node.start);
    ...
    function removeDynamicStrings(rawjs) {
        ranges = [0];
    ...
    const CSP_HEADERS = new Map();
    const CSP_REPORT_HEADERS = new Map();
    // Keeps track of scripts `fetch`-ed by the extension to ensure they are all
    // resolved from browser cache
    const CACHED_SCRIPTS_URLS = new Map();
    function getManifestMapForOrigin(origin) {
    ...
    function handleMessages(message, sender, sendResponse) {
        ...
        switch (message.type) {
            ...
            case MESSAGE_TYPE.LOAD_COMPANY_MANIFEST: {
                validateMetaCompanyManifest(message.rootHash, message.otherHashes, message.leaves, ORIGIN_HOST[message.origin], message.version).then(validationResult => {
                    var _a;
                    if (validationResult.valid) {
                        const manifestMap = getManifestMapForOrigin(message.origin);
                        const manifest = (_a = manifestMap.get(message.version)) !== null && _a !== void 0 ? _a : {
                            leaves: [],
                ...
                crypto.subtle.digest('SHA-256', encodedSrc).then(hashBuffer => {
                    ...
                    if (manifestObj.leaves.includes(hash)) {
                        ...
                        sendResponse({
                            valid: false,
                            hash: hash,
                            reason: 'Error: hash does not match ' +
                                message.origin +
                                ', ' +
                                message.version +
                                ', unmatched SRC is ' +
            ...
            case MESSAGE_TYPE.CONTENT_SCRIPT_START: {
                ...
                sendResponse({
                    success: true,
                    cspHeaders: (_a = CSP_HEADERS.get(validSender.tab.id)) === null || _a === void 0 ? void 0 : _a.get(validSender.frameId),
                    cspReportHeaders: (_b = CSP_REPORT_HEADERS === null || CSP_REPORT_HEADERS === void 0 ? void 0 : CSP_REPORT_HEADERS.get(validSender.tab.id)) === null || _b === void 0 ? void 0 : _b.get(validSender.frameId),
                });
                return;
            }
