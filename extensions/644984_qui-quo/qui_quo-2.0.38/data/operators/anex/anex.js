// в начале идёт код, скопированный из samo-tour.js
// при любых изменениях в samo-tour.js надо обязательно копировать сюда!!!!

// САМО-тур online бронирование
var showTopHotelsRating = true;
// фикс для бага в Google Chrome версия 66
document.addEventListener("scroll", scrolling);

function scrolling() {
    var popup = document.querySelector("#qq-adjust-price");
    var scrollTop = window.pageYOffset;
    if ( popup ) {
        popup.style.top = (scrollTop + document.documentElement.clientHeight/2 ) + "px";
...
function mapCountry(text) {
    if ( location.hostname.match(/anextour.lt/) ) {
...
function extractHotelUrl(td, td2) {
    var anchor = td.querySelector("td>a");
    if ( anchor )
        return anchor.href.replace('b2b.', '');
    anchor = td2.querySelector("td>a");
    return anchor ? anchor.href.replace('b2b.', '') : null
}

function getRoomType(tds) {
    var arr = [];
    var cols = [[/номер\s*\/\s*размещение/], [/^номер$/], [/^размещение$/], [/номер\s*\/\s*розміщення/], [/Numeris\s*\/\s*Regionas/i]];
    for ( var i = 0; i < cols.length; i++) {
        ...
        if ( text != null && text != "" ) {
            ...
            if ( html ) {
                ...
                tempElem.insertAdjacentHTML('afterBegin', html[0]);
                let tempText =   tempElem.innerText;
                if ( tempText.length > 50 ) {
                    tempText = tempText.split(/,\s*/)[0].slice(0, 50);
                }
                text = text + ", " +  tempText;
                delete tempElem;
            }
...
function extractBoardType(tds) {
    var board = colText(tds, [/питание/, /meal/, /харчування/, /Maistas/i]);
    if ( !board ) {
        if (  typeof isBoardTypeOptional != 'undefined' && isBoardTypeOptional == true ) {
            return "";
        } else {
            var tr = getHeaderRow();
            throw new Error("Не найдена информация про питание: " + (tr ? tr.innerHTML : null));
        }
    }
    return trim(board);
}
...
function extractBoardCode(tds) {
    var td = col(tds, [/питание/, /meal/, /харчування/, /Maistas/i]);
    if ( !td ) {
        return null;
    } else {
...
var COL_HOTEL = [/гостиница/,/отель/, /Viešbutis/i];
var COL_sURL = [/тур/, /Ekskursija/i];
function createOption(img) {
    ...
    var price = colText(tds, [/цена/, /ціна/]);
    ...
    var option = {
        ...
        city_from : getCityFrom(img.parentNode.parentNode.parentNode),
...
function injectData() {
	// фикс для бага в Google Chrome версия 66
    var popup = document.querySelector("#qq-adjust-price");
    if ( popup && !popup.getAttribute("focuseventadded")) {
        popup.addEventListener("focus", scrolling);
        simulateEvent(popup, "focus");
        popup.setAttribute("focuseventadded", "true");
    }
    ...
    for (var i = 0; i < trs.length; ++i) {
        ...
        // special check to hide buttons inside hidden area
        var tds = querySelectorAll(trs[i],"td").filter( td =>  !td.classList.contains("qq"));
        if ( tds.length == 0 )
            continue;
        var ths = querySelectorAll(getHeaderRow(), "th").filter(th => !th.classList.contains("qq"));
        if ( tds.length > ths.length ) {
            const td = tds.find( td => getText(td) === "");
            td ? td.remove(): null;
        }
        var dateAndDoW = colText(tds, [/заезд/, /дата/, /вылет/, /įsiregistruoti/]);
        if ( !dateAndDoW )
            continue;
        if ( !extractDate(dateAndDoW) )
            continue;

        if ( trs[i].querySelector(".qq") == null && tds.length > 6 ) {
...
function getDoc() {
    var frame = document.getElementById("framesamo");
    return frame ? frame.contentDocument : document;
}

var isTransportColToShrink = true;

function extractThumbnail(hotel, sURL) {
	var href = extractHotelUrl(hotel, sURL);
	if ( href ) {
		var hotelId = href.match(/HOTELINC=(\d+)/);
		if ( hotelId ) {
			return hotelId[1] ? "https://files.anextour.com/images/hotel?code=" + hotelId[1] + "&width=300" : null;
		}
	}
	return null;
}
...
function createOption(img) {
    ...
    var price = colText(tds, [/цена/, /ціна/]);
    if ( !price ) {
    	price = tr.nextElementSibling.querySelector(".price").textContent;
    }
    var sURL = col(tds, COL_sURL);
    var option = {
        checkinDt : extractDate(colText(tds, [/заезд/, /дата/, /вылет/, /įsiregistruoti/]))[1],
        region : extractRegion(tds, hotel),
        nights : extractNights(tr, tds),
        extra_nights: extractExtraNights(tr),
        hotelName : extractHotelName(hotel),
        ...
        city_from : getCityFrom(img.parentNode.parentNode.parentNode),
        occupancy : SEARCH_CRITERIA.occupancy,
        thumbnail : extractThumbnail(hotel, sURL)
    };
    return option;
}

// TEMP FIX FOR COLUMN WITHOUT TITLE
function injectTitle() {
    if ( needInjectTitle() ) {
    	addMissingColumnTitle();
        addTitle();
    }
}

function addMissingColumnTitle() {
	var tr = getHeaderRow();

	var firstColumn = tr.querySelector("th");
	if ( firstColumn && firstColumn.textContent.indexOf('Заезд') >= 0  ) {
	    firstColumn.parentNode.insertBefore(getDoc().createElement("th"), firstColumn);
	}
}
