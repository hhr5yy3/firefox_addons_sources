...
function afsrc(search) {
	var now = Date.now();
	if (search.indexOf('afsrc=') !== -1 || search.match(/afsrc=/i) || search.match(/CommissionJunction/ig) !== null || search.match(/Conversant/ig) !== null) {
...
/**
 * just compare the full domain with the base domain
 * ex: www.gmail.google.com (fullDomain) is same google.com (baseDomain)
 * @param fullDomain
 * @param baseDomain
 * @returns {boolean}
 */
function isSameDomain(fullDomain, baseDomain) {
  if (typeof fullDomain !== "string" || typeof baseDomain !== "string" || !fullDomain.length || !baseDomain.length) {
    return false;
  }
  fullDomain = extractHostname(fullDomain);
  baseDomain = extractHostname(baseDomain);
  // remove sub domain www
  baseDomain = baseDomain.replace(new RegExp("^www.\\b"), "");
  if (typeof baseDomain !== "string" || !baseDomain.length){
    return false;
  }
  return fullDomain.match(new RegExp("^\\S+." + baseDomain + "$|^" + baseDomain + "$")) !== null;
}

function hasCurrentUrl(data, url, callback) {
  var dataFilter = data.detectionTld.filter(function (item) {
    return isSameDomain(url, item.tld);
  });
  callback(!!dataFilter.length, dataFilter.length ? dataFilter[0] : null);
}
