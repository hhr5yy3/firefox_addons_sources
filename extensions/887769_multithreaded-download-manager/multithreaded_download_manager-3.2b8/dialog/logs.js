...
if (Number.isInteger(id)) {
    const timer = new Timer(async () => {
        ...
        for (const item of items) {
            ...
            tr.append(Object.assign(document.createElement('td'), { className: 'date', textContent: date, title: date }));
            tr.append(Object.assign(document.createElement('td'), { className: 'message', textContent: item.message, title: item.message }));
            tr.append(Object.assign(document.createElement('td'), { className: 'line', textContent: item.line, title: item.line }));
            tbody.append(tr);
        }
        if (scrollAtBottom)
...
rawDataButton.addEventListener('click', async () => {
    ...
    blobURL = URL.createObjectURL(new File([JSON.stringify(items, undefined, '\t')], `log.json`, { type: 'application/json' }));
