...
async function u_checklogin3a(res, ctx) {
    'use strict';
    var r = false;
    ...
    if (typeof res === 'object') {
        ...
        chrome.storage.local.set({u_handle});
...
function u_logout(logout) {
    ...
    u_sid = u_k = u_handle = u_attr = u_privk = undefined;
    ...
    if (logout) {
        ...
        M.RootID = false;
        pmReady = pminitialized = pmdb = currsn = login_next = pwmh = false;
        ...
        const oldapi = localStorage.apipath;

        localStorage.clear();
        localStorage.setItem('settings', {autofill: true, autosave: true});

        if (oldapi) {
            localStorage.setItem('apipath', oldapi);
        }

        u_k_aes = u_type = u_storage = u_checked  = undefined;
    ...
    if (emailChangeAccepted) {
        ...
        if (user) {
            ...
            if (ap.u === u_handle) {
                ...
                if (popupOpen) {
                    chrome.runtime.sendMessage({type: 'updateEmail', email: user.m});
                }
            }
        }
    }
