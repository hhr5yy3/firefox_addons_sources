...
browser.runtime.onInstalled.addListener((details) => {
  switch (details.reason) {
    case "install":
      ...
      browser.storage.local.set({ "skipCount" : 0, "spongeSiren" : 100 });
      getArtefacts();
      break;
    case "update":
      browser.storage.local.get(['skipCount'])
          .then((result) => (typeof result.skipCount !== "undefined") ? console.info("EXTENSION UPDATED") : browser.storage.local.set({ "skipCount" : 0, "spongeSiren" : 100 })
      )
      getArtefacts();
      break;
    case "browser_update":
    ...
    default:
      browser.storage.local.get(['skipCount'])
          .then((result) => (typeof result.skipCount !== "undefined") ? console.info("BROWSER UPDATED") : browser.storage.local.set({ "skipCount" : 0, "spongeSiren" : 100 })
      )
      getArtefacts();
      break;
  }
...
function txnModal() {
    browser.storage.local.get(['skipCount', 'spongeSiren'], function(res) {
          ...
          extpay.getUser().then(user => {
            if (!user.paid) {
                ...
                <div class="text">
                  <div class="title">
                    ...
                    <div class="popup-text">
                      For any questions and support you can reach out to 0x48piraj@gmail.com.
                    </div>
                  </div>
                  <div class="info">
              ...
              dialog.querySelector("#close-modal").addEventListener("click", () => {
                browser.storage.local.get(['spongeSiren'])
                    .then((result) => browser.storage.local.set({'spongeSiren':result.spongeSiren + 50}));
                dialog.close();
              });
...
browser.runtime.onMessage.addListener((res, sender) => {
