...
TspNativeHost.prototype.Connect = function()
{
  try
  {
    ...
    this.nmhPort.onDisconnect.addListener(function(port) {
      var error = null;
      if (port && typeof port.error != 'undefined')
        error = port.error; // Port error implementation under Mozilla Firefox
      else
        error = chrome.runtime.lastError;
      var errorMessage = '';
      if (error && typeof error.message != 'undefined')
        errorMessage = error.message;
      this.hstOnDisconnected(errorMessage);
    }.bind(this));
...
function TspBackground()
{
  ...
  this.MiniguiPopupTransport = spBackgroundTools.CreateMiniguiPopupTransport();
  ...
  this.TransportProtocol.onMiniguiExecuteUrl = this.tsOnMiniguiExecuteUrl.bind(this);

  this.DisableInternalPasswordManager();
  ...
  chrome.runtime.onInstalled.addListener(this.csOnInstalled.bind(this));
  // event handler for clicking on Caption Button
  spBackgroundTools.GetBrowserAction().onClicked.addListener(this.ProcessCaptionButtonClick.bind(this));
  // connect to tabs
...
TspBackground.prototype.DisableInternalPasswordManager = function ()
{
  try
  {
    var PrivacyServices = spPrivacyServices.CreatePrivacyServices();
    PrivacyServices.DisableInternalPasswordManager(this.ExtensionStorage);
  }
  catch (ErrorMessage)
  {
    spLog.logError(this.constructor.name + '.DisableInternalPasswordManager() Error: ' + ErrorMessage);
  }
};
...
TspBackground.prototype.LoadPasswordManagerConfiguration = function ()
{
  ...
  this.ExtensionStorage.get('PasswordManagerConfiguration', function (APasswordManagerConfiguration) {
    ...
    {
      spLog.logError(this.constructor.name + '.LoadPasswordManagerConfiguration() Error parsing PasswordManagerConfiguration: ' + ErrorMessage);
    }
  }.bind(this));
};
...
{
  spBackgroundTools.GetBrowserAction().setPopup({popup: 'spProtocolIncompatible.html'});
  spBackgroundTools.GetBrowserAction().setBadgeText({text: 'x'}); // show 'error' sub icon over main one
};
...
TspBackground.prototype.hstOnNotInitialized = function (wasInitialized)
{
  if (wasInitialized)
  {
    ...
    spBackgroundTools.GetBrowserAction().setPopup({popup: ''});
    spBackgroundTools.GetBrowserAction().setBadgeText({text: ''}); // reset sub icon over main one (if any error occured before)
  }
  else
  {
    // SP is not installed - initialize the default popup
    spBackgroundTools.GetBrowserAction().setPopup({popup: 'spPopup.html'});
    if (this.Host.Log.Connect)
      spBackgroundTools.GetBrowserAction().setBadgeText({text: 'x'}); // used for debug purposes
  }
};
...
TspBackground.prototype.UpdatePopupIfHostInitialized = function ()
{
  if (this.Host.IsInitialized())
  {
    ...
    if (this.UseMinigui &&
      ...
      spBackgroundTools.GetBrowserAction().setPopup({popup: 'minigui/index.html'});
    else
      spBackgroundTools.GetBrowserAction().setPopup({popup: ''});
  }
};
...
TspBackground.prototype.ClosePopupWindow = function ()
{
  try
  {
    ...
    {
      // Close the Minigui by getViews() API for Chrome Manifest V2
      var views = chrome.extension.getViews({ type: 'popup' });
      if (views && views.length)
      {
        var popupWindow = views[0];
        popupWindow.close();
      }
    }
    else
    {
      // Close the Minigui by CloseMinigui message for Chrome Manifest V3 as getViews() API doesn't supported
      this.TransportProtocol.CloseMinigui();
    }
  }
  catch (ErrorMessage)
  {
...
TspBackground.prototype.PostTabMessageToTransport = function (message, senderTab)
{
  ...
  chrome.windows.get(senderTab.windowId,
    function(window)
    {
      ...
      // if window is not focused - than any of its tabs are inactive too and such tabs must not be autofilled!
      if (typeof message.IsActiveTab != 'undefined')
      {
        if (!window.focused)
          message.IsActiveTab = false;
      }
...
TspBackground.prototype.ExecuteUrl = function(Url)
{
  if (this.Log.Info)
    spLog.logMessage(this.constructor.name + '.ExecuteUrl(), Url: <' + Url + '>');
  return spBackgroundTools.ExecuteUrl(Url, false, // InActiveTab
    function (AReusedTab)
    {
      if (!AReusedTab)
        return;
...
TspBackground.prototype.tsOnMiniguiExecuteUrl = function (Url)
{
  this.ExecuteUrl(Url);
};
  ...
  try
  {
    if (this.IsPasswordManagerLocked != message.IsLocked)
    {
      ...
      if (!this.IsPasswordManagerLocked)
        spBackgroundTools.GetBrowserAction().setIcon({
          path: {
            '16': 'skin/logo16.png',
            '20': 'skin/logo20.png',
            '32': 'skin/logo32.png'
...
TspBackground.prototype.tsExecuteUrl = function(message)
{
  if (!message)
    return;
  var Result = this.ExecuteUrl(message.Url);
...
TspBackground.prototype.csOnInstalled = function(details)
{
  if (details)
  try
  {
    ...
    {
      let url = 'https://www.stickypassword.com/applink?action=extension-firefox-pin&lng=%ui_lng%';
      url = url.replace('%ui_lng%', chrome.i18n.getUILanguage());
      chrome.tabs.create({
        url: url
      });
    }
  ...
  {
    spLog.logError(this.constructor.name + '.csOnInstalled() Error: ' + ErrorMessage);
  }
};
...
TspBackground.prototype.csOnMessage = function(message, sender, sendResponse)
{
  ...
  try
  {
    ...
    if (sender.tab)
    {
      ...
      else if (message.Action == 'RedirectTabToUrl')
        return this.csRedirectTabToUrl(message, sender, sender.tab, sendResponse);
...
TspBackground.prototype.csRedirectTabToUrl = function(message, sender, senderTab, sendResponse)
{
  chrome.tabs.update(senderTab.id, { url: message.Url });  
  if (sendResponse)
    sendResponse({});
};
