(() => {
  ...
  // ../common/content-scripts/serps/aol.ts
  (async () => {
  ...
  function processResults(searchResultToggler) {
    const items = document.querySelectorAll("ol.searchCenterMiddle li");
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      const a = item.querySelector(".compTitle a");
      if (!a) {
