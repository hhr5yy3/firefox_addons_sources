        ...
        function (request, sender, sendResponse) {
            ...
            switch (request.task) {
                case valueMap.requestKeys.getId: {
                    sendResponse({id: getExtensionId(), brversion: brVersion});
                    break;
                }
            }
        });
    ...
    chrome.runtime.onMessage.addListener(
        function (request, sender, sendResponse) {
            switch (request.task) {
                ...
                case valueMap.requestKeys.openHomepage: {
                        utilMap.openNewTab(chrome.runtime.getURL("html/homepage.html"))
                }
            }
            sendResponse({
                hasOptInInteracted: optInDecider.hasOptInInteracted()
            });
        });
} catch (err) {
}
