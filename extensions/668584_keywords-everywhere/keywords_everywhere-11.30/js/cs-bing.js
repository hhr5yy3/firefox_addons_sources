var Tool = (function(){
  ...
  var init = function(){
    initWindowMessaging();
    darkMode = isDarkMode();
    if (darkMode && document.location.pathname !== '/') {
  ...
  var isDarkMode = function() {
    if (document.body.classList.contains('b_dark')) return true;
    else return false;
  };
  ...
  var processChildList = function(children){
    for (var i = 0, len = children.length; i < len; i++) {
      var node = children[i];
      // var $node = $(node);
      if (node.id === 'sa_ul' || node.tagName === 'STRONG') {
        if (node.tagName === 'STRONG') node = $(node).closest('li')[0];
  ...
  var processSuggestion = function(node){
    ...
    // });
    var suggestionsList = {};
    $('ul[role=listbox] .sa_sg .sa_tm').map(function(i, item){
      if (item.parentNode.querySelector('.xt-suggestions-search')) return;
      var keyword = item.textContent;
      // console.log(keyword, item);
      suggestionsList[keyword] = item;
    });
