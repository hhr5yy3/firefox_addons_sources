...
class ConsentManager {
    ...
    async action(provider) {
        ...
        if (btn) {
            btn.click();
            console.log(`${extensionName()}: consent declined automatically.`);
            chrome.runtime.sendMessage({type: MessageName.WebsiteConsentRejected, url: window.location.href, provider, result: true});
        }
        else if (ConsentProviders[provider].fallBackActionFunc) {
            console.log(`${extensionName()}: trying to use custom flow to reject consent.`);
            ConsentProviders[provider].fallBackActionFunc();
        }
        else if (ConsentProviders[provider].shadowDom) {
            console.warn(`${extensionName()}: can't find action button in shadow DOM.`);
        }
        else {
            console.error(`${extensionName()}: can't decline consent.`);
            chrome.runtime.sendMessage({type: MessageName.WebsiteConsentRejected, url: window.location.href, provider, result: false});
        }
    }
        ...
        const callback = (mutationList, observer) => {
            for (const mutation of mutationList) {
                for (const el of mutation.addedNodes) {
                    ...
                    if (cmp) {
                        console.log(`${extensionName()}: found consent dialog (mutation), provider: ${ConsentProviders[cmp].cmpName}`);
                        observer.disconnect();
                        this.action(cmp);
                        return;
                    }
    ...
    processDoc() {
        ...
        if (cmp) {
            this.action(cmp);
            console.log(`${extensionName()}: found consent dialog (page load), provider: ${ConsentProviders[cmp].cmpName}`);
            return;
        }
        this.observeChanges();
    }
