(function (root, factory){
	...
	var django_top_level_domains=['com', 'edu', 'gov', 'int', 'mil', 'net', 'org', 'it', 'de'];
	var simple_email_re=/^\S+@\S+\.\S+$/;

	function htmlescape(html, options){
		var escaped=html.replace(/&/g, "&")
				.replace(/</g, "<")
				.replace(/>/g, ">")
				.replace(/"/g, "\"")
				.replace(/'/g, "'");
	...
	function urlize(text, options){
		...
		var simple_url_2_re=new RegExp('^www\\.|^(?!http)\\w[^@]+\\.('+(options.top_level_domains||django_top_level_domains).join('|')+')$', "i");
