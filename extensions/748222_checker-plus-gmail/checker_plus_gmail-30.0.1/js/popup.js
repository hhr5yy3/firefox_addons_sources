...
async function init() {
    ...
    window.addEventListener("resize", function() {
		console.log("window.resize: ", window.outerHeight, initialHeight);
        // in side panel, height would be 0 then quickly resize to correct height
		if (windowOpenTime.diffInSeconds() > -1 && !isSidePanel && window.outerHeight != initialHeight) {
	...
	onClick("#refresh", function() {
        ...
        if (window.lastRefresh && Date.now() - window.lastRefresh.getTime() <= 800) {
            ...
            showMessage(`${getMessage("refreshLinkTitle")}...`);
            refresh(true);
        } else {
            refresh();
        }
	...
	polymerPromise.then(async () => {
		...
		if (await shouldShowExtraFeature()) {
            ...
            Controller.getMinimumPayment().then(minPaymentObj => {
                $newsNotificationReducedDonationMessage.innerHTML = getMessage("reducedDonationAd_popup", [getMessage("extraFeatures"), formatCurrency(minPaymentObj.onetime_payment_reduced)]).replace(/([!?])/g, '$1<br>');
                show($newsNotificationReducedDonationMessage);
            });
            ...
            polymerPromise2.then(async () => { // polymerPromise2 required to fix paper-dialog-scrollable issue with 0 height
                ...
                if (!DetectClient.isFirefox() && !chrome.runtime.getContexts) {
                    ...
                    if (accountAddingMethod == "autoDetect" && !await hasGmailHostPermission()) {
                        if (await openDialog("hostPermissionsDialogTemplate") == "ok") {
