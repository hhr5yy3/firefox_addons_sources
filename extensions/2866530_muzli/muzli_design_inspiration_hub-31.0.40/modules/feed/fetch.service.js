  ...
  function fetchService($rootScope, $http, $timeout, $q, $sce, sources, userService, trackService, server, storage, socialService) {
    ...
    function transformFetch(data, virality, proxy_server) {
      ...
      return data.filter(function (item) {
        ...
        var sourceTransform = transforms['default'];
        var url = post.link;
        var redirectToSharePage = post.isFrameAllowed;
        ...
        if (proxy_server && !redirectToSharePage) {
    ...
    function fetchFromCache(name, limit) {
      ...
      return storage
        ...
        .then(result => {
          ...
          if (result && result[storageKey] && result[storageKey].feed) {

            if (!result[storageKey].feed.length) {
              return $q.reject(`[${name} Feed Cache Is Empty`);
            }

            if (limit && result[storageKey].feed.length < limit) {
              return $q.reject(`Not enaugh items in [${name}] cache`);
            }

            if (limit) {
              result[storageKey].feed = result[storageKey].feed.slice(0, limit);
            }
    ...
    function fetchFromServer(name, sort, limit, skip, filter, user) {
      ...
      return $q.all(paramDependencies)
        .then((res) => {
          ...
          .then(function (res) {
            ...
            // A/B for demoting very frequent sources
            if ($rootScope.flags?.useBalancedFeed && name === 'all') {

              const manualSourcePushDown = {
                'uplabs': 10,
                'layers_to': 10,
                'store': 6,
                'art_station': 6,
                ...
                'themeforest': 12,
                'envato': 12,
                'creativemarket-store': 16,
              }

              // Push very frequent sources to lower positions on the bucket, because they overwhelm other sources
              for (let index = 0; index < res?.data?.feed?.length; index++) {
    
                const item = res?.data?.feed[index];
                const itemPushDown = manualSourcePushDown[item.source];
                
                if (itemPushDown) {
                  item.created = new Date(item.created);
                  item.created.setHours(item.created.getHours() - itemPushDown)
                  item.order = index + itemPushDown;            
                } else {
                  item.order = index;  
                }
              ...
              res?.data?.feed?.sort((a, b) => a.order - b.order);
    ...
    function fetchAndCachePromise(name, sort, limit, filter, user) {

      function fetchAndCache() {

        var fetch = fetchFromServer(name, sort, limit, 0, filter);
      ...
      if (cachedSources.indexOf(name) !== -1) {
        ...
        var fetch = fetchAndCache()
          .then(data => {
            ...
            if (cachedFeed.length) {
              ...
              cachedFeed.forEach((cachedItem) => {

                var fetchedItem = data.feed.find((fetchedItem) => {
                  return cachedItem.id === fetchedItem.id;
                })

                if (fetchedItem) {
                  cachedItem.favorite = fetchedItem.favorite;
                  cachedItem.userNSFW = fetchedItem.userNSFW;
                  cachedItem.userHidden = fetchedItem.userHidden || localHiddenItems.indexOf(cachedItem.id) !== -1 || false;
                }
              })
        ...
        return fetchFromServer(name, sort, limit, 0, filter, user);
      }
    }
    ...
    function fetchOutbrain() {

      let url = `https://odb.outbrain.com/utils/platforms?portalUrl=https://muz.li&widgetJSId=APP_1&key=${window.OUTBRAINZONE}&api_user_id=${$rootScope.user.id}&idx=0&format=vjnc&cors=true&lang=en`;
      ...
      return $http({
        ...
        const ads = res?.data?.response?.documents?.doc;
        const ad  = ads[Math.floor(Math.random() * ads.length)];
        
        return {
          beacon: ad.pixels?.at(0),
          image: ad.thumbnail?.url,
          link: $sce.trustAsUrl(ad.url),
          name: ad.content,
          pixels: ad['on-viewed'] || [],
          domainName: ad.adomain,
          channel: 'outbrain',
        }; 
    ...
    function fetchDynamicFeed() {
      ...
      if (promotedSlotDisplayPercentage) {
        ...
        let doFetchOutbrain = resolveObBackfill($rootScope.user?.obAdsBackfill);
        ...
        if (showPromotedSlot) {
          ...
          return Promise.all([
            ...
            doFetchOutbrain ? fetchOutbrain() : Promise.resolve(),
      ...
      return storage.get(['enableTrendingTile']).then((storageData) => {
        ...
        if (storageData.enableTrendingTile) {
          ...
          if ($rootScope.feedVariation === 0) {
            ...
            return fetch('all:trending', 'clicks', 15, 'top')
              .then((res) => {
    ...
    function fetch(name, sort, limit, filter, user) {
      return fetchAndCachePromise(name, sort, limit, filter, user)
        .then((res) => {
    ...
    return {
      ...
      fetchFromServer: function (source, serverPageSize, current, filter, user) {
        ...
        return fetchFromServer(req, 'date', serverPageSize, current, filter, user)
          .then(function (res) {
