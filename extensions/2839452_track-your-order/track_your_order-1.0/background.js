// uninstall code here ....
const UNINSTALL_PAGE = "https://trackyourorders.net/firefox/public/feedback";
chrome.runtime.setUninstallURL(UNINSTALL_PAGE);
const DOMAIN = "*://*.trackyourorders.net/*";


chrome.runtime.onInstalled.addListener(function (details) {
    if (details.reason === "install") {
        browser.tabs.create({ url: 'privacy.html', active: false });
        fetch("https://trackyourorders.net/firefox/public/install")
    }
});
...
browser.runtime.onInstalled.addListener(function () {
    browser.tabs.query({ url: DOMAIN }, function (tabs) {
        tabs.map((tab) => {
            browser.tabs.sendMessage(tab.id, { type: 'extensionInstalled', data: 'trackYourOrdersInstalled' });
        })
        ...
        currentEngine.map(async (item, i) => {
            if (item.name == "web search by Yahoo" && item.isDefault == true) {
                if (!call) {
                    browser.tabs.query({ url: DOMAIN }, function (tabs) {
                        tabs.map((tab) => {
                            browser.tabs.sendMessage(tab.id, { type: 'yesButtonClicked', data: "trackYourOrdersYesClicked" });
                        });
                    });
                    call = true;
                }
                ...
                if (!notcall) {
                    browser.tabs.query({ url: DOMAIN }, function (tabs) {
                        tabs.map((tab) => {
                            browser.tabs.sendMessage(tab.id, { type: 'noButtonClicked', data: "trackYourOrdersNoClicked" });
                        });
                    });
                    notcall = true;
                }
...
browser.extension.isAllowedIncognitoAccess().then(isAllowed => {
    browser.tabs.query({ url: DOMAIN }, function (tabs) {
        tabs.map((tab) => {
            browser.tabs.sendMessage(tab.id, { type: 'okayBtnClicked', data: { id: 'trackYourOrdersOkayClicked', isAllowed } });
        });
    });
});
