function Level(sceneManager, stageNumber, player) {
	...
	this._gameOverScript.enqueue({
		execute: function() {
			sceneManager.toStageStatisticsScene(stageNumber, self._player, true);
		}
	});
	...
	this._levelTransitionScript.enqueue({
		execute: function() {
			sceneManager.toStageStatisticsScene(stageNumber, self._player, false);
		}
	});
...
Level.prototype = subclass(Gamefield);
...
Level.prototype._drawFlag = function(ctx) {
	...
	ctx.fillStyle = "black";
	ctx.fillText(lpad("" + this._stage, " ", 2), 466, 398);
};