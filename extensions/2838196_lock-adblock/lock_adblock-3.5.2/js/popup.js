...
async function init() {
    ...
    dom.text('#hostnameValue', punycode.toUnicode(tabHostname));
...
function updateMainBackgroundColor() {
    const slider = document.querySelector('.filteringModeSlider');
    const mainContainer = document.getElementById('main');
    if (!slider || !mainContainer) {
        console.error("Slider or main container not found.");
        return;
    }

    const level = slider.getAttribute('data-level');
    if (level === null) {
        console.error("Slider data-level attribute is null.");
        return;
    }

    const colorVarName = `--level-color-${parseInt(level) + 1}`;
    const colorValue = getComputedStyle(document.documentElement).getPropertyValue(colorVarName).trim();
    mainContainer.style.backgroundColor = colorValue;
    console.log(`Main container background color set to ${colorValue} for level ${level}`);
}

document.addEventListener('DOMContentLoaded', function() {
    var slider = document.querySelector('.filteringModeSlider');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const extensionId = browser.runtime.id; // Ensure 'browser' API is supported
    ...
    function getReviewPageURL() {
        ...
        if (userAgent.includes("Firefox")) {
            reviewPageURL = `https://addons.mozilla.org/en-US/firefox/addon/${extensionId}/reviews/`;
        } else if (userAgent.includes("Edg")) {
            reviewPageURL = `https://microsoftedge.microsoft.com/addons/detail/${extensionId}`;
        } else if (userAgent.includes("Chrome")) {
            reviewPageURL = `https://chrome.google.com/webstore/detail/${extensionId}/reviews`;
        }

        return reviewPageURL;
    }

    const reviewPageURL = getReviewPageURL();
    if (reviewButton && reviewPageURL) {
