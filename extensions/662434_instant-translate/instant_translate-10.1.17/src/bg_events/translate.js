                        ...
                        var handleTranslation = function (output_it_format, reversed) {
                            if (!output_it_format[3]) {
                                ...
                                var throwDataBack = function () {
                                    let correct_from = from === 'auto' ? output_it_format[5] : from;
                                    let correct_to = to;

                                    if (typeof ga != "undefined") ga('send', 'event', 'translation-by-lang-pairs', correct_from + ' <-> ' + correct_to);

                                    // =================================
                                    // save the character stats here too

                                    let stats = ke.ext.util.storageUtil.getDecodedVal('char_stats');
                                    stats.total += data.text.length;
                                    ++stats.translations;
                                    if (!stats.chars_per_pair[correct_from + '-' + correct_to]) {
                                        stats.chars_per_pair[correct_from + '-' + correct_to] = 0;
                                    }
                                    stats.chars_per_pair[correct_from + '-' + correct_to] += data.text.length;
                                    ke.ext.util.storageUtil.encodeAndSet('char_stats', stats);
                                ...
                                if (!data.dontSaveInCache && ke.ext.util.storageUtil.isTrueOption('history')) {
                                    ...
                                    if (reversed) {
                                        output_it_format[5] = from;
                                        output_it_format[6] = to;
                                    }

                                    ke.ext.cache.saveOrUpdate(from, to, data.text, output_it_format, data.source, throwDataBack);
                                } else {
                                    throwDataBack();
                                }
                            }
