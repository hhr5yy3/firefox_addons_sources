...
function ToolbarButton()
{
    ...
    {
        ApiCall(browsersApi.browserAction.setBadgeText, ApiCallType.DETACHED)
            .Start({ tabId: tabId, text: state.badgeText || "" });

        if (state.badgeBackgroundColor) 
        {
            ApiCall(browsersApi.browserAction.setBadgeBackgroundColor, ApiCallType.DETACHED)
        ...
        GetIconDetails(state.iconId, details =>
            {
                details.tabId = tabId;
                ApiCall(browsersApi.browserAction.setIcon, ApiCallType.DETACHED)
                    .Start(details);
            });
    }
    ...
    this.SetDefaultState = state =>
    {
        ...
        if (state.label)
        {
            ApiCall(browsersApi.browserAction.setTitle, ApiCallType.DETACHED)
