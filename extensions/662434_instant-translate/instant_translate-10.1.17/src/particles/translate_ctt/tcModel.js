        ...
        display: function (is_offline, data, manual_show) {
            ...
            ke.ext.util.storageUtil.requestProStatus((isPro) => {
                ke.ext.util.storageUtil.chainRequestBackgroundOption([
                    ...
                    if ($tt.find('.' + ke.getPrefix() + 'synonyms-paywall')[0] 
                        && $tt.find('.' + ke.getPrefix() + 'v-item')[1] 
                        && $tt.find('.' + ke.getPrefix() + 'variants-by-pos')[0]) {

                        let synonyms_paywall_height = $tt.find('.' + ke.getPrefix() + 'variants-by-pos').height() 
                            - parseInt($tt.find('.' + ke.getPrefix() + 'synonyms-paywall').css('top'));
                        let synonyms_paywall_top = ke.ext.dom.getPosition($tt.find('.' + ke.getPrefix() + 'v-item')[1])[1] 
                            - ke.ext.dom.getPosition($tt.find('.' + ke.getPrefix() + 'variants-by-pos')[0])[1]
                            - 3;
                        ...
                        $tt.find('.' + ke.getPrefix() + 'synonyms-paywall .' + ke.getPrefix() + 'upgrade-cta').on('click', () => {
                            let plan = '';
                            ...
                            chrome.runtime.sendMessage({
                                ...
                                url: chrome.extension.getURL('/pages/public/options.html#start_purchase,' + plan + 'tooltip-synonyms')
                            });
                        ...
                        $tt.find('.' + ke.getPrefix() + 'synonyms-paywall').remove();
                        ...
                        if (!isPro && Math.random() < 0.5 && !with_info_wrap) {
                            $tt
                                .find('.' + ke.getPrefix() + 'pp-' + data.old_data.id + '.' + ke.getPrefix() + 'hide')
                                ...
                                .on('click', () => {
                                    ...
                                    chrome.runtime.sendMessage({
                                        action: ke.processCall('app', 'opt', 'newTab'),
                                        url: chrome.extension.getURL('/pages/public/options.html#start_purchase,' + plan + 'tooltip-routine-promo')
                                    });
                                });
                            with_info_wrap = true;
                        }
            ...
            //ke.app.handlers.lastTranslationCallArgs.from = data.from; // update it in case it was `auto`
            ke.app.initDropdowns(data.from);
            $tt.find('.' + ke.getPrefix() + 'swap-arrow').unbind().bind('click', ke.app.handlers.reverseTranslation);
        ...
        showTranslation: function (text, from, to, win, iframe, type) {
            ...
            ke.ext.util.storageUtil.chainRequestBackgroundOption([
                ...
                that.getTranslation(selected_text, function (is_offline, data) {
                    ...
                    ke.app.handlers.lastTranslationCallArgs.from = data.from;
                    ke.app.handlers.lastTranslationCallArgs.to = data.to;
