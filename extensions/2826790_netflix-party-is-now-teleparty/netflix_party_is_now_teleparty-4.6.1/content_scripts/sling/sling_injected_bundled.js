...
const seekTimes = [];
const createHiddenElementWithData = (data) => {
    let hiddenElement = document.getElementById('custom-utils');
    if (!hiddenElement) {
        hiddenElement = document.createElement('div');
    }
    hiddenElement.style.display = 'none';
    hiddenElement.setAttribute('data-custom-data', JSON.stringify(data));
    hiddenElement.id = 'custom-utils';
    hiddenElement.seekFn = data.seekFn;
    if (!document.getElementById('custom-utils'))
        document.getElementById('root').appendChild(hiddenElement);
};
const findSeekUtils = function () {
    ...
    createHiddenElementWithData(utils);
    return utils;
};
setInterval(() => {
    findSeekUtils();
...
const isTimeValid = (time) => {
    let valid = true;
    seekTimes.forEach((st) => {
        if (st - 1000 < time && st + 1000 > time) {
            valid = false;
        }
    });
    return valid;
...
const seek = (ms) => {
    if (ms > 6000) {
        ...
        if (seekUtils && isTimeValid(ms)) {
            seekTimes.push(ms);
            seekUtils.seekFn(ms - Math.floor(seekUtils.currentPosition));
        }
    }
};
