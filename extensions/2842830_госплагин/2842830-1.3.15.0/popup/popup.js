...
document.addEventListener("DOMContentLoaded", function () {
     ...
     browser.storage.local.get('install', data => {
          if (!browser.runtime.lastError) {
               if (data.install) {
                    ...
                    if     ( 'install' == data.install.reason ) updateDownloadButton("Установить");
                    else if( 'update'  == data.install.reason ) updateDownloadButton("Обновить");
               }
          }
     });
     ...
     browser.runtime.sendMessage(new GosuslugiPluginMessage("popup", "system", "config", "", "", ""));
});
     ...
     if ( moduleType == "system" ) {
          if( methodType == "config" && !IGNORED_STATES.includes(+message.code) ) {
              ...
              if (message.error && message.error.length) {
                   ...
                   moduleNotFoundInfo.classList.add('list');
