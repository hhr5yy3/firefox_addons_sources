...
var mustKeepClassifier = (function() {
  ...
  function mustKeepMark(el) {
    if ((el.hasTagName('A') && el.node.getAttribute('name')) || el.nameMatches(MUST_KEEP_REGEXP)) {
      el.keep();
      return;
    }
  }
  ...
  return function setup(scorer) {
    scorer.registerMarkFunction(mustKeepMark);
  };
}());
var relatedArticlesClassifier = (function() {
  ...
  function isNonLinkInlineContent(el) {
    ...
    return score.wordsCount !== score.insideLinkWordsCount && (
      el.node.nodeType === Node.TEXT_NODE || el.node.getAttribute('data-pf_style_display') === 'inline'
    );
  }
  ...
  function calcScore(el) {
    ...
    if (
      ...
      // NOTE: check if element is not wrapper
      (el.node.parentNode && el.node.parentNode.childElementCount > 1)
    ) {
  ...
  function relatedBehavior(el, options) {
    ...
    if (
      ...
      commonScore.tagsCount > MANY_TAGS_THRESHOLD ||
  ...
  */
  ...
  function markAndCalcScoreElement(domScorer, element) {
    for(var i = 0, length = domScorer.markFunctions.length; i < length; i++) {
      domScorer.markFunctions[i](element);
    }
   ...
   */
  DomScorerModifier.prototype.registerMarkFunction = function(callback) {
    this.markFunctions.push(callback);
  };
...
var runDomScorerModifier = function () {
  var scorer = new DomScorerModifier();
  commonScore(scorer);

  mustKeepClassifier(scorer);
...
var cleanImgQueryString = (function () {
  ...
  return function (urlOrPath) {
    ...
    if (queryPlacement < 0) { return urlOrPath; }

    var url = urlOrPath.slice(0, queryPlacement);
    var query = urlOrPath.slice(queryPlacement + 1);
    ...
    if(!resultParts.length) { return url; }

    return url + '?' + resultParts.join('&');
  };
})();

if (typeof module !== 'undefined') { module.exports = cleanImgQueryString; }
;
...
var setup = {
  ...
  start: function() {
    try {
      ...
      document.body.innerHTML = this.defaultHtml + pfData.page.body;
      ...
      var scorer = runDomScorerModifier();
  ...
  hiddenImagesSelectors: ["img[original]", "img[data-lazy-src]", "img[data-href]", "img[data-src]", "img[data-ezsrc]", "img[data-layzr]", "img[data-pagespeed-lazy-src]", "img[data-original]", "img[data-mediaviewer-src]", "img[datasrc]", "img[data-ct-lazy]", "img[data-native-src]", "img[data-original-src]", "img[data-hi-res-src]", "img[data-low-res-src]", "img[data-raw-src]", "img[nitro-lazy-src]", "img[data-opt-src]", "img[data-orig-src]", "img[data-wpfc-original-src]", "img[data-flickity-lazyload]", "img[data-pin-media]", "img.lazyload", "img.lazyloaded", "img[itemprop=\"url\"]", "img[loading=\"lazy\"]", "img[data-load=\"false\"]", "img.js-progressiveMedia-image", "a[data-replace-image]", "amp-img"],
...
var readability = {
  ...
  titleText: null,
  titleTags: ['h1', 'h2', 'h3'],
  wpContentParentTags: ['.single-post article.hentry', '.hentry', '.single-post', 'article'],
  wpContentTags: ['.post-content .fusion-fullwidth', '.article', '.entry-content', '.entry_content',
    ...
    '.post_body', '.post_content', '.post_entry', '.post_page-content'],
  ...
  wrapTextNodes: function(node, scorer) {
    ...
    if(node.nodeType === Node.TEXT_NODE) {
      if ($.trim(node.textContent)) {
        ...
        DomScorerModifier.toElem(node.parentNode).replaceChild(s, node);
        s.appendChild(node);
      }
  ...
  init: function(scorer) {
    ...
    var fallbackHTML = document.body.innerHTML;
  ...
  getArticleTitle: function (scorer) {
    ...
    if(!$titleEl && $.trim($('title', document).text()) !== '') {
      var documentTitleText = $.trim($('title', document).text()).toLowerCase().replace(/\W+/g,'');
      var previousMatchLength = 0;
      $.each(htags, function(i, tag) {
        $doc.find(tag).each(function(j, el) {
          var match = documentTitleText.match($(el).text().toLowerCase().replace(/\W+/g,''));
  ...
  grabArticle: function (scorer, page) {
    ...
    if(candidates.length === 0) {
      switch(pfData.config.platform) {
        ...
        case 'wordpress':
          ...
          var selectors = ['.fl-module-fl-post-content', '.elementor-widget-theme-post-content', '[data-elementor-type="wp-post"]'];
    ...
    if(candidates.length > 0 && !candidatesHasBodyElem) {
      ...
      if($.trim(candidates.text()) !== '' || candidates.find('img,svg').length > 0 || $.inArray('IMG', $.map(candidates, function(el) {return el.nodeName; })) !== -1 ) {
        ...
        if(pfData.config.platform === 'wordpress') {
          $.each(readability.wpPostImageSelectors, function(i, selector) {
            ...
            if($wpPostImages.length === 1) {
              ...
              if (!primaryImage.findSameImage(primaryImage.getImageOrigSrc($wpPostImage[0], candidates))) {
        ...
        for (var i = 0, length = candidates.length; i < length; i++) {
          ...
          if ($(contentDiv.node).has(candidate).length === 0) { contentDiv.appendChild(candidate); }
        }
    ...
    while(node = queue.shift()) {
      ...
      if ($.inArray(node.tagName, primaryContentNodes) > -1 && !commonUtils.hasClass(node, 'pf-br-replacement')) {
        ...
        if(!commonUtils.hasClass(parentNode, listClass)) {
      ...
      /* Turn all divs that don't have children block level elements into p's */
        ...
        if (node.innerHTML.search(readability.regexps.divToPElements) === -1) {
          try {
            ...
            newNode.node.id = node.id;
            newNode.node.className = node.className;
    ...
    if(candidates.length > 0) {
      ...
      $.each(candidates, function(i, candidate) {
        ...
        */
        ...
        if ($candidate.find('.pf-candidate').length > 0) {
          if (
            ...
            candidatesAboveTheshold[0] = candidate;
          }
        }
        ...
        try {
          ...
          if (topCandidate.tagName === 'BODY') {
            var normalizeCandidate = function(c) {
              if(c.tagName === 'TBODY') { return c.parentElement; }
              else { return c; }
            };
    ...
    if (topCandidate.nodeName === 'TBODY') { topCandidate = topCandidate.parentNode; }
...
var primaryImage = {
  ...
  find: function() {
    ...
    try {
      var image = userSettingsImage || this.wpPostImage() || this.metaImage() || this.largeImage();

      if (image) {
        var src = this.getImageRestoreSrc(image);
        if (!src) { image = image.querySelector('img'); }
        src = this.getImageRestoreSrc(image);
        if (src) {
  ...
  largeImage: function() {
    var largeImages = document.querySelectorAll('.pf-large-image');

    for (var i = 0; i < largeImages.length; i++) {
      var image = largeImages[i];
  ...
  metaImage: function() {
    ...
    if (metaSrc) {
      var foundImg = this.findSameImage(metaSrc, document.body)
  ...
  prependTo: function(content) {
    ...
    var $img = $(this.primaryImageHTML);
    var origSrc = this.getImageOrigSrc($img[0]) || this.getImageRestoreSrc($img[0]);
    // if preview content doesn't have the same image then prepend it
    if (!this.findSameImage(origSrc, content)) {
      $img.addClass('blockImage').addClass('pf-primary-img');
      $(content).prepend($img);
  ...
  findSameImage: function(origSrc, context) {
    if(!origSrc) { return; }
    ...
    $('img, picture source', context).each(function() {
      ...
      if (srcSet.indexOf(origSrc) !== -1) {
        ...
        var score = primaryImage.similarityScore(src, origSrc)
        if (score < minScore) {
          image = this;
          minScore = score
        }
...
var imageProcessor = {
  ...
  lazyImgDataAttributes: ["original","data-lazy-src","data-href","data-src","data-ezsrc","data-layzr","data-pagespeed-lazy-src","data-original","data-mediaviewer-src","datasrc","data-ct-lazy","data-native-src","data-original-src","data-hi-res-src","data-low-res-src","data-raw-src","nitro-lazy-src","data-opt-src","data-orig-src","data-wpfc-original-src","data-flickity-lazyload","data-pin-media"],
  ...
  fixLazyLoadImages: function() {
    ...
    $('img[' + srcAttr + '=""]:not([srcset],img[original],img[data-lazy-src],img[data-href],img[data-src],img[data-ezsrc],img[data-layzr],img[data-pagespeed-lazy-src],img[data-original],img[data-mediaviewer-src],img[datasrc],img[data-ct-lazy],img[data-native-src],img[data-original-src],img[data-hi-res-src],img[data-low-res-src],img[data-raw-src],img[nitro-lazy-src],img[data-opt-src],img[data-orig-src],img[data-wpfc-original-src],img[data-flickity-lazyload],img[data-pin-media],img.lazyload,img.lazyloaded,img[itemprop="url"],img[loading="lazy"],img[data-load="false"],img.js-progressiveMedia-image,a[data-replace-image],amp-img)').remove();
    ...
    $doc.find('img[original],img[data-lazy-src],img[data-href],img[data-src],img[data-ezsrc],img[data-layzr],img[data-pagespeed-lazy-src],img[data-original],img[data-mediaviewer-src],img[datasrc],img[data-ct-lazy],img[data-native-src],img[data-original-src],img[data-hi-res-src],img[data-low-res-src],img[data-raw-src],img[nitro-lazy-src],img[data-opt-src],img[data-orig-src],img[data-wpfc-original-src],img[data-flickity-lazyload],img[data-pin-media],img.lazyload,img.lazyloaded,img[itemprop="url"],img[loading="lazy"],img[data-load="false"],img.js-progressiveMedia-image,a[data-replace-image],amp-img').each(function() {
    ...
    setImgClassAndDimensions: function($img) {
      ...
      **/
      ...
      if($img.hasClass('caption-img')) {
        ...
        if ($img[0].offsetWidth && $parents.find('img').length === 1) {
          $parents.width($img[0].offsetWidth);
        }
        ...
        if($img.hasClass('thumbimage')) {
          $img.parents('.thumbinner').width($img[0].offsetWidth).addClass('pf-caption flex-width').addClass(imgClass).addClass(imageProcessor.resizeImageCssClass);
        }
      }
...
var htmlPreProcessor = {
  run: function(scorer) {
    ...
    $('iframe.twitter-tweet-rendered, .twitter-tweet-rendered iframe').each(function() {
      var $el = $(this);
  ...
  replaceEmbeddedYoutube: function(scorer) {
    ...
    $("iframe[src^='" + embeddedYoutubePathPrefix +"']").each(function() {
      var video = { id: null, type: null, href: null, thumb: null };

      var videoIdMatch = this.src.match(new RegExp(embeddedYoutubePathPrefix + '([^?]+)'));
      if (videoIdMatch) {
        ...
        var playlistIdMatch = this.src.match(new RegExp(embeddedYoutubePathPrefix + '.*list=([^\;&]+)'));
        if (playlistIdMatch) {
...
var nsfwChecker = {
  ...
  checkText: function(text) {
    ...
    if ($.inArray(lang, supportedLanguages) != -1) {
      $.getJSON("https://v.printfriendly.com/assets/client/nsfw/nsfw-954d8813c8f5d4afc7d00e95053ce9d7243fd9b30afb00c6a6527c5bfbb06bd4.json", function(data) {
