...
(function _userAttributeHandling(global) {
    ...
    ns.get = function _getUserAttribute(
        ...
        // We do not have M.u as there is only one logged in user in extension.
        if ((attribute[0] === '^' || attribute[0] === '+') && userhandle === u_handle) {
        ...
        var settleFunction = function _settleFunction(res) {
            ...
            if (typeof res !== 'number' || res === ENOENT) {
                ...
                if (userhandle !== u_handle && M.u && (!M.u[userhandle] || M.u[userhandle].c !== 1)) {
                    exp = unixtime();
                }
    ...
    mBroadcaster.once('boot_done', function() {
        uaPacketParserHandler['firstname'] = function(userHandle) {
            if (u_attr) {
                u_attr.firstname = u_attr.lastname = "";
        ...
        // ua handers that potentially using in future
        ...
        uaPacketParserHandler['^!webtheme'] = function(userHandle) {
            ...
            mega.attr.get(userHandle, 'webtheme', -2, 1, function(res, ctx) {
                ...
                u_attr[ctx.ua] = res;
                chrome.runtime.sendMessage({type: 'u_attr:webtheme', payload: u_attr[ctx.ua]});
            });
        };
    ...
    ns.getFolderFactory = function(attribute, pub, nonHistoric, storeKey, name, decode, encode) {
        ...
        ns.get = promisify(function(resolve, reject, create) {
            factory.get().then(function(h) {
                return dbfetch.node([h]);
            }).always(function(res) {
                var node = res[0];
