...
async function openPanelIn(position) {
    if (position === 'tab') {
        if (await taskSyncRemote.activateTab())
            return;
        await browser.tabs.create({ url: panelURL });
    }
    else if (position === 'window') {
        if (await taskSyncRemote.activateWindow())
            return;
...
const linkMenuId = browser.menus.create({
    ...
    onclick: function ({ linkUrl, selectionText }, { url: tabURL }) {
        if (selectionText !== undefined || !isValidProtocolURL(linkUrl)) {
            openPopupWindow(browser.runtime.getURL('/dialog/create.html?selectedLinks=1'));
            return;
        }
...
localSettings.listen('inhibitSleep', () => updateWakeLock(), 'skip' /* called in updateBadge */);
Task.updateBadge = async function (suggestedState) {
    ...
    await browser.browserAction.setBadgeBackgroundColor({ color: DownloadState.colors[state || 'downloading'] });
};
...
async function updateIconColor() {
    ...
    if (iconColor === 'custom')
        ({ iconColorCode, iconColorAlpha } = S);
    ...
    const node = (await originalIconSVG).cloneNode(true);
    ...
    browser.browserAction.setIcon({ imageData: { [SIZE_PX]: context.getImageData(0, 0, size, size) } });
}
// Not needing `theme` permission [Firefox 62]
try {
...
let autoImportExtSet = new Set();
function autoImportExtHandler(download) {
    if (!isValidProtocolURL(download.url))
        return;
    if (!download.byExtensionId ||
        !autoImportExtSet.has(download.byExtensionId))
        return;
