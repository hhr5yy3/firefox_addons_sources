...
function api_reqfailed(channel, error) {
    ...
    if (e === ESID) {
        u_logout(true);
        if (popupOpen) {
            chrome.runtime.sendMessage({type: 'logoutFromSW'});
        }
    }
    ...
    // if suspended account
        ...
        api.req({ a: 'whyamiblocked' })
            .then(({result}) => chrome.runtime.sendMessage({type: 'whyamiblocked', result}))
            .catch((ex) => {
                if (ex === ESID) {
                    u_logout(true);
                }

                chrome.runtime.sendMessage({type: 'reset-loading-state'});
                tell(ex);
            });
    }
    else if (e === EPAYWALL) {
...
function setsn(sn) {
    ...
    if (pmdb) {
        ...
        pmdb.add('_sn', { i : 1, d : sn });
    }

    chrome.storage.local.set({sn});
}
...
function sc_residue(sc) {
    ...
    if (sc.sn) {
        ...
        if (sc.w) {
            // set params to get only the pwmh node related action packets
            waitsc.setURL(`${sc.w}?${this.sid}&sn=${currsn}&e=${pwmh}&ir=1&ap=1`);
        }
...
async function api_setattr(n) {
    ...
    return api.screq({a: 'a', n: n.h, at: ab_to_base64(crypto_makeattr(n)), vw: 1});
}
