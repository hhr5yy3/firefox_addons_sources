(function() {
	...
	const TYPES = {
		...
		"custom": {
			...
			getValue: function(DOMNode) {
				if(DOMNode.hasOwnProperty("dataCustomValue")) {
					return DOMNode.dataCustomValue;
				} else if(Object.getPrototypeOf(DOMNode).hasOwnProperty("value")) {
					try {
						return JSON.parse(DOMNode.value);
					} catch(e) {
						// Not required
					}
				}
			...
			setValue: function(DOMNode, value) {
				DOMNode.dataCustomValue = value;
				
				if(Object.getPrototypeOf(DOMNode).hasOwnProperty("value")) {
					DOMNode.value = JSON.stringify(value);
				}
			}
		},
		...
		"radio": {
			getTargetNodes: function(DOMNode) {
				return document.getElementsByName(DOMNode.name);
			},
	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		let hooks   = (typeof(OPTION_HOOKS) === "object" && OPTION_HOOKS) ? OPTION_HOOKS : {};
		let options = {};
		for(let DOMOption of document.querySelectorAll("body > *[data-option]")) {
			...
			if(typeof(TYPES[type].getTargetNodes) === "function") {
				for(let DOMNode of TYPES[type].getTargetNodes(DOMOptionTarget)) {
					DOMNode.addEventListener("change", option.triggerUserChange, false);
				}
			} else {
