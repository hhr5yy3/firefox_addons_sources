...
var ttss = [
    {
      word: "one",
      clue: "The first number",
    },
    {
      word: "two",
      clue: "The second number",
    },
    {
      word: "cat",
      clue: "An animal that sounds Miaow",
    },
    {
      word: "calculator",
      clue: "A tool to calculate.",
    },
    {
      word: "jakarta",
      clue: "Capital of Indonesia",
    },
    {
      word: "sweet",
      clue: "Taste of sugar",
    ...
    {
      word: "computer",
      clue: "A tool to compute",
    },
    {
      word: "fish",
      clue: "An animal that lives under water",
    },
    {
      word: "star",
  ...
  var CrosswordUtils = {
    PATH_TO_PNGS_OF_NUMBERS: "images/crosswordPuzzle/",
  ...
  window.addEventListener('DOMContentLoaded', function(e) {
    storageReplacer.init().then(function () {
      if (storageReplacer.getLocalStorageItem("ttsasyik") == undefined) {
        storageReplacer.getLocalStorageItem("ttsasyik")
        saveData();
      } else {
        appdata = (storageReplacer.getLocalStorageItem("ttsasyik"));    
      }
    })})

  
  function saveData() {
    storageReplacer.setLocalStorageItem("ttsasyik", appdata);
  }
  ...
  function startttsgame() {
    ...
    $('#crossword').html(CrosswordUtils.toHtml(
      grid,
      show_answers
    ));
  ...
  function activatetts() {
    $("td").click(function () {
      if (
        ...
        $(this).find(".canswer").html() != undefined
      ) {
  ...
  function initvkeyboard() {
    ...
    for (var i = 0; i < chars.length; i++) {
      $("#kbtnlist").append(
        `<div class='kbtn'>${chars[i]}</div>`
      );
      $('.kbtn').on('click', typechar)
    }
  ...
  $('.alfaClose').on('click',function(){
    $('#vkeyboard').hide();
  });
  $('#menuIco').on('click',toggledrawer);
  $('#toggleBtn').on('click',toggleAnswer);
  $('#randomizeBtn').on('click',function(){
    location.reload()
  });
  $('#threeWordCat').on('click',function(){
    setqcount(3);
  })
  $('#fiveWordCat').on('click',function(){
    setqcount(5);
  })
