function StageStatisticsScene(sceneManager, stage, player, gameOver) {
	...
	this._script.enqueue({
		execute: function() {
			...
			if (gameOver) {
				var playerScore = self._playerScore;
				playerScore = playerScore ? playerScore : 0;
				var maxScore = localStorage.getItem('maxPlayerScore');
				maxScore = maxScore ? maxScore : 20000;
				localStorage.setItem('lastPlayerScore', playerScore);
				if (playerScore > maxScore) {
					localStorage.setItem('maxPlayerScore', playerScore);
				}
...
StageStatisticsScene.prototype.draw = function(ctx) {
	...
	ctx.fillText("STAGE " + lpad("" + this._stage, " ", 2), 194, 78);
	...
	ctx.fillStyle = "#feac4e";
	ctx.fillText(lpad("" + this._player.getScore(), " ", 7), 66, 142);
	this._playerScore = this._player.getScore();
	...
	if (this._drawTotal) {
		ctx.fillText(lpad("" + this._player.getTanksCount(), " ", 2), 194, 366);
	}
};