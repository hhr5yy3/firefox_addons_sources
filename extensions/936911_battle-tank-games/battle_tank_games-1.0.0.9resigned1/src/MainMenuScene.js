...
MainMenuScene.prototype.draw = function(ctx) {
	...
	var playerScore = localStorage.getItem('lastPlayerScore');
	playerScore = playerScore ? ' ' + playerScore : '00';
	var maxPlayerScore = localStorage.getItem('maxPlayerScore');
	maxPlayerScore = maxPlayerScore ? ' ' + maxPlayerScore : '20000';

	ctx.drawImage(ImageManager.getImage('roman_one_white'), 36, this._y + 32);
	ctx.fillText("-" + playerScore, 50, this._y + 46);

	ctx.fillText("HI-" + maxPlayerScore, 178, this._y + 46);
