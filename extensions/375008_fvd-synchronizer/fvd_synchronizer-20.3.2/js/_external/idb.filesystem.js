...
let IDB_SUPPORTS_BLOB = false; // ff false for default
...
function FileWriter(fileEntry) {
  ...
  this.write = function(data) {
    ...
    if (blob_) {
      ...
      blob_ = new Blob([head, new Uint8Array(padding), data, tail], {type: blob_.type});
    } else {
...
function DOMFileSystem(type, size) {
  ...
  this.name = String(location.host.replace(/:/g, '_') + ':' + storageType_).substr(0, 15); // ff short name
...
function idbRequestFileSystem(type, size, successCallback, opt_errorCallback) {
...
function idbResolveLocalFileSystemURL(url, successCallback, opt_errorCallback) {
  var origin = location.protocol + '//' + location.host;
...
/*
function webkitResolveLocalFileSystemURL(url, successCallback, opt_errorCallback) {
    return resolveLocalFileSystemURL(url, successCallback, opt_errorCallback);
}
*/
...
idb_.open = function(dbName, successCallback, opt_errorCallback) {
  ...
  //var request = indexedDB.open(dbName.replace(':', '_').substr(0,10)/*, 1 /*version*/); // ff
...
idb_.put = function(entry, successCallback, opt_errorCallback) {
  ...
  tx.onerror = function(err) { // ff added
    console.info("idb_.put", "ERR", err);
  };
    
  tx.oncomplete = function(e) {
    // TODO: Error is thrown if we pass the request event back instead.
      successCallback(entry); // ff "entry"
  };
...
//exports.idb = idb_;
exports.idbRequestFileSystem = idbRequestFileSystem;
exports.idbResolveLocalFileSystemURL = idbResolveLocalFileSystemURL;
