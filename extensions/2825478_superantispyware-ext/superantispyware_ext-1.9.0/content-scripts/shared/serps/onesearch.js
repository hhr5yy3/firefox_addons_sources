(() => {
  ...
  // ../common/content-scripts/serps/onesearch.ts
  (async () => {
  ...
  function processResults(searchResultToggler) {
    const divs = document.querySelectorAll("#main #web ol li div.dd.algo");
    for (let i = 0; i < divs.length; i++) {
      const div = divs[i];
      const q = "div.compTitle div span:first-child";
      const span = div.querySelector(q);
      const domain = span.innerText.split("/").slice(0, 1);
      const url = `https://${domain}/`;
      getCategorization_default(url).then((categorization) => {
        if (!categorization) {
          log("Failed getting categorization.");
          return;
        }
        searchResultToggler.toggle(div, categorization.categories);
        const safetyBadge = new SafetyBadge_default(categorization, {
          width: "16px",
          height: "16px",
          "margin-right": "6px"
        });
        const a = div.querySelector("h3.title a");
        if (!a) {
          log("Missing A element.");
          return;
        }
        safetyBadge.draw(a);
      });
