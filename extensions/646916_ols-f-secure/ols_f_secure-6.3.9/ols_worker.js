...
class OlsWorker {
        ...
        const initComms = () => {
            if (NativeHost.hostName) {
                ...
                if (retryCount <= 10) {
                    ...
                    loge("Failed to initialize native messaging host after 10 retries");
                }
            }
        };
    ...
    async initCommsAddListeners() {
        ...
        if (result.settings) {
            ...
            logd("No settings in init response");
    ...
    addListeners() {
        ...
        if (chrome.runtime.getManifest().manifest_version == 3) {
            ...
            if (Browser.isSafari) {
                chrome.webNavigation.onBeforeNavigate.addListener((details) => { this.onBeforeNavigateSafari(details) }, { "url": [{"schemes": ["https", "http"]}] });
                return;
            }
    ...
    #clearConnectionStatusRequestQueue() {
        ...
        logd(`Sending connection status ${this.#nativeMessagingStatus} to ${this.#connectionStatusRequests.length} pending requests`);
    ...
    async adBlockerStats(details) {
        ...
        if (details.frameType == "sub_frame" && details.type != "main_frame") {
            ...
            const info = await this.orspInfoMessage(domain);
            if (info.categories) {
    ...
    getWebsiteType(categories) {
        ...
        for (let cat in categories) {
            if (Object.keys(categories[cat])[0] == "banking" || Object.keys(categories[cat])[0] == "paymentservice" || 
                Object.keys(categories[cat])[0] == "fso_banking" || Object.keys(categories[cat])[0] == "fso_paymentservice") {
