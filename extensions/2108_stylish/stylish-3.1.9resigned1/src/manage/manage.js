...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector("#serpEnabled").addEventListener("click", function (e) {
        // last-value is to fix multiple triggering of change event
        // for some reasone change is triggered 4 times
        if (this.getAttribute("last-value-serp") === this.checked.toString()) {
            return;
        }
        if (!this.checked && this.getAttribute("data-warned-serp") != "1") {
            analyticsEvent("Manage_installed_styles", "Opt out SERP clicked");
            this.setAttribute("data-warned-serp", "1");
            if(!optOutClicked) {
                alert(chrome.i18n.getMessage("serpEnabledAlert"));
            }
            prefs.get('popup.checkNewStyles').popupCheckSERPDisable();
        } else if (this.checked) {
            analyticsEvent("Manage_installed_styles", "Opt in SERP clicked");
            this.removeAttribute("data-warned-serp");
            prefs.get('popup.checkNewStyles').popupCheckSERPEnable();
            optOutClicked = false;
        }
        this.setAttribute("last-value-serp", this.checked);
    });
    document.querySelector("#gaEnabled").addEventListener("click", function (e) {
        // last-value is to fix multiple triggering of change event
        // for some reasone change is triggered 4 times
        if (this.getAttribute("last-value-ga") === this.checked.toString()) {
            return;
        }
        if (!this.checked) {
            prefs.get('popup.checkNewStyles').popupCheckGADisable();
        } else if (this.checked) {
            prefs.get('popup.checkNewStyles').popupCheckGAEnable();
        }
        this.setAttribute("last-value-ga", this.checked);
    });
