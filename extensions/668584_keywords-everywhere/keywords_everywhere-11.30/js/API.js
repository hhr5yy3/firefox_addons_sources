var API = (function(){
  ...
  var getPlan = function(cbProcessResponse){
    ...
    getJSON(url, {
      ...
      .then(function(json){
        if (json && json.error) {
          cbProcessResponse({error: true, data: json.error});
          return;
        }
        if (json && json.plan) {
  ...
  var getKeywordData = function( params, cbProcessResponse ){
    ...
    if (cachedResponse) {
      cbProcessResponse(cachedResponse);
      return;
    }
    data.t = Date.now();
    getJSON( url, data )
  ...
  var getURLMetrics = function(params, cbProcessResponse){
    ...
    var num = params.num ? params.num : numParam;
    var country = params.country;
  ...
  async function openAIFetchSEOReports() {
    const response = await fetch(API_URL + `templates/getSEOReports.php`);
    const data = await response.json();
    return data;
  }


  async function openAIFetchSEOReport(id) {
    const response = await fetch(API_URL + `templates/getSEOReport.php?id=${id}`);
    const data = await response.json();
    return data;
  }
  ...
  return {
    ...
    openAIFetchSEOReports: openAIFetchSEOReports,
    openAIFetchSEOReport: openAIFetchSEOReport
  };
