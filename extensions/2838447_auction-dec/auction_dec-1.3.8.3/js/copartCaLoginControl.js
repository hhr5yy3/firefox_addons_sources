(async () => {
   ...
   const copartURLs = {
      loginUrl: "https://www.copart.ca/processLogin",
      loginError: "https://www.copart.ca/public/loginAuthFailure",
      loginInternalError: "https://www.copart.ca/public/loginInternalServerError",
      loginSuccess: ["https://www.copart.ca/public/loginAccountSuspended", "https://www.copart.ca/loginSuccess.html"],
      login: "https://www.copart.ca/login"
   };
   ...
   if (appInfo && !appInfo?.ca_copart) return;

   const isUserLoggedIn = await checkIfUserLoggedIn();

   if (appInfo && appInfo?.ca_copart) {
   ...
   function isCopartSite() {
      return currentHost.includes("copart.ca");
   }
   ...
   async function attemptLogin() {
      ...
      const email = credentials?.ca_copart?.email;
      const password = credentials?.ca_copart?.password;
      ...
      for (let tries = 2; tries > 0; tries--) {
         ...
         const loginResponse = await fetch(copartURLs.loginUrl, {
            ...
            mode: "cors",
            credentials: "include",
            referrer: "https://www.copart.ca/login/",
         ...
         if (copartURLs.loginSuccess.includes(loginResponse.url) && loginResponse.status === 200) {
            if (!isLoggedInCopart) {
               ...
               const loggedInPrefix = "logged_in_ca_copart";
