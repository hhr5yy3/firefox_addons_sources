(() => {
  ...
  // ../common/content-scripts/serps/duckduckgo.ts
  (async () => {
  ...
  function processResults(searchResultToggler) {
    ...
    const Q = `ol.react-results--main li[data-layout=organic]:not([${ATTR_NAME}])`;
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      getCategorization_default(a.href).then((categorization) => {
        ...
        a.style.width = px(h2.offsetWidth - SIZE - MARGIN);
        safetyBadge.draw(h2);
      });
    }
  }
