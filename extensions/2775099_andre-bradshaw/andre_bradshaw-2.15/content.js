...
function monitorURLChangesVodClipper() {
    ...
    if(url != better_twitch_page_change_monitor_object?.page_context?.href) {
        createInitProgramBtn()
    }
    if(channel_login && (!recent_streamscharts[channel_login] || !recent_streamscharts[channel_login]?.length)) {
        getRecentStreamChartsData()
    }
}
