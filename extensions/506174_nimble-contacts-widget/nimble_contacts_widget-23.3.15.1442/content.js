...
;// CONCATENATED MODULE: ./source/content/frame-position.js
function frame_position_defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }




var getFramePositionKey = function getFramePositionKey() {
  return "frame_position__".concat(extractBaseDomain(location.href));
};

var getFramePosition = function getFramePosition() {
  var key = getFramePositionKey();
  return browser_polyfill_default().storage.local.get(key).then(function (result) {
    return result[key];
  });
};
var saveFramePosition = function saveFramePosition(newPosition) {
  return browser_polyfill_default().storage.local.set(frame_position_defineProperty({}, getFramePositionKey(), newPosition));
};
var removeFramePosition = function removeFramePosition() {
  return browser_polyfill_default().storage.local.remove(getFramePositionKey());
};
;// CONCATENATED MODULE: ./source/signin/index.js
...
var FormTextInput = function FormTextInput(_ref) {
  var type = _ref.type,
      ...
      label = _ref.label;
  var isNotEmptyClass = value && value.length ? '__not-empty' : '';
  return /*#__PURE__*/react.createElement("div", {
    className: "nimble-form-field"
  }, /*#__PURE__*/react.createElement("input", {
    type: type,
    className: "nimble-form-input nimble-form-input-text ".concat(isNotEmptyClass),
    value: value,
    ...
    className: "nimble-form-label nimble-form-label-overlay"
  }, label));
};
...
var Copyright = function Copyright() {
  var Year = /*#__PURE__*/react.createElement("span", null, "\xA9", year, " Nimble");
  var TosLink = /*#__PURE__*/react.createElement("a", {
    href: "http://www.nimble.com/company/terms-of-service/",
    target: "_blank",
    rel: "noreferrer noopener"
  }, "Terms of Service");
  var PpLink = /*#__PURE__*/react.createElement("a", {
    href: "http://www.nimble.com/company/privacy/",
    target: "_blank",
    rel: "noreferrer noopener"
  }, "Privacy Policy");
  return /*#__PURE__*/react.createElement("div", null, Year, " \xB7 ", TosLink, " \xB7 ", PpLink);
};

var SignInPage = function SignInPage(_ref2) {
  ...
  var formEl = (0,react.useRef)();

  var onSuccess = function onSuccess(token) {
    setLoading(false);
    setSuccess(true);
    setTimeout(function () {
      typeof onLogin === 'function' && onLogin(token);
    }, 300);
  };

  var shakeForm = function shakeForm() {
    formEl.current.classList.add('shake');
    setTimeout(function () {
      if (!formEl || !formEl.current) return;
      formEl.current.classList.remove('shake');
    }, 1000);
  };
  ...
  var submitForm = function submitForm() {
    ...
    setLoading(true);
    sendMessageToExtension('NIMBLE_EXT__LOGIN', {
      endpoint: endpoint,
      data: {
        email: email,
        password: password,
  ...
  return /*#__PURE__*/react.createElement("div", {
    className: "nimble-signin"
  }, /*#__PURE__*/react.createElement("div", {
    className: "nimble-signin-wrapper"
  }, /*#__PURE__*/react.createElement("div", {
    className: "nimble-signin-logo"
  }), /*#__PURE__*/react.createElement("div", {
    className: "nimble-signin-form ".concat(isLoading ? '__loading' : '', " ").concat(isSuccess ? '__success' : ''),
    ref: formEl
  }, /*#__PURE__*/react.createElement("div", {
    className: "nimble-signin-form-fields",
    role: "form"
    ...
    className: "nimble-signin-btn nimble-signin-btn-orange nimble-signin-form-submit",
    tabIndex: "-1",
    role: "button",
    onClick: submitForm
  }, "Sign in"), /*#__PURE__*/react.createElement("a", {
    href: "https://app.nimble.com/#remindPassword",
    target: "_blank",
    rel: "noreferrer noopener"
  }, "Trouble Signing In?")))), /*#__PURE__*/react.createElement("div", {
    className: "nimble-signin-copyright"
  }, /*#__PURE__*/react.createElement(Copyright, null)));
};

/* harmony default export */ const signin = (SignInPage);
;// CONCATENATED MODULE: ./source/content/index.js
function content_typeof(obj) { "@babel/helpers - typeof"; if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { content_typeof = function _typeof(obj) { return typeof obj; }; } else { content_typeof = function _typeof(obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; } return content_typeof(obj); }
...
(function () {
  if (window.location.href.includes('nimble.com/app/widget') && window.location.search.includes('?openProspector')) {
    log('RETURN');
    return;
  }

  var interval, lastUrl;

  var runApp = function runApp() {
    var wasInjected = window.IS_NIMBLE_EXTENSION_INJECTED || false;
    /*
    const wasInjectedMeta = document.querySelector('meta[name="nimble_prospector_injected"]');
    const wasInjected = wasInjectedMeta && wasInjectedMeta.content === 'true';
    */

    var sendMessageToFrame = function sendMessageToFrame(eventName, eventData) {
    ...
    var sendMessageToWidget = function sendMessageToWidget(eventName, eventData, eventSource) {
      sendMessageToFrame('NIMBLE_EXT__PROXY_TO_FRAME', {
        eventName: eventName,
        eventData: eventData,
        eventSource: eventSource
      });
    ...
    var useParser = function useParser() {
      var parser = findParser(window.location.href);

      if ((typeof pageParsers === "undefined" ? "undefined" : content_typeof(pageParsers)) === 'object' && pageParsers !== null && parser in pageParsers) {
        var parsedData = pageParsers[parser].parsePage();
    ...
    var hoverHandler = function hoverHandler(_ref) {
      ...
      if (/linkedin\.com/ig.test(location.host)) {
        el.querySelectorAll('.visually-hidden').forEach(function (el) {
          return el.remove();
        });
      }
      ...
      if (el.href.includes('angel.co/search/redirect')) {
        var url = el.href.split('&').find(function (op) {
          return op.includes('url=http');
        });
        selection = el.innerText + ' ' + (url ? decodeURIComponent(url.replace('url=', '')) : '');
      }

      if (/linkedin\.com\/jobs\/search/.test(el.href)) {
        var $companyName = el.querySelector('.job-card-container__company-name');

        if ($companyName && $companyName.innerText.trim().length) {
          selection = $companyName.innerText.trim();
        }
      } else if (/linkedin\.com\/in\/(\w+)/.test(el.href)) {
        var $name = el.querySelector('.name-and-icon .name');

        if ($name && $name.innerText.trim().length) {
          selection = $name.innerText.trim() + ' ' + el.href;
        } else {
          $name = el.querySelector('.feed-shared-feed-discovery-entity__name');

          if ($name && $name.innerText.trim().length) {
            selection = $name.innerText.trim() + ' ' + el.href;
          } else {
            $name = el.querySelector('[class*="name"]');
    ...
    var selectionHandler = function selectionHandler() {
      ...
      if (!selection.length) return; // added artificial timeout to have ability handle double mouse click: to cancel previous first-single click handler and to handle the last - the second one

      clearTimeout(selectionHandlingTimeout);
      selectionHandlingTimeout = setTimeout(function () {
        sendMessageToWidget('nimbleExtension.dataUpdate', {
          url: location.href,
          selection: selection,
    ...
    }; // init extension element


    var NimbleDiv = document.createElement('div');
    NimbleDiv.classList.add('__Nimble-Prospector');
    document.body.appendChild(NimbleDiv);
    var NimbleCloseBtn = document.createElement('div');
    NimbleCloseBtn.classList.add('__Nimble-Prospector__close-btn');
    NimbleCloseBtn.style.backgroundImage = "url('".concat(browser_polyfill_default().runtime.getURL('icons/close-24px.svg'), "')");
    NimbleCloseBtn.addEventListener('click', function () {
      sendMessageToWidget('nimbleExtension.hideToolbar');
      sendMessageToExtension('NIMBLE_EXT__HIDE_FRAME');
    }); // init widget frame

    var NimbleIframe = document.createElement('iframe');

    var appendFrame = function appendFrame() {
      log('append frame');
      NimbleDiv.appendChild(NimbleCloseBtn);
      NimbleDiv.appendChild(NimbleIframe);
    };

    var renderSignIn = function renderSignIn(endpoint) {
      log('render signin');
      (0,react_dom.render)( /*#__PURE__*/react.createElement(signin, {
        endpoint: endpoint,
    ...
    if (isSafari()) {
      log('is safari');
      options_storage.getAll().then(function (_ref2) {
        var endpoint = _ref2.endpoint;
        sendMessageToExtension('NIMBLE_EXT__REQUEST_TOKEN', {
          endpoint: endpoint
        }).then(function () {
          log('token ok');
          appendFrame();
    ...
    var onTokenRemove = function onTokenRemove() {
      if (!isSafari()) return;
      log('onTokenRemove');
      options_storage.getAll().then(function (storage) {
        renderSignIn(storage.endpoint);
      });
    };

    var onTokenCreate = function onTokenCreate() {
      log('onTokenCreate');
      if (!isSafari()) return;
      appendFrame();
    ...
    var setVisibility = function setVisibility(isVisible) {
      if (isVisible) {
        NimbleIframe.src = browser_polyfill_default().runtime.getURL('frame.html');
        sendMessageToFrame('NIMBLE_EXT__SHOW_FRAME');
        NimbleDiv.classList.add('__visible');
      } else {
        NimbleIframe.src = '';
        NimbleDiv.classList.remove('__visible');
      }

      [['mouseover', hoverHandler], ['mouseup', selectionHandler]].forEach(function (item) {
        return window.document[isVisible ? 'addEventListener' : 'removeEventListener'](item[0], item[1]);
      });
    };

    var setPosition = function setPosition(position) {
      return NimbleDiv.classList[position === 'left' ? 'add' : 'remove']('__position-left');
    }; // request frame position


    getFramePosition().then(setPosition); // handle visibility update

    var onFrameStateChange = function onFrameStateChange(state) {
      return setVisibility(state && state.isVisible);
    };

    var onFrameSetUrl = function onFrameSetUrl(eventData) {
      sendMessageToFrame('NIMBLE_EXT__UPDATE_URL', eventData); // sendMessageToWidget('NIMBLE_EXT__UPDATE_URL', eventData);
    }; // request frame state from background


    var requestFrameState = function requestFrameState() {
      sendMessageToExtension('NIMBLE_EXT__REQUEST_FRAME_STATE', {
        url: location.href,
        wasInjected: wasInjected
      }).then(onFrameStateChange);
    }; // handle position request from widget


    var onPositionRequest = function onPositionRequest() {
      getFramePosition().then(function (position) {
        sendMessageToWidget('nimbleExtension.updateFramePosition', position === 'left' ? 'left' : 'right');
      });
    }; // handle position toggle from widget


    var onPositionToggle = function onPositionToggle() {
      getFramePosition().then(function (storedPosition) {
        var position;
        ...
        if (storedPosition === 'left') {
          ...
          saveFramePosition('left');
          position = 'left';
        }

        setPosition(position);
        sendMessageToWidget('nimbleExtension.updateFramePosition', position);
      });
    }; //


    var onRequestParentUrl = function onRequestParentUrl() {
      return sendMessageToWidget('nimbleExtension.contactList.updateParentUrl', location.href);
    }; //


    var onRequestSavedSearches = function onRequestSavedSearches() {
      sendMessageToExtension('NIMBLE_EXT__REQUEST_SAVED_SEARCH').then(function (data) {
        return sendMessageToWidget('nimbleExtension.contactList.responseSavedSearches', data);
      });
    }; //


    var onUpdateSavedSearches = function onUpdateSavedSearches(data) {
      sendMessageToExtension('NIMBLE_EXT__UPDATE_SAVED_SEARCH', data);
    }; // handle messages from widget


    var onWidgetMessage = function onWidgetMessage(_ref3) {
      var eventName = _ref3.eventName,
          eventData = _ref3.eventData;

      //log('on widget message', eventName, eventData);
      switch (eventName) {
        case 'nimbleExtension.widgetIsReady':
          return useParser();

        case 'nimbleExtension.requestFramePosition':
          return onPositionRequest(eventData);

        case 'nimbleExtension.toggleFramePosition':
          return onPositionToggle(eventData);

        case 'nimbleExtension.contactList.requestParentUrl':
          return onRequestParentUrl();

        case 'nimbleExtension.contactList.requestSavedSearches':
          return onRequestSavedSearches();

        case 'nimbleExtension.contactList.updateSavedSearches':
          return onUpdateSavedSearches(eventData);
      }
    }; // handle messages
    ...
    var onMessage = function onMessage(_ref4) {
      ...
      log('CONTENT: on message', eventName, eventData);

      switch (eventName) {
        case 'NIMBLE_EXT__FRAME_STATE_CHANGED':
          return onFrameStateChange(eventData);

        case 'NIMBLE_EXT__SET_URL':
          return onFrameSetUrl(eventData);

        case 'NIMBLE_EXT__ON_TOKEN_REMOVE':
          return onTokenRemove();

        case 'NIMBLE_EXT__ON_TOKEN_CREATE':
          return onTokenCreate();

        case 'NIMBLE_EXT__SESSION':
          return;

        case 'NIMBLE_EXT__SESSION_INVALIDATE':
          return;
      }
    };
    ...
    }, 500); // listen widget messages

    window.addEventListener('message', function (_ref5) {
      var data = _ref5.data;
      return data.eventName === 'NIMBLE_EXT__PROXY_FROM_FRAME' && onWidgetMessage(data.eventData);
    });
    useParser();
    ...
    browser_polyfill_default().runtime.onMessage.addListener(onMessage); // request frame state from background

    requestFrameState(); // request session

    sendMessageToExtension('NIMBLE_EXT__SESSION_REQUEST').then(function (session) {
      return log('Got session:', session);
    });
  }; //


  if (!window.IS_NIMBLE_EXTENSION_RUNNING) runApp();
  window.IS_NIMBLE_EXTENSION_RUNNING = true;
  if ('IS_NIMBLE_EXTENSION_INJECTED' in window) delete window.IS_NIMBLE_EXTENSION_INJECTED;
  /*
   const isRunningMeta = document.querySelector('meta[name="nimble_prospector_running"]');
   if (!isRunningMeta) {
      const meta = document.createElement('meta');
      meta.name = 'nimble_prospector_running';
      meta.content = 'true';
      document.head.appendChild(meta);
      runApp();
  }
   const wasInjectedMeta = document.querySelector('meta[name="nimble_prospector_injected"]');
  if (wasInjectedMeta) wasInjectedMeta.remove();
   */
})();
