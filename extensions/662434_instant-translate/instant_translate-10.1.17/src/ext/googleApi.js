    ...
    pl.extend(ke.ext.googleApi, {
        ...
        getWordsInfo: function (words, fn) {
            ...
            var xhr = $.ajax({
                ...
                success: function (d) {
                    ...
                    const time_bucket = getTimeBucket(Date.now() - start_time);
                    if (typeof ga !== 'undefined') ga('send', 'event', 'translation', 'words-api-time-buckets', ke.ext.util.storageUtil.getVal('user_country') + ' - ' + time_bucket);
        ...
        getGoogleOldTranslation: function (from, to, text, fn) {
            $.ajax({
                ...
                success: function (d) {
                    // Building more suitable object for further parsing
                    const responseData = {
                        sentences: [
                            {
                                trans: d[0],
        ...
        getTextTranslation: function (from, to, text, fn) {
            ...
            var translateChunk = function (i) {
                if (i >= chunks.length) {
                    ...
                    var has_translit = (response.sentences && response.sentences.length) ?
                        typeof response.sentences[response.sentences.length - 1].translit != "undefined" :
                        false;
