...
//                                                                             }
//       Copyright (C) 2019-2024 Lamantine Software a.s.                       }
//                                                                             }
//*****************************************************************************}
...
var spHTMLElementVisibleManager =
{
  ...
  // isPositionOutOfDocument(AElement)
  isPositionOutOfDocument: function(AElement, AStyle)
  {
    if (AElement && AStyle)
    try
    {
      var rc = spHTMLTools.GetElementPositionWithoutScroll(AElement);
      var rcdoc = spHTMLTools.GetDocumentElementPositionWithoutScroll(AElement.ownerDocument.documentElement);
      const elementHiddenPercent = 0.5;
      var hiddenWidth = rc.GetWidth() * elementHiddenPercent;
      var hiddenHeight = rc.GetHeight() * elementHiddenPercent;
      if (hiddenWidth > 0 && rc.left + hiddenWidth <= rcdoc.left)
      {
        if (this.Log.Info)
          spLog.logMessage('spHTMLElementVisibleManager.isPositionOutOfDocument() ' + spHTMLTools.GetElementDebugInfo(AElement) + ' Out of left border: rc.left=' + Math.round(rc.left) +
            ' element position: ' + spTools.GetRectDebugInfo(rc) + ', document rect: ' + spTools.GetRectDebugInfo(rcdoc));
        return true;
      }
      if (hiddenHeight > 0 && rc.top + hiddenHeight <= rcdoc.top)
      {
        if (this.Log.Info)
          spLog.logMessage('spHTMLElementVisibleManager.isPositionOutOfDocument() ' + spHTMLTools.GetElementDebugInfo(AElement) + ' Out of top border: rc.top=' + Math.round(rc.top) +
            ' element position: ' + spTools.GetRectDebugInfo(rc) + ', document rect: ' + spTools.GetRectDebugInfo(rcdoc));
        return true;
      }
      // >> INFO: do not check for absolute & relative as H-scroll & V-scroll displayed instead of hiding the element
      if (AStyle.position != 'absolute' && AStyle.position != 'relative')
      {
        if (hiddenWidth > 0 && rc.right - hiddenWidth >= rcdoc.right)
        {
          if (this.Log.Info)
            spLog.logMessage('spHTMLElementVisibleManager.isPositionOutOfDocument() ' + spHTMLTools.GetElementDebugInfo(AElement) + ' Out of right border: rc.right=' + Math.round(rc.right) +
              ' element position: ' + spTools.GetRectDebugInfo(rc) + ', document rect: ' + spTools.GetRectDebugInfo(rcdoc));
          return true;
        }
        if (hiddenHeight > 0 && rc.bottom - hiddenHeight >= rcdoc.bottom)
        {
          if (this.Log.Info)
            spLog.logMessage('spHTMLElementVisibleManager.isPositionOutOfDocument() ' + spHTMLTools.GetElementDebugInfo(AElement) + ' Out of bottom border: rc.bottom=' + Math.round(rc.bottom) +
              ' element position: ' + spTools.GetRectDebugInfo(rc) + ', document rect: ' + spTools.GetRectDebugInfo(rcdoc));
          return true;
        }
      }
    ...
    {
      spLog.logError('spHTMLElementVisibleManager.isPositionOutOfDocument() Error: ' + ErrorMessage);
    }
    return false;
  },
  ...
  getElementVisibility: function(AElement)
  {
    ...
    if (style)
    {
      ...
      if (style.opacity === '0' || parseFloat(style.opacity) < 0.25)
      {
        if (this.Log.Info)
          spLog.logMessage('spHTMLElementVisibleManager.getElementVisibility() ' + spHTMLTools.GetElementDebugInfo(AElement) + ' ' +
      ...
      {
        if (AElement.offsetHeight > 0 || AElement.offsetWidth > 0)
        {
          if (this.isPositionOutOfDocument(AElement, style))
            return this.ElementVisibility.Hidden_PositionOutOfOffset;
        }
      }
      ...
      if (offsetWidth == 0 || offsetHeight == 0)
      {
        ...
        {
          var noChildren = true;
          if (typeof AElement.shadowRoot != 'undefined' && AElement.shadowRoot)
          {
            if (AElement.shadowRoot.children && AElement.shadowRoot.children.length)
              noChildren = false;
          }
          if (noChildren)
          {
  ...
  getElementVisibilityBasedOnParent: function(AElement, AParentVisibility)
  {
    ...
    if (AParentVisibility != this.ElementVisibility.Visible)
    {
      ...
      // skip parent visibility if element is visible,
      ...
      if (AParentVisibility != this.ElementVisibility.Hidden_VisibilityHidden &&
          AParentVisibility != this.ElementVisibility.Hidden_PositionOutOfOffset &&
      ...
      while (element)
      {
        ...
        if (element != AElement)
        {
          ...
          if (visibility == this.ElementVisibility.Hidden_VisibilityHidden ||
              visibility == this.ElementVisibility.Hidden_PositionOutOfOffset ||
  ...
  try
  {
    ...
    if (ADocument && typeof ADocument.elementFromPoint == 'function')
    {
      ...
      for (var a = 0; a < 2; a++)
      {
        ...
        var topElement = spHTMLTools.ElementFromPoint(AElement, x, y);
        if (topElement)
        {
          ...
          if (isOverlapping)
          {
            ...
            {
              // SVG element could be wider than right border of the element
              if (topElement.tagName.toUpperCase() != 'SVG')
              {
                if (this.Log.Info)
                  spLog.logMessage('TspContextButton._fixButtonRectOverlap() button for ' + spHTMLTools.GetElementDebugInfo(AElement) +
                    ' is not overlapped by the top element ' + spHTMLTools.GetElementDebugInfo(topElement) +
  ...
  try
  {
    ...
    if (spTools.IsChrome())
    {
      if (rcDiv.bottom - rcDiv.top == 0)
      {
        ...
        {
          yImage = '0';
          AdivButton.style.alignItems = 'center'; // need if any parent has style "align-items: center"
        }
      }
    }
  ...
  if (AElement && AdivButton)
  {
    ...
    if (style && style.zIndex)
    {
      ...
      else
      {
        ...
        {
          // if element position is relative/absolute without z-index (i.e. auto), then 
          // context button became under the element because its position is absolute;
          // possibly this should be done if element position <> static.
          zIndex = 2;
        }
        else
          zIndex = 'auto';
...
TspContextButtonImg.prototype.addButtonToElement = function (AWindow, AElement, AButtonInfo)
{
  ...
  try
  {
    ...
    if (typeof MutationObserver != 'undefined')
    {
      ...
      spHTMLTools.AddUnloadEventListener(AWindow, function (AEvent) {
...
TspContextButtonManager.prototype.hasContextButtonBackgroundImg = function (AElement)
{
  if (AElement)
  try
  {
    ...
    {
      var contextButton = this.ContextButtons[i];
      if (contextButton instanceof TspContextButtonBackgroundImg)
        return true;
    }
  }
  catch (ErrorMessage)
  {
    spLog.logError('TspContextButtonManager.hasContextButtonBackgroundImg() Error: ' + ErrorMessage);
  }
...
TspInputValueCacheManager.prototype._attachToInputMethod = function(AInput, AInputMethod, AAlreadySubscribedCallback)
{
  ...
  try
  {
    ...
    if (Window)
      spHTMLTools.AddUnloadEventListener(Window, this._onWindowUnloadHandler);
...
TspDOMChangesMonitor.prototype =
{
  ...
  _StartFrameMonitoring: function(AFrame)
  {
    if (AFrame)
    try
    {
      var frameDocument = spHTMLTools.GetWindowDocument(AFrame); // get the loaded document!
      if (this._StartDocumentMonitoring(frameDocument))
      {
        if (this.Log.Info)
          spLog.logMessage('TspDOMChangesMonitor._StartFrameMonitoring() ' + 
            'New MutationObserver subscribed for loaded frame ' +
    ...
    {
      spLog.logError('TspDOMChangesMonitor._StartFrameMonitoring() Error: ' + ErrorMessage);
    }
  },
    ...
    try
    {
      ...
      for(var i = 0, lenFrames = frameElements.length; i < lenFrames; i++)
      {
        ...
        frame.addEventListener('load', function _load(AEvent) {
          ...
          Self._StartFrameMonitoring(frame);
        }, true);
      }
    }
    catch (ErrorMessage)
  ...
  _FrameOnLoad: function (AEvent)
  {
    ...
    try
    {
      ...
      if (AFrame)
      {
        this._StartFrameMonitoring(AFrame);
  ...
  {
    // allowed to monitor Mutation events even after 1st showing of input password!
    this._StopLoadDelayTimer();
...
TspPageEventsMonitor.prototype.ConnectToSubmitElementClick = function(AElement)
{
  ...
  try
  {
    ...
    // attach submit events to the shadow element,
    // otherwise submit events in the shadow won't be handled
    var parentShadowRoot = spHTMLTools.GetParentShadowRoot(AElement);
    if (parentShadowRoot)
      this.ConnectToShadowRootSubmitEvents(parentShadowRoot);
  }
  catch (ErrorMessage)
  {
...
// ConnectToShadowRootSubmitEvents(AShadowRoot)
TspPageEventsMonitor.prototype.ConnectToShadowRootSubmitEvents = function(AShadowRoot)
{
  if (AShadowRoot)
  try
  {
    if (AShadowRoot.spSubmitEventsAttached)
      return;

    if (this.Log.Info)
      spLog.logMessage('TspPageEventsMonitor.ConnectToShadowRootSubmitEvents() attach events to shadowRoot element ' +
        spHTMLTools.GetElementDebugInfo(AShadowRoot)
      );
    AShadowRoot.spSubmitEventsAttached = true;
    this.ConnectToSubmitEvents(AShadowRoot);
  }
  catch (ErrorMessage)
  {
    spLog.logError('TspPageEventsMonitor.ConnectToShadowRootSubmitEvents() Error: ' + ErrorMessage);
  }
};
...
// AllowProcessCrossOriginPageLoaded(logInfo)
TspPageEventsMonitor.prototype.AllowProcessCrossOriginPageLoaded = function (logInfo)
{
  try
  {
    // process event only if this top page has been completely loaded
    let dtc = spTools.GetTickCount() - this.DOMChangesMonitor.PageLoadFiredTickCount;
    if (this.DOMChangesMonitor.PageLoadFired &&
        // prevent fire event twice (once for top window and again by child frame)
        this.DOMChangesMonitor.PageLoadFiredTickCount &&
        dtc > 1500
       )
    {
      this.DOMChangesMonitor.PageLoadFiredTickCount = spTools.GetTickCount();
      this.DOMChangesMonitor._StopLoadDelayTimer();
      if (logInfo)
        logInfo.dtc = dtc;
      return true;
    }
  ...
  {
    spLog.logError('TspPageEventsMonitor.AllowProcessCrossOriginPageLoaded() Error: ' + ErrorMessage);
  }
...
TspPageEventsMonitor.prototype.InstallHTMLFormElementPrototypeForDocument = function (ADocument)
{
  ...
  try
  {
    ...
    // Chrome Manifest V3 denied to use inline scripts, therefore move the code into separate JS file,
    // also some sites under Mozilla Firefox failed to work with inline scripts even for Chrome Manifest V2, thus use separate JS file if possible
    if (typeof chrome != 'undefined' &&
        typeof chrome.runtime != 'undefined' &&
        typeof chrome.runtime.getURL == 'function')
    {
      var scriptUrl = chrome.runtime.getURL('/spFormElementPrototypeEx.js');
...
{
  if (typeof window != 'undefined' && typeof window.DOMParser != 'undefined') // Opera 12 support
...
{
  if (typeof window != 'undefined' && typeof window.XMLSerializer != 'undefined') // Opera 12 support
...
TspRTIDContext.prototype._findElementByRuntimeIDInShadowRoot = function(rootElement, ARTID, selector)
{
  if (rootElement)
  try
  {
    if (!(rootElement instanceof HTMLDocument))
    {
      ...
      if (Elements)
      {
        for (let i = 0, lenElements = Elements.length; i < lenElements; i++)
        {
    ...
    for (let child of rootElement.querySelectorAll('*'))
    {
      if (typeof child.shadowRoot != 'undefined' && child.shadowRoot)
      {
        let element = this._findElementByRuntimeIDInShadowRoot(child.shadowRoot, ARTID, selector);
        if (element)
          return element;
      }
    }
  ...
  {
    spLog.logError('TspRTIDContext._findElementByRuntimeIDInShadowRoot() Error: ' + ErrorMessage);
  }
  return null;
};
...
TspRTIDContext.prototype.FindElementByRuntimeID = function(ADocumentList, ARTID)
{
  ...
  try
  {
    ...
    // search element within documents list
    ...
    // search element within shadowRoot children
    for (var i = 0, lenDocuments = ADocumentList.length; i < lenDocuments; i++)
    {
      var Document = ADocumentList[i];
      var element = this._findElementByRuntimeIDInShadowRoot(Document, ARTID, selector);
      if (element)
        return element;
    }
  }
...
TspCommonXmlParser.prototype.CreateNewXml = function ()
{
  ...
  try
  {
    ...
    else
    {
      ...
      this.XmlDocument = XmlParser.parseFromString('<' + this.TagName_RootNode + '/>', 'text/xml');
    }
...
TspCrossOriginDOMXmlParser.prototype.UpdateTopDocumentInfo = function (AUrl, ATitle)
{
  ...
  try
  {
    ...
    if (topXmlDocumentNode)
    {
      ...
      {
        XmlDocumentNode.setAttribute(this.AttributeName_Url, AUrl);
        Result = true;
      }
      if (typeof ATitle != 'undefined' && ATitle)
      {
        XmlDocumentNode.setAttribute(this.AttributeName_Title, ATitle);
        Result = true;
      }

      if (Result)
      {
        this.XmlRootNode.appendChild(XmlDocumentNode);
        XmlDocumentNode.appendChild(topXmlDocumentNode);
      }
    }
  }
  catch (ErrorMessage)
...
TspExportDOMManager.prototype._ExportElementBaseProperties = function(AElement, AXmlElementNode)
{
  ...
  try
  {
    ...
    // role, and other required attributes
    const aCustomAttributes = ['role', 'data-testid'];
    for (var i = 0, len = aCustomAttributes.length; i < len; i++)
    {
      const attributeName = aCustomAttributes[i];
      s = AElement.getAttribute(attributeName);
      if (!spStrings.IsStrEmpty(s))
        AXmlElementNode.setAttribute(attributeName, s);
    }
    // spAttributes
...
TspExportDOMManager.prototype._isElementReallyHidden = function (AElementVisibility)
{
  if (AElementVisibility == spHTMLElementVisibleManager.ElementVisibility.Visible ||
      ...
      AElementVisibility == spHTMLElementVisibleManager.ElementVisibility.Hidden_PositionOutOfOffset ||
...
TspExportDOMManager.prototype.IsSandboxed = function(AWindow)
{
  // handle sandbox rule in CSP and sandboxed iframe
  return AWindow && (AWindow.origin == null || AWindow.origin === 'null');
};
...
TspExportDOMManager.prototype.ExportDOMToXml = function(AWindow, AExportDOMToXmlCallback)
{
  ...
  try
  {
    ...
    var isSandboxed = this.IsSandboxed(AWindow);
...
function TspAutofillManager()
{
  ...
  this.AutofillCmd_pressEnterKey = 'pressEnterKey';
...
TspAutofillManager.prototype._pressEnterKey = function(AXmlNode)
{
  var element = this._findElementFromXmlNode(AXmlNode);
  if (element)
  {
    spHTMLTools.ElementSetFocus(element);
    spHTMLTools.FireElementKeyEvent(element, 'keydown', 13, 'Enter');
    spHTMLTools.FireElementKeyEvent(element, 'keyup', 13, 'Enter');
  }
};
  ...
  try
  {
    while (AXmlCommands.length)
    {
      ...
      try
      {
        ...
        else if (spStrings.SameText(tagName, this.AutofillCmd_pressEnterKey))
          this._pressEnterKey(xmlNode);
...
TspRect.prototype.GetWidth = function ()
{
  return this.right - this.left;
};

TspRect.prototype.GetHeight = function ()
{
...
TspRect.prototype.SetHeight = function (value)
{
...
var spTools =
{
  ...
  GetBrowserClientName: function()
  {
    if (navigator.userAgentData)
    {
      ...
      //   because the userAgentData.brands also could include the 'Chromium' text
      // ComodoDragon doesn't specify itself in the userAgentData.brands, only 'Chromium' exists there
      if (brands.some(b => b.brand.includes('Opera')))
        return this.BrowserClientName.OperaChromium;
      else if (brands.some(b => b.brand.includes('Microsoft Edge')))      
        return this.BrowserClientName.Edge;
      //   therefore the Chrome check should be the latest one
      else if (brands.some(b => b.brand.includes('Google Chrome') || b.brand.includes('Chromium')))
        return this.BrowserClientName.Chrome;

      // Mozilla based
      // TODO: Firefox still doesn't implement this API
      // else if (brands.some(b => b.brand.includes('Firefox')))
      //   return this.BrowserClientName.Firefox;

      // Safari based
      // TODO: Safari still doesn't implement this API
      // else if (brands.some(b => b.brand.includes('Safari')))
    ...
    // ComodoDragon doesn't specify itself in the userAgent, only 'Chrome/' exists there
...
function TspOSXScreenCoordsConvertor()
{
  ...
  spHTMLTools.AddUnloadEventListener(window, function (AEvent) {
    // remove listeners, no longer needed
...
var spHTMLTools =
{
  ...
  GetElementDebugInfo: function(AHtmlElement)
  {
    ...
    {
      var s;
      if (this.IsInstanceOfShadowRoot(AHtmlElement))
      {
        // <#shadow-root>
        Result = '<#shadow-root>';
        // host
        s = this.GetElementDebugInfo(AHtmlElement.host);
        if (s)
          Result += ' of host: ' + s;
      }
      else
  ...
  IsInstanceOfShadowRoot: function (AElement)
  {
    if (AElement)
    try
    {
      if (AElement instanceof ShadowRoot ||
          // special case when "instanceof ShadowRoot" failed on some sites
          AElement.constructor.name === 'ShadowRoot' ||
          // ShadowRoot constructor doesn't have "name" property in Firefox!
          (typeof AElement.host != 'undefined' &&
           typeof AElement.mode != 'undefined')
         )
      {
        return true;
      }
    ...
    {
      spLog.logError('spHTMLTools.IsInstanceOfShadowRoot() Error: ' + ErrorMessage);
    }
    return false;
  },

  IsElementInShadow: function (AElement)
  {
    if (AElement)
    try
    {
      if (typeof AElement.getRootNode == 'function')
      {
        var rootNode = AElement.getRootNode();
        if (this.IsInstanceOfShadowRoot(rootNode))
          return true;
      }
    }
    catch (ErrorMessage)
    {
      spLog.logError('spHTMLTools.IsElementInShadow() Error: ' + ErrorMessage);
    }
    return false;
  },

  GetParentShadowRoot: function (AElement)
  {
    if (AElement)
    try
    {
      if (this.IsElementInShadow(AElement))
      {
        // getRootNode returns the most parent shadowRoot element comparing
        // to this algorythm which returns the first found parent shadowRoot
        var element = AElement.parentNode;
        while (element)
        {
          if (typeof element.shadowRoot != 'undefined' &&
              element.shadowRoot)
            return element.shadowRoot;
          else if (this.IsInstanceOfShadowRoot(element))
            return element;
          // is it top element?
          if (!element.parentNode)
            break;
    ...
    {
      spLog.logError('spHTMLTools.GetParentShadowRoot() Error: ' + ErrorMessage);
    }
    return null;
  },

  // ElementFromPoint(AElement, x, y)
  ElementFromPoint: function (AElement, x, y)
  {
    if (AElement)
    try
    {
      var element = null;
      // in case if element lays in shadow - the document will return 
      // the shadow parent element from the specified coords instead of an element
      // inside the shadow; thus get the shadow root and ask the element from it.
      var parentShadowRoot = this.GetParentShadowRoot(AElement);
      if (parentShadowRoot &&
          typeof parentShadowRoot.elementFromPoint == 'function')
        element = parentShadowRoot.elementFromPoint(x, y);
      else
        element = AElement.ownerDocument.elementFromPoint(x, y);
      return element;
    }
    catch (ErrorMessage)
    {
      spLog.logError('spHTMLTools.ElementFromPoint() Error: ' + ErrorMessage);
    }
    return null;
  },      
  ...
  // GetElementPositionWithoutScroll(AHtmlElement)
  GetElementPositionWithoutScroll: function (AHtmlElement)
  {
    var Result = new TspRect();
    ...
    try
    {
      Result = this.GetElementPosition(AHtmlElement);
      var documentElement = AHtmlElement.ownerDocument.documentElement;
      spTools.OffsetRect(Result, documentElement.scrollLeft, documentElement.scrollTop);
    }
    catch (ErrorMessage)
    {
      spLog.logError('spHTMLTools.GetElementPositionWithoutScroll() Error: ' + ErrorMessage);
    }
    return Result;
  },
  
  // GetDocumentElementPositionWithoutScroll(ADocumentElement)
  GetDocumentElementPositionWithoutScroll: function (ADocumentElement)
  {
    var Result = new TspRect();
    ...
    try
    {
      Result = this.GetElementPositionWithoutScroll(ADocumentElement);
      if (Result.GetHeight() == 0 &&
          ADocumentElement.ownerDocument.body)
      {
        // the document element could return zero height if CSS styles position = fixed for body element
        Result.SetHeight(ADocumentElement.ownerDocument.body.offsetHeight);
      }
      if (Result.GetHeight() < ADocumentElement.scrollHeight &&
          ADocumentElement.scrollHeight > 0)
      {
        // the document element could return zero height (and even 8px on one site)
        // from documentElement.getBoundingClientRect() which is equal to documentElement.offsetHeight,
        // but in fact the document has real height specified in scrollHeight property more than returned in rect one!
        Result.SetHeight(ADocumentElement.scrollHeight);
      }
    }
    catch (ErrorMessage)
    {
      spLog.logError('spHTMLTools.GetDocumentElementPositionWithoutScroll() Error: ' + ErrorMessage);
    }
    return Result;
  },
      ...
      if (spTools.IsMacOSPlatform())
      {
        ...
        if (this.IsTopWindow(AWindow))
        {
          ...
          if (browserClientName == spTools.BrowserClientName.Chrome ||
              browserClientName == spTools.BrowserClientName.OperaChromium ||
              browserClientName == spTools.BrowserClientName.Edge)
  ...
  AllowProcessDocument: function(ADocument)
  {
    try
    {
      ...
      if (spStrings.IsStrEmpty(Url))
        return false;
      //  *   blank URL
      if (spStrings.SearchTextPos('about:', Url) == 0 ||   // ignore any about page
          spStrings.SearchTextPos('chrome://', Url) == 0)  // ignore any chrome page
        return false;
    }
    catch (ErrorMessage)
    {
  ...
  GetDocumentFocusedElement: function (ADocument)
  {
    ...
    try
    {
      ...
      // if active element located in shadowRoot - we get it from the shadowRoot
      if (Result &&
          typeof Result.shadowRoot != 'undefined' &&
          Result.shadowRoot
         )
      {
        if (this.IsInstanceOfShadowRoot(Result.shadowRoot))
          Result = this.GetDocumentFocusedElement(Result.shadowRoot);
      }
      // if active element located in iframe/frame - we get it from the frame element
      if (Result && 
  ...
  // FireElementKeyEvent()
  FireElementKeyEvent: function (AHtmlElement, AEventName, AKeyCode, AKey)
  {
    if (AHtmlElement && AEventName)
    try
    {
      if (typeof AKey != 'undefined')
      {
        ...
        {
          const event = new KeyboardEvent(AEventName, {
            key: AKey,
            keyCode: AKeyCode,
            bubbles: true,
            cancelable: true
  ...
  AddUnloadEventListener: function (AWindow, AEventListener)
  {
    if (AWindow && typeof AEventListener == 'function')
    {
      const terminationEvent = 'onpagehide' in AWindow ? 'pagehide' : 'unload';
      AWindow.addEventListener(terminationEvent, function _unload(AEvent) {
        if (event && !event.persisted)
        {
          AWindow.removeEventListener(terminationEvent, _unload, false);
          AEventListener(event);
        }
      }, false);
    }
  },

  IsBlankPageUrl: function (Url)
  {
    if (Url)
    {
      if (spStrings.SameText(Url, 'about:blank') ||
          spStrings.SameText(Url, 'about:home') ||              // Mozilla Firefox
          spStrings.SameText(Url, 'about:newtab') ||            // Mozilla Firefox
          spStrings.SameText(Url, 'chrome://new-tab-page/') ||  // Chromium
          spStrings.SameText(Url, 'chrome://newtab/')           // Chromium
         )
      {
        return true;
      }
...
TspWindowElementsEnumerator.prototype._EnumShadowRoot = function(AParentElement, AParentVisible, AEnumCallback)
{
  if (!AParentElement || !AEnumCallback)
    return;

  let enumAllChildDocuments = this.EnumAllChildDocuments;
  this.EnumAllChildDocuments = false; // do not enum any iframes of shadowRoot
  try
  {
    var Self = this;    
    ...
    function findShadowRoots(node)
    {
      if (node.shadowRoot)
      {
        var isElementVisible = Self._IsElementVisible(node, AParentVisible);
        if (!isElementVisible)
          return;
        // <*.shadowRoot>
        Self._EnumElements(node.shadowRoot, AParentVisible, AEnumCallback);
        if (Self.CancelEnum)
          return;

        // recursively find more shadow roots inside this shadowRoot
        for (let i = 0, lenChildren = node.shadowRoot.children.length; i < lenChildren; i++)
        {
      ...
      for (let i = 0, lenChildren = node.children.length; i < lenChildren; i++)
      {
    ...
    findShadowRoots(AParentElement);
  }
  catch (ErrorMessage)
  {
    spLog.logError('TspWindowElementsEnumerator._EnumShadowRoot() Error: ' + ErrorMessage);
  }
  this.EnumAllChildDocuments = enumAllChildDocuments;
};

TspWindowElementsEnumerator.prototype._EnumElements = function(AParentElement, AParentVisible, AEnumCallback)
{
  if (!AParentElement || !AEnumCallback)
    return;
...
TspWindowElementsEnumerator.prototype._EnumWindow = function(AWindow, AParentVisible, AEnumCallback)
{
  ...
  try
  {
    ...
    if (Document)
    {
      ...
      {
        // <body> tag
        this._EnumElements(Document.body, AParentVisible, AEnumCallback);
        if (this.CancelEnum)
          return;
        // <*.shadowRoot>
        this._EnumShadowRoot(Document.body, AParentVisible, AEnumCallback);
        if (this.CancelEnum)
          return;
      }
    }
