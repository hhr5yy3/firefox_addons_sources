...
async function autogigsLancarAtividade(
	...
	if(tipo.includes('Comentário')){
		autogigsLancarComentario(comentario,salvar)
		return
	}
	...
	function preencherPrazo(){
		...
		if(obterData(prazo)) autogigsPreencherCampo('dataPrazo',prazo)
		else autogigsPreencherCampo('dias',prazo)
	}

	function salvarLancamento(){
		if(salvar) setTimeout(() => {
			clicar('[aria-label="Salva as alterações"]')
			if(comentario) autogigsLancarComentario(comentario)
		},1000)
	}
}
...
async function autogigsLancarComentario(
	...
	function salvarLancamento(){
		if(salvar) setTimeout(() => clicar('[aria-label="Salva as alterações"]'),1000)
	}
}
