...
tineye.saveOptions = function() {
    ...
    for(var i = 0; i < sort_order_radio.length; i++) {
        if(sort_order_radio[i].checked)
            apiNamespace.storage.local.set({'sort_order': sort_order_radio[i].value});
    }

    for(var k = 0; k < tab_visibility_radio.length; k++) {
        if(tab_visibility_radio[k].checked)
            apiNamespace.storage.local.set({'tab_visibility': tab_visibility_radio[k].value});
    }
};
...
tineye.restoreOptions = function() {
    ...
    apiNamespace.storage.local.get(['sort_order', 'tab_visibility'], function(result) {
        ...
        if(result.sort_order) {
            ...
            apiNamespace.storage.local.set({'sort_order': 'last_used'});
        }
        ...
        if(result.tab_visibility) {
            ...
            apiNamespace.storage.local.set({'tab_visibility': 'background'});
        }
