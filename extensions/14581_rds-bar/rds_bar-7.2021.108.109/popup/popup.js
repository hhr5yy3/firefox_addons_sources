        ...
        Parameters: rdz.Backbone.Collection.extend({
            ...
            sendALLRequest: function () {
                rdz.pmessenger.post({
                    ...
                    receiver: 'popup',
                    method: 'message',
                    request: 'OPERA_ALL_PARAMETERS_PopupRequest'
                });
            }
...
window.rdz.view.popup = {
    ...
    App: rdz.Backbone.View.extend({
        render: function () {
            ...
            chrome.storage.local.get(['OtherData', 'Parameters'], settings => { this.showBanner(settings); });
            this.showAhrefsRank();
            ...
            //var history_counters = rdz.view.parameters.Parameter.extend();
            new window.rdz.view.Popup.History_Counters.Factory({model: rdz.app.popup.history_counters});

            rdz.app.popup.parameters.sendALLRequest();
...
//	    var parameters = rdz.utils.getOptions({options: ['Parameters']}),
//                mc = parameters.filter(function(p) {return p.name === 'MassChecking';}),
//		mcActive = mc && mc[0] && mc[0].active;
//
//	    if ( mcActive && (AppLocale.get_locale_str() === 'ru') ) {
//		rdz.$('#banner-cnt').css('display', 'block');
//	//        document.getElementById('banner-object').onmousedown = function () {
//	//	    setTimeout(function() {
//	//		// main.rdz.cache.set(['ApplicationData', 'selectedUrls'], [window.rdz.tab.url]);
//	//	        main.rdz.utils.openDBPage("#checkpages/1/1");
//	//	    }, 500);
//	//        }
//	        rdz.$('#banner-cnt').click(function() {
//		    setTimeout(function() {
//		        main.rdz.utils.openDBPage("#checkpages/1/1");
//		    }, 300);
//		});
//	    }
            ...
            var locale = AppLocale.get_locale_str(),
                domain = rdz.utils.domainFromUri(window.rdz.tab.url).domain,
                currBanner, banner, bannerURL;


            //if (locale === 'ru') {
                currBanner = main.rdz.user.banners.updateCurrBanner('popup', settings);

                if (currBanner) {
                    rdz.$('#banner-cnt').empty().css('display', 'block');
                    bannerURL = currBanner.url.replace(/curr_domain/, domain);

                    // .png or .swf
                    if (/\.png$|\.gif$|\.jpg$/.test(currBanner.file_name)) {
                        banner = rdz.$('<img />', {src: currBanner.data_url});

                        rdz.$('#banner-cnt').click(function () {
                            setTimeout(function () {
                                main.rdz.utils.openPage(bannerURL);
                                main.rdz.user.banners.updateClicksStats({
                                    locale: locale,
                                    place: 'popup',
                                    banner: currBanner
                                });
                        ...
                        rdz.$('#banner-cnt').append(banner);
                    } else if (/\.swf/.test(currBanner.file_name)) {
                        banner = rdz.$('<object id="banner-object" width="468" height="60" style="margin: 0 auto; cursor: pointer;">' +
                        ( /OPR/.test(navigator.userAgent) ? '<embed wmode="transparent" allowscriptaccess="always" src="/resources/' + currBanner.file_name + '" width="468" height="60"></embed>' :
                        '<param name="movie" value="/resources/' + currBanner.file_name + '">' ) +
                        '</object>');

                        rdz.$('#banner-cnt').append(banner); // have to duplicate

                        document.getElementById('banner-object').onmousedown = function (e) {
                            if (e.button === 0) {
                                setTimeout(function () {
                                    main.rdz.utils.openPage(bannerURL);
                                    main.rdz.user.banners.updateClicksStats({
                    ...
                    } else if (/\.html|\.htm/.test(currBanner.file_name)) {
                        banner = rdz.$('<iframe id="banner-object" width="468" height="60" src="https://www.recipdonor.com/rdsbarad/' +
                                       currBanner.file_name + '">' + '</iframe>');
                    ...
                    main.rdz.user.banners.updateDisplayingStats({locale: locale, place: 'popup'});
                }

            //}

        },
        showAhrefsRank: function () {
            var name = 'AhrefsRank',
                el = rdz.$('.' + name);

            el.html('<div class="rds-nm">' +
            rdz.locale.popup.parameters[name].name + ': ' +
              ' </div><div class="rds-prm"><a target="_blank" class="rds-value_ar">?</a> (<a target="_blank" class="rds-value_dr">?</a>)</div>');
            el.attr('title', rdz.locale.popup.parameters[name].ttl);
        }
    })
};
...
window.rdz.view.Popup.AhrefsPage = {
    template: function (model) {
        var name = model.get('name'),
            html = rdz.$(`<div class="rds-nm">${rdz.locale.popup.parameters[name].name}:</div><div class="rds-prm"><a target="_blank" class="rds-value_bl">?</a> (<a target="_blank" class="rds-value_rd">?</a>)</div>`);
    ...
    showPageRank: function () {
        ...
        if (v && v.AhrefsPage && v.AhrefsPage.ar) {
            var value_ar = this.checkError(v && v.AhrefsPage ? v.AhrefsPage.ar : v);
            rdz.$('.AhrefsRank .rds-value_ar').html(this.formatValue(value_ar))
                .attr('href', this.model.get('requests')['AhrefsPage'].displayUrl);
            this.showCached({name: 'AhrefsPage', parent: '.AhrefsRank', class: 'rds-italic_Ahrefs'});
        }
    }
};

window.rdz.view.Popup.AhrefsDomain = {
    template: function (model) {
        var name = model.get('name'),
            html = rdz.$(`<div class="rds-nm">${rdz.locale.popup.parameters[name].name}:</div><div class="rds-prm"><a target="_blank" class="rds-value_bl">?</a> (<a target="_blank" class="rds-value_rd">?</a>)</div>`);
    ...
    showDomainRank: function () {
        var v = this.model.get('value');
        if (v && v.AhrefsDomain && v.AhrefsDomain.dr) {
            var value_dr = this.checkError(v && v.AhrefsDomain ? v.AhrefsDomain.dr : v);
            rdz.$('.AhrefsRank .rds-value_dr').html(this.formatValue(value_dr))
                .attr('href', this.model.get('requests')['AhrefsDomain'].displayUrl);
            this.showCached({name: 'AhrefsDomain', parent: '.AhrefsRank', class: 'rds-italic_Ahrefs'});
        }
    }
};
