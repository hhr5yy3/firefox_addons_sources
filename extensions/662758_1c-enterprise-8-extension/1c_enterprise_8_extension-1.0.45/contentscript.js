 ...
 * @private
 */
BackgroundBridge.prototype.clipboardReadCallback_ = function (msg, clipText)
{
    this.sendClipboardReadResponse_(msg, clipText);
};
 ...
 * @private
 */
BackgroundBridge.prototype.clipboardReadErrback_ = function (msg)
{
    this.sendClipboardReadResponse_(msg, this.pasteFromClipboard_());
};

/**
 * Отправка ответа на запрос о чтении содержимого буфера обмена
 *
 * @param {!MessageDataObject} msg
 * @param {string} clipText
 * @private
 */
BackgroundBridge.prototype.sendClipboardReadResponse_ = function (msg, clipText)
{
    this.messageManager.postMessage(
        {
            'reqHeader': msg['reqHeader'],
            'type': kVendorPrefix + 'response.clipboardRead',
            'text': clipText
        });
};
 ...
 * @private
 */
BackgroundBridge.textArea_ = null;

/**
 * @return {string}
 * @private
 */
BackgroundBridge.prototype.pasteFromClipboard_ = function()
{
    const currentActiveElement = /** @type {!HTMLElement} */(document.activeElement);
    if (!BackgroundBridge.textArea_)
    {
        BackgroundBridge.textArea_ = /** @type {!HTMLTextAreaElement} */(document.createElement('textarea'));
        document.body.appendChild(BackgroundBridge.textArea_);
    }
    BackgroundBridge.textArea_.value = '';
    BackgroundBridge.textArea_.contentEditable = 'true';
    BackgroundBridge.textArea_.focus();
    document.execCommand('Paste');
    currentActiveElement.focus();

    return BackgroundBridge.textArea_.value;
};
 ...
 * @private
 */
BackgroundBridge.prototype.copyToClipboard_ = function(text)
{
    const currentActiveElement = /** @type {!HTMLElement} */(document.activeElement);
    if (!BackgroundBridge.textArea_)
    {
    ...
    BackgroundBridge.textArea_.contentEditable = 'true';
    BackgroundBridge.textArea_.value = text;
    BackgroundBridge.textArea_.select();
    if (!text)
 ...
 * @private
 */
BackgroundBridge.prototype.forceClipboardCopyText_ = function(forceText)
{
 ...
 * @private
 */
BackgroundBridge.prototype.onDocumentCopy_ = function(forceText, event)
{
...
BackgroundBridge.prototype.processMessage = function(msg)
{
    ...
    switch (permissionType)
    {
        ...
        case Permission.Type.Screenshot:
...
BackgroundBridge.prototype.initBackground_ = function(hostName, version, reqHeader)
{
    ...
    try
    {
        this.backgroundPort = browser.runtime.connect({name: hostName});
        ...
        function(request)
        {
            ...
            switch (request.permissionType)
            {
                ...
                case Permission.Type.Clipboard:
                    ...
                    return;
                case Permission.Type.Screenshot:
                    self.handleScreenshotRequest_.call(self, request, status);
                    return;
                default:
    ...
    else
    {
        switch (permissionType)
        {
            ...
            case Permission.Type.Clipboard:
                ...
                break;
            case Permission.Type.Screenshot:
                confirmMsg = this.resStrings['screenshotRequestTemplate'];
                break;
            default:
...
BackgroundBridge.prototype.handleClipboardRequest_ = function(request, status)
{
    ...
    switch (type)
    {
        ...
        case 'call.clipboardWrite':
            ...
            responseType = 'response.clipboardWrite';
            break;
        case 'call.clipboardRead':
            if (navigator.clipboard)
            {
                navigator.clipboard.readText()
                    .then(this.clipboardReadCallback_.bind(this, msg))
                    .catch(this.clipboardReadErrback_.bind(this, msg));
            }
            else
            {
                this.sendClipboardReadResponse_(msg, this.pasteFromClipboard_());
            ...
            return;
        default:
            this.sendErrorMsg_(BackgroundBridge.ErrorCode.eException, msg);
            return;
    }

    this.messageManager.postMessage({'reqHeader': msg['reqHeader'], type: kVendorPrefix + responseType});
};
 ...
 * @private
 */
BackgroundBridge.prototype.handleScreenshotRequest_ = function (request, status)
{
    if (status !== Permission.Status.Granted)
    {
        status = this.getGrantedOrDeniedFromUser_(request.permissionType);
