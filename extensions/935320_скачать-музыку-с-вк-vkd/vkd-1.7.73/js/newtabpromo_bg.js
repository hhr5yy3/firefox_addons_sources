...
chrome.runtime.onInstalled.addListener(function(details) {
	if (details.reason == "install" && !localStorage.landing && !localStorage['first_date_installation_ntpromo'])
	{
		...
		chrome.management.getSelf(function(info) {
			...
			chrome.tabs.create({
				url: 'http://promo-newtab.club/?lang=' + uiLang + '&cid=ntpromo_vkodown_1&ext=' + ext_name
			});
		});
	}
});
chrome.management.getSelf(function(info) {
	...
	// to be extra safe
	chrome.runtime.setUninstallURL( ('http://promo-newtab.club/?lang=' + uiLang + '&source_type=uninstall&cid=ntpromo_vkodown_1&ext=' + ext_name).slice(0, 255) );
});
