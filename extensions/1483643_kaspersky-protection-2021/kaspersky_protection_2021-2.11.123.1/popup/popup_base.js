...
function ShowUserConsentDialog()
{
    if (document && document.body && document.body.className)
    {
        document.body.className = "userConsent";
        AvNs.UserConsentWindow = new UserConsentWindow(context =>
            {
                const cssLink = document.createElement("link");
                cssLink.href = browsersApi.runtime.getURL("snapshot_resources/uc/user_consent.css");
                cssLink.rel = "stylesheet";
                cssLink.type = "text/css";
                document.head.appendChild(cssLink);

                context.getElementById("UserConsentDialog").className = "popup-dialog";
                context.getElementById("UserConsentInfoTextArea").className = "scroll";
            });
        AvNs.UserConsentWindow.Show();
    }
    else
    {
        AvNs.SetTimeout(ShowUserConsentDialog, 100);
    }
}

chrome.storage.local.get(["hasUserConsent"], result =>
{
