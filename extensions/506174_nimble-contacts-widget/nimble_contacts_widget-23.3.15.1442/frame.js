...
;// CONCATENATED MODULE: ./source/frame/index.js
...
NimbleIframe.id = 'nimble-widget-frame'; //NimbleIframe.src = browser.runtime.getURL('frame.html');

options_storage.getAll().then(function (storage) {
  var isInbox = urlParams.get('inbox') !== null;
  var path = isSafari() ? '/app/safari-login-middleware' : '/app/widget';
  var url = isInbox ? "".concat(storage.endpoint).concat(path, "?inbox#/contact-list/_/_/").concat(encodeURIComponent(urlParams.get('data'))) : "".concat(storage.endpoint).concat(path, "/#/contact-list");
  endpoint = storage.endpoint; //NimbleIframe.contentWindow.location.replace(url);
  //https://stackoverflow.com/questions/2245883/browser-back-acts-on-nested-iframe-before-the-page-itself-is-there-a-way-to-av

  NimbleIframe.src = url;
  document.body.appendChild(NimbleIframe);
}); //const NimbleIframe = document.querySelector('#nimble-widget-frame');

var ALLOWED_ORIGINS = ['https://app.nimble.com', 'https://live.devnimble.com', 'https://vbox1.devnimble.com', 'https://vbox2.devnimble.com', 'https://vbox3.devnimble.com', 'https://vbox4.devnimble.com', 'https://vbox5.devnimble.com', 'https://vbox6.devnimble.com', 'https://vbox7.devnimble.com', 'https://vbox8.devnimble.com', 'https://vbox9.devnimble.com', 'https://vbox10.devnimble.com', 'https://vbox11.devnimble.com', 'https://vbox12.devnimble.com', 'https://vbox13.devnimble.com', 'https://vbox14.devnimble.com', 'https://app.dev.nimble.com:9000'];
var ALLOWED_INCOMING_MESSAGES_LIST = ['NIMBLE_EXT__SHOW_FRAME', 'NIMBLE_EXT__PROXY_TO_FRAME', 'NIMBLE_EXT__UPDATE_URL', 'nimbleExtension.widgetIsReady', 'nimbleExtension.dataUpdate', 'nimbleExtension.hideToolbar', 'nimbleExtension.updateFramePosition', 'nimbleExtension.contactList.updateParentUrl', 'nimbleExtension.contactList.responseSavedSearches', 'nimbleExtension.inboxDataUpdate'];
var ALLOWED_OUTGOING_MESSAGES_LIST = ['nimbleExtension.toggleFramePosition', 'nimbleExtension.requestFramePosition', 'nimbleExtension.contactList.requestSavedSearches', 'nimbleExtension.contactList.updateSavedSearches', 'nimbleExtension.contactList.requestParentUrl', 'NIMBLE_EXT__REQUEST_TOKEN', 'NIMBLE_EXT__REMOVE_TOKEN'];
/*const loadFrame = () => {
    optionsStorage.getAll().then(storage => {
        const url = urlParams.get('inbox') !== null
            ? `${storage.endpoint}/app/widget?inbox#/contact-list/_/_/${encodeURIComponent(urlParams.get('data'))}`
            : `${storage.endpoint}/app/widget/#/contact-list`;
        NimbleIframe.contentWindow.location.replace(url);
        //https://stackoverflow.com/questions/2245883/browser-back-acts-on-nested-iframe-before-the-page-itself-is-there-a-way-to-av
        //NimbleIframe.src = url;
    });
};*/
...
window.addEventListener('message', function (e) {
  var _e$data = e.data,
      ...
      eventData = _e$data.eventData,
      origin = e.origin;
  /*if (!NimbleIframe.contentWindow.location.href.includes('/app/')) {
      loadFrame();
  }*/

  log('FRAME on message', origin, e, {
    eventName: eventName,
    eventData: eventData
  });
  if (!eventName || !ALLOWED_INCOMING_MESSAGES_LIST.includes(eventName)) return;
  log('FRAME on inner message', origin, e, {
    eventName: eventName,
    eventData: eventData
  });

  switch (eventName) {
    case 'NIMBLE_EXT__PROXY_TO_FRAME':
      if (!ALLOWED_INCOMING_MESSAGES_LIST.includes(eventData.eventName)) return;

      if (NimbleIframe && NimbleIframe.contentWindow) {
        NimbleIframe.contentWindow.postMessage(eventData, '*');
      }

      break;

    case 'NIMBLE_EXT__UPDATE_URL':
      // NimbleIframe.src = `${endpoint}/app/widget/?rnd=${Math.random() * 999}${eventData.hash}`;
      NimbleIframe.contentWindow.postMessage({
        eventName: 'NIMBLE_EXT__UPDATE_URL',
        eventData: eventData
      }, '*');
      break;

    /*case 'NIMBLE_EXT__SHOW_FRAME':
        loadFrame();
        break;*/
  }
}); //loadFrame();
...
window.addEventListener('message', function (e) {
  var _e$data2 = e.data,
      eventName = _e$data2.eventName,
      eventData = _e$data2.eventData,
      eventSource = _e$data2.eventSource,
      origin = e.origin;
  if (eventName === 'NIMBLE_EXT__PROXY_TO_FRAME' || !ALLOWED_ORIGINS.includes(origin) || !ALLOWED_OUTGOING_MESSAGES_LIST.includes(eventName)) return;
  log('FRAME on message', origin, e, {
    eventName: eventName,
    eventData: eventData
  });

  if (eventName === 'NIMBLE_EXT__REQUEST_TOKEN') {
    log('sendMessageToExtension NIMBLE_EXT__REQUEST_TOKEN', origin);
    sendMessageToExtension('NIMBLE_EXT__REQUEST_TOKEN', {
      endpoint: origin
    }, '').then(function (token) {
      log('sending NIMBLE_EXT__RESPONSE_TOKEN +');
      NimbleIframe.contentWindow.postMessage({
        eventName: 'NIMBLE_EXT__RESPONSE_TOKEN',
        eventData: token
      }, origin);
    })["catch"](function () {
      log('sending NIMBLE_EXT__RESPONSE_TOKEN -');
  ...
  if (eventName === 'NIMBLE_EXT__REMOVE_TOKEN') sendMessageToExtension('NIMBLE_EXT__REMOVE_TOKEN', eventData, '');
  window.parent.postMessage({
    eventName: 'NIMBLE_EXT__PROXY_FROM_FRAME',
    eventData: {
      eventName: eventName,
      eventData: eventData,
      eventSource: eventSource
