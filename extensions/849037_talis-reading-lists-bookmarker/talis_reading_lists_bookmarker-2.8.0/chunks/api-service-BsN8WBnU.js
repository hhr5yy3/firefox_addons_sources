...
const Yc = typeof process == "object" && define_process_env_default && define_process_env_default.NODE_DEBUG && /\bsemver\b/i.test(define_process_env_default.NODE_DEBUG) ? (...e) => console.error("SEMVER", ...e) : () => {
};
var Ot = Yc;
(function(e, t) {
  const {
...
const fetchClientId = async () => {
  ...
  if (typeof result.clientId === "string" && Boolean(result.clientId)) {
...
const fetchSessionId = async () => {
  const result = await browser.storage.local.get("sessionData");
  const timestamp = Date.now();
  if (isSessionData(result.sessionData)) {
    const duration = timestamp - result.sessionData.timestamp;
    if (duration < SESSION_EXPIRATION) {
      ...
      return result.sessionData.sessionId;
    }
  }
...
const legacyActiveTenantV2Schema = z.object({
  code: z.string()
});
...
const fetchTenant = async () => {
  const result = await storage.get(["selectedTenant", "activeTenant"]);
  const { selectedTenant, activeTenant } = result;
...
const postParseResource = async (parsingRequest) => {
  ...
  const response = await send(url, "POST", JSON.stringify(parsingRequest));
  if (!lh(response)) {
...
const getTenantList = async () => {
  ...
  const response = await send(new URL(configurationUrl));
  try {
...
const fetchApiRegistry = async () => {
  try {
    const apiOrigin = await getBaseUrlForCustomerOrDefault();
    const response = await send(new URL("1/configuration", apiOrigin));
    if (!mh(response)) {
      return fh;
    }
    return response;
