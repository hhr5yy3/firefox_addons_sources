...
(function (undefined) {
    ...
    pl.extend(ke.app.handlers, {
        ...
        handleMessagesFromIFrames: function(event) {
            if (event.origin !== "https://gikken.co") {
                return;
            }

            if (event.data.indexOf('height') > -1) {
                let height = Math.ceil(parseFloat(event.data.split('=').pop()));
                if (height > 0) {
                    $('.survey-layout .popup:visible iframe').css('height', height + 'px');
                }
            } else if (event.data === 'get_dark_mode') {
                ke.ext.util.storageUtil.requestBackgroundOption('isTrueOption', ['dark_mode'], function (dark_mode) {
                    event.source.postMessage('dark_mode=' + dark_mode, event.origin);
                });
            } else if (event.data === 'finished') {
                setTimeout(() => {
                    let is_cancelled = $('.combo-change-window:visible').hasClass('cancelled');

                    ke.particles.pro_block.model.dismissSurvey();
                ...
                ke.ext.util.storageUtil.setVal('already_submitted_screw_sub_survey', true);
            }
        },
        ...
        showSharingWindow: function (event) {
            ...
            if (platform === 'email') {
                ...
                window.open(ke.ext.tpl.compile(ke.share_links[platform], {
                    link: encodeURIComponent(ke.storeLink),
                    text: encodeURIComponent(ke.getLocale('Settings_ShareText').replace('{{platform}}', ke.browserName))
        ...
        handleLoginStateChange: function (data) {
            ...
            } else if (parts.lib === 'app' && parts.cmd === 'pro' && parts.exact === 'finish-checkout') {
                ke.particles.pro_block.model.finishCheckoutCallback(data.email);
            }
        },
        ...
        saveTooltipSize: function () {
            if (ke.isProUser) {
                ...
                ke.ui.pro_alert.show(ke.getLocale('Options_TooltipSizePro'), 'toolbar-unpin');
