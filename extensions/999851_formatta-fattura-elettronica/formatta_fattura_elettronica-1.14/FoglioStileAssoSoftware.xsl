...
<xsl:stylesheet
  ...
  <xsl:template name="getEstensione">
    <xsl:param name="nomeFile"/>
    <xsl:value-of select="substring-after($nomeFile, '.')"/>
  </xsl:template>

  <xsl:template name="getMime">
    <xsl:param name="estensione"/>
    <xsl:choose>
      <xsl:when test="$estensione = 'txt'">
        <xsl:value-of select="'text'"/>
      </xsl:when>
      <xsl:when test="$estensione = 'TXT'">
        <xsl:value-of select="'text'"/>
      </xsl:when>
      <xsl:when test="$estensione = 'jpeg'">
        <xsl:value-of select="'image'"/>
      </xsl:when>
      <xsl:when test="$estensione = 'jpg'">
        <xsl:value-of select="'image'"/>
      </xsl:when>
      <xsl:when test="$estensione = 'png'">
        <xsl:value-of select="'image'"/>
      </xsl:when>
      <xsl:when test="$estensione = 'gif'">
        <xsl:value-of select="'image'"/>
      </xsl:when>
      <xsl:when test="$estensione = 'JPEG'">
        <xsl:value-of select="'image'"/>
          ...
          <xsl:if test="Allegati">
            ...
            <ul class="ulAllegati">
              <xsl:for-each select="Allegati">
                <xsl:variable name="estensione">
		  <xsl:call-template name = "getEstensione" >
  	            <xsl:with-param name="nomeFile" select="NomeAttachment"/>
		  </xsl:call-template>
		</xsl:variable>
                <xsl:variable name="mime">
		  <xsl:call-template name = "getMime" >
  	            <xsl:with-param name="estensione" select="$estensione"/>
		  </xsl:call-template>
		</xsl:variable>
                <li>
                  <div class="tx-small">
                       <a target="_blank" title="{NomeAttachment}" name="myLink" href="data:{$mime}/{$estensione};base64,{normalize-space(Attachment)}" download="{NomeAttachment}">
