class MegaForm extends MegaComponent {
        ...
        if (options.actions) {
            ...
            for (let i = options.actions.length; i--;) {
                ...
                if (options.actions[i].href) {
                    this.addLink(options.actions[i]);
                }
                else {
    ...
    addLink(options) {
        const {text, classname, type, parentNode, href, target} = options;

        const actionBtn = new MegaLink({
            parentNode: parentNode || this.domNode,
            text,
            componentClassname: classname,
            type,
    ...
    discard(isFormChanged) {
        return new Promise(resolve => {
            ...
            mega.ui.dialog.show({
                ...
                title: l.discard_changes,
                contents: [l.discard_changes_msg],
                showClose: true,
