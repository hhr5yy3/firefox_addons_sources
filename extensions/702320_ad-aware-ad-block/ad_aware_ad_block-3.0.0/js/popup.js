...
const renderPrivacyExposure = function() {
    ...
    const summary = allDomainCount - touchedDomainCount;
    dom.text('[data-i18n^="popupDisplayStatsOthersBlocked"] + span', summary);
};
...
const renderPopup = function() {
    ...
    let isAdsBlockingOff = popupData.pageURL === '' || isFiltering !== true;
    let switchStatus = isAdsBlockingOff ? "switch-text-off" : "switch-text-on";
    let switchText = isAdsBlockingOff ? "Protection is disabled" : "Protection is enabled";
    let powerControlStatus = isAdsBlockingOff ? 'toggle-btn-control transform-control transform-control-active' : 'toggle-btn-control transform-control';
    let powerButtonStatus = isAdsBlockingOff ? 'toggle-btn-default transform-btn transform-btn-active' : 'toggle-btn-default transform-btn';
    
    qs$('#switchText').className = switchStatus;
    qs$('#switchText').innerHTML = switchText;
    qs$('#mainControlSwitch').className = powerControlStatus;
    qs$('#mainButtonSwitch').className = powerButtonStatus;
    
    // dom.cl.toggle(qs$('#switchText'), 'switch-text-off', switchStatus);
    ...
    dom.text('[data-i18n^="popupDisplayStatsAdsBlocked"] + span', text);
...
dom.on('[data-i18n="popupAnyRulePrompt"]', 'click', ev => {
    ...
    if ( ev.shiftKey && ev.ctrlKey ) {
        messaging.send('popupPanel', {
            what: 'gotoURL',
            details: {
                url: `popup-adaware.html?tabId=${popupData.tabId}&intab=1`,
                select: true,
                index: -1,
            },
        });
...

{
    ...
    const checkViewport = async function() {
        if (
            ...
            await nextFrames(8);
            const main = qs$('#main');
            const minWidth = (main.offsetWidth) / 1.1;
            if (
                selfURL.searchParams.get('portrait') ||
...
dom.on('#mainControlSwitch', 'click', (e) => {
    // toggleNetFilteringSwitch(e);
    dom.cl.toggle(qs$('#mainButtonSwitch'), 'transform-btn-active');
    dom.cl.toggle(qs$('#mainControlSwitch'), 'transform-control-active');
});

dom.on('#mainButtonSwitch', 'click', (e) => {
    // toggleNetFilteringSwitch(e);
    dom.cl.toggle(qs$('#mainButtonSwitch'), 'transform-btn-active');
    dom.cl.toggle(qs$('#mainControlSwitch'), 'transform-control-active');
});

dom.on("#gotoPick", 'click', (e) => {
