...
TspPageFocusChangedMonitor.prototype.ConnectToWindowEvents = function(AWindow, AOnWindowUnloadCallback)
{
  ...
  try
  {
    ...
    if (ADocument)
    {
      ...
      var Self = this;
      spAutofillCore.HTMLTools.AddUnloadEventListener(AWindow, function (AEvent) {
...
// ConnectToShadowRootEvents(AShadowRoot)
TspPageFocusChangedMonitor.prototype.ConnectToShadowRootEvents = function(AShadowRoot)
{
  if (AShadowRoot)
  try
  {
    if (AShadowRoot.spFocusChangedEventsAttached)
      return;

    if (this.Log.Info)
      spLog.logMessage('TspPageFocusChangedMonitor.ConnectToShadowRootEvents() attach events to shadowRoot element ' +
        spAutofillCore.HTMLTools.GetElementDebugInfo(AShadowRoot)
    );
    AShadowRoot.spFocusChangedEventsAttached = true;
    AShadowRoot.addEventListener('blur', this._onBlurHandler, true);
    AShadowRoot.addEventListener('focus', this._onFocusHandler, true);
    // looks like it doesn't make any sense to handle scroll event from shadow?
    AShadowRoot.addEventListener('keydown', this._onKeydownHandler, true);
    AShadowRoot.addEventListener('mousedown', this._onMousedownHandler, true);
  }
  catch (ErrorMessage)
  {
    spLog.logError('TspPageFocusChangedMonitor.ConnectToShadowRootEvents() Error: ' + ErrorMessage);
  }
};
  ...
  if (AEvent.target)
  {
    if (typeof AEvent.target.nodeType != 'undefined' && AEvent.target.nodeType == 1)
    {
      ...
      if (spAutofillCore.HTMLTools.IsInputEditableElement(AEvent.target))
      {
        ...
        this.WasInputFocusEvent = true;
        this.LastFocusEvent = this.FocusEventType.Focus;
        this.TryToProcessFocusedElement();
      }
      else if (!this.WasInputFocusEvent)
      {
        // this could be a shadowRoot which hasn't been subscribed to focus events yet
        let focusedElement = spAutofillCore.HTMLTools.GetDocumentFocusedElement(AEvent.target.ownerDocument);
        if (focusedElement != AEvent.target &&
            spAutofillCore.HTMLTools.IsInputEditableElement(focusedElement))
        {
          this.WasInputFocusEvent = true;
          this.LastFocusEvent = this.FocusEventType.Focus;
          this.TryToProcessFocusedElement();
...
TspPageFocusChangedMonitor.prototype.ProcessFocusedElement = function()
{
  try
  {
    ...
    if (this.LastFocusEvent == this.FocusEventType.Focus &&
        ...
        isTextElement)
    {
      // attach focus events to the shadow element,
      // otherwise focus events between elements in the shadow won't be handled
      var parentShadowRoot = spAutofillCore.HTMLTools.GetParentShadowRoot(focusedElement);
      if (parentShadowRoot)
        this.ConnectToShadowRootEvents(parentShadowRoot);
      ...
      var topElement = spAutofillCore.HTMLTools.ElementFromPoint(focusedElement, x, y);
      if (topElement &&
...
function TspAutofillAdapter()
{
  ...
  this.FocusChangedPostponeTimerID = 0;
...
TspAutofillAdapter.prototype.ConnectToPageFocusChanged = function()
{
  ...
  this.PageFocusChangedMonitor.onPageFocusChangedCallback = function (AWindow, AFocusedElement, AFocusedElementScreenRect) {
    if (spAutofillCore.ContextButtonManager.hasContextButtonBackgroundImg(AFocusedElement))
      Self.StartFocusChangedPostpone(() => {
...
// StartFocusChangedPostpone()
TspAutofillAdapter.prototype.StartFocusChangedPostpone = function(AFocusChangedCallback)
{
  try
  {
    if (typeof AFocusChangedCallback != 'function')
      throw 'AFocusChangedCallback is undefined!';

    this.StopFocusChangedPostpone();
    this.FocusChangedPostponeTimerID = setTimeout((AEvent) => {
      this.FocusChangedPostponeTimerID = 0;
      AFocusChangedCallback();
    }, 125);
  }
  catch (ErrorMessage)
  {
    spLog.logError('TspAutofillAdapter.StartFocusChangedPostpone() Error: ' + ErrorMessage);
  }
};

// StopFocusChangedPostpone()
TspAutofillAdapter.prototype.StopFocusChangedPostpone = function()
{
  try
  {
    if (this.FocusChangedPostponeTimerID)
    {
      clearTimeout(this.FocusChangedPostponeTimerID);
      this.FocusChangedPostponeTimerID = 0;
    }
  }
  catch (ErrorMessage)
  {
    spLog.logError('TspAutofillAdapter.StopFocusChangedPostpone() Error: ' + ErrorMessage);
  }
};
...
TspChromiumAutofillAdapter.prototype.bgCrossOriginPageLoaded = function(AWindow, message)
{
  ...
  {
    let logInfo = {};
    if (this.PageEventsMonitor.AllowProcessCrossOriginPageLoaded(logInfo))
    {
      if (this.Log.OnLoad)
      {
        this._logPageEvent('CrossOriginPageLoaded', AWindow, 
          'allowed cross-origin page loaded, dtc=' + logInfo.dtc + ' tc=' + spAutofillCore.Tools.GetTickCount()
        );
      }
      this.ProcessPageLoaded(AWindow);
    }
  ...
  {
    spLog.logError('TspChromiumAutofillAdapter.bgCrossOriginPageLoaded() Error: ' + ErrorMessage);
  }
};
