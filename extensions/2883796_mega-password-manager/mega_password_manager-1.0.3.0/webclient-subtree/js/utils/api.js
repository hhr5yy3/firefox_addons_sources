...
class MEGAPIRequest {
    ...
    get queryString() {
        ...
        // set params to get only the pwmh node related action packets for wsc requests
        return wsc ? `${value}&e=${pwmh}&ir=1&ap=1` : `id=${++this.seqNo}&${value}`;
    }
...
class MEGAKeepAliveStream {
    constructor(url, options, handlers) {
        ...
        mBroadcaster.addListener('online', this);
        mBroadcaster.addListener('offline', this);
    }
    ...
    destroy(reason) {
        if (!this.destroyed) {
            ...
            mBroadcaster.removeListener('online', this);
            mBroadcaster.removeListener('offline', this);
    ...
    return freeze({
        ...
        getURLSearchParams(options, mode) {
            ...
            if (!gSearchParams) {
                ...
                if (is_pwm_extension) {
                    obj.domain = 'meganz';
                    obj.ext = `PWM/${buildVersion.pwmChrome}` || '';
                }
                else {
