var safeInsertHTML = function (app, template, append) {
    ...
    if (!app) throw new Error('safeInsertHTML: Please provide a valid element to inject content into');

    // Check for browser support
    if (!supports()) throw new Error('safeInsertHTML: Your browser is not supported.');
...
const showError = (message) => {
    ...
    safeInsertHTML(window.document.getElementById('errorMessage'), message);
}

const setCustomMessage = (customMessage) => {
    window.document.getElementById('customMessage').style.display = 'block';
    safeInsertHTML(window.document.getElementById('customMessage'), customMessage);
}

const setConfirmMessage = (confirmMessage, confirmTitle) => {
    window.document.getElementById('loadingMessage').style.display = 'none';
    window.document.getElementById('confirmContainer').style.display = 'block';
    safeInsertHTML(window.document.getElementById('confirmMessage'), confirmMessage);
    if (confirmTitle) {
        safeInsertHTML(window.document.getElementById('confirmAction'), confirmTitle);
    }
}
