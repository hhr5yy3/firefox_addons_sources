    ...
    async function loadDefaultSearches() {
        ...
        if(savedSearches.length === 0) {
            ...
            let pinnedSearches = await new Promise(resolve => {
                chrome.storage.local.get(['pinnedSearches'], data => {
            ...
            for(let i in savedSearches) {
                ...
                removeTopic.addEventListener('click',async () => {
                    if(savedSearches[i].local) {
                        ...
                        chrome.storage.local.set({pinnedSearches: pinnedSearches});
                        topicElement.remove();
                        removeTopic.remove();
    ...
    if(!vars.disableHotkeys) {
        function processHotkeys() {
            if (keysHeld['Alt'] && keysHeld['Control'] && keysHeld['KeyO']) {
                ...
                if(vars.developerMode) chrome.storage.local.get('extensiveLogging', res => {
                    chrome.storage.local.set({ extensiveLogging: !res.extensiveLogging }, () => {
